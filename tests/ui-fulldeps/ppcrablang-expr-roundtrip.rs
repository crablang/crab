...
use crablangc_ast::mut_visit::{self, visit_clobber, MutVisitor};
use crablangc_ast::ptr::P;
use crablangc_ast::*;
use crablangc_ast_pretty::ppcrablang;
use crablangc_parse::new_parser_from_source_str;
use crablangc_session::parse::ParseSess;
use crablangc_span::source_map::FilePathMapping;
use crablangc_span::source_map::{FileName, Spanned, DUMMY_SP};
...
fn main() {
    crablangc_span::create_default_session_globals_then(|| run());
}

fn run() {
    let ps = ParseSess::new(vec![crablangc_parse::DEFAULT_LOCALE_RESOURCE], FilePathMapping::empty());

    iter_exprs(2, &mut |mut e| {
        // If the pretty printer is correct, then `parse(print(e))` should be identical to `e`,
        // modulo placement of `Paren` nodes.
        let printed = ppcrablang::expr_to_string(&e);
        ...
        if let Some(mut parsed) = parse_expr(&ps, &printed) {
            ...
            let text1 = ppcrablang::expr_to_string(&e);
            RemoveParens.visit_expr(&mut parsed);
            AddParens.visit_expr(&mut parsed);
            let text2 = ppcrablang::expr_to_string(&parsed);
            assert!(
                text1 == text2,
                "exprs are not equal:\n  e =      {:?}\n  parsed = {:?}",
                text1,
