...
// `#[global_allocator]` in code blocks built by `crablangdoc`.
//
// ## Background
//
// Changes in lang-item visibility injected failures that were only
// exposed when compiling with `-C prefer-dynamic`. But `crablangdoc` used
...
// decided to change `crablangdoc` to behave more like the compiler's
