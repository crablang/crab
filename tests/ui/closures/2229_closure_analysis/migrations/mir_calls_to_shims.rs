    ...
    F: FnOnce(),
{
    ...
    let result = panic::catch_unwind(move || {
        ...
        //~| NOTE: in CrabLang 2018, this closure implements `UnwindSafe`
        //~| NOTE: in CrabLang 2018, this closure implements `RefUnwindSafe`
        //~| NOTE: for more information, see
        //~| HELP: add a dummy let to cause `f` to be fully captured
        f.0()
        //~^ NOTE: in CrabLang 2018, this closure captures all of `f`, but in CrabLang 2021, it will only capture `f.0`
    });
    if let Ok(..) = result {
