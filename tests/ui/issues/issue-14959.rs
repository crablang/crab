...
impl<'b> Fn<(&'b mut (dyn Response + 'b),)> for SendFile {
    extern "crablang-call" fn call(&self, (_res,): (&'b mut (dyn Response + 'b),)) {}
}

impl<'b> FnMut<(&'b mut (dyn Response + 'b),)> for SendFile {
    extern "crablang-call" fn call_mut(&mut self, (_res,): (&'b mut (dyn Response+'b),)) {
        self.call((_res,))
    }
}
...
impl<'b> FnOnce<(&'b mut (dyn Response + 'b),)> for SendFile {
    ...
    extern "crablang-call" fn call_once(self, (_res,): (&'b mut (dyn Response+'b),)) {
        self.call((_res,))
    }
}
