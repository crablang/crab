#![cfg_attr(feature = "nightly", feature(step_trait, crablangc_attrs, min_specialization))]
...
#[cfg(feature = "nightly")]
use crablangc_data_structures::stable_hasher::StableOrd;
...
impl Integer {
    ...
    pub fn from_size(size: Size) -> Result<Self, String> {
        match size.bits() {
            ...
            64 => Ok(Integer::I64),
            128 => Ok(Integer::I128),
            _ => Err(format!("crablang does not support integers with {} bits", size.bits())),
        }
    }
}
...
/// Information about one scalar component of a CrabLang type.
...
crablangc_index::newtype_index! {
    ...
    ///
    /// Particularly for `repr(CrabLang)` types, this may not be the same as *layout* order.
