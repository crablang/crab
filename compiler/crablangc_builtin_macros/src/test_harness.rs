...
use crablangc_ast::entry::EntryPointType;
use crablangc_ast::mut_visit::{ExpectOne, *};
use crablangc_ast::ptr::P;
use crablangc_ast::{attr, ModKind};
use crablangc_expand::base::{ExtCtxt, ResolverExpand};
use crablangc_expand::expand::{AstFragment, ExpansionConfig};
use crablangc_feature::Features;
use crablangc_session::Session;
use crablangc_span::hygiene::{AstPass, SyntaxContext, Transparency};
...
// Beware, this is duplicated in libcrablangc_passes/entry.rs (with
// `crablangc_hir::Item`), so make sure to keep them in sync.
fn entry_point_type(item: &ast::Item, depth: usize) -> EntryPointType {
    match item.kind {
        ast::ItemKind::Fn(..) => {
            if attr::contains_name(&item.attrs, sym::start) {
                EntryPointType::Start
            } else if attr::contains_name(&item.attrs, sym::crablangc_main) {
                EntryPointType::CrabLangcMainAttr
...
impl<'a> MutVisitor for EntryPointCleaner<'a> {
    fn flat_map_item(&mut self, i: P<ast::Item>) -> SmallVec<[P<ast::Item>; 1]> {
        ...
        // Remove any #[crablangc_main] or #[start] from the AST so it doesn't
        // clash with the one we're going to add, but mark it as
        // #[allow(dead_code)] to avoid printing warnings.
        let item = match entry_point_type(&item, self.depth) {
            EntryPointType::MainNamed | EntryPointType::CrabLangcMainAttr | EntryPointType::Start => {
                item.map(|ast::Item { id, ident, attrs, kind, vis, span, tokens }| {
                    ...
                    let attrs = attrs
                        .into_iter()
                        .filter(|attr| {
                            !attr.has_name(sym::crablangc_main) && !attr.has_name(sym::start)
                        })
...
fn mk_main(cx: &mut TestCtxt<'_>) -> P<ast::Item> {
    ...
    // #[crablangc_main]
    let main_attr = ecx.attr_word(sym::crablangc_main, sp);
...
fn get_test_name(i: &ast::Item) -> Option<Symbol> {
    attr::first_attr_value_str_by_name(&i.attrs, sym::crablangc_test_marker)
}

fn get_test_runner(sd: &crablangc_errors::Handler, krate: &ast::Crate) -> Option<ast::Path> {
