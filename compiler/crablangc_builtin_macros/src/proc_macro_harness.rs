...
use crablangc_ast::{self as ast, attr, NodeId};
use crablangc_ast_pretty::ppcrablang;
use crablangc_expand::base::{parse_macro_name_and_helper_attrs, ExtCtxt, ResolverExpand};
    ...
    fn visit_item(&mut self, item: &'a ast::Item) {
        ...
        for attr in &item.attrs {
            if attr.is_proc_macro_attr() {
                if let Some(prev_attr) = found_attr {
                    ...
                    let path_str = ppcrablang::path_to_string(&item.path);
                        ...
                        == prev_item.path.segments[0].ident.name
                    {
                        ...
                        format!(
                            ...
                            path_str,
                            ppcrablang::path_to_string(&prev_item.path),
                        )
                    };
        ...
        if !is_fn {
            let msg = format!(
                ...
                ppcrablang::path_to_string(&attr.get_normal_item().path),
            );
...
fn mk_decls(cx: &mut ExtCtxt<'_>, macros: &[ProcMacro]) -> P<ast::Item> {
    let expn_id = cx.resolver.expansion_for_ast_pass(
        ...
        &[sym::crablangc_attrs, sym::proc_macro_internals],
        None,
    );
    ...
    let decls_static = cx
        ...
        .map(|mut i| {
            i.attrs.push(cx.attr_word(sym::crablangc_proc_macro_decls, span));
