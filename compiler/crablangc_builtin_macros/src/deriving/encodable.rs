//! The compiler code necessary to implement the `#[derive(CrabLangcEncodable)]`
//! (and `CrabLangcDecodable`, in `decodable.rs`) extension. The idea here is that
//! type-defining items may be tagged with
//! `#[derive(CrabLangcEncodable, CrabLangcDecodable)]`.
//!
...
use crablangc_ast::{AttrVec, ExprKind, MetaItem, Mutability};
use crablangc_expand::base::{Annotatable, ExtCtxt};
use crablangc_span::symbol::{sym, Ident, Symbol};
use crablangc_span::Span;
use thin_vec::{thin_vec, ThinVec};

pub fn expand_deriving_crablangc_encodable(
    cx: &mut ExtCtxt<'_>,
    span: Span,
    mitem: &MetaItem,
    item: &Annotatable,
    ...
    let krate = sym::crablangc_serialize;
    let typaram = sym::__S;
    ...
    match substr.fields {
        Struct(_, fields) => {
            let fn_emit_struct_field_path =
                cx.def_site_path(&[sym::crablangc_serialize, sym::Encoder, sym::emit_struct_field]);
            ...
            let fn_emit_struct_path =
                cx.def_site_path(&[sym::crablangc_serialize, sym::Encoder, sym::emit_struct]);
        ...
        EnumMatching(idx, _, variant, fields) => {
            ...
            let fn_emit_enum_variant_arg_path: Vec<_> =
                cx.def_site_path(&[sym::crablangc_serialize, sym::Encoder, sym::emit_enum_variant_arg]);
            ...
            let fn_emit_enum_variant_path: Vec<_> =
                cx.def_site_path(&[sym::crablangc_serialize, sym::Encoder, sym::emit_enum_variant]);
            ...
            let fn_emit_enum_path: Vec<_> =
                cx.def_site_path(&[sym::crablangc_serialize, sym::Encoder, sym::emit_enum]);
            let expr = cx.expr_call_global(
                trait_span,
                fn_emit_enum_path,
