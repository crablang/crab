...
use crablangc_ast::{attr, walk_list, EnumDef, VariantData};
use crablangc_expand::base::{Annotatable, DummyResult, ExtCtxt};
...
fn extract_default_variant<'a>(
    ...
    enum_def: &'a EnumDef,
    trait_span: Span,
) -> Result<&'a crablangc_ast::Variant, ()> {
...
fn validate_default_attribute(
    cx: &mut ExtCtxt<'_>,
    default_variant: &crablangc_ast::Variant,
) -> Result<(), ()> {
...
impl<'a, 'b> crablangc_ast::visit::Visitor<'a> for DetectNonVariantDefaultAttr<'a, 'b> {
    fn visit_attribute(&mut self, attr: &'a crablangc_ast::Attribute) {
        if attr.has_name(kw::Default) {
            self.cx.emit_err(errors::NonUnitDefault { span: attr.span });
        }

        crablangc_ast::visit::walk_attribute(self, attr);
    }
    fn visit_variant(&mut self, v: &'a crablangc_ast::Variant) {
...
fn has_a_default_variant(item: &Annotatable) -> bool {
    ...
    impl<'ast> crablangc_ast::visit::Visitor<'ast> for HasDefaultAttrOnVariant {
        fn visit_variant(&mut self, v: &'ast crablangc_ast::Variant) {
