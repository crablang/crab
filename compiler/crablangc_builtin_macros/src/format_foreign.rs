pub(crate) mod printf {
    ...
    impl<'a> Substitution<'a> {
        ...
        /// Translate this substitution into an equivalent CrabLang formatting directive.
        ///
    ...
    impl Format<'_> {
        /// Translate this directive into an equivalent CrabLang formatting directive.
        ///
        /// Returns `Err` in cases where the `printf` directive does not have an exact CrabLang
        ...
        pub fn translate(&self) -> Result<String, Option<String>> {
            ...
            // Has a special form in CrabLang for numbers.
            let fill = c_zero.then_some("0");

            let align = c_left.then_some("<");

            // CrabLang doesn't have an equivalent to the `' '` flag.
            ...
            let width = match self.width {
                Some(Num::Next) => {
                    // NOTE: CrabLang doesn't support this.
                    return Err(Some(
            ...
            // NOTE: although length *can* have an effect, we can't duplicate the effect in CrabLang, so
            // we just ignore it.
            ...
            let (fill, width, precision) = match (is_int, width, precision) {
                (true, Some(_), Some(_)) => {
                    // CrabLang can't duplicate this insanity.
                    return Err(Some(
    ...
    pub fn parse_next_substitution(s: &str) -> Option<(Substitution<'_>, &str)> {
        ...
        // Sadly, CrabLang isn't *quite* smart enough to know these *must* be initialised by the end.
