...
use crablangc_errors::{Applicability, MultiSpan, PResult, SingleLabelManySpans};
use crablangc_expand::base::{self, *};
use crablangc_parse_format as parse;
use crablangc_span::symbol::{Ident, Symbol};
use crablangc_span::{BytePos, InnerSpan, Span};

use crablangc_lint_defs::builtin::NAMED_ARGUMENTS_USED_POSITIONALLY;
use crablangc_lint_defs::{BufferedEarlyLint, BuiltinLintDiagnostics, LintId};
...
//  3. Much later, in AST lowering (crablangc_ast_lowering), that `FormatArgs` structure will be turned
//     into the expression of type `core::fmt::Arguments`.

// See crablangc_ast/src/format.rs for the FormatArgs structure and glossary.
...
struct MacroInput {
    ...
    ///
    /// [RFC 2795]: https://crablang.github.io/rfcs/2795-format-args-implicit-identifiers.html#macro-hygiene
...
fn make_format_args(
    ...
    let str_style = match fmt_style {
        crablangc_ast::StrStyle::Cooked => None,
        crablangc_ast::StrStyle::Raw(raw) => Some(raw as usize),
    };
    ...
    let to_span = |inner_span: crablangc_parse_format::InnerSpan| {
    ...
    if invalid_refs.is_empty() && ecx.sess.err_count() == 0 {
        for &(index, span, used_as) in &numeric_refences_to_named_arg {
            ...
            ecx.buffered_early_lint.push(BufferedEarlyLint {
                ...
                node_id: crablangc_ast::CRATE_NODE_ID,
...
fn invalid_placeholder_type_error(
    ecx: &ExtCtxt<'_>,
    ty: &str,
    ty_span: Option<crablangc_parse_format::InnerSpan>,
    fmt_span: Span,
) {
...
fn report_invalid_references(
    ...
    if template.iter().all(|piece| match piece {
        ...
        e = ecx.create_err(errors::FormatPositionalMismatch {
            ...
            highlight: SingleLabelManySpans {
                ...
                label: "",
                kind: crablangc_errors::LabelKind::Label,
            },
        });
    ...
    if template.iter().any(|piece| match piece {
        ...
        e.note("for information about formatting flags, visit https://doc.crablang.org/std/fmt/index.html");
    }
