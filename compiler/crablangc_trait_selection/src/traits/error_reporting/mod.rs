...
use crablangc_data_structures::fx::{FxHashMap, FxIndexMap};
...
use crablangc_hir::{GenericParam, Item, Node};
use crablangc_infer::infer::error_reporting::TypeErrCtxt;
use crablangc_infer::infer::{InferOk, TypeTrace};
use crablangc_middle::traits::select::OverflowError;
use crablangc_middle::ty::abstract_const::NotConstEvaluatable;
use crablangc_middle::ty::error::{ExpectedFound, TypeError};
use crablangc_middle::ty::fast_reject::TreatProjections;
use crablangc_middle::ty::fold::{TypeFolder, TypeSuperFoldable};
use crablangc_middle::ty::print::{with_forced_trimmed_paths, FmtPrinter, Print};
...
use crablangc_span::{ExpnKind, Span, DUMMY_SP};
use std::fmt;
use std::iter;
use std::ops::ControlFlow;
use suggestions::TypeErrCtxtExt as _;

pub use crablangc_infer::traits::error_reporting::*;
        ...
        let mut err = match *error {
                    ...
                    ty::PredicateKind::Clause(ty::Clause::Trait(trait_predicate)) => {
                        ...
                        if Some(trait_ref.def_id()) == tcx.lang_items().tuple_trait() {
                            match obligation.cause.code().peel_derives() {
                                ObligationCauseCode::CrabLangCall => {
                                    err.set_primary_message("functions with the \"crablang-call\" ABI must take a single non-self tuple argument");
                                }
                                ObligationCauseCode::BindingObligation(def_id, _)
                                | ObligationCauseCode::ItemObligation(def_id)
                                    if tcx.is_fn_trait(*def_id) =>
                                {
                                    err.code(crablangc_errors::error_code!(E0059));
                            ...
                            && predicate_is_const
                        {
                            ...
                            err.note("See <https://github.com/crablang/crablang/pull/94901> for more details");
                        }
                        ...
                        if let Some(ref s) = label {
                            // If it has a custom `#[crablangc_on_unimplemented]`
                        ...
                        if let Some(ref s) = note {
                            // If it has a custom `#[crablangc_on_unimplemented]` note, let's display it
                            err.note(s.as_str());
                        }
                        ...
                        if is_unsize {
                            ...
                            err.note(
                                ...
                                <https://doc.crablang.org/stable/std/marker/trait.Unsize.html> \
                            ...
                            && self.fallback_has_occurred
                        {
                            ...
                            if self.predicate_may_hold(&unit_obligation) {
                                err.note(
                                    ...
                                    CrabLang's type-inference algorithm (see issue #48950 \
                                    <https://github.com/crablang/crablang/issues/48950> \
                                    for more information)",
                                );
            ...
            SelectionError::NotConstEvaluatable(NotConstEvaluatable::MentionsParam) => {
                if !self.tcx.features().generic_const_exprs {
                    ...
                    // (see https://github.com/crablang/crablang/issues/66962#issuecomment-575907083).
                    //
...
impl<'tcx> InferCtxtPrivExt<'tcx> for TypeErrCtxt<'_, 'tcx> {
    ...
    fn report_similar_impl_candidates(
        ...
        // https://github.com/crablang/crablang/pull/57475#issuecomment-455519507
        //
                ...
                if let Some(ty::subst::GenericArgKind::Type(_)) = subst.map(|subst| subst.unpack())
                {
                    ...
                    if let Some(hir::Expr {
                        ...
                        && let [
                            ...
                            trait_path_segment @ hir::PathSegment {
                                res: crablangc_hir::def::Res::Def(crablangc_hir::def::DefKind::Trait, trait_id),
                                ..
                            },
                            hir::PathSegment {
                                ident: assoc_item_name,
                                res: crablangc_hir::def::Res::Def(_, item_id),
                                ..
                            }
                        ] = path.segments
                        ...
                        && let None = self.tainted_by_errors()
                    {
                        ...
                        err = self.tcx.sess.struct_span_err_with_code(
                            ...
                            crablangc_errors::error_code!(E0790),
                        );
