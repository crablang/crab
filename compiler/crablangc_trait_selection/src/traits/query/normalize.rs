...
use crablangc_infer::traits::Normalized;
use crablangc_middle::ty::fold::{FallibleTypeFolder, TypeFoldable, TypeSuperFoldable};
use crablangc_middle::ty::visit::{TypeSuperVisitable, TypeVisitable, TypeVisitableExt};
use crablangc_middle::ty::{self, Ty, TyCtxt, TypeVisitor};
...
pub use crablangc_middle::traits::query::NormalizationResult;
...
impl<'cx, 'tcx> FallibleTypeFolder<TyCtxt<'tcx>> for QueryNormalizer<'cx, 'tcx> {
    ...
    fn try_fold_ty(&mut self, ty: Ty<'tcx>) -> Result<Ty<'tcx>, Self::Error> {
        ...
        // See note in `crablangc_trait_selection::traits::project` about why we
        ...
        let res = match kind {
            ...
            ty::Projection => {
                // See note in `crablangc_trait_selection::traits::project`
                ...
                if result.is_ambiguous() {
                    // CrabLangdoc normalizes possibly not well-formed types, so only
                    // treat this as a bug if we're not in crablangdoc.
