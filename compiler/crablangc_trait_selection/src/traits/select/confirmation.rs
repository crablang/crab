...
//! type error. See the [crablangc dev guide] for more details.
//!
//! [crablangc dev guide]:
//! https://crablangc-dev-guide.crablang.org/traits/resolution.html#confirmation
use crablangc_data_structures::stack::ensure_sufficient_stack;
use crablangc_hir::lang_items::LangItem;
use crablangc_infer::infer::LateBoundRegionConversionTime::HigherRankedType;
...
impl<'cx, 'tcx> SelectionContext<'cx, 'tcx> {
    ...
    fn confirm_transmutability_candidate(
        ...
        let src_and_dst = predicate.map_bound(|p| crablangc_transmute::Types {
        ...
        let Some(assume) =
            crablangc_transmute::Assume::from_const(self.infcx.tcx, obligation.param_env, const_at(3)) else {
                return Err(Unimplemented);
            };

        let cause = obligation.cause.clone();

        let mut transmute_env = crablangc_transmute::TransmuteTypeEnv::new(self.infcx);

        let maybe_transmutable = transmute_env.is_transmutable(cause, src_and_dst, scope, assume);

        use crablangc_transmute::Answer;
