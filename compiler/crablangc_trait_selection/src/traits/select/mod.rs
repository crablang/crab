//! Candidate selection. See the [crablangc dev guide] for more information on how this works.
//!
//! [crablangc dev guide]: https://crablangc-dev-guide.crablang.org/traits/resolution.html#selection
...
use crablangc_data_structures::fx::{FxHashSet, FxIndexMap, FxIndexSet};
...
use crablangc_infer::infer::LateBoundRegionConversionTime;
use crablangc_infer::traits::TraitEngine;
use crablangc_infer::traits::TraitEngineExt;
use crablangc_middle::dep_graph::{DepKind, DepNodeIndex};
use crablangc_middle::mir::interpret::ErrorHandled;
use crablangc_middle::ty::abstract_const::NotConstEvaluatable;
use crablangc_middle::ty::fold::BottomUpFolder;
use crablangc_middle::ty::relate::TypeRelation;
use crablangc_middle::ty::SubstsRef;
use crablangc_middle::ty::{self, EarlyBinder, PolyProjectionPredicate, ToPolyTraitRef, ToPredicate};
use crablangc_middle::ty::{Ty, TyCtxt, TypeFoldable, TypeVisitableExt};
...
pub use crablangc_middle::traits::select::*;
...
impl<'cx, 'tcx> SelectionContext<'cx, 'tcx> {
        ...
        if let ImplCandidate(def_id) = candidate {
            if let ty::ImplPolarity::Reservation = tcx.impl_polarity(def_id) {
                if let Some(intercrate_ambiguity_clauses) = &mut self.intercrate_ambiguity_causes {
                    let value = tcx
                        .get_attr(def_id, sym::crablangc_reservation_impl)
    ...
    /// certain cases (currently only libcrablangdoc's blanket impl finder),
...
///
/// In Issue #60010, we found a bug in crablangc where it would cache
