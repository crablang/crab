...
CrabLang, trait object types were written as plain `Trait` (just the name of the
...
The reasoning behind this is somewhat subtle. It derives from the fact that CrabLang
requires (and defines) that every trait object type `dyn Trait` automatically
implements `Trait`. CrabLang does this to simplify error reporting and ease
...
to the rule that `dyn Trait` always implements `Trait`, CrabLang chooses to prohibit
