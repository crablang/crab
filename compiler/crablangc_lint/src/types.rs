...
use crablangc_errors::DiagnosticMessage;
use crablangc_hir as hir;
use crablangc_hir::{is_range_literal, Expr, ExprKind, Node};
use crablangc_middle::ty::layout::{IntegerExt, LayoutOf, SizeSkeleton};
...
use crablangc_target::abi::{Abi, Size, WrappingRange};
use crablangc_target::abi::{Integer, TagEncoding, Variants};
...
declare_lint! {
    ...
    ///
    /// [`Box`]: https://doc.crablang.org/std/boxed/index.html
...
fn lint_overflowing_range_endpoint<'tcx>(
    ...
    use crablangc_ast::{LitIntType, LitKind};
...
declare_lint! {
    ...
    ///
    /// [`extern` function]: https://doc.crablang.org/reference/items/functions.html#extern-function-qualifier
    ///
    /// ### Example
    ///
    /// ```crablang
...
pub(crate) fn nonnull_optimization_guaranteed<'tcx>(
    ...
    tcx.has_attr(def.did(), sym::crablangc_nonnull_optimization_guaranteed)
}
...
fn get_nullable_type<'tcx>(cx: &LateContext<'tcx>, ty: Ty<'tcx>) -> Option<Ty<'tcx>> {
    ...
    Some(match *ty.kind() {
        ...
        ty::FnPtr(..) => {
            // There is no nullable equivalent for CrabLang's function pointers -- you
...
impl<'a, 'tcx> ImproperCTypesVisitor<'a, 'tcx> {
    ...
    fn check_variant_for_ffi(
        ...
        // We can't completely tcrablang repr(C) markings; make sure the fields are
        // actually safe.
    ...
    fn is_internal_abi(&self, abi: SpecAbi) -> bool {
        matches!(
            abi,
            SpecAbi::CrabLang | SpecAbi::CrabLangCall | SpecAbi::CrabLangIntrinsic | SpecAbi::PlatformIntrinsic
        )
    }
}
