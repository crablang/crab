...
use crablangc_ast::util::{classify, parser};
use crablangc_ast::{ExprKind, StmtKind};
...
declare_lint! {
    ...
    ///
    /// [the reference]: https://doc.crablang.org/reference/attributes/diagnostics.html#the-must_use-attribute
    pub UNUSED_MUST_USE,
    Warn,
...
declare_lint! {
    ...
    /// values.
    ///
    /// [`must_use` attribute]: https://doc.crablang.org/reference/attributes/diagnostics.html#the-must_use-attribute
...
impl<'tcx> LateLintPass<'tcx> for UnusedResults {
    fn check_stmt(&mut self, cx: &LateContext<'_>, s: &hir::Stmt<'_>) {
        ...
        if !fn_warned && type_lint_emitted_or_suppressed {
            ...
            // (See https://github.com/crablang/crablang/issues/43806 for details.)
            return;
        }
...
trait UnusedDelimLint {
    ...
    fn check_expr(&mut self, cx: &EarlyContext<'_>, e: &ast::Expr) {
        ...
        let (value, ctx, followed_by_block, left_pos, right_pos) = match e.kind {
            ...
            {
                let left = e.span.lo() + crablangc_span::BytePos(2);
            ...
            While(ref cond, ref block, ..)
                if !matches!(cond.kind, Let(_, _, _))
                    || Self::LINT_EXPR_IN_PATTERN_MATCHING_CTX =>
            {
                let left = e.span.lo() + crablangc_span::BytePos(5);
                let right = block.span.lo();
            ...
            Ret(Some(ref value)) => {
                let left = e.span.lo() + crablangc_span::BytePos(3);
                ...
                if !Self::is_expr_delims_necessary(inner, followed_by_block, followed_by_else)
                    ...
                    && (ctx != UnusedDelimsCtx::LetScrutineeExpr
                        || !matches!(inner.kind, ast::ExprKind::Binary(
                                crablangc_span::source_map::Spanned { node, .. },
                                _,
                                _,
                            ) if node.lazy()))
                {
...
impl EarlyLintPass for UnusedParens {
    ...
    fn enter_where_predicate(&mut self, _: &EarlyContext<'_>, pred: &ast::WherePredicate) {
        use crablangc_ast::{WhereBoundPredicate, WherePredicate};
