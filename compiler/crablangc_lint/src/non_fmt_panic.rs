...
use crablangc_parse_format::{ParseMode, Parser, Piece};
use crablangc_session::lint::FutureIncompatibilityReason;
use crablangc_span::edition::Edition;
use crablangc_span::{hygiene, sym, symbol::kw, InnerSpan, Span, Symbol};
...
declare_lint! {
    ...
    /// ### Explanation
    ///
    /// In CrabLang 2018 and earlier, `panic!(x)` directly uses `x` as the message.
    /// That means that `panic!("{}")` panics with the message `"{}"` instead
    /// of using it as a formatting string, and `panic!(123)` will panic with
    /// an `i32` as message.
    ///
    /// CrabLang 2021 always interprets the first argument as format string.
    NON_FMT_PANICS,
    Warn,
