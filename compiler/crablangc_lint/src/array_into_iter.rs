...
use crablangc_session::lint::FutureIncompatibilityReason;
...
declare_lint! {
    ...
    /// ### Explanation
    ///
    /// Since CrabLang 1.53, arrays implement `IntoIterator`. However, to avoid
    /// breakage, `array.into_iter()` in CrabLang 2015 and 2018 code will still
    /// behave as `(&array).into_iter()`, returning an iterator over
    /// references, just like in CrabLang 1.52 and earlier.
    ...
    pub ARRAY_INTO_ITER,
    Warn,
    "detects calling `into_iter` on arrays in CrabLang 2015 and 2018",
    @future_incompatible = FutureIncompatibleInfo {
        reference: "<https://doc.crablang.org/nightly/edition-guide/crablang-2021/IntoIterator-for-arrays.html>",
