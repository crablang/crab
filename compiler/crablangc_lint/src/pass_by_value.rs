...
use crablangc_hir::{GenericArg, PathSegment, QPath, TyKind};
use crablangc_middle::ty;
use crablangc_span::symbol::sym;

declare_tool_lint! {
    /// The `crablangc_pass_by_value` lint marks a type with `#[crablangc_pass_by_value]` requiring it to
    ...
    pub crablangc::PASS_BY_VALUE,
    Warn,
    "pass by reference of a type flagged as `#[crablangc_pass_by_value]`",
...
fn path_for_pass_by_value(cx: &LateContext<'_>, ty: &hir::Ty<'_>) -> Option<String> {
    if let TyKind::Path(QPath::Resolved(_, path)) = &ty.kind {
        match path.res {
            Res::Def(_, def_id) if cx.tcx.has_attr(def_id, sym::crablangc_pass_by_value) => {
            ...
            Res::SelfTyAlias { alias_to: did, is_trait_impl: false, .. } => {
                if let ty::Adt(adt, substs) = cx.tcx.type_of(did).subst_identity().kind() {
                    if cx.tcx.has_attr(adt.did(), sym::crablangc_pass_by_value) {
