...
impl<'a> DecorateLint<'a, ()> for BuiltinMissingDebugImpl<'_> {
    fn decorate_lint<'b>(
        self,
        diag: &'b mut crablangc_errors::DiagnosticBuilder<'a, ()>,
    ) -> &'b mut crablangc_errors::DiagnosticBuilder<'a, ()> {
...
impl<'a> DecorateLint<'a, ()> for BuiltinUngatedAsyncFnTrackCaller<'_> {
    fn decorate_lint<'b>(
        ...
        crablangc_session::parse::add_feature_diagnostics(
            diag,
            &self.parse_sess,
...
pub struct SuggestChangingAssocTypes<'a, 'b> {
    pub ty: &'a crablangc_hir::Ty<'b>,
}

impl AddToDiagnostic for SuggestChangingAssocTypes<'_, '_> {
    fn add_to_diagnostic_with<F>(self, diag: &mut crablangc_errors::Diagnostic, _: F)
    where
        F: Fn(
        ...
        ) -> crablangc_errors::SubdiagnosticMessage,
    {
        ...
        struct WalkAssocTypes<'a> {
            err: &'a mut crablangc_errors::Diagnostic,
        }
        impl Visitor<'_> for WalkAssocTypes<'_> {
            fn visit_qpath(
                &mut self,
                qpath: &crablangc_hir::QPath<'_>,
                id: crablangc_hir::HirId,
                span: Span,
            ) {
