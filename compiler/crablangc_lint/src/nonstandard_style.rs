...
use crablangc_hir::{GenericParamKind, PatKind};
        ...
        match &fk {
            FnKind::Method(ident, sig, ..) => match method_context(cx, id) {
                MethodLateContext::PlainImpl => {
                    if sig.header.abi != Abi::CrabLang && cx.tcx.has_attr(id, sym::no_mangle) {
                        return;
                    }
            ...
            FnKind::ItemFn(ident, _, header) => {
                ...
                if header.abi != Abi::CrabLang && cx.tcx.has_attr(id, sym::no_mangle) {
                    return;
                }
