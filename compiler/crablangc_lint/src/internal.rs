...
use crablangc_hir::{def_id::DefId, Expr, ExprKind, GenericArg, PatKind, Path, PathSegment, QPath};
use crablangc_hir::{HirId, Impl, Item, ItemKind, Node, Pat, Ty, TyKind};
...
declare_tool_lint! {
    ...
    /// required as input is assumed to be tcrablanged.
    pub crablangc::DEFAULT_HASH_TYPES,
    Allow,
...
impl LateLintPass<'_> for DefaultHashTypes {
    fn check_path(&mut self, cx: &LateContext<'_>, path: &Path<'_>, hir_id: HirId) {
        let Res::Def(crablangc_hir::def::DefKind::Struct, def_id) = path.res else { return };
...
declare_tool_lint! {
    ...
    ///
    /// Due to the [incremental compilation](https://crablangc-dev-guide.crablang.org/queries/incremental-compilation.html) model,
    ...
    pub crablangc::POTENTIAL_QUERY_INSTABILITY,
    Allow,
...
impl LateLintPass<'_> for QueryStability {
    fn check_expr(&mut self, cx: &LateContext<'_>, expr: &Expr<'_>) {
        ...
        if let Ok(Some(instance)) = ty::Instance::resolve(cx.tcx, cx.param_env, def_id, substs) {
            ...
            if cx.tcx.has_attr(def_id, sym::crablangc_lint_query_instability) {
                cx.emit_spanned_lint(
...
declare_tool_lint! {
    ...
    pub crablangc::USAGE_OF_TY_TYKIND,
    Allow,
...
declare_tool_lint! {
    ...
    pub crablangc::USAGE_OF_QUALIFIED_TY,
    Allow,
...
declare_tool_lint! {
    ...
    pub crablangc::LINT_PASS_IMPL_WITHOUT_MACRO,
    Allow,
...
declare_tool_lint! {
    ...
    pub crablangc::EXISTING_DOC_KEYWORD,
    Allow,
...
impl<'tcx> LateLintPass<'tcx> for ExistingDocKeyword {
    fn check_item(&mut self, cx: &LateContext<'_>, item: &crablangc_hir::Item<'_>) {
...
declare_tool_lint! {
    ...
    ///
    /// More details on translatable diagnostics can be found [here](https://crablangc-dev-guide.crablang.org/diagnostics/translation.html).
    pub crablangc::UNTRANSLATABLE_DIAGNOSTIC,
    Allow,
...
declare_tool_lint! {
    ...
    ///
    /// More details on diagnostics implementations can be found [here](https://crablangc-dev-guide.crablang.org/diagnostics/diagnostic-structs.html).
    pub crablangc::DIAGNOSTIC_OUTSIDE_OF_IMPL,
    Allow,
...
impl LateLintPass<'_> for Diagnostics {
    fn check_expr(&mut self, cx: &LateContext<'_>, expr: &Expr<'_>) {
        ...
        let has_attr = ty::Instance::resolve(cx.tcx, cx.param_env, def_id, substs)
            .ok()
            .and_then(|inst| inst)
            .map(|inst| cx.tcx.has_attr(inst.def_id(), sym::crablangc_lint_diagnostics))
            .unwrap_or(false);
        if !has_attr {
            return;
        }
        ...
        for (hir_id, parent) in cx.tcx.hir().parent_iter(expr.hir_id) {
            if let Some(owner_did) = hir_id.as_owner() {
                found_parent_with_attr = found_parent_with_attr
                    || cx.tcx.has_attr(owner_did, sym::crablangc_lint_diagnostics);
            }
...
impl LateLintPass<'_> for BadOptAccess {
    fn check_expr(&mut self, cx: &LateContext<'_>, expr: &Expr<'_>) {
        ...
        // Skip types without `#[crablangc_lint_opt_ty]` - only so that the rest of the lint can be
        // avoided.
        if !cx.tcx.has_attr(adt_def.did(), sym::crablangc_lint_opt_ty) {
            return;
        }

        for field in adt_def.all_fields() {
            if field.name == target.name &&
                let Some(attr) = cx.tcx.get_attr(field.did, sym::crablangc_lint_opt_deny_field_access) &&
