lint_array_into_iter =
    this method call resolves to `<&{$target} as IntoIterator>::into_iter` (due to backwards compatibility), but will resolve to <{$target} as IntoIterator>::into_iter in CrabLang 2021
...
lint_default_hash_types = prefer `{$preferred}` over `{$used}`, it has better performance
    .note = a `use crablangc_data_structures::fx::{$preferred}` may be necessary
...
lint_non_fmt_panic = panic message is not a string literal
    .note = this usage of `{$name}!()` is deprecated; it will be a hard error in CrabLang 2021
    .more_info_note = for more information, see <https://doc.crablang.org/nightly/edition-guide/crablang-2021/panic-macro-consistency.html>
...
lint_non_fmt_panic_unused =
    ...
    .note = this message is not used as a format string when given without arguments, but will be in CrabLang 2021
...
lint_non_fmt_panic_braces =
    ...
    .note = this message is not used as a format string, but will be in CrabLang 2021
...
lint_improper_ctypes_fnptr_reason = this function pointer has CrabLang-specific calling convention
...
lint_builtin_overridden_symbol_name =
    the linker's behavior with multiple libraries exporting duplicate symbol names is undefined and CrabLang cannot provide guarantees when you manually override them

lint_builtin_overridden_symbol_section =
    the program's behavior with overridden link sections on items is unpredictable and CrabLang cannot provide guarantees when you manually override them
...
lint_builtin_unused_doc_comment = unused doc comment
    .label = crablangdoc does not generate documentation for {$kind}
...
lint_builtin_incomplete_features = the feature `{$name}` is incomplete and may not be safe to use and/or cause compiler crashes
    .note = see issue #{$n} <https://github.com/crablang/crablang/issues/{$n}> for more information
