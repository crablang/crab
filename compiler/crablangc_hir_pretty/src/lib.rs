...
use crablangc_ast::util::parser::{self, AssocOp, Fixity};
use crablangc_ast_pretty::pp::Breaks::{Consistent, Inconsistent};
use crablangc_ast_pretty::pp::{self, Breaks};
use crablangc_ast_pretty::ppcrablang::{Comments, PrintState};
use crablangc_hir as hir;
use crablangc_hir::LifetimeParamKind;
use crablangc_hir::{BindingAnnotation, ByRef, GenericArg, GenericParam, GenericParamKind, Node, Term};
use crablangc_hir::{GenericBound, PatKind, RangeEnd, TraitBoundModifier};
use crablangc_span::source_map::SourceMap;
use crablangc_span::symbol::{kw, Ident, IdentPrinter, Symbol};
use crablangc_span::{self, FileName};
use crablangc_target::spec::abi::Abi;

use std::cell::Cell;
use std::vec;

pub fn id_to_string(map: &dyn crablangc_hir::intravisit::Map<'_>, hir_id: hir::HirId) -> String {
...
impl PpAnn for &dyn crablangc_hir::intravisit::Map<'_> {
...
impl<'a> State<'a> {
    pub fn bclose_maybe_open(&mut self, span: crablangc_span::Span, close_box: bool) {
    ...
    pub fn bclose(&mut self, span: crablangc_span::Span) {
    ...
    pub fn print_variants(&mut self, variants: &[hir::Variant<'_>], span: crablangc_span::Span) {
        self.bopen();
        for v in variants {
            self.space_if_not_bol();
