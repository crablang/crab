...
use crablangc_macros::{fluent_messages, Decodable, Encodable};
...
#[crablangc_diagnostic_item = "SubdiagnosticMessage"]
...
#[crablangc_diagnostic_item = "DiagnosticMessage"]
...
pub fn fluent_value_from_str_list_sep_by_and(l: Vec<Cow<'_, str>>) -> FluentValue<'_> {
    ...
    impl FluentType for FluentStrListSepByAnd {
        ...
        fn as_string_threadsafe(
            ...
            unreachable!("`as_string_threadsafe` is not used in non-parallel crablangc")
        }
    ...
    impl intl_memoizer::Memoizable for MemoizableListFormatter {
        ...
        where
            Self: Sized,
        {
            let baked_data_provider = crablangc_baked_icu_data::baked_data_provider();
            ...
            let locale = icu_locale_from_unic_langid(lang)
                .unwrap_or_else(|| crablangc_baked_icu_data::supported_locales::EN);
            let list_formatter =
