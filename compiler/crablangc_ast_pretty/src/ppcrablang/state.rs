...
use crablangc_ast::attr::AttrIdGenerator;
use crablangc_ast::ptr::P;
use crablangc_ast::token::{self, BinOpToken, CommentKind, Delimiter, Nonterminal, Token, TokenKind};
use crablangc_ast::tokenstream::{TokenStream, TokenTree};
use crablangc_ast::util::classify;
use crablangc_ast::util::comments::{gather_comments, Comment, CommentStyle};
use crablangc_ast::util::parser;
use crablangc_ast::{self as ast, AttrArgs, AttrArgsEq, BlockCheckMode, PatKind};
use crablangc_ast::{attr, BindingAnnotation, ByRef, DelimArgs, RangeEnd, RangeSyntax, Term};
use crablangc_ast::{GenericArg, GenericBound, SelfKind, TraitBoundModifier};
use crablangc_ast::{InlineAsmOperand, InlineAsmRegOrRegClass};
use crablangc_ast::{InlineAsmOptions, InlineAsmTemplatePiece};
use crablangc_span::edition::Edition;
use crablangc_span::source_map::{SourceMap, Spanned};
use crablangc_span::symbol::{kw, sym, Ident, IdentPrinter, Symbol};
...
pub fn print_crate<'a>(
    ...
    if is_expanded && !krate.attrs.iter().any(|attr| attr.has_name(sym::no_core)) {
        ...
        // Currently, in CrabLang 2018 we don't have `extern crate std;` at the crate
        // root, so this is not needed, and actually breaks things.
        if edition.is_crablang_2015() {
            // `#![no_std]`
...
pub trait PrintState<'a>: std::ops::Deref<Target = pp::Printer> + std::ops::DerefMut {
    ...
    fn maybe_print_trailing_comment(&mut self, span: crablangc_span::Span, next_pos: Option<BytePos>) {
        if let Some(cmnts) = self.comments() {
    ...
    fn bclose_maybe_open(&mut self, span: crablangc_span::Span, empty: bool, close_box: bool) {
    ...
    fn bclose(&mut self, span: crablangc_span::Span, empty: bool) {
        let close_box = true;
