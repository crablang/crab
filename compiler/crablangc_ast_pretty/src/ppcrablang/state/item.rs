...
use crate::ppcrablang::state::delimited::IterDelimited;
use crate::ppcrablang::state::{AnnNode, PrintState, State, INDENT_UNIT};

use ast::StaticItem;
use crablangc_ast as ast;
use crablangc_ast::GenericBound;
use crablangc_ast::ModKind;
...
impl<'a> State<'a> {
    ...
    fn print_variants(&mut self, variants: &[ast::Variant], span: crablangc_span::Span) {
        self.bopen();
        for v in variants {
    ...
    fn print_record_struct_body(&mut self, fields: &[ast::FieldDef], span: crablangc_span::Span) {
        self.nbsp();
        self.bopen();
