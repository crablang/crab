...
/// <https://crablang.github.io/rfcs/2603-crablang-symbol-name-mangling-v0.html>).
...
// FIXME(rcvalle): Move to compiler/crablangc_middle/src/ty/sty.rs after C types work is done, possibly
...
/// CrabLang types that are not used at the FFI boundary.
...
/// Encodes substs using the Itanium C++ ABI with vendor extended type qualifiers and types for CrabLang
...
fn encode_ty_name(tcx: TyCtxt<'_>, def_id: DefId) -> String {
    ...
    // https://crablang.github.io/rfcs/2603-crablang-symbol-name-mangling-v0.html#methods.
    let mut s = String::new();
...
fn encode_ty<'tcx>(
    ...
    match ty.kind() {
        ...
        // Compound types
        // () in CrabLang is equivalent to void return type in C
        _ if ty.is_unit() => {
            typeid.push('v');
        }
...
/// extended type qualifiers and types for CrabLang types that are not used at the FFI boundary.
#[instrument(level = "trace", skip(tcx))]
