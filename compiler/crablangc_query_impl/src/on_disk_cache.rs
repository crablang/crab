...
use crablangc_data_structures::sync::{HashMapExt, Lock, Lrc, RwLock};
use crablangc_data_structures::unhash::UnhashMap;
use crablangc_data_structures::unord::UnordSet;
use crablangc_hir::def_id::{CrateNum, DefId, DefIndex, LocalDefId, StableCrateId, LOCAL_CRATE};
use crablangc_hir::definitions::DefPathHash;
use crablangc_index::vec::{Idx, IndexVec};
use crablangc_middle::dep_graph::{DepNodeIndex, SerializedDepNodeIndex};
use crablangc_middle::mir::interpret::{AllocDecodingSession, AllocDecodingState};
use crablangc_middle::mir::{self, interpret};
use crablangc_middle::ty::codec::{RefDecodable, TyDecoder, TyEncoder};
use crablangc_middle::ty::{self, Ty, TyCtxt};
use crablangc_query_system::dep_graph::DepContext;
use crablangc_query_system::query::{QueryCache, QuerySideEffects};
use crablangc_serialize::{
...
use crablangc_span::source_map::{SourceMap, StableSourceFileId};
use crablangc_span::{BytePos, ExpnData, ExpnHash, Pos, SourceFile, Span};
use crablangc_span::{CachingSourceMapView, Symbol};
...
impl<'sess> crablangc_middle::ty::OnDiskCache<'sess> for OnDiskCache<'sess> {
...
impl<'sess> OnDiskCache<'sess> {
    pub fn as_dyn(&self) -> &dyn crablangc_middle::ty::OnDiskCache<'sess> {
        self as _
    }
...
crablangc_middle::implement_ty_decoder!(CacheDecoder<'a, 'tcx>);
...
impl<'a, 'tcx> Decodable<CacheDecoder<'a, 'tcx>> for SyntaxContext {
    fn decode(decoder: &mut CacheDecoder<'a, 'tcx>) -> Self {
        ...
        crablangc_span::hygiene::decode_syntax_context(decoder, decoder.hygiene_context, |this, id| {
...
impl<'a, 'tcx> Decodable<CacheDecoder<'a, 'tcx>> for ExpnId {
    fn decode(decoder: &mut CacheDecoder<'a, 'tcx>) -> Self {
        ...
        let expn_id = if krate == LOCAL_CRATE {
            ...
            let expn_id = crablangc_span::hygiene::register_local_expn_id(data, hash);
...
// copy&paste impl from crablangc_metadata
...
impl<'a, 'tcx> Decodable<CacheDecoder<'a, 'tcx>> for &'tcx [crablangc_ast::InlineAsmTemplatePiece] {
    fn decode(d: &mut CacheDecoder<'a, 'tcx>) -> Self {
        RefDecodable::decode(d)
    }
}
...
impl_ref_decoder! {<'tcx>
    ...
    ty::Variance,
    crablangc_span::def_id::DefId,
    crablangc_span::def_id::LocalDefId,
    (crablangc_middle::middle::exported_symbols::ExportedSymbol<'tcx>, crablangc_middle::middle::exported_symbols::SymbolExportInfo),
...
impl<'a, 'tcx> Encodable<CacheEncoder<'a, 'tcx>> for SyntaxContext {
    fn encode(&self, s: &mut CacheEncoder<'a, 'tcx>) {
        crablangc_span::hygiene::raw_encode_syntax_context(*self, s.hygiene_context, s);
    }
}
