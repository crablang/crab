...
use crablangc_data_structures::sync::{AtomicU64, Lock};
use crablangc_errors::{Diagnostic, Handler};
use crablangc_middle::dep_graph::{
    self, DepKind, DepKindStruct, DepNode, DepNodeIndex, SerializedDepNodeIndex,
};
use crablangc_middle::query::Key;
use crablangc_middle::ty::tls::{self, ImplicitCtxt};
use crablangc_middle::ty::{self, TyCtxt};
use crablangc_query_system::dep_graph::{DepNodeParams, HasDepContext};
...
use crablangc_query_system::{LayoutOfDepth, QueryOverflow};
...
impl<'tcx> HasDepContext for QueryCtxt<'tcx> {
    type DepKind = crablangc_middle::dep_graph::DepKind;
...
impl QueryContext for QueryCtxt<'_> {
    fn next_job_id(self) -> QueryJobId {
        QueryJobId(
            NonZeroU64::new(
                self.queries.jobs.fetch_add(1, crablangc_data_structures::sync::Ordering::Relaxed),
            )
            .unwrap(),
        )
    }
...
impl<'tcx> QueryCtxt<'tcx> {
    ...
    pub fn try_print_query_stack(
        ...
        crablangc_query_system::query::print_query_stack(self, query, handler, num_frames)
    }
}
...
macro_rules! handle_cycle_error {
    ([]) => {{
        crablangc_query_system::HandleCycleError::Error
    }};
    ([(fatal_cycle) $($rest:tt)*]) => {{
        crablangc_query_system::HandleCycleError::Fatal
    }};
    ([(cycle_delay_bug) $($rest:tt)*]) => {{
        crablangc_query_system::HandleCycleError::DelayBug
    }};
...
// invoked by `crablangc_query_append`.
     ...
     $($(#[$attr:meta])*
        ...
        $(impl<'tcx> QueryConfig<QueryCtxt<'tcx>> for queries::$name<'tcx> {
            ...
            fn cache_on_disk(self, tcx: TyCtxt<'tcx>, key: &Self::Key) -> bool {
                ::crablangc_middle::query::cached::$name(tcx, key)
            }
            ...
            fn try_load_from_disk(
                ...
                _qcx: QueryCtxt<'tcx>,
                _key: &Self::Key
            ) -> crablangc_query_system::query::TryLoadFromDisk<QueryCtxt<'tcx>, Self::Value> {
                should_ever_cache_on_disk!([$($modifiers)*] {
                    if ::crablangc_middle::query::cached::$name(_qcx.tcx, _key) {
            ...
            #[inline(always)]
            fn dep_kind(self) -> crablangc_middle::dep_graph::DepKind {
                dep_graph::DepKind::$name
            }

            #[inline(always)]
            fn handle_cycle_error(self) -> crablangc_query_system::HandleCycleError {
                handle_cycle_error!([$($modifiers)*])
            }

            #[inline(always)]
            fn hash_result(self) -> crablangc_query_system::query::HashResult<Self::Value> {
        ...
        mod query_structs {
            ...
            use $crate::profiling_support::QueryKeyStringCache;
            use crablangc_query_system::query::QueryMap;
            use crablangc_middle::dep_graph::DepKind;
            ...
            pub(super) const fn $name<'tcx>() -> QueryStruct<'tcx> { QueryStruct {
                try_collect_active_jobs: |tcx, qmap| {
                    let make_query = |tcx, key| {
                        let kind = crablangc_middle::dep_graph::DepKind::$name;
                        let name = stringify!($name);
                        $crate::plumbing::create_query_frame(tcx, crablangc_middle::query::descs::$name, key, kind, name)
                    };
     ...
     input: ($(([$($modifiers:tt)*] [$($attr:tt)*] [$name:ident]))*)) => {
        ...
        pub struct Queries<'tcx> {
            ...
            $(
                ...
                $name: QueryState<
                    ...
                    crablangc_middle::dep_graph::DepKind,
                >,
            )*
        }
        ...
        impl<'tcx> Queries<'tcx> {
            pub(crate) fn try_collect_active_jobs(
                &'tcx self,
                tcx: TyCtxt<'tcx>,
            ) -> Option<QueryMap<crablangc_middle::dep_graph::DepKind>> {
