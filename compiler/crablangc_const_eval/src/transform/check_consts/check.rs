...
use crablangc_errors::{Diagnostic, ErrorGuaranteed};
use crablangc_hir as hir;
use crablangc_hir::def_id::DefId;
use crablangc_index::bit_set::BitSet;
use crablangc_infer::infer::TyCtxtInferExt;
use crablangc_infer::traits::{ImplSource, Obligation, ObligationCause};
...
use crablangc_middle::ty::{self, adjustment::PointerCast, Instance, InstanceDef, Ty, TyCtxt};
use crablangc_middle::ty::{Binder, TraitRef, TypeVisitableExt};
...
use crablangc_trait_selection::traits::{self, ObligationCauseCode, ObligationCtxt, SelectionContext};
...
type QualifResults<'mir, 'tcx, Q> =
    crablangc_mir_dataflow::ResultsCursor<'mir, 'tcx, FlowSensitiveAnalysis<'mir, 'mir, 'tcx, Q>>;
...
impl<'mir, 'tcx> Checker<'mir, 'tcx> {
    ...
    pub fn check_body(&mut self) {
        ...
        if !tcx.has_attr(def_id, sym::crablangc_do_not_const_check) {
        ...
        let gate = match op.status_in_item(self.ccx) {
            ...
            Status::Unstable(gate) if self.tcx.features().enabled(gate) => {
                let unstable_in_stable = self.ccx.is_const_stable_const_fn()
                    && !super::crablangc_allow_const_fn_unstable(self.tcx, self.def_id(), gate);
        ...
        match &terminator.kind {
            TerminatorKind::Call { func, args, fn_span, from_hir_call, .. } => {
                ...
                if let Some((gate, implied_by)) = is_unstable_const_fn(tcx, callee) {
                    ...
                    // `#[crablangc_allow_const_fn_unstable(the_gate)]`.
                    ...
                    if super::crablangc_allow_const_fn_unstable(tcx, caller, gate) {
                        trace!("crablangc_allow_const_fn_unstable gate active");
                        return;
                    }
                ...
                // have no `crablangc_const_stable` attributes to be const-unstable as well. This
                ...
                if callee_is_unstable_unmarked {
                    ...
                    // use `crablangc_const_(un)stable` attributes to mean that the intrinsic is `const`
