...
use crablangc_middle::ty::{suggest_constraining_type_param, Adt, Closure, FnDef, FnPtr, Param, Ty};
use crablangc_middle::ty::{Binder, TraitRef};
use crablangc_session::parse::feature_err;
use crablangc_span::symbol::sym;
use crablangc_span::{BytePos, Pos, Span, Symbol};
use crablangc_trait_selection::traits::SelectionContext;
                ...
                if Some(trait_id) == ccx.tcx.lang_items().eq_trait() {
                    match (substs[0].unpack(), substs[1].unpack()) {
                                ...
                                && self_ty.peel_refs().is_primitive() =>
                        {
                            ...
                            while let crablangc_middle::ty::Ref(_, inner_ty, _) = tmp_ty.kind() {
                                num_refs += 1;
                                tmp_ty = *inner_ty;
                            }
...
/// A call to an `#[unstable]` const fn or `#[crablangc_const_unstable]` function.
///
