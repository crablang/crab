use crablangc_middle::ty::layout::{LayoutCx, LayoutError, LayoutOf, TyAndLayout, ValidityRequirement};
use crablangc_middle::ty::{ParamEnv, ParamEnvAnd, Ty, TyCtxt};
use crablangc_session::Limit;
use crablangc_target::abi::{Abi, FieldsShape, Scalar, Variants};
...
/// CrabLang UB as long as there is no risk of miscompilations. The `strict_init_checks` can be set to
/// do a full check against CrabLang UB instead (in which case we will also ignore the 0x01-filling and
...
fn might_permit_raw_init_strict<'tcx>(
    ...
    let mut cx = InterpCx::new(tcx, crablangc_span::DUMMY_SP, ParamEnv::reveal_all(), machine);
