...
use crablangc_ast::ast::InlineAsmOptions;
...
use crablangc_target::abi::call::{ArgAbi, ArgAttribute, ArgAttributes, FnAbi, PassMode};
...
impl<'mir, 'tcx: 'mir, M: Machine<'mir, 'tcx>> InterpCx<'mir, 'tcx, M> {
        ...
        match instance.def {
            ...
            | ty::InstanceDef::Item(_) => {
                ...
                let res: InterpResult<'tcx> = try {
                    ...
                    // For where they come from: If the ABI is CrabLangCall, we untuple the
                    // last incoming argument. These two iterators do not have the same type,
                    // so to keep the code paths uniform we accept an allocation
                    // (for CrabLangCall ABI only).
                    let caller_args: Cow<'_, [OpTy<'tcx, M::Provenance>]> =
                        if caller_abi == Abi::CrabLangCall && !args.is_empty() {
                            // Untuple
                ...
                let receiver_place = loop {
                    match receiver.layout.ty.kind() {
                        ty::Ref(..) | ty::RawPtr(..) => {
                            ...
                            // Also see <https://github.com/crablang/miri/issues/2786>.
    ...
    fn drop_in_place(
        ...
        // implementation fail -- a problem shared by crablangc.
