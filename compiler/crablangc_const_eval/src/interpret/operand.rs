...
use crablangc_middle::ty::print::{FmtPrinter, PrettyPrinter};
use crablangc_middle::ty::{ConstInt, Ty, ValTree};
use crablangc_middle::{mir, ty};
use crablangc_span::Span;
use crablangc_target::abi::{self, Abi, Align, HasDataLayout, Size};
...
/// An `Immediate` represents a single immediate self-contained CrabLang value.
///
/// For optimization of a few very common cases, there is also a representation for a pair of
/// primitive values (`ScalarPair`). It allows Miri to avoid making allocations for checked binary
/// operations and wide pointers. This idea was taken from crablangc's codegen.
...
pub struct OpTy<'tcx, Prov: Provenance = AllocId> {
    ...
    pub layout: TyAndLayout<'tcx>,
    /// crablangc does not have a proper way to represent the type of a field of a `repr(packed)` struct:
