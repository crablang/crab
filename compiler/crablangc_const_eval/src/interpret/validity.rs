...
use crablangc_middle::mir::interpret::InterpError;
...
macro_rules! throw_validation_failure {
    ($where:expr, { $( $what_fmt:tt )* } $( expected { $( $expected_fmt:tt )* } )?) => {{
        ...
        let path = crablangc_middle::ty::print::with_no_trimmed_paths!({
            let where_ = &$where;
            if !where_.is_empty() {
...
fn write_path(out: &mut String, path: &[PathElem]) {
    ...
    for elem in path.iter() {
        match elem {
            ...
            // `.<deref>` does not match CrabLang syntax, but it is more readable for long paths -- and
            // some of the other items here also are not CrabLang syntax. Actually we can't
    ...
    fn aggregate_field_path_elem(&mut self, layout: TyAndLayout<'tcx>, field: usize) -> PathElem {
        ...
        match layout.ty.kind() {
            ...
            ty::Closure(def_id, _) | ty::Generator(def_id, _, _) => {
                ...
                // https://github.com/crablang/project-rfc-2229/issues/46
    ...
    for ValidityVisitor<'rt, 'mir, 'tcx, M>
{
    ...
    fn visit_value(&mut self, op: &OpTy<'tcx, M::Provenance>) -> InterpResult<'tcx> {
        ...
        // the fields say (`crablangc_layout_scalar_valid_range_start`).
