//! Contains the logic to lower crablangc types into Chalk types
//!
//! In many cases there is a 1:1 relationship between a crablangc type and a Chalk type.
...
//! [Chalk book](https://crablang.github.io/chalk/book/types/crablang_types.html).
//! The most notable difference lies with `Param`s. To convert from crablangc to
...
//!
//! ## `Region` lowering
//! Regions are handled in crablangc and Chalk is quite differently. In crablangc, there
...
//! regions (Lifetimes) have an associated index. In crablangc, only `BrAnon`s have
...
//! Another difference between crablangc and Chalk lies in the handling of binders.
//! Chalk requires that we store the bound parameter kinds, whereas crablangc does
...
use crablangc_middle::traits::{ChalkEnvironmentAndGoal, ChalkCrabLangInterner as CrabLangInterner};
...
use crablangc_hir::Unsafety;
use std::collections::btree_map::{BTreeMap, Entry};
use std::ops::ControlFlow;

/// Essentially an `Into` with a `&CrabLangInterner` parameter
pub(crate) trait LowerInto<'tcx, T> {
    /// Lower a crablangc construct (e.g., `ty::TraitPredicate`) to a chalk type, consuming `self`.
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> T;
}

impl<'tcx> LowerInto<'tcx, chalk_ir::Substitution<CrabLangInterner<'tcx>>> for SubstsRef<'tcx> {
    fn lower_into(
        self,
...
impl<'tcx> LowerInto<'tcx, SubstsRef<'tcx>> for &chalk_ir::Substitution<CrabLangInterner<'tcx>> {
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> SubstsRef<'tcx> {
        interner
            .tcx
...
impl<'tcx> LowerInto<'tcx, chalk_ir::InEnvironment<chalk_ir::Goal<CrabLangInterner<'tcx>>>>
    for ChalkEnvironmentAndGoal<'tcx>
{
    fn lower_into(
        self,
        interner: CrabLangInterner<'tcx>,
    ) -> chalk_ir::InEnvironment<chalk_ir::Goal<CrabLangInterner<'tcx>>> {
        ...
        let goal: chalk_ir::GoalData<CrabLangInterner<'tcx>> = self.goal.lower_into(interner);
...
impl<'tcx> LowerInto<'tcx, chalk_ir::GoalData<CrabLangInterner<'tcx>>> for ty::Predicate<'tcx> {
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> chalk_ir::GoalData<CrabLangInterner<'tcx>> {
        let (predicate, binders, _named_regions) =
...
impl<'tcx> LowerInto<'tcx, chalk_ir::TraitRef<CrabLangInterner<'tcx>>>
    for crablangc_middle::ty::TraitRef<'tcx>
{
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> chalk_ir::TraitRef<CrabLangInterner<'tcx>> {
        chalk_ir::TraitRef {
...
impl<'tcx> LowerInto<'tcx, chalk_ir::AliasEq<CrabLangInterner<'tcx>>>
    for crablangc_middle::ty::ProjectionPredicate<'tcx>
{
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> chalk_ir::AliasEq<CrabLangInterner<'tcx>> {
...
/*
// FIXME(...): Where do I add this to Chalk? I can't find it in the crablangc repo anywhere.
impl<'tcx> LowerInto<'tcx, chalk_ir::Term<CrabLangInterner<'tcx>>> for crablangc_middle::ty::Term<'tcx> {
  fn lower_into(self, interner: CrabLangInterner<'tcx>) -> chalk_ir::Term<CrabLangInterner<'tcx>> {
    match self {
...
impl<'tcx> LowerInto<'tcx, chalk_ir::Ty<CrabLangInterner<'tcx>>> for Ty<'tcx> {
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> chalk_ir::Ty<CrabLangInterner<'tcx>> {
...
impl<'tcx> LowerInto<'tcx, Ty<'tcx>> for &chalk_ir::Ty<CrabLangInterner<'tcx>> {
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> Ty<'tcx> {
        use chalk_ir::TyKind;
...
impl<'tcx> LowerInto<'tcx, chalk_ir::Lifetime<CrabLangInterner<'tcx>>> for Region<'tcx> {
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> chalk_ir::Lifetime<CrabLangInterner<'tcx>> {
        match *self {
            ty::ReEarlyBound(_) => {
...
impl<'tcx> LowerInto<'tcx, Region<'tcx>> for &chalk_ir::Lifetime<CrabLangInterner<'tcx>> {
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> Region<'tcx> {
...
impl<'tcx> LowerInto<'tcx, chalk_ir::Const<CrabLangInterner<'tcx>>> for ty::Const<'tcx> {
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> chalk_ir::Const<CrabLangInterner<'tcx>> {
...
impl<'tcx> LowerInto<'tcx, ty::Const<'tcx>> for &chalk_ir::Const<CrabLangInterner<'tcx>> {
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> ty::Const<'tcx> {
...
impl<'tcx> LowerInto<'tcx, chalk_ir::GenericArg<CrabLangInterner<'tcx>>> for GenericArg<'tcx> {
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> chalk_ir::GenericArg<CrabLangInterner<'tcx>> {
        match self.unpack() {
...
impl<'tcx> LowerInto<'tcx, ty::subst::GenericArg<'tcx>>
    for &chalk_ir::GenericArg<CrabLangInterner<'tcx>>
{
    fn lower_into(self, interner: CrabLangInterner<'tcx>) -> ty::subst::GenericArg<'tcx> {
        match self.data(interner) {
...
impl<'tcx> LowerInto<'tcx, Option<chalk_ir::QuantifiedWhereClause<CrabLangInterner<'tcx>>>>
...
impl<'tcx> LowerInto<'tcx, chalk_ir::Binders<chalk_ir::QuantifiedWhereClauses<CrabLangInterner<'tcx>>>>
    for &'tcx ty::List<ty::PolyExistentialPredicate<'tcx>>
{
    fn lower_into(
        self,
        interner: CrabLangInterner<'tcx>,
    ) -> chalk_ir::Binders<chalk_ir::QuantifiedWhereClauses<CrabLangInterner<'tcx>>> {
        // `Self` has one binder:
...
impl<'tcx> LowerInto<'tcx, chalk_ir::FnSig<CrabLangInterner<'tcx>>>
    for ty::Binder<'tcx, ty::FnSig<'tcx>>
{
    fn lower_into(self, _interner: CrabLangInterner<'_>) -> FnSig<CrabLangInterner<'tcx>> {
        chalk_ir::FnSig {
            abi: self.abi(),
...
// eventually be removed.
impl<'tcx> LowerInto<'tcx, Option<chalk_solve::crablang_ir::QuantifiedInlineBound<CrabLangInterner<'tcx>>>>
    for ty::Predicate<'tcx>
{
    fn lower_into(
        self,
        interner: CrabLangInterner<'tcx>,
    ) -> Option<chalk_solve::crablang_ir::QuantifiedInlineBound<CrabLangInterner<'tcx>>> {
        let (predicate, binders, _named_regions) =
            collect_bound_vars(interner, interner.tcx, self.kind());
        match predicate {
            ty::PredicateKind::Clause(ty::Clause::Trait(predicate)) => {
                Some(chalk_ir::Binders::new(
                    binders,
                    chalk_solve::crablang_ir::InlineBound::TraitBound(
                        predicate.trait_ref.lower_into(interner),
                    ),
                ))
            }
            ty::PredicateKind::Clause(ty::Clause::Projection(predicate)) => {
                Some(chalk_ir::Binders::new(
                    binders,
                    chalk_solve::crablang_ir::InlineBound::AliasEqBound(predicate.lower_into(interner)),
                ))
            }
...
impl<'tcx> LowerInto<'tcx, chalk_solve::crablang_ir::TraitBound<CrabLangInterner<'tcx>>>
    for ty::TraitRef<'tcx>
{
    fn lower_into(
        self,
        interner: CrabLangInterner<'tcx>,
    ) -> chalk_solve::crablang_ir::TraitBound<CrabLangInterner<'tcx>> {
        chalk_solve::crablang_ir::TraitBound {
            trait_id: chalk_ir::TraitId(self.def_id),
...
impl<'tcx> LowerInto<'tcx, chalk_ir::Mutability> for ast::Mutability {
    fn lower_into(self, _interner: CrabLangInterner<'tcx>) -> chalk_ir::Mutability {
        match self {
            crablangc_ast::Mutability::Mut => chalk_ir::Mutability::Mut,
            crablangc_ast::Mutability::Not => chalk_ir::Mutability::Not,
        }
    }
}

impl<'tcx> LowerInto<'tcx, ast::Mutability> for chalk_ir::Mutability {
    fn lower_into(self, _interner: CrabLangInterner<'tcx>) -> ast::Mutability {
        match self {
...
impl<'tcx> LowerInto<'tcx, chalk_solve::crablang_ir::Polarity> for ty::ImplPolarity {
    fn lower_into(self, _interner: CrabLangInterner<'tcx>) -> chalk_solve::crablang_ir::Polarity {
        match self {
            ty::ImplPolarity::Positive => chalk_solve::crablang_ir::Polarity::Positive,
            ty::ImplPolarity::Negative => chalk_solve::crablang_ir::Polarity::Negative,
            // FIXME(chalk) reservation impls
            ty::ImplPolarity::Reservation => chalk_solve::crablang_ir::Polarity::Negative,
        }
    }
}
impl<'tcx> LowerInto<'tcx, chalk_ir::Variance> for ty::Variance {
    fn lower_into(self, _interner: CrabLangInterner<'tcx>) -> chalk_ir::Variance {
        match self {
...
impl<'tcx> LowerInto<'tcx, chalk_solve::crablang_ir::AliasEqBound<CrabLangInterner<'tcx>>>
    for ty::ProjectionPredicate<'tcx>
{
    fn lower_into(
        self,
        interner: CrabLangInterner<'tcx>,
    ) -> chalk_solve::crablang_ir::AliasEqBound<CrabLangInterner<'tcx>> {
        let (trait_ref, own_substs) = self.projection_ty.trait_ref_and_own_substs(interner.tcx);
        chalk_solve::crablang_ir::AliasEqBound {
...
pub(crate) fn collect_bound_vars<'tcx, T: TypeFoldable<TyCtxt<'tcx>>>(
    ...
    tcx: TyCtxt<'tcx>,
    ty: Binder<'tcx, T>,
) -> (T, chalk_ir::VariableKinds<CrabLangInterner<'tcx>>, BTreeMap<DefId, u32>) {
...
pub(crate) struct BoundVarsCollector<'tcx> {
    ...
    pub(crate) parameters: BTreeMap<u32, chalk_ir::VariableKind<CrabLangInterner<'tcx>>>,
...
pub(crate) struct ParamsSubstitutor<'tcx> {
    tcx: TyCtxt<'tcx>,
    binder_index: ty::DebruijnIndex,
    list: Vec<crablangc_middle::ty::ParamTy>,
    next_ty_placeholder: usize,
    pub(crate) params: crablangc_data_structures::fx::FxHashMap<u32, crablangc_middle::ty::ParamTy>,
...
impl<'tcx> ParamsSubstitutor<'tcx> {
    pub(crate) fn new(tcx: TyCtxt<'tcx>, next_ty_placeholder: usize) -> Self {
        ParamsSubstitutor {
            ...
            list: vec![],
            next_ty_placeholder,
            params: crablangc_data_structures::fx::FxHashMap::default(),
...
pub(crate) struct ReverseParamsSubstitutor<'tcx> {
    tcx: TyCtxt<'tcx>,
    params: crablangc_data_structures::fx::FxHashMap<u32, crablangc_middle::ty::ParamTy>,
}
