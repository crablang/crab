...
) -> &crablangc_data_structures::steal::Steal<Body<'_>> {
...
fn construct_fn<'tcx>(
    ...
    if let DefKind::Closure = tcx.def_kind(fn_def.did) {
        // HACK(eddyb) Avoid having CrabLangCall on closures,
    ...
    body.spread_arg = if abi == Abi::CrabLangCall {
        // CrabLangCall pseudo-ABI untuples the last argument.
...
impl<'a, 'tcx> Builder<'a, 'tcx> {
    fn new(
        ...
        let mut check_overflow = attr::contains_name(attrs, sym::crablangc_inherit_overflow_checks);
...
pub(crate) fn parse_float_into_scalar(
    ...
    match float_ty {
        ty::FloatTy::F32 => {
            let Ok(crablang_f) = num.parse::<f32>() else { return None };
            let mut f = num.parse::<Single>().unwrap_or_else(|e| {
                panic!("apfloat::ieee::Single failed to parse `{}`: {:?}", num, e)
            });

            assert!(
                u128::from(crablang_f.to_bits()) == f.to_bits(),
                "apfloat::ieee::Single gave different result for `{}`: \
                 {}({:#x}) vs CrabLang's {}({:#x})",
                crablang_f,
                f,
                f.to_bits(),
                Single::from_bits(crablang_f.to_bits().into()),
                crablang_f.to_bits()
            );
        ...
        ty::FloatTy::F64 => {
            let Ok(crablang_f) = num.parse::<f64>() else { return None };
            ...
            assert!(
                ...
                f,
                f.to_bits(),
                Double::from_bits(crablang_f.to_bits().into()),
                crablang_f.to_bits()
            );
