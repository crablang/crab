...
use crablangc_middle::thir::{Expr, LintLevel};

use crablangc_span::{DesugaringKind, Span, DUMMY_SP};
    ...
    pub(crate) fn maybe_new_source_scope(
        ...
        let (current_root, parent_root) =
            if self.tcx.sess.opts.unstable_opts.maximal_hir_to_mir_coverage {
                // Some consumers of crablangc need to map MIR locations back to HIR nodes. Currently the
                // the only part of crablangc that tracks MIR -> HIR is the `SourceScopeLocalData::lint_root`
