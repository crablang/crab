...
use crablangc_middle::hir::place::Place as HirPlace;
use crablangc_middle::hir::place::PlaceBase as HirPlaceBase;
use crablangc_middle::hir::place::ProjectionKind as HirProjectionKind;
use crablangc_middle::middle::region;
use crablangc_middle::mir::{self, BinOp, BorrowKind, UnOp};
...
impl<'tcx> Cx<'tcx> {
        ...
        let kind = match expr.kind {
            ...
            hir::ExprKind::Call(ref fun, ref args) => {
                if self.typeck_results().is_method_call(expr) {
                    ...
                    if attrs.iter().any(|a| a.name_or_empty() == sym::crablangc_box) {
                        if attrs.len() != 1 {
                            tcx.sess.emit_err(errors::CrabLangcBoxAttributeError {
                                span: attrs[0].span,
                                reason: errors::CrabLangcBoxAttrReason::Attributes,
                            });
                                ...
                                && let [value] = args
                            {
                                ...
                                tcx.sess.emit_err(errors::CrabLangcBoxAttributeError {
                                    span: expr.span,
                                    reason: errors::CrabLangcBoxAttrReason::NotBoxNew
                                });
                            }
                        } else {
                            tcx.sess.emit_err(errors::CrabLangcBoxAttributeError {
                                span: attrs[0].span,
                                reason: errors::CrabLangcBoxAttrReason::MissingBox,
                            });
                        }
                    }
                    let adt_data =
    ...
    fn convert_captured_hir_place(
        ...
        // The result of capture analysis in `crablangc_hir_analysis/check/upvar.rs`represents a captured path
