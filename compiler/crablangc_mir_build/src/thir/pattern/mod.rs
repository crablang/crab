...
use crablangc_hir::pat_util::EnumerateAndAdjustIterator;
use crablangc_hir::RangeEnd;
use crablangc_index::vec::Idx;
use crablangc_middle::mir::interpret::{
    ConstValue, ErrorHandled, LitToConstError, LitToConstInput, Scalar,
};
use crablangc_middle::mir::{self, UserTypeProjection};
use crablangc_middle::mir::{BorrowKind, Mutability};
use crablangc_middle::thir::{Ascription, BindingMode, FieldPat, LocalVarId, Pat, PatKind, PatRange};
use crablangc_middle::ty::subst::{GenericArg, SubstsRef};
use crablangc_middle::ty::CanonicalUserTypeAnnotation;
use crablangc_middle::ty::{self, AdtDef, ConstKind, Region, Ty, TyCtxt, UserType};
...
impl<'a, 'tcx> PatCtxt<'a, 'tcx> {
    fn lower_pattern(&mut self, pat: &'tcx hir::Pat<'tcx>) -> Box<Pat<'tcx>> {
        ...
        // determined in crablangc_hir_analysis::check::match). The adjustments would be
        //
        ...
        match (end, cmp) {
            ...
            (RangeEnd::Excluded, _) => {
                ...
                if let Some(hir::Expr { kind: hir::ExprKind::Lit(lit), .. }) = lo_expr
                    && let crablangc_ast::ast::LitKind::Int(val, _) = lit.node
                {
...
pub(crate) fn compare_const_vals<'tcx>(
    ...
    match *ty.kind() {
        ty::Float(ty::FloatTy::F32) => {
            let a = crablangc_apfloat::ieee::Single::from_bits(a);
            let b = crablangc_apfloat::ieee::Single::from_bits(b);
            a.partial_cmp(&b)
        }
        ty::Float(ty::FloatTy::F64) => {
            let a = crablangc_apfloat::ieee::Double::from_bits(a);
            let b = crablangc_apfloat::ieee::Double::from_bits(b);
            a.partial_cmp(&b)
        }
        ty::Int(ity) => {
            use crablangc_middle::ty::layout::IntegerExt;
            let size = crablangc_target::abi::Integer::from_int_ty(&tcx, ity).size();
