...
mir_build_unsafe_op_in_unsafe_fn_initializing_type_with_requires_unsafe =
    initializing type with `crablangc_layout_scalar_valid_range` attr is unsafe and requires unsafe
    block (error E0133)
    .note = initializing a layout restricted type's field with a value outside the valid range is undefined behavior
    .label = initializing type with `crablangc_layout_scalar_valid_range` attr
...
mir_build_unsafe_op_in_unsafe_fn_extern_static_requires_unsafe =
    ...
    .note = extern statics are not controlled by the CrabLang type system: invalid data, aliasing violations or data races will cause undefined behavior
...
mir_build_initializing_type_with_requires_unsafe =
    initializing type with `crablangc_layout_scalar_valid_range` attr is unsafe and requires unsafe block
    .note = initializing a layout restricted type's field with a value outside the valid range is undefined behavior
    .label = initializing type with `crablangc_layout_scalar_valid_range` attr

mir_build_initializing_type_with_requires_unsafe_unsafe_op_in_unsafe_fn_allowed =
    initializing type with `crablangc_layout_scalar_valid_range` attr is unsafe and requires unsafe function or block
    .note = initializing a layout restricted type's field with a value outside the valid range is undefined behavior
    .label = initializing type with `crablangc_layout_scalar_valid_range` attr
...
mir_build_pointer_pattern = function pointers and unsized pointers in patterns behave unpredictably and should not be relied upon. See https://github.com/crablang/crablang/issues/70861 for details.
...
mir_build_type_not_structural_more_info = see https://doc.crablang.org/stable/std/marker/trait.StructuralEq.html for details
...
mir_build_more_information = for more information, visit https://doc.crablang.org/book/ch18-02-refutability.html
...
mir_build_crablangc_box_attribute_error = `#[crablangc_box]` attribute used incorrectly
    .attributes = no other attributes may be applied
    .not_box = `#[crablangc_box]` may only be applied to a `Box::new()` call
    .missing_box = `#[crablangc_box]` requires the `owned_box` lang item
