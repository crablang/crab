...
use crablangc_middle::ty::util::{needs_drop_components, AlwaysRequiresDrop};
use crablangc_middle::ty::{self, EarlyBinder, Ty, TyCtxt};
...
fn drop_tys_helper<'tcx>(
    tcx: TyCtxt<'tcx>,
    ty: Ty<'tcx>,
    param_env: crablangc_middle::ty::ParamEnv<'tcx>,
...
fn adt_consider_insignificant_dtor<'tcx>(
    ...
    move |adt_def: ty::AdtDef<'tcx>| {
        let is_marked_insig = tcx.has_attr(adt_def.did(), sym::crablangc_insignificant_dtor);
        if is_marked_insig {
            // In some cases like `std::collections::HashMap` where the struct is a wrapper around
            // a type that is a Drop type, and the wrapped type (eg: `hashbrown::HashMap`) lies
            // outside stdlib, we might choose to still annotate the wrapper (std HashMap) with
            // `crablangc_insignificant_dtor`, even if the type itself doesn't have a `Drop` impl.
            ...
            // No destructor found nor the type is annotated with `crablangc_insignificant_dtor`, we
