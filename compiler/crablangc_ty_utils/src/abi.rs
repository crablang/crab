...
use crablangc_middle::ty::{self, InstanceDef, Ty, TyCtxt};
use crablangc_session::config::OptLevel;
use crablangc_span::def_id::DefId;
use crablangc_target::abi::call::{
    ArgAbi, ArgAttribute, ArgAttributes, ArgExtension, Conv, FnAbi, PassMode, Reg, RegKind,
};
use crablangc_target::abi::*;
use crablangc_target::spec::abi::Abi as SpecAbi;
...
fn fn_sig_for_fn_abi<'tcx>(
    ...
    if let InstanceDef::ThreadLocalShim(..) = instance.def {
        return ty::Binder::dummy(tcx.mk_fn_sig(
            ...
            false,
            hir::Unsafety::Normal,
            crablangc_target::spec::abi::Abi::Unadjusted,
        ));
    }
        ...
        ty::Generator(did, substs, _) => {
            ...
            ty::Binder::bind_with_vars(
                tcx.mk_fn_sig(
                    ...
                    false,
                    hir::Unsafety::Normal,
                    crablangc_target::spec::abi::Abi::CrabLang,
                ),
                bound_vars,
            )
        }
...
fn conv_from_spec_abi(tcx: TyCtxt<'_>, abi: SpecAbi) -> Conv {
    ...
    match tcx.sess.target.adjust_abi(abi) {
        CrabLangIntrinsic | PlatformIntrinsic | CrabLang | CrabLangCall => Conv::CrabLang,
        CrabLangCold => Conv::CrabLangCold,
...
// Handle safe CrabLang thin and fat pointers.
fn adjust_for_crablang_scalar<'tcx>(
    ...
    if let Some(pointee) = layout.pointee_info_at(&cx, offset) {
        if let Some(kind) = pointee.safe {
            ...
            // <https://github.com/crablang/crablang/pull/98017>), and for mutable references to
            // potentially self-referential types (see
            // <https://github.com/crablang/unsafe-code-guidelines/issues/381>). If LLVM had a way
            ...
            // See https://github.com/crablang/unsafe-code-guidelines/issues/326
            ...
            // (see <https://github.com/crablang/unsafe-code-guidelines/issues/385#issuecomment-1368055745>).
...
fn fn_abi_new_uncached<'tcx>(
    ...
    let extra_args = if sig.abi == CrabLangCall {
        ...
        if let Some(input) = sig.inputs().last() {
            if let ty::Tuple(tupled_arguments) = input.kind() {
                ...
                bug!(
                    "argument to function with \"crablang-call\" ABI \
                        is not a tuple"
                );
            }
        } else {
    ...
    use SpecAbi::*;
    let crablang_abi = matches!(sig.abi, CrabLangIntrinsic | PlatformIntrinsic | CrabLang | CrabLangCall);
    ...
    let arg_of = |ty: Ty<'tcx>, arg_idx: Option<usize>| -> Result<_, FnAbiError<'tcx>> {
        ...
        let mut arg = ArgAbi::new(cx, layout, |layout, scalar, offset| {
            ...
            adjust_for_crablang_scalar(*cx, &mut attrs, scalar, *layout, offset, is_return);
            attrs
        });
...
fn fn_abi_adjust_for_abi<'tcx>(
    ...
    if abi == SpecAbi::CrabLang
        || abi == SpecAbi::CrabLangCall
        || abi == SpecAbi::CrabLangIntrinsic
