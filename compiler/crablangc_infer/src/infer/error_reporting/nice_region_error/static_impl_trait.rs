...
use crablangc_errors::{AddToDiagnostic, Applicability, Diagnostic, ErrorGuaranteed, MultiSpan};
...
pub fn suggest_new_region_bound(
    tcx: TyCtxt<'_>,
    err: &mut Diagnostic,
    fn_returns: Vec<&crablangc_hir::Ty<'_>>,
                ...
                if let Some(span) = opaque.bounds.iter().find_map(|arg| match arg {
                    ...
                    if let Some(id) = scope_def_id
                        ...
                        && let mut spans_suggs = generics
                            ...
                            .map(|p|
                                  if p.span.hi() - p.span.lo() == crablangc_span::BytePos(1) { // Ampersand (elided without '_)
