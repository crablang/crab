...
use crablangc_errors::{pluralize, struct_span_err, Diagnostic, ErrorGuaranteed, IntoDiagnosticArg};
use crablangc_errors::{Applicability, DiagnosticBuilder, DiagnosticStyledString};
...
use crablangc_middle::dep_graph::DepContext;
use crablangc_middle::ty::print::with_forced_trimmed_paths;
use crablangc_middle::ty::relate::{self, RelateResult, TypeRelation};
use crablangc_middle::ty::{
    self, error::TypeError, List, Region, Ty, TyCtxt, TypeFoldable, TypeSuperVisitable,
    TypeVisitable, TypeVisitableExt,
};
use crablangc_span::{sym, symbol::kw, BytePos, DesugaringKind, Pos, Span};
...
impl<'tcx> TypeErrCtxt<'_, 'tcx> {
    ...
    fn check_and_note_conflicting_crates(&self, err: &mut Diagnostic, terr: TypeError<'tcx>) {
        ...
        use crablangc_hir::definitions::DisambiguatedDefPathData;
    ...
    fn cmp_fn_sig(
        ...
        // unsafe extern "C" for<'a> fn(&'a T) -> &'a T
        //        ^^^^^^^^^^
        if sig1.abi != abi::Abi::CrabLang {
            values.0.push(format!("extern {} ", sig1.abi), sig1.abi != sig2.abi);
        }
        if sig2.abi != abi::Abi::CrabLang {
