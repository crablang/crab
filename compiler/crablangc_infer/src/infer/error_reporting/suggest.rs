...
use crablangc_middle::ty::{self as ty, IsSuggestable, Ty, TypeVisitableExt};
...
impl<'tcx> TypeErrCtxt<'_, 'tcx> {
    ...
    // FIXME: Remove once `crablangc_hir_typeck` is migrated to diagnostic structs
...
impl<'tcx> TypeErrCtxt<'_, 'tcx> {
    ...
    pub fn could_remove_semicolon(
        ...
        let span = if last_stmt.span.from_expansion() {
            let mac_call = crablangc_span::source_map::original_sp(last_stmt.span, blk.span);
