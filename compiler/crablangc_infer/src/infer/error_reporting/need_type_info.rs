...
use crablangc_errors::IntoDiagnostic;
use crablangc_errors::{DiagnosticBuilder, ErrorGuaranteed, IntoDiagnosticArg};
use crablangc_hir as hir;
use crablangc_hir::def::Res;
use crablangc_hir::def::{CtorOf, DefKind, Namespace};
use crablangc_hir::def_id::{DefId, LocalDefId};
use crablangc_hir::intravisit::{self, Visitor};
use crablangc_hir::{Body, Closure, Expr, ExprKind, FnRetTy, HirId, Local, LocalSource};
...
use crablangc_middle::ty::print::{FmtPrinter, PrettyPrinter, Print, Printer};
use crablangc_middle::ty::{self, InferConst};
use crablangc_middle::ty::{GenericArg, GenericArgKind, SubstsRef};
use crablangc_middle::ty::{IsSuggestable, Ty, TyCtxt, TypeckResults};
...
impl Into<crablangc_errors::DiagnosticId> for TypeAnnotationNeeded {
    fn into(self) -> crablangc_errors::DiagnosticId {
        match self {
            Self::E0282 => crablangc_errors::error_code!(E0282),
            Self::E0283 => crablangc_errors::error_code!(E0283),
            Self::E0284 => crablangc_errors::error_code!(E0284),
        }
    }
}
...
impl IntoDiagnosticArg for UnderspecifiedArgKind {
    fn into_diagnostic_arg(self) -> crablangc_errors::DiagnosticArgValue<'static> {
        ...
        crablangc_errors::DiagnosticArgValue::Str(kind.into())
    }
}
...
fn ty_to_string<'tcx>(
    ...
    match (ty.kind(), called_method_def_id) {
        ...
        //
        // We do have to hide the `extern "crablang-call"` ABI in that case though,
...
/// doesn't work as they actually use the "crablang-call" API.
