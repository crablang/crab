...
use crablangc_ast::TraitObjectSyntax;
...
use crablangc_hir::def::{CtorOf, DefKind, Namespace, Res};
use crablangc_hir::def_id::{DefId, LocalDefId};
use crablangc_hir::intravisit::{walk_generics, Visitor as _};
use crablangc_hir::{GenericArg, GenericArgs, OpaqueTyOrigin};
...
use crablangc_middle::middle::stability::AllowUnstable;
use crablangc_middle::ty::fold::FnMutDelegate;
use crablangc_middle::ty::subst::{self, GenericArgKind, InternalSubsts, SubstsRef};
use crablangc_middle::ty::GenericParamDefKind;
use crablangc_middle::ty::{self, Const, IsSuggestable, Ty, TyCtxt, TypeVisitableExt};
use crablangc_middle::ty::{DynKind, ToPredicate};
use crablangc_session::lint::builtin::{AMBIGUOUS_ASSOCIATED_ITEMS, BARE_TRAIT_OBJECTS};
...
use crablangc_span::{sym, Span, DUMMY_SP};
use crablangc_target::spec::abi;
use crablangc_trait_selection::traits::error_reporting::{
    report_object_safety_error, suggestions::NextTypeParamName,
};
use crablangc_trait_selection::traits::wf::object_region_bounds;
use crablangc_trait_selection::traits::{self, astconv_object_safety_violations, ObligationCtxt};
...
impl<'o, 'tcx> dyn AstConv<'tcx> + 'o {
        ...
        if regular_traits.len() > 1 {
            ...
            err.note(
                "auto-traits like `Send` and `Sync` are traits that have special properties; \
                 ...
                 <https://doc.crablang.org/reference/special-types-and-traits.html#auto-traits>",
            );
            err.emit();
        }
        ...
        let existential_trait_refs = regular_traits.iter().map(|i| {
            i.trait_ref().map_bound(|trait_ref: ty::TraitRef<'tcx>| {
                ...
                if references_self {
                    ...
                    err.note(
                        crablangc_middle::traits::ObjectSafetyViolation::SupertraitSelf(smallvec![])
                            .error_msg(),
                    );
                    err.emit();
                }
    ...
    fn check_assoc_ty(
        ...
        if !tcx.visibility(item).is_accessible_from(def_scope, tcx) {
            ...
            tcx.sess
                .struct_span_err_with_code(span, &msg, crablangc_errors::error_code!(E0624))
        ...
        if !self.allow_ty_infer() && !(visitor.0.is_empty() && infer_replacements.is_empty()) {
            ...
            if !infer_replacements.is_empty() {
                diag.multipart_suggestion(
                    ...
                    crablangc_errors::pluralize!(infer_replacements.len()),
                ),
                    infer_replacements,
                    Applicability::MachineApplicable,
                );
    ...
    fn maybe_lint_bare_trait(&self, self_ty: &hir::Ty<'_>, in_path: bool) {
            ...
            self_ty.kind
        {
            ...
            if self_ty.span.edition() >= Edition::Edition2021 {
                ...
                let mut diag =
                    crablangc_errors::struct_span_err!(tcx.sess, self_ty.span, E0782, "{}", msg);
