...
use crablangc_errors::{error_code, struct_span_err};
...
fn enforce_trait_manually_implementable(
    ...
    // Disallow *all* explicit impls of traits marked `#[crablangc_deny_explicit_impl]`
    if tcx.has_attr(trait_def_id, sym::crablangc_deny_explicit_impl) {
        ...
        tcx.trait_def(trait_def_id).specialization_kind
    {
        if !tcx.features().specialization && !tcx.features().min_specialization {
            tcx.sess
                .struct_span_err(
                    impl_header_span,
                    "implementing `crablangc_specialization_trait` traits is unstable",
                )
