...
use crablangc_middle::ty::fast_reject::{simplify_type, SimplifiedType, TreatParams};
use crablangc_middle::ty::{self, CrateInherentImpls, Ty, TyCtxt};
...
const ADD_ATTR_TO_TY: &str = "alternatively add `#[crablangc_has_incoherent_inherent_impls]` to the type \
     and `#[crablangc_allow_incoherent_impl]` to the relevant impl items";
const ADD_ATTR: &str =
    "alternatively add `#[crablangc_allow_incoherent_impl]` to the relevant impl items";
...
impl<'tcx> InherentCollect<'tcx> {
    fn check_def_id(&mut self, impl_def_id: LocalDefId, self_ty: Ty<'tcx>, ty_def_id: DefId) {
        ...
        if self.tcx.features().crablangc_attrs {
            let items = self.tcx.associated_item_def_ids(impl_def_id);

            if !self.tcx.has_attr(ty_def_id, sym::crablangc_has_incoherent_inherent_impls) {
            ...
            for &impl_item in items {
                if !self.tcx.has_attr(impl_item, sym::crablangc_allow_incoherent_impl) {
                    let impl_span = self.tcx.def_span(impl_def_id);
                    struct_span_err!(
                        self.tcx.sess,
                        impl_span,
    ...
    fn check_primitive_impl(&mut self, impl_def_id: LocalDefId, ty: Ty<'tcx>) {
        let items = self.tcx.associated_item_def_ids(impl_def_id);
        if !self.tcx.hir().crablangc_coherence_is_core() {
            if self.tcx.features().crablangc_attrs {
                for &impl_item in items {
                    if !self.tcx.has_attr(impl_item, sym::crablangc_allow_incoherent_impl) {
