...
use crablangc_data_structures::fx::{FxHashMap, FxHashSet, FxIndexSet};
use crablangc_errors::{pluralize, struct_span_err, Applicability, DiagnosticBuilder, ErrorGuaranteed};
use crablangc_hir as hir;
use crablangc_hir::def_id::{DefId, LocalDefId};
use crablangc_hir::lang_items::LangItem;
use crablangc_hir::ItemKind;
use crablangc_infer::infer::outlives::env::{OutlivesEnvironment, RegionBoundPairs};
use crablangc_infer::infer::outlives::obligations::TypeOutlives;
use crablangc_infer::infer::{self, InferCtxt, TyCtxtInferExt};
use crablangc_middle::mir::ConstraintCategory;
use crablangc_middle::ty::query::Providers;
use crablangc_middle::ty::trait_def::TraitSpecializationKind;
use crablangc_middle::ty::{
    self, AdtKind, GenericParamDefKind, Ty, TyCtxt, TypeFoldable, TypeSuperVisitable,
    TypeVisitable, TypeVisitableExt, TypeVisitor,
};
use crablangc_middle::ty::{GenericArgKind, InternalSubsts};
...
use crablangc_trait_selection::traits::outlives_bounds::InferCtxtExt as _;
...
/// ```crablang,ignore (this code fails due to this lint)
/// trait IntoIter {
        ...
        if !unsatisfied_bounds.is_empty() {
            ...
            err.note(
                "we are soliciting feedback, see issue #87479 \
                 <https://github.com/crablang/crablang/issues/87479> \
...
fn region_known_to_outlive<'tcx>(
    ...
    resolve_regions_with_wf_tys(tcx, id, param_env, &wf_tys, |mut infcx, _| {
        use crablangc_infer::infer::outlives::obligations::TypeOutlivesDelegate;
...
fn check_impl<'tcx>(
    ...
    enter_wf_checking_ctxt(tcx, item.span, item.owner_id.def_id, |wfcx| {
        match ast_trait_ref {
            Some(ast_trait_ref) => {
                // `#[crablangc_reservation_impl]` impls are not real impls and
...
fn check_fn_or_method<'tcx>(
    ...
    if sig.abi == Abi::CrabLangCall {
        ...
        if let Some(ty) = inputs.next() {
            wfcx.register_bound(
                ObligationCause::new(span, wfcx.body_def_id, ObligationCauseCode::CrabLangCall),
                wfcx.param_env,
                *ty,
            ...
            tcx.sess.span_err(
                ...
                "functions with the \"crablang-call\" ABI must take a single non-self tuple argument",
            );
        }
...
fn report_bivariance(
    tcx: TyCtxt<'_>,
    param: &crablangc_hir::GenericParam<'_>,
