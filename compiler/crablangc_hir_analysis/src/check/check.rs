...
use crablangc_errors::{Applicability, ErrorGuaranteed, MultiSpan};
...
use crablangc_hir::{ItemKind, Node, PathSegment};
use crablangc_infer::infer::opaque_types::ConstrainOpaqueTypeRegionVisitor;
use crablangc_infer::infer::outlives::env::OutlivesEnvironment;
use crablangc_infer::infer::{DefiningAnchor, RegionVariableOrigin, TyCtxtInferExt};
use crablangc_infer::traits::{Obligation, TraitEngineExt as _};
use crablangc_lint::builtin::REPR_TRANSPARENT_EXTERNAL_PRIVATE_FIELDS;
use crablangc_middle::hir::nested_filter;
use crablangc_middle::middle::stability::EvalResult;
use crablangc_middle::ty::layout::{LayoutError, MAX_SIMD_LANES};
...
use crablangc_session::lint::builtin::{UNINHABITED_STATIC, UNSUPPORTED_CALLING_CONVENTIONS};
...
use crablangc_trait_selection::traits::error_reporting::on_unimplemented::OnUnimplementedDirective;
use crablangc_trait_selection::traits::error_reporting::TypeErrCtxtExt as _;
use crablangc_trait_selection::traits::{self, ObligationCtxt, TraitEngine, TraitEngineExt as _};
...
fn check_opaque(tcx: TyCtxt<'_>, id: hir::ItemId) {
    ...
    // Since crablangdoc doesn't care about the concrete type behind `impl Trait`, just don't look at it!
            ...
            for &assoc_item in assoc_items.in_definition_order() {
                match assoc_item.kind {
                    ...
                    ty::AssocKind::Type if assoc_item.defaultness(tcx).has_value() => {
                        ...
                        let _: Result<_, crablangc_errors::ErrorGuaranteed> = check_type_bounds(
                            tcx,
                            assoc_item,
                            assoc_item,
...
fn check_impl_items_against_trait<'tcx>(
    ...
    if let Ok(ancestors) = trait_def.ancestors(tcx, impl_id.to_def_id()) {
        ...
        if let Some(missing_items) = must_implement_one_of {
            let attr_span = tcx
                .get_attr(impl_trait_ref.def_id, sym::crablangc_must_implement_one_of)
                .map(|attr| attr.span);
        ...
        let (span, display_discr) = match var.discr {
            ty::VariantDiscr::Explicit(discr_def_id) => {
                ...
                if let hir::Node::AnonConst(expr) = tcx.hir().get_by_def_id(discr_def_id.expect_local())
                    ...
                    && let crablangc_ast::LitKind::Int(lit_value, _int_kind) = &lit.node
                    && *lit_value != dis.val
                {
