...
use crablangc_errors::{pluralize, struct_span_err, Applicability, Diagnostic, DiagnosticBuilder};
...
use crablangc_span::{self, BytePos, Span, Symbol};
use crablangc_target::abi::VariantIdx;
use crablangc_target::spec::abi::Abi;
use crablangc_trait_selection::traits::error_reporting::suggestions::ReturnsVisitor;
...
/// Forbid defining intrinsics in CrabLang code,
/// as they must always be defined by the compiler.
// FIXME: Move this to a more appropriate place.
pub fn fn_maybe_err(tcx: TyCtxt<'_>, sp: Span, abi: Abi) {
    if let Abi::CrabLangIntrinsic | Abi::PlatformIntrinsic = abi {
        tcx.sess.span_err(sp, "intrinsic must be in `extern \"crablang-intrinsic\" { ... }` block");
    }
}
...
fn default_body_is_unstable(
    ...
    crablangc_session::parse::add_feature_diagnostics_for_issue(
        &mut err,
        &tcx.sess.parse_sess,
        feature,
        crablangc_feature::GateIssue::Library(issue),
    );
    err.emit();
}
