//! Type-checking for the crablang-intrinsic and platform-intrinsic
...
use crablangc_errors::{struct_span_err, DiagnosticMessage};
use crablangc_hir as hir;
use crablangc_middle::traits::{ObligationCause, ObligationCauseCode};
...
pub fn intrinsic_operation_unsafety(tcx: TyCtxt<'_>, intrinsic_id: DefId) -> hir::Unsafety {
    let has_safe_attr = match tcx.has_attr(intrinsic_id, sym::crablangc_safe_intrinsic) {
...
/// Remember to add all intrinsics here, in `compiler/crablangc_codegen_llvm/src/intrinsic.rs`,
...
pub fn check_intrinsic_type(tcx: TyCtxt<'_>, it: &hir::ForeignItem<'_>) {
    ...
    let (n_tps, n_lts, inputs, output, unsafety) = if name_str.starts_with("atomic_") {
        ...
        let (n_tps, inputs, output) = match intrinsic_name {
            ...
            sym::crablangc_peek => (1, vec![param(0)], param(0)),
    ...
    let sig = tcx.mk_fn_sig(inputs, output, false, unsafety, Abi::CrabLangIntrinsic);
