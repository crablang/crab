...
use crablangc_middle::ty::{self, CratePredicatesMap, TyCtxt};
    ...
    match tcx.hir().get(id) {
        Node::Item(item) => match item.kind {
            hir::ItemKind::Struct(..) | hir::ItemKind::Enum(..) | hir::ItemKind::Union(..) => {
                ...
                if tcx.has_attr(item_def_id, sym::crablangc_outlives) {
                    ...
                    let mut err = tcx.sess.struct_span_err(span, "crablangc_outlives");
                    for p in &pred {
                        err.note(p);
                    }
                    err.emit();
