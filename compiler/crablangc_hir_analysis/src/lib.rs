...
use crablangc_middle::util;
use crablangc_session::{config::EntryFnType, parse::feature_err};
use crablangc_span::def_id::{DefId, LocalDefId, CRATE_DEF_ID};
use crablangc_span::{symbol::sym, Span, DUMMY_SP};
use crablangc_target::spec::abi::Abi;
use crablangc_trait_selection::traits::error_reporting::TypeErrCtxtExt as _;
use crablangc_trait_selection::traits::{self, ObligationCause, ObligationCauseCode, ObligationCtxt};
...
fn check_main_fn_ty(tcx: TyCtxt<'_>, main_def_id: DefId) {
    ...
    if !tcx.codegen_fn_attrs(main_def_id).target_features.is_empty()
        ...
        && !tcx.sess.opts.actually_crablangdoc
    {
    ...
    let se_ty = tcx.mk_fn_ptr(expected_return_type.map_bound(|expected_return_type| {
        tcx.mk_fn_sig([], expected_return_type, false, hir::Unsafety::Normal, Abi::CrabLang)
    }));
...
pub fn check_crate(tcx: TyCtxt<'_>) -> Result<(), ErrorGuaranteed> {
    ...
    // NOTE: This is copy/pasted in libcrablangdoc/core.rs and should be kept in sync.
...
/// A quasi-deprecated helper used in crablangdoc and clippy to get
