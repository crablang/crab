...
//! details of each item in CrabLang. Collection is specifically concerned
...
use crablangc_errors::{Applicability, DiagnosticBuilder, ErrorGuaranteed, StashKey};
use crablangc_hir as hir;
use crablangc_hir::def_id::{DefId, LocalDefId};
use crablangc_hir::intravisit::{self, Visitor};
use crablangc_hir::{GenericParamKind, Node};
use crablangc_infer::infer::{InferCtxt, TyCtxtInferExt};
use crablangc_infer::traits::ObligationCause;
use crablangc_middle::hir::nested_filter;
use crablangc_middle::ty::query::Providers;
use crablangc_middle::ty::util::{Discr, IntTypeExt};
use crablangc_middle::ty::{self, AdtKind, Const, IsSuggestable, ToPredicate, Ty, TyCtxt};
...
use crablangc_trait_selection::traits::error_reporting::suggestions::NextTypeParamName;
...
///
/// [`InferCtxt`]: crablangc_infer::infer::InferCtxt
///
/// # Trait predicates
///
...
fn trait_def(tcx: TyCtxt<'_>, def_id: LocalDefId) -> ty::TraitDef {
    ...
    let paren_sugar = tcx.has_attr(def_id, sym::crablangc_paren_sugar);
    if paren_sugar && !tcx.features().unboxed_closures {
        tcx.sess.emit_err(errors::ParenSugarAttribute { span: item.span });
    }

    let is_marker = tcx.has_attr(def_id, sym::marker);
    let crablangc_coinductive = tcx.has_attr(def_id, sym::crablangc_coinductive);
    let skip_array_during_method_dispatch =
        tcx.has_attr(def_id, sym::crablangc_skip_array_during_method_dispatch);
    let specialization_kind = if tcx.has_attr(def_id, sym::crablangc_unsafe_specialization_marker) {
        ty::trait_def::TraitSpecializationKind::Marker
    } else if tcx.has_attr(def_id, sym::crablangc_specialization_trait) {
        ty::trait_def::TraitSpecializationKind::AlwaysApplicable
    } else {
        ty::trait_def::TraitSpecializationKind::None
    };
    let must_implement_one_of = tcx
        .get_attr(def_id, sym::crablangc_must_implement_one_of)
        // Check that there are at least 2 arguments of `#[crablangc_must_implement_one_of]`
        ...
        .and_then(|attr| match attr.meta_item_list() {
            ...
            Some(items) => items
                ...
                .ok()
                .zip(Some(attr.span)),
            // Error is reported by `crablangc_attr!`
            None => None,
        })
        // Check that all arguments of `#[crablangc_must_implement_one_of]` reference
    ...
    ty::TraitDef {
        ...
        is_marker,
        is_coinductive: crablangc_coinductive || is_auto,
            ...
            if ret_ty.is_closure() {
                diag.note(
                    "for more information on `Fn` traits and closure types, see \
                     https://doc.crablang.org/book/ch13-01-closures.html",
                );
            }
            diag.emit();
...
fn impl_polarity(tcx: TyCtxt<'_>, def_id: LocalDefId) -> ty::ImplPolarity {
    let is_crablangc_reservation = tcx.has_attr(def_id, sym::crablangc_reservation_impl);
    let item = tcx.hir().expect_item(def_id);
    match &item.kind {
...
fn compute_sig_of_foreign_fn_decl<'tcx>(
    ...
    let unsafety = if abi == abi::Abi::CrabLangIntrinsic {
    ...
    if abi != abi::Abi::CrabLangIntrinsic
...
fn generator_kind(tcx: TyCtxt<'_>, def_id: LocalDefId) -> Option<hir::GeneratorKind> {
    match tcx.hir().get_by_def_id(def_id) {
        Node::Expr(&crablangc_hir::Expr {
            kind: crablangc_hir::ExprKind::Closure(&crablangc_hir::Closure { body, .. }),
            ..
