...
use crablangc_data_structures::stable_hasher::ToStableHashKey;
...
/// An attribute that is not a macro; e.g., `#[inline]` or `#[crablangfmt::skip]`.
#[derive(Clone, Copy, PartialEq, Eq, Encodable, Decodable, Hash, Debug)]
#[derive(HashStable_Generic)]
pub enum NonMacroAttrKind {
    /// Single-segment attribute defined by the language (`#[inline]`)
    Builtin(Symbol),
    /// Multi-segment custom attribute living in a "tool module" (`#[crablangfmt::skip]`).
    Tool,
...
pub enum DefKind {
    ...
    ///
    /// [RFC 2593]: https://github.com/crablang/rfcs/pull/2593
...
///
/// For every path or identifier in CrabLang, the compiler must determine
...
pub enum Res<Id = hir::HirId> {
    ...
    /// A tool attribute module; e.g., the `crablangfmt` in `#[crablangfmt::skip]`.
    ///
    /// **Belongs to the type namespace.**
    ToolMod,
    ...
    /// E.g., `#[inline]` and `#[crablangfmt::skip]`, which are essentially directives,
    /// as opposed to `#[test]`, which is a builtin macro.
    ///
    /// **Belongs to the macro namespace.**
    NonMacroAttr(NonMacroAttrKind), // e.g., `#[inline]` or `#[crablangfmt::skip]`
...
pub enum Namespace {
    ...
    /// like `#[inline]` and `#[crablangfmt::skip]`.
    MacroNS,
}
