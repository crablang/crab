...
use crablangc_hir::{InlineAsmOperand, ItemId};
use crablangc_session::config::{OutputFilenames, OutputType};

use crate::prelude::*;

pub(crate) fn codegen_global_asm_item(tcx: TyCtxt<'_>, global_asm: &mut String, item_id: ItemId) {
    let item = tcx.hir().item(item_id);
    if let crablangc_hir::ItemKind::GlobalAsm(asm) = item.kind {
                ...
                InlineAsmTemplatePiece::Placeholder { operand_idx, modifier: _, span: op_sp } => {
                    match asm.operands[operand_idx].0 {
                        InlineAsmOperand::Const { ref anon_const } => {
                            ...
                            let string = crablangc_codegen_ssa::common::asm_const_to_str(
                                tcx,
                                op_sp,
                                const_value,
...
pub(crate) fn compile_global_asm(
    ...
    if !config.asm_enabled {
        if global_asm.contains("__crablang_probestack") {
            return Ok(None);
        }
        ...
        if cfg!(not(feature = "inline_asm")) {
            return Err(
                "asm! and global_asm! support is disabled while compiling crablangc_codegen_cranelift"
                    .to_owned(),
            );
        } else {
