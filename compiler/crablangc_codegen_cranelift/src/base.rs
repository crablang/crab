...
pub(crate) fn codegen_fn<'tcx>(
    ...
    let _mir_guard = crate::PrintOnPanic(|| {
        ...
        with_no_trimmed_paths!({
            crablangc_middle::mir::pretty::write_mir_fn(tcx, mir, &mut |_, _| Ok(()), &mut buf)
                .unwrap();
        });
                ...
                if let TerminatorKind::Return = fx.mir[*target].terminator().kind {
                    ...
                    for stmt in &fx.mir[*target].statements {
                        if let StatementKind::StorageDead(_) = stmt.kind {
                            ...
                            // FIXME Can sometimes happen, see crablang/crablang#70531
            ...
            TerminatorKind::Assert { cond, expected, msg, target, unwind: _ } => {
                ...
                match msg {
                    AssertKind::BoundsCheck { ref len, ref index } => {
                        ...
                        codegen_panic_inner(
                            fx,
                            crablangc_hir::LangItem::PanicBoundsCheck,
                            &[index, len, location],
                            source_info.span,
                        );
                    }
    ...
    match &stmt.kind {
        ...
        StatementKind::Intrinsic(ref intrinsic) => match &**intrinsic {
            ...
            NonDivergingIntrinsic::CopyNonOverlapping(mir::CopyNonOverlapping {
                ...
                let pointee = dst
                    .layout()
                    .pointee_info_at(fx, crablangc_target::abi::Size::ZERO)
...
pub(crate) fn codegen_panic<'tcx>(
    ...
    codegen_panic_inner(fx, crablangc_hir::LangItem::Panic, &args, source_info.span);
}
...
pub(crate) fn codegen_panic_nounwind<'tcx>(
    ...
    codegen_panic_inner(fx, crablangc_hir::LangItem::PanicNounwind, &args, source_info.span);
}
...
pub(crate) fn codegen_panic_cannot_unwind<'tcx>(
    ...
    codegen_panic_inner(fx, crablangc_hir::LangItem::PanicCannotUnwind, &args, source_info.span);
}

fn codegen_panic_inner<'tcx>(
    fx: &mut FunctionCx<'_, '_, 'tcx>,
    lang_item: crablangc_hir::LangItem,
    args: &[Value],
    span: Span,
) {
    let def_id = fx
