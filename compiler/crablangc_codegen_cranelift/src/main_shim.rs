...
use crablangc_session::config::{sigpipe, EntryFnType};
use crablangc_span::symbol::Ident;

use crate::prelude::*;

/// Create the `main` function which will initialize the crablang runtime and call
/// users main function.
pub(crate) fn maybe_create_entry_wrapper(
    ...
    fn create_entry_fn(
        ...
        let main_ret_ty = tcx.fn_sig(crablang_main_def_id).no_bound_vars().unwrap().output();
        ...
        let instance = Instance::mono(tcx, crablang_main_def_id).polymorphize(tcx);
