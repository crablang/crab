...
use crablangc_target::abi::call::{Conv, FnAbi};
...
pub(crate) fn conv_to_call_conv(sess: &Session, c: Conv, default_call_conv: CallConv) -> CallConv {
    match c {
        Conv::CrabLang | Conv::C => default_call_conv,
        Conv::CrabLangCold => CallConv::Cold,
    ...
    pub(crate) fn lib_call(
        ...
        if self.tcx.sess.target.is_like_windows {
                ...
                params
                    ...
                    .map(|(param, &arg)| {
                        if param.value_type == types::I128 {
                            ...
                            arg_ptr.store(self, arg, MemFlags::tcrablanged());
            ...
            if indirect_ret_val {
                ...
                return Cow::Owned(vec![ret_ptr.load(self, types::I128, MemFlags::tcrablanged())]);
            } else {
...
fn make_local_place<'tcx>(
    ...
    let place = if is_ssa {
        if let crablangc_target::abi::Abi::ScalarPair(_, _) = layout.abi {
...
pub(crate) fn codegen_fn_prelude<'tcx>(fx: &mut FunctionCx<'_, '_, 'tcx>, start_block: Block) {
    ...
    let func_params = fx
        ...
        .map(|local| {
            ...
            // Adapted from https://github.com/crablang/crablang/blob/145155dc96757002c7b2e9de8489416e2fdbbd57/src/libcrablangc_codegen_llvm/mir/mod.rs#L442-L482
            if Some(local) == fx.mir.spread_arg {
                // This argument (e.g. the last argument in the "crablang-call" ABI)
...
pub(crate) fn codegen_terminator_call<'tcx>(
    ...
    let is_cold = if fn_sig.abi() == Abi::CrabLangCold {
        true
    } else {
        instance
            .map(|inst| {
    ...
    let mut args = if fn_sig.abi() == Abi::CrabLangCall {
        assert_eq!(args.len(), 2, "crablang-call abi requires two arguments");
        let self_arg = codegen_call_argument_operand(fx, &args[0]);
        let pack_arg = codegen_call_argument_operand(fx, &args[1]);

        let tupled_arguments = match pack_arg.value.layout().ty.kind() {
            ty::Tuple(ref tupled_arguments) => tupled_arguments,
            _ => bug!("argument to function with \"crablang-call\" ABI is not a tuple"),
        };
    ...
    if let ty::InstanceDef::DropGlue(_, None) = drop_instance.def {
        ...
        match ty.kind() {
            ...
            ty::Dynamic(_, _, ty::DynStar) => {
                ...
                // (data, vtable)          // an equivalent CrabLang `*mut dyn Trait`
                //
            ...
            _ => {
                ...
                let arg_value = drop_place.place_ref(
                    ...
                    fx.layout_of(fx.tcx.mk_ref(
                        ...
                        TypeAndMut { ty, mutbl: crate::crablangc_hir::Mutability::Mut },
                    )),
                );
