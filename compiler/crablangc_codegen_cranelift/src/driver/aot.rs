...
use crablangc_codegen_ssa::back::metadata::create_compressed_metadata_file;
use crablangc_codegen_ssa::{CodegenResults, CompiledModule, CrateInfo, ModuleKind};
...
use crablangc_middle::mir::mono::{CodegenUnit, MonoItem};
use crablangc_session::cgu_reuse_tracker::CguReuse;
use crablangc_session::config::{DebugInfo, OutputFilenames, OutputType};
        ...
        for module_codegen in self.modules {
            ...
            if let Some((work_product_id, work_product)) = existing_work_product {
                ...
                let work_product = if backend_config.disable_incr_cache {
                    ...
                    crablangc_incremental::copy_cgu_workproduct_to_incr_comp_cache_dir(
                        sess,
                        &module_regular.name,
                        &[
...
fn reuse_workproduct_for_cgu(
    ...
    let source_file_regular = crablangc_incremental::in_incr_comp_dir_sess(
        &tcx.sess,
        &work_product.saved_files.get("o").expect("no saved object file in work product"),
    );

    if let Err(err) = crablangc_fs_util::link_or_copy(&source_file_regular, &obj_out_regular) {
        return Err(format!(
            "unable to copy {} to {}: {}",
    ...
    let has_global_asm = if let Some(asm_o) = work_product.saved_files.get("asm.o") {
        let source_file_global_asm = crablangc_incremental::in_incr_comp_dir_sess(&tcx.sess, asm_o);
        if let Err(err) = crablangc_fs_util::link_or_copy(&source_file_global_asm, &obj_out_global_asm)
        {
            return Err(format!(
                "unable to copy {} to {}: {}",
                source_file_regular.display(),
...
pub(crate) fn run_aot(
                    ...
                    CguReuse::No => {
                        ...
                        tcx.dep_graph
                            .with_task(
                                ...
                                module_codegen,
                                Some(crablangc_middle::dep_graph::hash_result),
                            )
                            .0
                    }
    ...
    let metadata_module = if need_metadata_module {
        let (metadata_cgu_name, tmp_file) = tcx.sess.time("write compressed metadata", || {
            use crablangc_middle::mir::mono::CodegenUnitNameBuilder;
            ...
            let symbol_name = crablangc_middle::middle::exported_symbols::metadata_symbol_name(tcx);
...
// Adapted from https://github.com/crablang/crablang/blob/303d8aff6092709edd4dbd35b1c88e9aa40bf6d8/src/libcrablangc_codegen_ssa/base.rs#L922-L953
