...
use crablangc_codegen_ssa::CrateInfo;
...
/// The Sender owned by the crablangc thread
static GLOBAL_MESSAGE_SENDER: OnceCell<Mutex<mpsc::Sender<UnsafeMessage>>> = OnceCell::new();

/// A message that is sent from the jitted runtime to the crablangc thread.
...
pub(crate) fn run_jit(tcx: TyCtxt<'_>, backend_config: BackendConfig) -> ! {
    ...
    if !tcx.sess.crate_types().contains(&crablangc_session::config::CrateType::Executable) {
    ...
    println!(
        "CrabLangc codegen cranelift will JIT run the executable, because -Cllvm-args=mode=jit was passed"
    );
    ...
    // Spawn the jitted runtime in a new thread so that this crablangc thread can handle messages
...
extern "C" fn clif_jit_fn(
    ...
    // send the JIT request to the crablangc thread, with a channel for the response
    let (tx, rx) = mpsc::channel();
    UnsafeMessage::JitFn { instance_ptr, trampoline_ptr, tx }
        .send()
        .expect("crablangc thread hung up before lazy JIT request was sent");

    // block on JIT compilation result
    rx.recv().expect("crablangc thread hung up before responding to sent lazy JIT request")
}

fn jit_fn(instance_ptr: *const Instance<'static>, trampoline_ptr: *const u8) -> *const u8 {
    crablangc_middle::ty::tls::with(|tcx| {
        ...
        LAZY_JIT_STATE.with(|lazy_jit_state| {
            ...
            // This does not need synchronization as this code is executed only by a sole crablangc
            // thread.
...
fn dep_symbol_lookup_fn(
    ...
    let data = &crate_info
        .dependency_formats
        .iter()
        .find(|(crate_type, _data)| *crate_type == crablangc_session::config::CrateType::Executable)
        .unwrap()
        .1;
    ...
    for &cnum in crate_info.used_crates.iter().rev() {
        ...
        match data[cnum.as_usize() - 1] {
            ...
            Linkage::Static => {
                ...
                err.note("crablangc_codegen_cranelift can only load dylibs in JIT mode.");
                err.emit();
            }
            Linkage::Dynamic => {
