...
use crablangc_target::abi::{Integer, Primitive};
...
pub(crate) fn has_ptr_meta<'tcx>(tcx: TyCtxt<'tcx>, ty: Ty<'tcx>) -> bool {
    let ptr_ty = tcx.mk_ptr(TypeAndMut { ty, mutbl: crablangc_hir::Mutability::Not });
...
pub(crate) fn codegen_bitcast(fx: &mut FunctionCx<'_, '_, '_>, dst_ty: Type, val: Value) -> Value {
    ...
    flags.set_endianness(match fx.tcx.data_layout.endian {
        crablangc_target::abi::Endian::Big => cranelift_codegen::ir::Endianness::Big,
        crablangc_target::abi::Endian::Little => cranelift_codegen::ir::Endianness::Little,
    });
...
impl<'tcx> crablangc_target::abi::HasDataLayout for FunctionCx<'_, '_, 'tcx> {
    fn data_layout(&self) -> &crablangc_target::abi::TargetDataLayout {
    ...
    pub(crate) fn set_debug_loc(&mut self, source_info: mir::SourceInfo) {
        if let Some(debug_context) = &mut self.cx.debug_context {
            ...
            if let Some((ref last_source_file, last_file_id)) = self.last_source_file {
                ...
                if crablangc_data_structures::sync::Lrc::ptr_eq(last_source_file, &file) {
    ...
    pub(crate) fn get_caller_location(&mut self, mut source_info: mir::SourceInfo) -> CValue<'tcx> {
        let span_to_caller_location = |fx: &mut FunctionCx<'_, '_, 'tcx>, span: Span| {
            ...
            let const_loc = fx.tcx.const_caller_location((
                crablangc_span::symbol::Symbol::intern(
...
impl<'tcx> crablangc_target::abi::HasDataLayout for RevealAllLayoutCx<'tcx> {
    fn data_layout(&self) -> &crablangc_target::abi::TargetDataLayout {
        &self.0.data_layout
    }
}
