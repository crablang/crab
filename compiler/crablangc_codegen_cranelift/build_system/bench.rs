...
fn benchmark_simple_raytracer(dirs: &Dirs, bootstrap_host_compiler: &Compiler) {
    ...
    if !SIMPLE_RAYTRACER_REPO.source_dir().to_path(dirs).exists() {
        ...
        spawn_and_wait(SIMPLE_RAYTRACER.fetch(
            ...
            &bootstrap_host_compiler.crablangc,
            dirs,
        ));
    }
    ...
    let clean_cmd = format!(
        "CRABLANGC=crablangc cargo clean --manifest-path {manifest_path} --target-dir {target_dir}",
        manifest_path = manifest_path.display(),
        target_dir = target_dir.display(),
    );
    let llvm_build_cmd = format!(
        "CRABLANGC=crablangc cargo build --manifest-path {manifest_path} --target-dir {target_dir}",
        manifest_path = manifest_path.display(),
        target_dir = target_dir.display(),
    );
    let clif_build_cmd = format!(
        "CRABLANGC=crablangc {cargo_clif} build --manifest-path {manifest_path} --target-dir {target_dir}",
