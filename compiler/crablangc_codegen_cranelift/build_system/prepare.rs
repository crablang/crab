...
use super::build_sysroot::{BUILD_SYSROOT, ORIG_BUILD_SYSROOT, SYSROOT_CRABLANGC_VERSION, SYSROOT_SRC};
use super::path::{Dirs, RelPath};
use super::crablangc_info::{get_default_sysroot, get_crablangc_version};
...
pub(crate) fn prepare(dirs: &Dirs) {
    ...
    spawn_and_wait(super::build_backend::CG_CLIF.fetch("cargo", "crablangc", dirs));

    prepare_stdlib(dirs);
    spawn_and_wait(super::build_sysroot::STANDARD_LIBRARY.fetch("cargo", "crablangc", dirs));

    prepare_coretests(dirs);
    spawn_and_wait(super::tests::LIBCORE_TESTS.fetch("cargo", "crablangc", dirs));

    super::tests::RAND_REPO.fetch(dirs);
    spawn_and_wait(super::tests::RAND.fetch("cargo", "crablangc", dirs));
    super::tests::REGEX_REPO.fetch(dirs);
    spawn_and_wait(super::tests::REGEX.fetch("cargo", "crablangc", dirs));
    super::tests::PORTABLE_SIMD_REPO.fetch(dirs);
    spawn_and_wait(super::tests::PORTABLE_SIMD.fetch("cargo", "crablangc", dirs));
}

fn prepare_stdlib(dirs: &Dirs) {
    let sysroot_src_orig = get_default_sysroot(Path::new("crablangc")).join("lib/crablanglib/src/crablang");
    ...
    let crablangc_version = get_crablangc_version(Path::new("crablangc"));
    fs::write(SYSROOT_CRABLANGC_VERSION.to_path(dirs), &crablangc_version).unwrap();
