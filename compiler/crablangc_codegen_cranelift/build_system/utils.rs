...
use super::crablangc_info::{get_cargo_path, get_crablangc_path, get_crablangdoc_path};

#[derive(Clone, Debug)]
pub(crate) struct Compiler {
    pub(crate) cargo: PathBuf,
    pub(crate) crablangc: PathBuf,
    pub(crate) crablangdoc: PathBuf,
    pub(crate) crablangflags: String,
    pub(crate) crablangdocflags: String,
...
impl Compiler {
    pub(crate) fn bootstrap_with_triple(triple: String) -> Compiler {
        Compiler {
            ...
            crablangc: get_crablangc_path(),
            crablangdoc: get_crablangdoc_path(),
    ...
    pub(crate) fn set_cross_linker_and_runner(&mut self) {
        match self.triple.as_str() {
            "aarch64-unknown-linux-gnu" => {
                ...
                self.crablangflags += " -Clinker=aarch64-linux-gnu-gcc";
                self.crablangdocflags += " -Clinker=aarch64-linux-gnu-gcc";
                self.runner = vec![
            ...
            "s390x-unknown-linux-gnu" => {
                ...
                self.crablangflags += " -Clinker=s390x-linux-gnu-gcc";
                self.crablangdocflags += " -Clinker=s390x-linux-gnu-gcc";
                self.runner = vec![
                    "qemu-s390x".to_owned(),
                    "-L".to_owned(),
...
impl CargoProject {
    ...
    fn build_cmd(&self, command: &str, compiler: &Compiler, dirs: &Dirs) -> Command {
        ...
        cmd.env("CRABLANGC", &compiler.crablangc);
        cmd.env("CRABLANGDOC", &compiler.crablangdoc);
        cmd.env("CRABLANGFLAGS", &compiler.crablangflags);
        cmd.env("CRABLANGDOCFLAGS", &compiler.crablangdocflags);
    ...
    pub(crate) fn fetch(
        ...
        crablangc: impl AsRef<Path>,
        dirs: &Dirs,
    ) -> Command {
        let mut cmd = Command::new(cargo.as_ref());

        cmd.env("CRABLANGC", crablangc.as_ref())
            .arg("fetch")
            .arg("--manifest-path")
            .arg(self.manifest_path(dirs));
...
// Based on the retry function in crablang's src/ci/shared.sh
#[track_caller]
