...
// Note: copied from crablangc_span
...
impl<'a> Parser<'a> {
    ...
    fn position(&mut self) -> Option<Position<'a>> {
        if let Some(i) = self.integer() {
            ...
            match self.cur.peek() {
                Some(&(_, c)) if crablangc_lexer::is_id_start(c) => Some(ArgumentNamed(self.word())),
    ...
    /// CrabLang identifier, except that it can't start with `_` character.
    fn word(&mut self) -> &'a str {
        let start = match self.cur.peek() {
            Some(&(pos, c)) if crablangc_lexer::is_id_start(c) => {
                self.cur.next();
                pos
            }
            _ => {
        ...
        let mut end = None;
        while let Some(&(pos, c)) = self.cur.peek() {
            if crablangc_lexer::is_id_continue(c) {
                self.cur.next();
            } else {
                end = Some(pos);
                break;
...
crablangc_data_structures::static_assert_size!(Piece<'_>, 16);
