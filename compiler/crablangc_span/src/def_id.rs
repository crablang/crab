...
use crablangc_data_structures::unhash::Unhasher;
...
/// `crablangc_middle` (with ~50000 `LocalDefId`s as of the time of writing) there
...
/// further trouble.
///
/// For more information on the possibility of hash collisions in crablangc,
...
impl StableCrateId {
    ...
    pub fn new(crate_name: Symbol, is_exe: bool, mut metadata: Vec<String>) -> StableCrateId {
        ...
        // Also incorporate the crablangc version. Otherwise, with -Zsymbol-mangling-version=v0
        // and no -Cmetadata, symbols from the same crate compiled with different versions of
        // crablangc are named the same.
        //
        // CRABLANGC_FORCE_CRABLANGC_VERSION is used to inject crablangc version information
        // during testing.
        if let Some(val) = std::env::var_os("CRABLANGC_FORCE_CRABLANGC_VERSION") {
...
// is used throughout crablangc, has problems distributing the entropy from the high bits, so reversing
...
crablangc_data_structures::define_id_collections!(DefIdMap, DefIdSet, DefIdMapEntry, DefId);
...
// See https://github.com/crablang/crablang/issues/90317.
...
crablangc_data_structures::define_id_collections!(
    LocalDefIdMap,
    LocalDefIdSet,
    LocalDefIdMapEntry,
    LocalDefId
