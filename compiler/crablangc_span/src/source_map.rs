...
use crablangc_data_structures::sync::{AtomicU32, Lrc, MappedReadGuard, ReadGuard, RwLock};
use std::cmp;
use std::hash::Hash;
...
impl SourceMap {
    ...
    pub fn new_source_file(&self, filename: FileName, src: String) -> Lrc<SourceFile> {
        self.try_new_source_file(filename, src).unwrap_or_else(|OffsetOverflowError| {
            eprintln!("fatal error: crablangc does not support files larger than 4GB");
...
impl FilePathMapping {
    ...
    pub fn map_prefix<'a>(&'a self, path: impl Into<Cow<'a, Path>>) -> (Cow<'a, Path>, bool) {
        ...
        if path.as_os_str().is_empty() {
            ...
            // This is mostly to reduce spam for `CRABLANGC_LOG=[remap_path_prefix]`.
            return (path, false);
        }
