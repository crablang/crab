...
use crablangc_data_structures::stable_hasher::HashingControls;
pub mod def_id;
...
// use of magic `/crablang/1.x/...` paths across the board.
...
pub enum RealFileName {
    ...
    Remapped {
        ...
        /// `virtual_name` is the stable path crablangc will store internally within
        /// build artifacts.
...
impl Hash for RealFileName {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) {
        ...
        // virtualized paths to sysroot crates (/crablang/$hash or /crablang/$version)
...
pub enum FileName {
    ...
    /// Command line.
    Anon(u64),
    /// Hack in `src/libcrablangc_ast/parse.rs`.
    // FIXME(jseyfried)
    MacroExpansion(u64),
