...
// candidates here, and do the actual check in crablangc_hir_analysis/check_unused.rs.
//
...
//    calculate the spans that can be removed by crablangfix; This is done in a
//    separate step to be able to collapse the adjacent spans that crablangfix
//    will remove
//
...
use crablangc_errors::{pluralize, MultiSpan};
use crablangc_hir::def::{DefKind, Res};
use crablangc_session::lint::builtin::{MACRO_USE_EXTERN_CRATE, UNUSED_EXTERN_CRATES, UNUSED_IMPORTS};
...
fn calc_unused_spans(
    ...
    // This tells crablangfix to remove the whole item if all the imports are unused
        ...
        ast::UseTreeKind::Nested(ref nested) => {
            ...
            for (pos, (use_tree, use_tree_id)) in nested.iter().enumerate() {
                ...
                if let Some(remove) = remove {
                    ...
                    // overlapping removals, which are not supported by crablangfix
...
impl Resolver<'_, '_> {
    pub(crate) fn check_unused(&mut self, krate: &ast::Crate) {
        ...
        for extern_crate in visitor.extern_crate_items {
            ...
            // If we are not in CrabLang 2018 edition, then we don't make any further
            // suggestions.
            if !tcx.sess.crablang_2018() {
                continue;
            }
