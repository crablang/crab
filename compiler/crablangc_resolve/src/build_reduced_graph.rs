...
use crablangc_ast::{self as ast, AssocItem, AssocItemKind, MetaItemKind, StmtKind};
use crablangc_ast::{Block, Fn, ForeignItem, ForeignItemKind, Impl, Item, ItemKind, NodeId};
use crablangc_attr as attr;
use crablangc_data_structures::sync::Lrc;
use crablangc_errors::{struct_span_err, Applicability};
use crablangc_expand::expand::AstFragment;
use crablangc_hir::def::{self, *};
use crablangc_hir::def_id::{DefId, LocalDefId, CRATE_DEF_ID};
use crablangc_metadata::creader::LoadedMacro;
use crablangc_middle::metadata::ModChild;
use crablangc_middle::{bug, ty};
use crablangc_span::hygiene::{ExpnId, LocalExpnId, MacroKind};
...
impl<'a, 'tcx> Resolver<'a, 'tcx> {
    ...
    /// (<https://github.com/crablang/crablang/pull/77984#issuecomment-712445508>).
...
impl<'a, 'b, 'tcx> BuildReducedGraphVisitor<'a, 'b, 'tcx> {
        ...
        match vis.kind {
            ...
            ast::VisibilityKind::Restricted { ref path, id, .. } => {
                ...
                let crate_root = if ident.is_path_segment_keyword() {
                    None
                } else if ident.span.is_crablang_2015() {
    ...
    fn build_reduced_graph_for_use_tree(
        ...
        let crate_root = match prefix_iter.peek() {
            Some(seg) if !seg.ident.is_path_segment_keyword() && seg.ident.span.is_crablang_2015() => {
                Some(seg.ident.span.ctxt())
            }
            None if is_glob && use_tree.span.is_crablang_2015() => Some(use_tree.span.ctxt()),
            _ => None,
        }
        .map(|ctxt| {
