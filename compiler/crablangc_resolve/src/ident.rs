use crablangc_ast::{self as ast, NodeId};
use crablangc_feature::is_builtin_attr_name;
use crablangc_hir::def::{DefKind, Namespace, NonMacroAttrKind, PartialRes, PerNS};
use crablangc_hir::PrimTy;
use crablangc_middle::bug;
use crablangc_middle::ty;
use crablangc_session::lint::builtin::PROC_MACRO_DERIVE_RESOLUTION_FALLBACK;
use crablangc_session::lint::BuiltinLintDiagnostics;
use crablangc_span::def_id::LocalDefId;
use crablangc_span::hygiene::{ExpnId, ExpnKind, LocalExpnId, MacroKind, SyntaxContext};
...
impl<'a, 'tcx> Resolver<'a, 'tcx> {
    ...
    pub(crate) fn visit_scopes<T>(
        ...
        let crablang_2015 = ctxt.edition().is_crablang_2015();
        ...
        loop {
            let visit = match scope {
                ...
                Scope::Module(..) => true,
                Scope::MacroUsePrelude => use_prelude || crablang_2015,
        ...
        for (i, &Segment { ident, id, .. }) in path.iter().enumerate() {
            ...
            if ns == TypeNS {
                ...
                if i == 0 {
                    ...
                    if name == kw::PathRoot && ident.span.crablang_2018() {
                        module = Some(ModuleOrUniformRoot::ExternPrelude);
                        continue;
                    }
                    if name == kw::PathRoot
                        && ident.span.is_crablang_2015()
                        && self.tcx.sess.crablang_2018()
                    {
