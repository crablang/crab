...
use crablangc_ast::visit::{self, Visitor};
use crablangc_ast::{self as ast, Crate, ItemKind, ModKind, NodeId, Path, CRATE_NODE_ID};
use crablangc_ast_pretty::ppcrablang;
use crablangc_data_structures::fx::FxHashSet;
use crablangc_errors::{
    pluralize, Applicability, Diagnostic, DiagnosticBuilder, ErrorGuaranteed, MultiSpan,
};
use crablangc_errors::{struct_span_err, SuggestionStyle};
use crablangc_feature::BUILTIN_ATTRIBUTES;
use crablangc_hir::def::Namespace::{self, *};
use crablangc_hir::def::{self, CtorKind, CtorOf, DefKind, NonMacroAttrKind, PerNS};
...
use crablangc_session::lint::builtin::ABSOLUTE_PATHS_NOT_STARTING_WITH_CRATE;
use crablangc_session::lint::builtin::MACRO_EXPANDED_MACRO_EXPORTS_ACCESSED_BY_ABSOLUTE_PATHS;
...
impl<'a, 'tcx> Resolver<'a, 'tcx> {
    ...
    fn report_with_use_injections(&mut self, krate: &Crate) {
            ...
            self.use_injections.drain(..)
        {
            ...
            if !candidates.is_empty() {
                ...
                err.stash(segment.ident.span, crablangc_errors::StashKey::CallIntoMethod);
            } else {
                err.emit();
            }
        }
    ...
    pub(crate) fn report_conflict<'b>(
        ...
        // See https://github.com/crablang/crablang/issues/32354
    ...
    pub(crate) fn lint_if_path_starts_with_module(
        ...
        let first_name = match path.get(0) {
            ...
            Some(seg) if seg.ident.span.is_crablang_2015() && self.tcx.sess.is_crablang_2015() => {
                seg.ident.name
            }
            _ => return,
        };
        ...
        match vis_resolution_error {
            VisResolutionError::Relative2018(span, path) => {
                self.tcx.sess.create_err(errs::Relative2018 {
                    ...
                    path_str: ppcrablang::path_to_string(&path),
                })
            }
            ...
            in_module.for_each_child(self, |this, ident, ns, name_binding| {
                    ...
                    && !ident.span.normalize_to_macros_2_0().from_expansion()
                {
                    ...
                    if filter_fn(res) {
                        ...
                        if lookup_ident.span.crablang_2018() {
                            // crate-local absolute paths start with `crate::` in edition 2018
                            // FIXME: may also be stabilized for CrabLang 2015 (Issues #45477, #44660)
                        ...
                        if candidates.iter().all(|v: &ImportSuggestion| v.did != did) {
                            ...
                            let note = if let Some(did) = did {
                                let requires_note = !did.is_local()
                                    && this.tcx.get_attrs(did, sym::crablangc_diagnostic_item).any(
                                        |attr| {
                ...
                if let Some(module) = name_binding.module() {
                    ...
                    let is_extern_crate_that_also_appears_in_prelude =
                        name_binding.is_extern_crate() && lookup_ident.span.crablang_2018();
    ...
    fn report_ambiguity_error(&self, ambiguity_error: &AmbiguityError<'_>) {
        ...
        let mut could_refer_to = |b: &NameBinding<'_>, misc: AmbiguityErrorMisc, also: &str| {
            ...
            if b.is_extern_crate() && ident.span.crablang_2018() {
    ...
    pub(crate) fn report_path_resolution_error(
        ...
        if module_res == self.graph_root.res() {
            ...
            candidates
                .sort_by_cached_key(|c| (c.path.segments.len(), ppcrablang::path_to_string(&c.path)));
            if let Some(candidate) = candidates.get(0) {
                (
                    String::from("unresolved import"),
                    Some((
                        vec![(ident.span, ppcrablang::path_to_string(&candidate.path))],
                        String::from("a similar path exists"),
                        Applicability::MaybeIncorrect,
                    )),
                )
            } else if self.tcx.sess.is_crablang_2015() {
                (
    ...
    pub(crate) fn make_path_suggestion(
        ...
        match (path.get(0), path.get(1)) {
            ...
            // `ident::...` on 2018.
            (Some(fst), _)
                if fst.ident.span.crablang_2018() && !fst.ident.is_path_segment_keyword() =>
            {
    ...
    fn make_missing_crate_suggestion(
        ...
        if let PathResult::Module(..) = result {
            Some((
                path,
                Some(
                    "`use` statements changed in CrabLang 2018; read more at \
                     <https://doc.crablang.org/edition-guide/crablang-2018/module-system/path-\
                     clarity.html>"
                        .to_string(),
                ),
            ))
    ...
    fn make_external_crate_suggestion(
        ...
        if path[1].ident.span.is_crablang_2015() {
            return None;
        }
