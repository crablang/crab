...
//!
//! Type-relative name resolution (methods, fields, associated items) happens in `crablangc_hir_analysis`.
...
#![feature(never_type)]
#![recursion_limit = "256"]
#![allow(crablangdoc::private_intra_doc_links)]
#![allow(crablangc::potential_query_instability)]

#[macro_use]
extern crate tracing;

use crablangc_arena::{DroplessArena, TypedArena};
use crablangc_ast::node_id::NodeMap;
use crablangc_ast::{self as ast, attr, NodeId, CRATE_NODE_ID};
use crablangc_ast::{AngleBracketedArg, Crate, Expr, ExprKind, GenericArg, GenericArgs, LitKind, Path};
...
use crablangc_data_structures::sync::{Lrc, MappedReadGuard};
use crablangc_errors::{
    Applicability, DiagnosticBuilder, DiagnosticMessage, ErrorGuaranteed, SubdiagnosticMessage,
};
use crablangc_expand::base::{DeriveResolutions, SyntaxExtension, SyntaxExtensionKind};
use crablangc_hir::def::Namespace::{self, *};
use crablangc_hir::def::{self, CtorOf, DefKind, DocLinkResMap, LifetimeRes, PartialRes, PerNS};
use crablangc_hir::def_id::{CrateNum, DefId, LocalDefId, LocalDefIdMap, LocalDefIdSet};
use crablangc_hir::def_id::{CRATE_DEF_ID, LOCAL_CRATE};
use crablangc_hir::definitions::DefPathData;
use crablangc_hir::TraitCandidate;
use crablangc_index::vec::IndexVec;
use crablangc_macros::fluent_messages;
use crablangc_metadata::creader::{CStore, CrateLoader};
...
use crablangc_middle::ty::{self, MainDefinition, RegisteredTools, TyCtxt};
use crablangc_middle::ty::{ResolverGlobalCtxt, ResolverOutputs};
use crablangc_query_system::ich::StableHashingContext;
use crablangc_session::lint::LintBuffer;
use crablangc_span::hygiene::{ExpnId, LocalExpnId, MacroKind, SyntaxContext, Transparency};
...
enum ResolutionError<'a> {
    ...
    TraitImplMismatch {
        ...
        code: crablangc_errors::DiagnosticId,
    },
...
///
/// Note that a "module" in resolve is broader than a `mod` that you declare in CrabLang code. It may be one of these:
///
/// * `mod`
...
impl<'a> NameBinding<'a> {
    ...
    // https://github.com/crablang/crablang/pull/53778#issuecomment-419224049
    fn may_appear_after(
        &self,
...
impl<'a, 'tcx> Resolver<'a, 'tcx> {
    ...
    /// CrabLangdoc uses this to resolve doc link paths in a recoverable way. `PathResult<'a>`
    /// isn't something that can be returned because it can't be made to live that long,
    /// and also it's a private type. Fortunately crablangdoc doesn't need to know the error,
    /// just that an error occurred.
    fn resolve_crablangdoc_path(
        &mut self,
        path_str: &str,
        ns: Namespace,
        parent_scope: ParentScope<'a>,
    ...
    /// `#[crablangc_legacy_const_generics]` and returns the argument index list
    /// from the attribute.
    fn legacy_const_generic_args(&mut self, expr: &Expr) -> Option<Vec<usize>> {
        if let ExprKind::Path(None, path) = &expr.kind {
            ...
            if let Res::Def(def::DefKind::Fn, def_id) = res {
                ...
                let attr = self.tcx.get_attr(def_id, sym::crablangc_legacy_const_generics)?;
                let mut ret = Vec::new();
