...
use crablangc_ast::util::comments::beautify_doc_string;
...
pub enum MalformedGenerics {
    ...
    /// brackets because crablangdoc mostly ignores what's inside angle brackets (except for
...
/// Has `#[crablangc_doc_primitive]` or `#[doc(keyword)]`.
pub fn has_primitive_or_keyword_docs(attrs: &[ast::Attribute]) -> bool {
    for attr in attrs {
        if attr.has_name(sym::crablangc_doc_primitive) {
            return true;
...
/// Simplified version of the corresponding function in crablangdoc.
/// If the crablangdoc version returns a successful result, this function must return the same result.
...
/// Simplified version of `preprocessed_markdown_links` from crablangdoc.
