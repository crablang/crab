...
use crablangc_ast::visit::{FnCtxt, FnKind, LifetimeCtxt};
use crablangc_ast::{
    self as ast, AssocItemKind, Expr, ExprKind, GenericParam, GenericParamKind, Item, ItemKind,
    MethodCall, NodeId, Path, Ty, TyKind, DUMMY_NODE_ID,
};
use crablangc_ast_pretty::ppcrablang::path_segment_to_string;
...
use crablangc_hir::def::{self, CtorKind, CtorOf, DefKind};
...
impl<'a: 'ast, 'ast, 'tcx> LateResolutionVisitor<'a, '_, 'ast, 'tcx> {
    fn make_base_error(
        ...
        if let Some(res) = res {
            ...
            {
                ("`async` blocks are only allowed in CrabLang 2018 or later".to_string(), suggestion)
            } else {
    ...
    fn err_code_special_cases(
        ...
        if let Some(err_code) = &err.code {
            if err_code == &crablangc_errors::error_code!(E0425) {
            ...
            } else if err_code == &crablangc_errors::error_code!(E0412) {
                if let Some(correct) = Self::likely_crablang_type(path) {
                    err.span_suggestion(
                        span,
    ...
    fn suggest_self_ty(
        ...
        err.code(crablangc_errors::error_code!(E0411));
    ...
    fn suggest_self_value(
        ...
        err.code(crablangc_errors::error_code!(E0424));
        err.span_label(
            span,
            match source {
                PathSource::Pat => {
    ...
    fn smart_resolve_context_dependent_help(
        ...
        match (res, source) {
            (
                ...
                if path_str == "try" && span.is_crablang_2015() {
                    err.note("if you want the `try` keyword, you need CrabLang 2018 or later");
                }
            }
    ...
    // Returns the name of the CrabLang type approximately corresponding to
    // a type name in another programming language.
    fn likely_crablang_type(path: &[Segment]) -> Option<Symbol> {
                    ...
                    if higher_ranked {
                        ...
                        err.note_once(
                            "for more information on higher-ranked polymorphism, visit \
                             https://doc.crablang.org/nomicon/hrtb.html",
                        );
    ...
    pub(crate) fn emit_non_static_lt_in_const_generic_error(&self, lifetime_ref: &ast::Lifetime) {
        ...
        .note(
            ...
            <https://github.com/crablang/crablang/issues/74052>",
        )
        .emit();
    }
