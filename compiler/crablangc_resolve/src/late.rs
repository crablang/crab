...
use crate::{path_names_to_string, crablangdoc, BindingError, Finalize, LexicalScopeBinding};
use crate::{Module, ModuleOrUniformRoot, NameBinding, ParentScope, PathResult};
use crate::{ResolutionError, Resolver, Segment, UseError};

use crablangc_ast::ptr::P;
use crablangc_ast::visit::{self, AssocCtxt, BoundKind, FnCtxt, FnKind, Visitor};
use crablangc_ast::*;
use crablangc_data_structures::fx::{FxHashMap, FxHashSet, FxIndexMap};
use crablangc_errors::{Applicability, DiagnosticArgValue, DiagnosticId, IntoDiagnosticArg};
use crablangc_hir::def::Namespace::{self, *};
use crablangc_hir::def::{self, CtorKind, DefKind, LifetimeRes, PartialRes, PerNS};
use crablangc_hir::def_id::{DefId, LocalDefId, CRATE_DEF_ID, LOCAL_CRATE};
use crablangc_hir::{BindingAnnotation, PrimTy, TraitCandidate};
use crablangc_middle::middle::resolve_bound_vars::Set1;
use crablangc_middle::{bug, span_bug};
use crablangc_session::config::{CrateType, ResolveDocLinks};
use crablangc_session::lint;
use crablangc_span::source_map::{respan, Spanned};
use crablangc_span::symbol::{kw, sym, Ident, Symbol};
use crablangc_span::{BytePos, Span, SyntaxContext};
...
impl<'a> PathSource<'a> {
    ...
    fn error_code(self, has_unexpected_resolution: bool) -> DiagnosticId {
        use crablangc_errors::error_code;
...
struct LateResolutionVisitor<'a, 'b, 'ast, 'tcx> {
    ...
    ///
    /// In particular, crablangdoc uses this to avoid giving errors for `cfg()` items.
                ...
                match fn_kind {
                    FnKind::Fn(_, _, sig, _, generics, body) => {
                        ...
                        if let Some(body) = body {
                            // Ignore errors in function bodies if this is crablangdoc
...
impl<'a: 'ast, 'b, 'ast, 'tcx> LateResolutionVisitor<'a, 'b, 'ast, 'tcx> {
    ...
    // allowed within blocks in CrabLang) and jump straight to the current module
    // graph node.
    //
    ...
    fn visit_generic_params(&mut self, params: &'ast [GenericParam], add_self_upper: bool) {
        ...
        // crablang/crablang#61631: The type `Self` is essentially
        ...
        for (i, rib) in self.lifetime_ribs.iter().enumerate().rev() {
            ...
            match rib.kind {
                ...
                LifetimeRibKind::AnonymousReportError => {
                    ...
                    let mut diag = crablangc_errors::struct_span_err!(
                        self.r.tcx.sess,
                        lifetime.ident.span,
                        E0637,
                        "{}",
            ...
            for rib in self.lifetime_ribs.iter().rev() {
                match rib.kind {
                    ...
                    LifetimeRibKind::AnonymousCreateParameter { report_in_path: true, .. } => {
                        ...
                        crablangc_errors::add_elided_lifetime_in_path_suggestion(
                            sess.source_map(),
                            &mut err,
                            expected_lifetimes,
                            path_span,
    ...
    fn future_proof_import(&mut self, use_tree: &UseTree) {
        ...
        if !segments.is_empty() {
            ...
            if ident.is_path_segment_keyword() || ident.span.is_crablang_2015() {
                return;
            }
    ...
    fn resolve_item(&mut self, item: &'ast Item) {
        let mod_inner_docs =
            matches!(item.kind, ItemKind::Mod(..)) && crablangdoc::inner_docs(&item.attrs);
        ...
        F: FnOnce(&mut Self),
    {
        ...
        for param in params {
            ...
            if param.ident.name == kw::UnderscoreLifetime {
                crablangc_errors::struct_span_err!(
                    self.r.tcx.sess,
                    param.ident.span,
                    E0637,
        ...
        F: FnOnce(Ident, String, Option<Symbol>) -> ResolutionError<'a>,
    {
        ...
        let (code, kind) = match kind {
            AssocItemKind::Const(..) => (crablangc_errors::error_code!(E0323), "const"),
            AssocItemKind::Fn(..) => (crablangc_errors::error_code!(E0324), "method"),
            AssocItemKind::Type(..) => (crablangc_errors::error_code!(E0325), "type"),
                ...
                if candidates.is_empty() {
                    if path.len() == 2 && prefix_path.len() == 1 {
                        ...
                        err.stash(
                            ...
                            crablangc_errors::StashKey::CallAssocMethod,
                        );
                    } else {
    ...
    ///
    /// This doesn't emit errors for function bodies if this is crablangdoc.
    ...
    #[inline]
    /// If we're actually crablangdoc then avoid giving a name resolution error for `cfg()` items.
    fn should_report_errs(&self) -> bool {
        !(self.r.tcx.sess.opts.actually_crablangdoc && self.in_func_body)
    }
    ...
    fn resolve_and_cache_crablangdoc_path(&mut self, path_str: &str, ns: Namespace) -> bool {
        ...
        let res = doc_link_resolutions
            ...
            .or_insert_with_key(|(path, ns)| {
                let res = self.r.resolve_crablangdoc_path(path.as_str(), *ns, self.parent_scope);
                if let Some(res) = res
    ...
    fn resolve_doc_links(&mut self, attrs: &[Attribute], maybe_exported: MaybeExported<'_>) {
        match self.r.tcx.sess.opts.resolve_doc_links {
            ...
            ResolveDocLinks::Exported
                if !maybe_exported.eval(self.r)
                    && !crablangdoc::has_primitive_or_keyword_docs(attrs) =>
            {
                return;
            }
            ResolveDocLinks::ExportedMetadata
        ...
        for path_str in crablangdoc::attrs_to_preprocessed_links(attrs) {
            // Resolve all namespaces due to no disambiguator or for diagnostics.
            let mut any_resolved = false;
            let mut need_assoc = false;
            for ns in [TypeNS, ValueNS, MacroNS] {
                if self.resolve_and_cache_crablangdoc_path(&path_str, ns) {
                    any_resolved = true;
                } else if ns != MacroNS {
                    need_assoc = true;
                }
            ...
            if need_assoc || !any_resolved {
                ...
                while let Some(idx) = path.rfind("::") {
                    ...
                    for ns in [TypeNS, ValueNS, MacroNS] {
                        self.resolve_and_cache_crablangdoc_path(path, ns);
                    }
                }
            }
        }
