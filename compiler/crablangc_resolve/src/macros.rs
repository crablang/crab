...
use crablangc_ast::{self as ast, attr, Inline, ItemKind, ModKind, NodeId};
use crablangc_ast_pretty::ppcrablang;
use crablangc_attr::StabilityLevel;
use crablangc_data_structures::intern::Interned;
use crablangc_data_structures::sync::Lrc;
use crablangc_errors::{struct_span_err, Applicability};
use crablangc_expand::base::{Annotatable, DeriveResolutions, Indeterminate, ResolverExpand};
use crablangc_expand::base::{SyntaxExtension, SyntaxExtensionKind};
use crablangc_expand::compile_declarative_macro;
use crablangc_expand::expand::{AstFragment, Invocation, InvocationKind, SupportsMacroExpansion};
use crablangc_hir::def::{self, DefKind, NonMacroAttrKind};
use crablangc_hir::def_id::{CrateNum, LocalDefId};
use crablangc_middle::middle::stability;
use crablangc_middle::ty::RegisteredTools;
use crablangc_middle::ty::TyCtxt;
use crablangc_session::lint::builtin::{LEGACY_DERIVE_HELPERS, SOFT_UNSTABLE};
use crablangc_session::lint::builtin::{UNUSED_MACROS, UNUSED_MACRO_RULES};
...
use crablangc_span::hygiene::{self, ExpnData, ExpnKind, LocalExpnId};
...
pub(crate) fn registered_tools(tcx: TyCtxt<'_>, (): ()) -> RegisteredTools {
    ...
    // We implicitly add `crablangfmt` and `clippy` to known tools,
    // but it's not an error to register them explicitly.
    let predefined_tools = [sym::clippy, sym::crablangfmt];
...
fn soft_custom_inner_attributes_gate(path: &ast::Path, invoc: &Invocation) -> bool {
    match &path.segments[..] {
        ...
        // `#![crablangfmt::skip]` on out-of-line modules
        [seg1, seg2] if seg1.ident.name == sym::crablangfmt && seg2.ident.name == sym::skip => {
...
impl<'a, 'tcx> Resolver<'a, 'tcx> {
    ...
    fn smart_resolve_macro_path(
        ...
        for segment in &path.segments {
            ...
            if kind == MacroKind::Attr && segment.ident.as_str().starts_with("crablangc") {
                self.tcx.sess.span_err(
                    segment.ident.span,
                    "attributes starting with `crablangc` are reserved for use by the `crablangc` compiler",
                );
            }
        }
        ...
        if let Some((article, expected)) = unexpected_res {
            let path_str = ppcrablang::path_to_string(path);
    ...
    fn check_stability_and_deprecation(
        ...
        if let Some(depr) = &ext.deprecation {
            let path = ppcrablang::path_to_string(&path);
    ...
    pub(crate) fn compile_macro(
        ...
        if let Some(builtin_name) = result.builtin_name {
            // The macro was marked with `#[crablangc_builtin_macro]`.
