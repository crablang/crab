...
use crablangc_ast::*;
use crablangc_ast::{walk_list, StaticItem};
use crablangc_ast_pretty::ppcrablang::{self, State};
use crablangc_data_structures::fx::FxIndexMap;
use crablangc_macros::Subdiagnostic;
use crablangc_parse::validate_attr;
use crablangc_session::lint::builtin::{
    DEPRECATED_WHERE_CLAUSE_LOCATION, MISSING_ABI, PATTERNS_IN_FNS_WITHOUT_BODY,
};
use crablangc_session::lint::{BuiltinLintDiagnostics, LintBuffer};
...
impl<'a> AstValidator<'a> {
    ...
    fn err_handler(&self) -> &crablangc_errors::Handler {
    ...
    fn check_decl_attrs(&self, fn_decl: &FnDecl) {
        fn_decl
            ...
            .filter(|attr| {
                ...
                !arr.contains(&attr.name_or_empty()) && crablangc_attr::is_builtin_attr(attr)
            })
            .for_each(|attr| {
    ...
    fn correct_generic_order_suggestion(&self, data: &AngleBracketedArgs) -> String {
        ...
        let lt_sugg = data.args.iter().filter_map(|arg| match arg {
            AngleBracketedArg::Arg(lt @ GenericArg::Lifetime(_)) => {
                Some(ppcrablang::to_string(|s| s.print_generic_arg(lt)))
            }
            _ => None,
        });
        let args_sugg = data.args.iter().filter_map(|a| match a {
            ...
            AngleBracketedArg::Arg(arg) => Some(ppcrablang::to_string(|s| s.print_generic_arg(arg))),
        });
        // Constraints always come last.
        let constraint_sugg = data.args.iter().filter_map(|a| match a {
            AngleBracketedArg::Arg(_) => None,
            AngleBracketedArg::Constraint(c) => {
                Some(ppcrablang::to_string(|s| s.print_assoc_constraint(c)))
            }
        });
        format!(
            "<{}>",
...
fn validate_generic_param_order(
    ...
    for (idx, param) in generics.iter().enumerate() {
        ...
        let (ord_kind, ident) = match &param.kind {
            ...
            GenericParamKind::Const { ty, .. } => {
                let ty = ppcrablang::ty_to_string(ty);
    ...
    if !out_of_order.is_empty() {
        ...
        for (kind, _, bounds, _, ident) in param_idents {
            ...
            if !bounds.is_empty() {
                ordered_params += ": ";
                ordered_params += &ppcrablang::bounds_to_string(&bounds);
            }

            match kind {
                GenericParamKind::Type { default: Some(default) } => {
                    ordered_params += " = ";
                    ordered_params += &ppcrablang::ty_to_string(default);
                }
                GenericParamKind::Type { default: None } => (),
                GenericParamKind::Lifetime => (),
                GenericParamKind::Const { ty: _, kw_span: _, default: Some(default) } => {
                    ordered_params += " = ";
                    ordered_params += &ppcrablang::expr_to_string(&default.value);
                }
        ...
        for predicate in &generics.where_clause.predicates {
            match predicate {
                WherePredicate::BoundPredicate(bound_pred) => {
                    ...
                    // the syntax of CrabLang permits quantification in two places in where clauses,
            ...
            match (ctxt, modify) {
                (BoundKind::SuperTraits, TraitBoundModifier::Maybe) => {
                    self.err_handler().emit_err(errors::OptionalTraitSupertrait {
                        span: poly.span,
                        path_str: ppcrablang::path_to_string(&poly.trait_ref.path)
                    });
                }
                            ...
                            match &full_path.segments[qself.position..] {
                                [PathSegment { ident, args, .. }] => {
                                    ...
                                    let arg = AngleBracketedArg::Constraint(AssocConstraint {
                                        id: crablangc_ast::node_id::DUMMY_NODE_ID,
                                        ident: *ident,
                                        gen_args,
                                    ...
                                    err.assoc = Some(errors::AssociatedSuggestion {
                                        ...
                                        ident: *ident,
                                        param: *param,
                                        path: ppcrablang::path_to_string(&assoc_path),
                                    })
                                }
                                _ => {}
                            };
                    ...
                    for bound in &param.bounds {
                        if let ast::GenericBound::Trait(trait_ref, TraitBoundModifier::None) = bound
                        {
                            if let [trait_segment] = &trait_ref.trait_ref.path.segments[..] {
                                let assoc = ppcrablang::path_to_string(&ast::Path::from_ident(
                                    potential_assoc.ident,
                                ));
                                let ty = ppcrablang::ty_to_string(&predicate.rhs_ty);
