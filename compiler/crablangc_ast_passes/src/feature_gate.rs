...
use crablangc_ast::visit::{self, AssocCtxt, FnCtxt, FnKind, Visitor};
use crablangc_ast::{attr, AssocConstraint, AssocConstraintKind, NodeId};
use crablangc_ast::{PatKind, RangeEnd};
use crablangc_errors::{Applicability, StashKey};
use crablangc_feature::{AttributeGate, BuiltinAttribute, Features, GateIssue, BUILTIN_ATTRIBUTE_MAP};
use crablangc_session::parse::{feature_err, feature_err_issue, feature_warn};
    ...
    fn visit_attribute(&mut self, attr: &ast::Attribute) {
        ...
        if attr.has_name(sym::doc) {
            for nested_meta in attr.meta_item_list().unwrap_or_default() {
                ...
                if nested_meta.has_name(sym::keyword) {
                    ...
                    gate_feature_post!(self, crablangdoc_internals, attr.span, msg);
                }
        ...
        if !self.features.staged_api {
            if attr.has_name(sym::unstable)
                ...
                || attr.has_name(sym::crablangc_const_unstable)
                || attr.has_name(sym::crablangc_const_stable)
                || attr.has_name(sym::crablangc_default_body_unstable)
            {
...
fn check_incompatible_features(sess: &Session) {
    ...
    for (f1, f2) in crablangc_feature::INCOMPATIBLE_FEATURES
        .iter()
