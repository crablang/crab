...
use crablangc_parse::{new_parser_from_source_str, parser::Parser, source_file_to_stream};
use crablangc_session::parse::ParseSess;
use crablangc_span::create_default_session_if_not_set_then;
...
use crablangc_errors::{Handler, MultiSpan, PResult, TerminalUrl};
...
pub(crate) fn string_to_stream(source_str: String) -> TokenStream {
    let ps = ParseSess::new(
        vec![crate::DEFAULT_LOCALE_RESOURCE, crablangc_parse::DEFAULT_LOCALE_RESOURCE],
...
pub(crate) fn matches_codepattern(a: &str, b: &str) -> bool {
    ...
    loop {
        let (a, b) = match (a_iter.peek(), b_iter.peek()) {
            ...
            (None, _) => return false,
            (Some(&a), None) => {
                if crablangc_lexer::is_whitespace(a) {
        ...
        if crablangc_lexer::is_whitespace(a) && crablangc_lexer::is_whitespace(b) {
            // Skip whitespace for `a` and `b`.
            scan_for_non_ws_or_end(&mut a_iter);
            scan_for_non_ws_or_end(&mut b_iter);
        } else if crablangc_lexer::is_whitespace(a) {
    ...
    a_iter.all(crablangc_lexer::is_whitespace)
}

/// Advances the given peekable `Iterator` until it reaches a non-whitespace character.
fn scan_for_non_ws_or_end<I: Iterator<Item = char>>(iter: &mut Peekable<I>) {
    while iter.peek().copied().map(crablangc_lexer::is_whitespace) == Some(true) {
        iter.next();
    }
}
