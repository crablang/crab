...
use crablangc_ast::token::{Delimiter, Token, TokenKind};
use crablangc_ast::tokenstream::{AttrTokenStream, AttrTokenTree};
use crablangc_ast::tokenstream::{DelimSpan, Spacing};
use crablangc_ast::tokenstream::{LazyAttrTokenStream, TokenTree};
use crablangc_ast::NodeId;
use crablangc_ast::{self as ast, AttrStyle, Attribute, HasAttrs, HasTokens, MetaItem};
...
use crablangc_feature::{Feature, Features, State as FeatureState};
use crablangc_feature::{
    ACCEPTED_FEATURES, ACTIVE_FEATURES, REMOVED_FEATURES, STABLE_REMOVED_FEATURES,
};
use crablangc_parse::validate_attr;
use crablangc_session::parse::feature_err;
use crablangc_session::Session;
use crablangc_span::edition::{Edition, ALL_EDITIONS};
...
pub fn features(sess: &Session, krate_attrs: &[Attribute]) -> Features {
    ...
    for &edition in ALL_EDITIONS {
        if edition <= crate_edition {
            ...
            // (i.e., `#![feature(crablang_20XX_preview)]` isn't needed on edition 20XX).
...
impl<'a> StripUnconfigured<'a> {
    ...
    pub(crate) fn expand_cfg_attr(&self, attr: &Attribute, recursive: bool) -> Vec<Attribute> {
        let Some((cfg_predicate, expanded_attrs)) =
            crablangc_parse::parse_cfg_attr(attr, &self.sess.parse_sess) else {
                return vec![];
            };
        ...
        if expanded_attrs.is_empty() {
            self.sess.parse_sess.buffer_lint(
                crablangc_lint_defs::builtin::UNUSED_ATTRIBUTES,
                attr.span,
                ast::CRATE_NODE_ID,
    ...
    fn expand_cfg_attr_item(
        ...
        if attr.has_name(sym::crate_type) {
            self.sess.parse_sess.buffer_lint(
                crablangc_lint_defs::builtin::DEPRECATED_CFG_ATTR_CRATE_TYPE_NAME,
                attr.span,
                ast::CRATE_NODE_ID,
