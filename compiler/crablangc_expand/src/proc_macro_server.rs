...
use crablangc_ast::tokenstream::{self, Spacing::*, TokenStream};
use crablangc_ast::util::literal::escape_byte_str_symbol;
use crablangc_ast_pretty::ppcrablang;
use crablangc_data_structures::fx::FxHashMap;
use crablangc_data_structures::sync::Lrc;
use crablangc_errors::{MultiSpan, PResult};
use crablangc_parse::lexer::nfc_normalize;
use crablangc_parse::parse_stream_from_source_str;
use crablangc_session::parse::ParseSess;
use crablangc_span::def_id::CrateNum;
use crablangc_span::symbol::{self, sym, Symbol};
use crablangc_span::{BytePos, FileName, Pos, SourceFile, Span};
...
impl FromInternal<(TokenStream, &mut CrabLangc<'_, '_>)> for Vec<TokenTree<TokenStream, Span, Symbol>> {
    fn from_internal((stream, crablangc): (TokenStream, &mut CrabLangc<'_, '_>)) -> Self {
        use crablangc_ast::token::*;
        ...
        while let Some(tree) = cursor.next() {
            ...
            match kind {
                ...
                Interpolated(nt) => {
                    ...
                    if crate::base::nt_pretty_printing_compatibility_hack(&nt, crablangc.sess()) {
                        trees.extend(Self::from_internal((stream, crablangc)));
                    } else {
                        trees.push(TokenTree::Group(Group {
...
impl ToInternal<SmallVec<[tokenstream::TokenTree; 2]>>
    for (TokenTree<TokenStream, Span, Symbol>, &mut CrabLangc<'_, '_>)
{
    fn to_internal(self) -> SmallVec<[tokenstream::TokenTree; 2]> {
        use crablangc_ast::token::*;

        let (tree, crablangc) = self;
        match tree {
            ...
            TokenTree::Ident(self::Ident { sym, is_raw, span }) => {
                crablangc.sess().symbol_gallery.insert(sym, span);
...
impl ToInternal<crablangc_errors::Level> for Level {
    fn to_internal(self) -> crablangc_errors::Level {
        match self {
            Level::Error => crablangc_errors::Level::Error { lint: false },
            Level::Warning => crablangc_errors::Level::Warning(None),
            Level::Note => crablangc_errors::Level::Note,
            Level::Help => crablangc_errors::Level::Help,
...
pub(crate) struct CrabLangc<'a, 'b> {
    ecx: &'a mut ExtCtxt<'b>,
    def_site: Span,
    call_site: Span,
    mixed_site: Span,
...
impl server::Types for CrabLangc<'_, '_> {
...
impl server::FreeFunctions for CrabLangc<'_, '_> {
    ...
    fn literal_from_str(&mut self, s: &str) -> Result<Literal<Self::Span, Self::Symbol>, ()> {
        ...
        let mut parser = crablangc_parse::new_parser_from_source_str(self.sess(), name, s.to_owned());
    ...
    fn emit_diagnostic(&mut self, diagnostic: Diagnostic<Self::Span>) {
        let mut diag =
            crablangc_errors::Diagnostic::new(diagnostic.level.to_internal(), diagnostic.message);
...
impl server::TokenStream for CrabLangc<'_, '_> {
    ...
    fn to_string(&mut self, stream: &Self::TokenStream) -> String {
        ppcrablang::tts_to_string(stream)
    }

    fn expand_expr(&mut self, stream: &Self::TokenStream) -> Result<Self::TokenStream, ()> {
        // Parse the expression from our tokenstream.
        let expr: PResult<'_, _> = try {
            let mut p = crablangc_parse::stream_to_parser(
                self.sess(),
                stream.clone(),
...
impl server::SourceFile for CrabLangc<'_, '_> {
...
impl server::Span for CrabLangc<'_, '_> {
...
impl server::Symbol for CrabLangc<'_, '_> {
    fn normalize_and_validate_ident(&mut self, string: &str) -> Result<Self::Symbol, ()> {
        let sym = nfc_normalize(string);
        if crablangc_lexer::is_ident(sym.as_str()) { Ok(sym) } else { Err(()) }
    }
}

impl server::Server for CrabLangc<'_, '_> {
