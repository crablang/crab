...
use crablangc_ast::token::{self, Delimiter};
use crablangc_ast::tokenstream::TokenStream;
use crablangc_ast::visit::{self, AssocCtxt, Visitor};
use crablangc_ast::{AssocItemKind, AstNodeWrapper, AttrArgs, AttrStyle, AttrVec, ExprKind};
use crablangc_ast::{ForeignItemKind, HasAttrs, HasNodeId};
use crablangc_ast::{Inline, ItemKind, MacStmtStyle, MetaItemKind, ModKind};
use crablangc_ast::{NestedMetaItem, NodeId, PatKind, StmtKind, TyKind};
use crablangc_ast_pretty::ppcrablang;
use crablangc_data_structures::flat_map_in_place::FlatMapInPlace;
use crablangc_data_structures::sync::Lrc;
use crablangc_errors::PResult;
use crablangc_feature::Features;
use crablangc_parse::parser::{
    AttemptLocalParseRecovery, CommaRecoveryMode, ForceCollect, Parser, RecoverColon, RecoverComma,
};
use crablangc_parse::validate_attr;
use crablangc_session::lint::builtin::{UNUSED_ATTRIBUTES, UNUSED_DOC_COMMENTS};
...
use crablangc_span::symbol::{sym, Ident};
use crablangc_span::{FileName, LocalExpnId, Span};
            ...
            InvocationKind::Attr { attr, pos, mut item, derives } => match ext {
                SyntaxExtensionKind::Attr(expander) => {
                    ...
                    let tokens = match &item {
                        ...
                        Annotatable::Crate(krate) => crablangc_parse::fake_token_stream_for_crate(
                        ...
                        {
                            crablangc_parse::fake_token_stream_for_item(
                                &self.cx.sess.parse_sess,
                                item_inner,
                            )
                        }
...
pub fn ensure_complete_parse<'a>(
    ...
    if parser.token != token::Eof {
        let token = ppcrablang::token_to_string(&parser.token);
...
impl<'a, 'b> InvocationCollector<'a, 'b> {
        ...
        for (pos, attr) in item.attrs().iter().enumerate() {
            if !attr.is_doc_comment() && !self.cx.expanded_inert_attrs.is_marked(attr) {
                ...
                if name == Some(sym::cfg) || name == Some(sym::cfg_attr) {
                    ...
                    && !name.map_or(false, crablangc_feature::is_builtin_attr_name)
                {
    ...
    fn check_attributes(&self, attrs: &[ast::Attribute], call: &ast::MacCall) {
        ...
        while let Some(attr) = attrs.next() {
            crablangc_ast_passes::feature_gate::check_attribute(attr, self.cx.sess, features);
            ...
            } else if crablangc_attr::is_builtin_attr(attr) {
                ...
                if attr_name != sym::cfg && attr_name != sym::cfg_attr {
                    self.cx.sess.parse_sess.buffer_lint_with_diagnostic(
                        ...
                        BuiltinLintDiagnostics::UnusedBuiltinAttribute {
                            attr_name,
                            macro_name: ppcrablang::path_to_string(&call.path),
    ...
    fn expand_cfg_true(
        ...
        if res {
            ...
            // and some tools like crablangdoc and clippy rely on that. Find a way to remove them
