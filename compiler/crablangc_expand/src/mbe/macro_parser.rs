//! This is an NFA-based parser, which calls out to the main CrabLang parser for named non-terminals
...
//! a CrabLang non-terminal like `$e:expr`, and `next_mps` with threads that are waiting
//! on a particular token. Most of the logic concerns moving the · through the
//! repetitions indicated by Kleene stars. The rules for moving the · without
//! consuming any input are called epsilon transitions. It only advances or calls
//! out to the real CrabLang parser when no `cur_mps` threads remain.
//!
//! Example:
//!
//! ```text, ignore
...
use crablangc_ast::token::{self, DocComment, Nonterminal, NonterminalKind, Token};
...
use crablangc_parse::parser::{NtOrTt, Parser};
use crablangc_span::symbol::Ident;
use crablangc_span::symbol::MacroRulesNormalizedIdent;
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            MatcherLoc::Token { token } | MatcherLoc::SequenceSep { separator: token } => {
                write!(f, "`{}`", ppcrablang::token_to_string(token))
            }
...
crablangc_data_structures::static_assert_size!(MatcherPos, 16);
...
pub(crate) enum ParseResult<T, F> {
    ...
    Error(crablangc_span::Span, String),
...
pub(crate) enum NamedMatch {
    ...
    MatchedTokenTree(crablangc_ast::tokenstream::TokenTree),
...
impl TtParser {
    ...
    fn ambiguity_error<F>(
        &self,
        matcher: &[MatcherLoc],
        token_span: crablangc_span::Span,
    ) -> NamedParseResult<F> {
        let nts = self
            .bb_mps
            .iter()
