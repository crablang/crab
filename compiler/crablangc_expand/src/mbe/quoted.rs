...
use crablangc_ast::{tokenstream, NodeId};
            ...
            match next {
                ...
                Some(tokenstream::TokenTree::Delimited(delim_span, delim, tts)) => {
                    if parsing_patterns {
                        ...
                        match delim {
                            ...
                            _ => {
                                let tok = ppcrablang::token_kind_to_string(&token::OpenDelim(delim));
                ...
                Some(tokenstream::TokenTree::Token(token, _)) => {
                    let msg = format!(
                        ...
                        ppcrablang::token_to_string(&token),
                    );
...
fn span_dollar_dollar_or_metavar_in_the_lhs_err(sess: &ParseSess, token: &Token) {
    sess.span_diagnostic
        .span_err(token.span, &format!("unexpected token: {}", ppcrablang::token_to_string(token)));
