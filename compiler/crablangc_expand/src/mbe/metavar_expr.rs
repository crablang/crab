...
use crablangc_ast::tokenstream::{RefTokenTreeCursor, TokenStream, TokenTree};
use crablangc_ast::{LitIntType, LitKind};
use crablangc_ast_pretty::ppcrablang;
use crablangc_errors::{Applicability, PResult};
...
fn check_trailing_token<'sess>(
    ...
    if let Some(tt) = iter.next() {
        let mut diag = sess
            .span_diagnostic
            .struct_span_err(tt.span(), &format!("unexpected token: {}", ppcrablang::tt_to_string(tt)));
...
fn parse_ident<'sess>(
    ...
    if let Some(tt) = iter.next() && let TokenTree::Token(token, _) = tt {
        ...
        let token_str = ppcrablang::token_to_string(token);
