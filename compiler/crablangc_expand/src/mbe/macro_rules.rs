...
use crablangc_ast::token::{self, Delimiter, NonterminalKind, Token, TokenKind, TokenKind::*};
use crablangc_ast::tokenstream::{DelimSpan, TokenStream, TokenTree};
use crablangc_ast::{NodeId, DUMMY_NODE_ID};
use crablangc_ast_pretty::ppcrablang;
use crablangc_attr::{self as attr, TransparencyError};
use crablangc_data_structures::fx::{FxHashMap, FxIndexMap};
use crablangc_errors::{Applicability, ErrorGuaranteed};
use crablangc_lint_defs::builtin::{
    CRABLANG_2021_INCOMPATIBLE_OR_PATTERNS, SEMICOLON_IN_EXPRESSIONS_FROM_MACROS,
};
use crablangc_lint_defs::BuiltinLintDiagnostics;
use crablangc_parse::parser::{Parser, Recovery};
...
use crablangc_span::symbol::{kw, sym, Ident, MacroRulesNormalizedIdent};
...
fn expand_macro<'cx>(
    ...
    if cx.trace_macros() {
        let msg = format!("expanding `{}! {{ {} }}`", name, ppcrablang::tts_to_string(&arg));
    ...
    match try_success_result {
        Ok((i, named_matches)) => {
            ...
            if cx.trace_macros() {
                let msg = format!("to `{}`", ppcrablang::tts_to_string(&tts));
                trace_macros_note(&mut cx.expansions, sp, msg);
            }
...
pub fn compile_declarative_macro(
    ...
    let create_parser = || {
        ...
        Parser::new(&sess.parse_sess, body, true, crablangc_parse::MACRO_ARGUMENTS)
    };
                    ...
                    TokenTree::Sequence(sp, seq_rep) => {
                        ...
                        match sets.first.entry(sp.entire()) {
                            ...
                            Entry::Occupied(mut occ) => {
                                ...
                                // In such a case, the map cannot be tcrablanged; so mark this
                                // entry as unusable.
                                occ.insert(None);
                            }
                        }
        ...
        for tt in &last.tokens {
            if let &TokenTree::MetaVarDecl(span, name, Some(kind)) = tt.get() {
                for next_token in &suffix_first.tokens {
                    ...
                    // to warn about incompatibility with CrabLang 2021.
                        ...
                        && matches!(next_token, TokenTree::Token(token) if token.kind == BinOp(token::BinOpToken::Or))
                    {
                        ...
                        sess.buffer_lint_with_diagnostic(
                            ...
                            span,
                            ast::CRATE_NODE_ID,
                            "the meaning of the `pat` fragment specifier is changing in CrabLang 2021, which may affect this macro",
...
fn quoted_tt_to_string(tt: &mbe::TokenTree) -> String {
    match tt {
        mbe::TokenTree::Token(token) => ppcrablang::token_to_string(&token).into(),
...
pub(super) fn parser_from_cx(sess: &ParseSess, tts: TokenStream, recovery: Recovery) -> Parser<'_> {
    Parser::new(sess, tts, true, crablangc_parse::MACRO_ARGUMENTS).recovery(recovery)
}
