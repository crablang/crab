...
use crablangc_ast::mut_visit::{self, MutVisitor};
use crablangc_ast::token::{self, Delimiter, Token, TokenKind};
use crablangc_ast::tokenstream::{DelimSpan, Spacing, TokenStream, TokenTree};
use crablangc_data_structures::fx::FxHashMap;
use crablangc_errors::{pluralize, PResult};
use crablangc_errors::{DiagnosticBuilder, ErrorGuaranteed};
use crablangc_span::hygiene::{LocalExpnId, Transparency};
use crablangc_span::symbol::{sym, Ident, MacroRulesNormalizedIdent};
                ...
                if let Some(cur_matched) = lookup_cur_matched(ident, interp, &repeats) {
                    match cur_matched {
                        ...
                        MatchedNonterminal(nt) => {
                            ...
                            // `Interpolated` is currently used for such groups in crablangc parser.
