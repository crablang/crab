...
use crablangc_ast::token::{self, Token, TokenKind};
use crablangc_ast::tokenstream::TokenStream;
use crablangc_ast_pretty::ppcrablang;
use crablangc_errors::{Applicability, Diagnostic, DiagnosticBuilder, DiagnosticMessage};
...
pub(super) fn emit_frag_parse_err(
    mut e: DiagnosticBuilder<'_, crablangc_errors::ErrorGuaranteed>,
    parser: &Parser<'_>,
...
pub(super) fn parse_failure_msg(tok: &Token) -> String {
    match tok.kind {
        ...
        _ => format!("no rules expected the token `{}`", ppcrablang::token_to_string(tok),),
    }
}
