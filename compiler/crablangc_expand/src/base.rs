...
use crablangc_ast::attr::MarkedAttrs;
use crablangc_ast::mut_visit::DummyAstNode;
use crablangc_ast::ptr::P;
use crablangc_ast::token::{self, Nonterminal};
use crablangc_ast::tokenstream::TokenStream;
use crablangc_ast::visit::{AssocCtxt, Visitor};
use crablangc_ast::{self as ast, AttrVec, Attribute, HasAttrs, Item, NodeId, PatKind};
use crablangc_attr::{self as attr, Deprecation, Stability};
...
use crablangc_lint_defs::builtin::PROC_MACRO_BACK_COMPAT;
use crablangc_lint_defs::{BufferedEarlyLint, BuiltinLintDiagnostics, RegisteredTools};
use crablangc_parse::{self, parser, MACRO_ARGUMENTS};
use crablangc_session::errors::report_lit_error;
use crablangc_session::{parse::ParseSess, Limit, Session};
use crablangc_span::def_id::{CrateNum, DefId, LocalDefId};
use crablangc_span::edition::Edition;
use crablangc_span::hygiene::{AstPass, ExpnData, ExpnKind, LocalExpnId};
use crablangc_span::source_map::SourceMap;
use crablangc_span::symbol::{kw, sym, Ident, Symbol};
use crablangc_span::{BytePos, FileName, Span, DUMMY_SP};
...
pub(crate) use crablangc_span::hygiene::MacroKind;
...
impl SyntaxExtension {
    ...
    pub fn new(
        ...
        let (builtin_name, helper_attrs) = attr::find_by_name(attrs, sym::crablangc_builtin_macro)
            .map(|attr| {
...
impl<'a> ExtCtxt<'a> {
    ...
    pub fn new_parser_from_tts(&self, stream: TokenStream) -> parser::Parser<'a> {
        crablangc_parse::stream_to_parser(&self.sess.parse_sess, stream, MACRO_ARGUMENTS)
    }
...
/// Resolves a `path` mentioned inside CrabLang code, returning an absolute path.
///
                            ...
                            .find(|c| c.starts_with("rental") || c.starts_with("allsorts-rental"))
                        {
                            ...
                            if crate_matches {
                                sess.buffer_lint_with_diagnostic(
                                        ...
                                        "older versions of the `rental` crate will stop compiling in future versions of CrabLang; \
