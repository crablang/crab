...
use crablangc_ast::{Attribute, LitKind, MetaItem, MetaItemKind, MetaItemLit, NestedMetaItem, NodeId};
use crablangc_ast_pretty::ppcrablang;
use crablangc_feature::{find_gated_cfg, is_builtin_attr_name, Features, GatedCfg};
use crablangc_macros::HashStable_Generic;
use crablangc_session::lint::builtin::UNEXPECTED_CFGS;
use crablangc_session::lint::BuiltinLintDiagnostics;
use crablangc_session::parse::{feature_err, ParseSess};
use crablangc_session::Session;
use crablangc_span::hygiene::Transparency;
use crablangc_span::{symbol::sym, symbol::Symbol, Span};
...
///
/// For more, see [this pull request](https://github.com/crablang/crablang/pull/100591).
pub const VERSION_PLACEHOLDER: &str = "CURRENT_CRABLANGC_VERSION";

pub fn crablang_version_symbol() -> Symbol {
...
/// Represents the `#[crablangc_const_unstable]` and `#[crablangc_const_stable]` attributes.
#[derive(Encodable, Decodable, Copy, Clone, Debug, PartialEq, Eq, Hash)]
#[derive(HashStable_Generic)]
pub struct ConstStability {
    pub level: StabilityLevel,
    pub feature: Symbol,
    /// whether the function has a `#[crablangc_promotable]` attribute
    pub promotable: bool,
}
...
/// Represents the `#[crablangc_default_body_unstable]` attribute.
#[derive(Encodable, Decodable, Copy, Clone, Debug, PartialEq, Eq, Hash)]
#[derive(HashStable_Generic)]
...
pub enum StabilityLevel {
    ...
    Unstable {
        ...
        /// Relevant `crablang/crablang` issue.
    ...
    /// `#[stable]`
    Stable {
        /// CrabLang release which stabilized this feature.
        since: Symbol,
...
/// Collects stability info from `stable`/`unstable`/`crablangc_allowed_through_unstable_modules`
...
pub fn find_stability(
    ...
    for attr in attrs {
        match attr.name_or_empty() {
            sym::crablangc_allowed_through_unstable_modules => allowed_through_unstable_modules = true,
            sym::unstable => {
                if stab.is_some() {
    ...
    if allowed_through_unstable_modules {
        match &mut stab {
            ...
            _ => {
                sess.emit_err(session_diagnostics::CrabLangcAllowedUnstablePairing { span: item_sp });
            }
        }
    }
...
/// Collects stability info from `crablangc_const_stable`/`crablangc_const_unstable`/`crablangc_promotable`
/// attributes in `attrs`. Returns `None` if no stability attributes are found.
pub fn find_const_stability(
    ...
    for attr in attrs {
        match attr.name_or_empty() {
            sym::crablangc_promotable => promotable = true,
    ...
    if promotable {
        match &mut const_stab {
            ...
            _ => _ = sess.emit_err(session_diagnostics::CrabLangcPromotablePairing { span: item_sp }),
        }
    }
...
/// Collects stability info from `crablangc_default_body_unstable` attributes in `attrs`.
...
pub fn find_body_stability(
    ...
    for attr in attrs {
        if attr.has_name(sym::crablangc_default_body_unstable) {
...
/// Read the content of a `stable`/`crablangc_const_stable` attribute, and return the feature name and
...
fn parse_stability(sess: &Session, attr: &Attribute) -> Option<(Symbol, StabilityLevel)> {
    ...
    let insert_or_error = |meta: &MetaItem, item: &mut Option<Symbol>| {
        if item.is_some() {
            handle_errors(
                &sess.parse_sess,
                meta.span,
                AttrError::MultipleItem(ppcrablang::path_to_string(&meta.path)),
            );
            return false;
        }
            ...
            _ => {
                handle_errors(
                    ...
                    AttrError::UnknownMetaItem(
                        ppcrablang::path_to_string(&mi.path),
                        &["feature", "since"],
                    ),
                );
                return None;
    ...
    if let Some(s) = since && s.as_str() == VERSION_PLACEHOLDER {
        since = Some(crablang_version_symbol());
    }
...
/// Read the content of a `unstable`/`crablangc_const_unstable`/`crablangc_default_body_unstable`
...
fn parse_unstability(sess: &Session, attr: &Attribute) -> Option<(Symbol, StabilityLevel)> {
    ...
    match (feature, reason, issue) {
        (Some(feature), reason, Some(_)) => {
            if !crablangc_lexer::is_ident(feature.as_str()) {
...
pub fn eval_condition(
    ...
    match &cfg.kind {
        ast::MetaItemKind::List(mis) if cfg.name_or_empty() == sym::version => {
            ...
            let crablangc_version = parse_version(env!("CFG_RELEASE"), true).unwrap();

            // See https://github.com/crablang/crablang/issues/64796#issuecomment-640851454 for details
            if sess.assume_incomplete_release {
                crablangc_version > min_version
            } else {
                crablangc_version >= min_version
            }
        }
            ...
            match cfg.name_or_empty() {
                ...
                _ => {
                    sess.emit_err(session_diagnostics::InvalidPredicate {
                        span: cfg.span,
                        predicate: ppcrablang::path_to_string(&cfg.path),
                    });
                    false
                }
            }
...
pub struct Deprecation {
    ...
    /// Whether to treat the since attribute as being a CrabLang version identifier
    /// (rather than an opaque string).
    pub is_since_crablangc_version: bool,
}
    ...
    I: Iterator<Item = &'a Attribute>,
{
    ...
    let is_crablangc = sess.features_untracked().staged_api;
    ...
    'outer: for attr in attrs_iter {
        ...
        depr = Some((
            Deprecation { since, note, suggestion, is_since_crablangc_version: is_crablangc },
            attr.span,
        ));
    }
...
pub fn find_transparency(
    ...
    let mut error = None;
    for attr in attrs {
        if attr.has_name(sym::crablangc_macro_transparency) {
...
pub fn crablangc_allow_const_fn_unstable<'a>(
    sess: &'a Session,
    attrs: &'a [Attribute],
) -> impl Iterator<Item = Symbol> + 'a {
    allow_unstable(sess, attrs, sym::crablangc_allow_const_fn_unstable)
}
...
pub fn parse_alignment(node: &ast::LitKind) -> Result<u32, &'static str> {
    if let ast::LitKind::Int(literal, ast::LitIntType::Unsuffixed) = node {
        if literal.is_power_of_two() {
            // crablangc_middle::ty::layout::Align restricts align to <= 2^29
