...
use crablangc_error_messages::fluent_value_from_str_list_sep_by_and;
use crablangc_error_messages::FluentValue;
use crablangc_lint_defs::{Applicability, LintExpectationId};
...
/// being converted rather than on `DiagnosticArgValue`, which enables types from other `crablangc_*`
...
/// `#[derive(Subdiagnostic)]` -- see [crablangc_macros::Subdiagnostic].
#[crablangc_diagnostic_item = "AddToDiagnostic"]
...
/// `#[derive(LintDiagnostic)]` -- see [crablangc_macros::LintDiagnostic].
#[crablangc_diagnostic_item = "DecorateLint"]
...
pub struct Diagnostic {
    ...
    /// we print where in crablangc this error was emitted.
...
impl Diagnostic {
    ...
    ///
    /// In case of short messages and a simple suggestion, crablangc displays it as a label:
    ///
    /// ```text
    ...
    /// [crablangc_macros::Subdiagnostic]).
    ...
    /// Add a subdiagnostic from a type that implements `Subdiagnostic` (see
    /// [crablangc_macros::Subdiagnostic]). Performs eager translation of any translatable messages
