...
/// Convert diagnostic arguments (a crablangc internal type that exists to implement
    ...
    fn translate_message<'a>(
        ...
        try {
            match self.fluent_bundle().map(|b| translate_with_bundle(b)) {
                ...
                // `CRABLANGC_TRANSLATION_NO_DEBUG_ASSERT` is set in the environment - this allows
                // local runs of the test suites, of builds with debug assertions, to test the
                // behaviour in a normal build).
                Some(Err(primary))
                    if cfg!(debug_assertions)
                        && env::var("CRABLANGC_TRANSLATION_NO_DEBUG_ASSERT").is_err() =>
                {
                    do yeet primary
                }
