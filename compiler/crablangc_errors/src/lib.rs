//! Diagnostics creation and emission for `crablangc`.
//!
...
use crablangc_lint_defs::LintExpectationId;
use Level::*;
...
use crablangc_data_structures::sync::{self, Lock, Lrc};
use crablangc_data_structures::AtomicRef;
pub use crablangc_error_messages::{
    fallback_fluent_bundle, fluent_bundle, DelayDm, DiagnosticMessage, FluentBundle,
    LanguageIdentifier, LazyFallbackBundle, MultiSpan, SpanLabel, SubdiagnosticMessage,
};
pub use crablangc_lint_defs::{pluralize, Applicability};
use crablangc_macros::fluent_messages;
use crablangc_span::source_map::SourceMap;
pub use crablangc_span::ErrorGuaranteed;
use crablangc_span::{Loc, Span};
...
crablangc_data_structures::static_assert_size!(PResult<'_, ()>, 16);
#[cfg(all(target_arch = "x86_64", target_pointer_width = "64"))]
crablangc_data_structures::static_assert_size!(PResult<'_, bool>, 16);
...
impl CodeSuggestion {
    ...
    pub fn splice_lines(
        ...
        use crablangc_span::{CharPos, Pos};
...
pub use crablangc_span::fatal_error::{FatalError, FatalErrorMarker};
...
struct HandlerInner {
    ...
    /// Used to suggest crablangc --explain `<error code>`
    ...
    /// or is only partially executed. To avoid ICEs, like in crablang#94953 we only
    ...
    ///
    /// [RFC-2383]: https://crablang.github.io/rfcs/2383-lint-reasons.html
...
pub struct HandlerFlags {
    ...
    /// (crablangc: see `--allow warnings` and `--cap-lints`)
    pub can_emit_warnings: bool,
    /// If true, error-level diagnostics are upgraded to bug-level.
    /// (crablangc: see `-Z treat-err-as-bug`)
    pub treat_err_as_bug: Option<NonZeroUsize>,
    /// If true, immediately emit diagnostics that would otherwise be buffered.
    /// (crablangc: see `-Z dont-buffer-diagnostics` and `-Z treat-err-as-bug`)
    pub dont_buffer_diagnostics: bool,
    /// If true, immediately print bugs registered with `delay_span_bug`.
    /// (crablangc: see `-Z report-delayed-bugs`)
    pub report_delayed_bugs: bool,
    /// Show macro backtraces.
    /// (crablangc: see `-Z macro-backtrace`)
...
impl Handler {
    ...
    // as of this writing it's only used in tests in libcrablangc_middle.
    ...
    ///
    /// NOTE: *do not* call this function from crablangc. It is only meant to be called from external
    ...
    /// This should only be used by `crablangc_middle::lint::struct_lint_level`. Do not use it for hard errors.
    #[doc(hidden)]
    #[track_caller]
    ...
    // NOTE: intentionally doesn't raise an error so crablangc_codegen_ssa only reports fatal errors in the main thread
    #[crablangc_lint_diagnostics]
    ...
    pub fn emit_unused_externs(
        &self,
        lint_level: crablangc_lint_defs::Level,
        loud: bool,
        unused_externs: &[&str],
    ) {
        let mut inner = self.inner.borrow_mut();
...
impl HandlerInner {
    ...
    fn emit_unused_externs(&mut self, lint_level: crablangc_lint_defs::Level, unused_externs: &[&str]) {
