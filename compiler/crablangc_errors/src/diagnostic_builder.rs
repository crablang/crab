...
/// `#[derive(Diagnostic)]` -- see [crablangc_macros::Diagnostic].
#[crablangc_diagnostic_item = "IntoDiagnostic"]
...
crablangc_data_structures::static_assert_size!(
...
impl<'a> DiagnosticBuilder<'a, crablangc_span::fatal_error::FatalError> {
    /// Convenience function for internal use, clients should use one of the
    /// `struct_*` methods on [`Handler`].
    #[track_caller]
...
impl EmissionGuarantee for crablangc_span::fatal_error::FatalError {
    fn diagnostic_builder_emit_producing_guarantee(db: &mut DiagnosticBuilder<'_, Self>) -> Self {
        match db.inner.state {
            // First `.emit()` call, the `&Handler` is still available.
            DiagnosticBuilderState::Emittable(handler) => {
        ...
        // Then fatally error..
        crablangc_span::fatal_error::FatalError
    }
