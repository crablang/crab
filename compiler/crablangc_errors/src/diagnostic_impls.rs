...
use crablangc_lint_defs::Level;
use crablangc_span::edition::Edition;
use crablangc_span::symbol::{Ident, MacroRulesNormalizedIdent, Symbol};
use crablangc_span::Span;
use crablangc_target::abi::TargetDataLayoutErrors;
use crablangc_target::spec::{PanicStrategy, SplitDebuginfo, StackProtector, TargetTriple};
use crablangc_type_ir as type_ir;
use std::borrow::Cow;
use std::fmt;
...
impl IntoDiagnosticArg for ast::Path {
    fn into_diagnostic_arg(self) -> DiagnosticArgValue<'static> {
        DiagnosticArgValue::Str(Cow::Owned(ppcrablang::path_to_string(&self)))
    }
}

impl IntoDiagnosticArg for ast::token::Token {
    fn into_diagnostic_arg(self) -> DiagnosticArgValue<'static> {
        DiagnosticArgValue::Str(ppcrablang::token_to_string(&self))
    }
}

impl IntoDiagnosticArg for ast::token::TokenKind {
    fn into_diagnostic_arg(self) -> DiagnosticArgValue<'static> {
        DiagnosticArgValue::Str(ppcrablang::token_kind_to_string(&self))
    }
}
...
impl IntoDiagnosticArg for crablangc_data_structures::small_c_str::SmallCStr {
    fn into_diagnostic_arg(self) -> DiagnosticArgValue<'static> {
        DiagnosticArgValue::Str(Cow::Owned(self.to_string_lossy().into_owned()))
    }
}
...
impl IntoDiagnosticArg for ast::Visibility {
    fn into_diagnostic_arg(self) -> DiagnosticArgValue<'static> {
        let s = ppcrablang::vis_to_string(&self);
