...
use crablangc_data_structures::sync::{par_for_each_in, Send, Sync};
...
use crablangc_span::def_id::StableCrateId;
...
impl<'hir> Map<'hir> {
    ...
    pub fn crablangc_coherence_is_core(self) -> bool {
        self.krate_attrs().iter().any(|attr| attr.has_name(sym::crablangc_coherence_is_core))
    }
