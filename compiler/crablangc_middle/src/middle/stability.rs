...
use crablangc_attr::{self as attr, ConstStability, DefaultBodyStability, Deprecation, Stability};
use crablangc_data_structures::fx::FxHashMap;
use crablangc_errors::{Applicability, Diagnostic};
use crablangc_feature::GateIssue;
...
use crablangc_session::lint::builtin::{DEPRECATED, DEPRECATED_IN_FUTURE, SOFT_UNSTABLE};
use crablangc_session::lint::{BuiltinLintDiagnostics, Level, Lint, LintBuffer};
use crablangc_session::parse::feature_err_issue;
...
/// deprecated (i.e., whether X is not greater than the current crablangc version).
pub fn deprecation_in_effect(depr: &Deprecation) -> bool {
    let is_since_crablangc_version = depr.is_since_crablangc_version;
    ...
    if let Some(since) = since {
        ...
        if let Some(crablangc) = option_env!("CFG_RELEASE") {
            let since: Vec<u32> = parse_version(&since);
            let crablangc: Vec<u32> = parse_version(crablangc);
            // We simply treat invalid `since` attributes as relating to a previous
            // CrabLang version, thus always displaying the warning.
            if since.len() != 3 {
                return true;
            }
            return since <= crablangc;
        }
    };

    // Assume deprecation is in effect if "since" field is missing
    // or if we can't determine the current CrabLang version.
    true
}
...
fn deprecation_message(
    ...
    let message = if is_in_effect {
        ...
        if since == Some("TBD") {
            format!("use of {} `{}` that will be deprecated in a future CrabLang version", kind, path)
        } else {
            format!(
    ...
    pub fn eval_stability_allow_unstable(
        ...
        if let Some(id) = id {
            if let Some(depr_entry) = self.lookup_deprecation_entry(def_id) {
                ...
                if !skip || depr_attr.is_since_crablangc_version {
        ...
        match stability {
            Some(Stability {
                ...
                // `crablangc_private` crate (e.g., a compiler crate) and we also have
                // the `-Z force-unstable-if-unmarked` flag present (we're
                // compiling a compiler crate), then let this missing feature
                // annotation slide.
                if feature == sym::crablangc_private && issue == NonZeroU32::new(27812) {
