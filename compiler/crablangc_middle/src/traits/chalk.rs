...
use crablangc_middle::ty::{self, AdtDef, TyCtxt};
...
#[derive(Copy, Clone)]
pub struct CrabLangInterner<'tcx> {
    pub tcx: TyCtxt<'tcx>,
}

/// We don't ever actually need this. It's only required for derives.
impl<'tcx> Hash for CrabLangInterner<'tcx> {
    fn hash<H: Hasher>(&self, _state: &mut H) {}
}

/// We don't ever actually need this. It's only required for derives.
impl<'tcx> Ord for CrabLangInterner<'tcx> {
...
/// We don't ever actually need this. It's only required for derives.
impl<'tcx> PartialOrd for CrabLangInterner<'tcx> {
...
/// We don't ever actually need this. It's only required for derives.
impl<'tcx> PartialEq for CrabLangInterner<'tcx> {
...
/// We don't ever actually need this. It's only required for derives.
impl<'tcx> Eq for CrabLangInterner<'tcx> {}

impl fmt::Debug for CrabLangInterner<'_> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "CrabLangInterner")
    }
}
...
impl<'tcx> chalk_ir::interner::Interner for CrabLangInterner<'tcx> {
...
impl<'tcx> chalk_ir::interner::HasInterner for CrabLangInterner<'tcx> {
    type Interner = Self;
}
