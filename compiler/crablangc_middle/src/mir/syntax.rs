...
use crablangc_hir::{self as hir};
use crablangc_hir::{self, GeneratorKind};
...
pub enum MirPhase {
    ...
    ///    that CrabLang itself has them. Where exactly these are is generally subject to change, and so we
...
pub enum StatementKind<'tcx> {
    /// Assign statements roughly correspond to an assignment in CrabLang proper (`x = ...`) except
    ...
    ///
    /// [#68364]: https://github.com/crablang/crablang/issues/68364
    ///
    ...
    /// convenience method at `crablangc_mir_dataflow::storage::always_storage_live_locals` for
    ...
    /// <https://internals.crablang.org/t/stacked-borrows-an-aliasing-model-for-crablang/8153/> for
    /// more details.
    ///
...
/// object as places in CrabLang. This of course means that what exactly they are is undecided and part
/// of the CrabLang memory model. However, they will likely contain at least the following pieces of
...
/// `#![feature(unsized_locals)]` in CrabLang. Besides just putting "unsized locals are special and
...
///
/// [UCG#319]: https://github.com/crablang/unsafe-code-guidelines/issues/319
///
/// CrabLang currently requires that every place obey those two rules. This is checked by MIRI and taken
...
pub enum ProjectionElem<V, T> {
    ...
    /// crablangc can identify that a field projection refers to either two different regions of memory
    /// or the same one between the base and the 'projection element'.
    /// Read more about projections in the [crablangc-dev-guide][mir-datatypes]
    ///
    /// [mir-datatypes]: https://crablangc-dev-guide.crablang.org/mir/index.html#mir-data-types
    Field(FieldIdx, T),

    /// Index into a slice/array.
    ///
    /// Note that this does not also dereference, and so it does not exactly correspond to slice
    /// indexing in CrabLang. In other words, in the below CrabLang code:
    ///
    /// ```crablang
    /// let x = &[1, 2, 3, 4];
    /// let i = 2;
