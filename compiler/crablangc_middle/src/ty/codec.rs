...
pub use crablangc_type_ir::{TyDecoder, TyEncoder};
use std::hash::Hash;
use std::intrinsics;
...
impl<'tcx, D: TyDecoder<I = TyCtxt<'tcx>>> Decodable<D> for Ty<'tcx> {
    ...
    fn decode(decoder: &mut D) -> Ty<'tcx> {
        ...
        if decoder.positioned_at_shorthand() {
            ...
            tcx.mk_ty_from_kind(crablangc_type_ir::TyKind::decode(decoder))
        }
    }
}
...
crablangc_hir::arena_types!(impl_arena_allocatable_decoders);
...
macro_rules! implement_ty_decoder {
    ($DecoderName:ident <$($typaram:tt),*>) => {
        mod __ty_decoder_impl {
            use std::borrow::Cow;
            use crablangc_serialize::Decoder;
