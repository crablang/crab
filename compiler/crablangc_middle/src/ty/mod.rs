...
//!
//! - [`crablangc_middle::ty::Ty`], used to represent the semantics of a type.
//! - [`crablangc_middle::ty::TyCtxt`], the central data structure in the compiler.
//!
//! For more information, see ["The `ty` module: representing types"] in the crablangc-dev-guide.
//!
//! ["The `ty` module: representing types"]: https://crablangc-dev-guide.crablang.org/ty.html
...
use crablangc_ast::node_id::NodeMap;
use crablangc_attr as attr;
use crablangc_data_structures::fx::{FxHashMap, FxHashSet, FxIndexMap, FxIndexSet};
...
use crablangc_data_structures::tagged_ptr::CopyTaggedPtr;
use crablangc_errors::ErrorGuaranteed;
use crablangc_hir as hir;
use crablangc_hir::def::{CtorKind, CtorOf, DefKind, DocLinkResMap, LifetimeRes, Res};
use crablangc_hir::def_id::{CrateNum, DefId, DefIdMap, LocalDefId, LocalDefIdMap};
use crablangc_hir::Node;
use crablangc_index::vec::IndexVec;
use crablangc_macros::HashStable;
use crablangc_query_system::ich::StableHashingContext;
use crablangc_serialize::{Decodable, Encodable};
use crablangc_session::lint::LintBuffer;
pub use crablangc_session::lint::RegisteredTools;
use crablangc_span::hygiene::MacroKind;
use crablangc_span::symbol::{kw, sym, Ident, Symbol};
use crablangc_span::{ExpnId, ExpnKind, Span};
use crablangc_target::abi::{Align, FieldIdx, Integer, IntegerType, VariantIdx};
pub use crablangc_target::abi::{ReprFlags, ReprOptions};
use crablangc_type_ir::WithCachedTypeInfo;
pub use subst::*;
pub use vtable::*;
...
pub use crablangc_type_ir::AliasKind::*;
pub use crablangc_type_ir::DynKind::*;
pub use crablangc_type_ir::InferTy::*;
pub use crablangc_type_ir::RegionKind::*;
pub use crablangc_type_ir::TyKind::*;
pub use crablangc_type_ir::*;
...
pub enum ImplPolarity {
    ...
    /// `impl !Trait for Type`
    Negative,
    /// `#[crablangc_reservation_impl] impl Trait for Type`
    ///
    /// This is a "stability hack", not a real CrabLang feature.
...
impl crablangc_errors::IntoDiagnosticArg for Predicate<'_> {
    fn into_diagnostic_arg(self) -> crablangc_errors::DiagnosticArgValue<'static> {
        crablangc_errors::DiagnosticArgValue::Str(std::borrow::Cow::Owned(self.to_string()))
    }
}
...
unsafe impl crablangc_data_structures::tagged_ptr::Tag for ParamTag {
    const BITS: usize = 2;
    #[inline]
...
pub enum ImplOverlapKind {
    ...
    ///
    /// Some background: in CrabLang 1.0, the trait-object types `Send + Sync` (today's
...
impl<'tcx> TyCtxt<'tcx> {
    ...
    pub fn impls_are_allowed_to_overlap(
        ...
        match (self.impl_polarity(def_id1), self.impl_polarity(def_id2)) {
            (ImplPolarity::Reservation, _) | (_, ImplPolarity::Reservation) => {
                // `#[crablangc_reservation_impl]` impls don't overlap with anything
                return Some(ImplOverlapKind::Permitted { marker: false });
            }
    ...
    pub fn get_attrs(
        ...
        } else if cfg!(debug_assertions) && crablangc_feature::is_builtin_only_local(attr) {
    ...
    pub fn get_attr(self, did: impl Into<DefId>, attr: Symbol) -> Option<&'tcx ast::Attribute> {
        if cfg!(debug_assertions) && !crablangc_feature::is_valid_for_get_attr(attr) {
            let did: DefId = did.into();
    ...
    pub fn has_attr(self, did: impl Into<DefId>, attr: Symbol) -> bool {
        let did: DefId = did.into();
        if cfg!(debug_assertions) && !did.is_local() && crablangc_feature::is_builtin_only_local(attr) {
            bug!("tried to access the `only_local` attribute `{}` from an extern crate", attr);
        } else {
    ...
    /// an auto trait or because it has the `#[crablangc_coinductive]` attribute.
    ...
    pub fn is_builtin_derived(self, def_id: DefId) -> bool {
        if self.is_automatically_derived(def_id)
            ...
            && self.has_attr(outer.macro_def_id.unwrap(), sym::crablangc_builtin_macro)
        {
            true
        } else {
            false
