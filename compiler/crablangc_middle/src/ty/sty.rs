...
use crablangc_errors::{DiagnosticArgValue, IntoDiagnosticArg};
...
use crablangc_target::abi::{FieldIdx, VariantIdx, FIRST_VARIANT};
use crablangc_target::spec::abi::{self, Abi};
use std::borrow::Cow;
...
use crablangc_type_ir::RegionKind as IrRegionKind;
use crablangc_type_ir::TyKind as IrTyKind;

// Re-export the `TyKind` from `crablangc_type_ir` here for convenience
#[crablangc_diagnostic_item = "TyKind"]
...
impl<'tcx> ClosureSubsts<'tcx> {
    ...
    // see `crablangc_hir_analysis::check::closure`.
...
impl<'tcx> GeneratorSubsts<'tcx> {
    ...
    /// It is calculated in crablangc_mir_transform::generator::StateTransform.
...
impl<'tcx> List<ty::PolyExistentialPredicate<'tcx>> {
    ...
    ///
    /// A CrabLang trait object type consists (in addition to a lifetime bound)
...
impl<'tcx> PolyFnSig<'tcx> {
    ...
    pub fn is_fn_trait_compatible(&self) -> bool {
        matches!(
            self.skip_binder(),
            ty::FnSig {
                unsafety: crablangc_hir::Unsafety::Normal,
                abi: Abi::CrabLang,
                c_variadic: false,
                ..
            }
...
impl<'tcx> Ty<'tcx> {
            ...
            Adt(def, substs) => {
                ...
                match f0_ty.kind() {
                    ...
                    Array(f0_elem_ty, f0_len) => {
                        // FIXME(repr_simd): https://github.com/crablang/crablang/pull/78863#discussion_r522784112
    ...
    /// contents are abstract to crablangc.)
    #[inline]
    ...
    pub fn discriminant_for_variant(
        ...
        match self.kind() {
            TyKind::Adt(adt, _) if adt.variants().is_empty() => {
                ...
                // https://github.com/crablang/crablang/issues/89765.
                None
            }
    ...
    /// inferred. Once upvar inference (in `crablangc_hir_analysis/src/check/upvar.rs`)
