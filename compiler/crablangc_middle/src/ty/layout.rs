...
use crablangc_errors::{DiagnosticBuilder, Handler, IntoDiagnostic};
...
use crablangc_target::spec::{abi::Abi as SpecAbi, HasTargetSpec, PanicStrategy, Target};
...
impl IntegerExt for Integer {
    ...
    fn repr_discr<'tcx>(
        ...
        let at_least = if repr.c() {
            ...
            // repr(CrabLang) enums try to be as small as possible
            I8
        };
...
pub type TyAndLayout<'tcx> = crablangc_target::abi::TyAndLayout<'tcx, Ty<'tcx>>;
    ...
    fn ty_and_layout_for_variant(
        ...
        let layout = match this.variants {
            Variants::Single { index }
                ...
                // For more details see https://github.com/crablang/crablang/issues/69763.
...
///   flags is the empty set. This is only applicable for CrabLang-defined
...
///
/// Note that in this case unwinding is not necessarily panicking in CrabLang. CrabLang
...
/// Notably unwinding is disallowed for more non-CrabLang ABIs unless it's
...
///
/// CrabLang functions are classified whether or not they can unwind based on the
/// active "panic strategy". In other words CrabLang functions are considered to
/// unwind in `-Cpanic=unwind` mode and cannot unwind in `-Cpanic=abort` mode.
/// Note that CrabLang supports intermingling panic=abort and panic=unwind code, but
...
///
/// FIXME: this is actually buggy with respect to CrabLang functions. CrabLang functions
...
pub fn fn_can_unwind(tcx: TyCtxt<'_>, fn_def_id: Option<DefId>, abi: SpecAbi) -> bool {
    if let Some(did) = fn_def_id {
        ...
        // function defined in CrabLang is also required to abort.
    ...
    // ABIs have such an option. Otherwise the only other thing here is CrabLang
    // itself, and those ABIs are determined by the panic strategy configured
    // for this compilation.
    //
    // Unfortunately at this time there's also another caveat. CrabLang [RFC
    ...
    // crablangc behavior as well as plan for future crablangc behavior.
    //
    ...
    // externally declared functions as well as CrabLang-defined function.
    //
    // To fix this UB crablangc would like to change in the future to catch unwinds
    // from function calls that may unwind within a CrabLang-defined `extern "C"`
    ...
    //
    // Note that this means that unlike historical compilers crablangc now, by
    ...
    // ABI.
    //
    // [rfc]: https://github.com/crablang/rfcs/blob/master/text/2945-c-unwind-abi.md
    use SpecAbi::*;
    match abi {
        C { unwind }
        | System { unwind }
        ...
        | PlatformIntrinsic
        | Unadjusted => false,
        CrabLang | CrabLangCall | CrabLangCold => tcx.sess.panic_strategy() == PanicStrategy::Unwind,
    }
}
