...
use crablangc_index::bit_set::FiniteBitSet;
use crablangc_macros::HashStable;
use crablangc_middle::ty::normalize_erasing_regions::NormalizationError;
...
pub enum InstanceDef<'tcx> {
    ...
    /// An intrinsic `fn` item (with `"crablang-intrinsic"` or `"platform-intrinsic"` ABI).
    ///
...
impl<'tcx> InstanceDef<'tcx> {
    ...
    pub fn get_attrs(
        ...
        tcx: TyCtxt<'tcx>,
        attr: Symbol,
    ) -> impl Iterator<Item = &'tcx crablangc_ast::Attribute> {
...
fn fmt_instance(
    ...
    instance: &Instance<'_>,
    type_length: crablangc_session::Limit,
) -> fmt::Result {
    ty::tls::with(|tcx| {
...
impl<'a, 'tcx> fmt::Display for ShortInstance<'a, 'tcx> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt_instance(f, self.0, crablangc_session::Limit(self.1))
    }
}
