...
use crablangc_apfloat::ieee::{Double, Single};
use crablangc_data_structures::fx::{FxHashMap, FxIndexMap};
use crablangc_data_structures::sso::SsoHashSet;
use crablangc_hir as hir;
use crablangc_hir::def::{self, CtorKind, DefKind, Namespace};
use crablangc_hir::def_id::{DefId, DefIdSet, CRATE_DEF_ID, LOCAL_CRATE};
use crablangc_hir::definitions::{DefKey, DefPathData, DefPathDataName, DisambiguatedDefPathData};
use crablangc_hir::LangItem;
use crablangc_session::config::TrimmedDefPaths;
use crablangc_session::cstore::{ExternCrate, ExternCrateSource};
use crablangc_session::Limit;
use crablangc_span::symbol::{kw, Ident, Symbol};
use crablangc_span::FileNameDisplayPreference;
...
define_helper!(
    ...
    /// of various crablangc types, for example `std::vec::Vec` would be trimmed to `Vec`,
    ...
    fn pretty_print_type(mut self, ty: Ty<'tcx>) -> Result<Self::Type, Self::Error> {
        ...
        match *ty.kind() {
            ...
            ty::Alias(ty::Opaque, ty::AliasTy { def_id, substs, .. }) => {
                ...
                // from `crablangc_middle::ty` during the computation of `tcx.predicates_of`),
                ...
                if !args.is_empty() || !assoc_items.is_empty() {
                    ...
                    for (assoc_item_def_id, term) in assoc_items {
                        ...
                        let term = if let Some(ty) = term.skip_binder().ty()
                            ...
                            && assoc.name == crablangc_span::sym::Return
                        {
    ...
    fn path_crate(mut self, cnum: CrateNum) -> Result<Self::Path, Self::Error> {
        ...
        if cnum == LOCAL_CRATE {
            if self.tcx.sess.crablang_2018() {
                // We add the `crate::` keyword on CrabLang 2018, only when desired.
...
///
/// See also [`DelayDm`](crablangc_error_messages::DelayDm) and [`with_no_trimmed_paths`].
fn trimmed_def_paths(tcx: TyCtxt<'_>, (): ()) -> FxHashMap<DefId, Symbol> {
    ...
    if let TrimmedDefPaths::GoodPath = tcx.sess.opts.trimmed_def_paths {
        ...
        //
        // For good paths causing this bug, the `crablangc_middle::ty::print::with_no_trimmed_paths`
