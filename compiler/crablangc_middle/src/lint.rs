...
use crablangc_errors::{Diagnostic, DiagnosticBuilder, DiagnosticId, DiagnosticMessage, MultiSpan};
use crablangc_hir::{HirId, ItemLocalId};
...
use crablangc_span::source_map::{DesugaringKind, ExpnKind};
use crablangc_span::{symbol, Span, Symbol, DUMMY_SP};
...
pub struct LintExpectation {
    ...
    /// goes for `crablangdoc`. This will be `None` for crablangc lints
    ...
    fn struct_lint_level_impl(
        ...
        let mut err = match (level, span) {
            ...
            (Level::Expect(expect_id), _) => {
                ...
                // need the lint diagnostic to be emitted to `crablangc_error::HandlerInner`.
                //
        ...
        if err.span.primary_spans().iter().any(|s| in_external_macro(sess, *s)) {
            ...
            // emit shouldn't be automatically fixed by crablangfix.
        ...
        if let Some(future_incompatible) = future_incompatible {
            let explanation = match future_incompatible.reason {
                ...
                FutureIncompatibilityReason::EditionError(edition) => {
                    ...
                    format!(
                        "this is accepted in the current edition (CrabLang {}) but is a hard error in CrabLang {}!",
                        current_edition, edition
                    )
                }
                FutureIncompatibilityReason::EditionSemanticsChange(edition) => {
                    format!("this changes meaning in CrabLang {}", edition)
                }
