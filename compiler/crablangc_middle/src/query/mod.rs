...
//! ["Queries: demand-driven compilation"](https://crablangc-dev-guide.crablang.org/query.html).
...
crablangc_queries! {
    ...
    /// of crablangc_middle::hir::source_map.
    ...
    query hir_crate_items(_: ()) -> &'tcx crablangc_middle::hir::ModuleItems {
        arena_cache
        eval_always
    ...
    query hir_module_items(key: LocalDefId) -> &'tcx crablangc_middle::hir::ModuleItems {
        arena_cache
    ...
    /// Returns the [`Ty`][crablangc_middle::ty::Ty] of the given [`DefId`]. If the [`DefId`] points
    /// to an alias, it will "skip" this alias to return the aliased type.
    ///
    /// [`DefId`]: crablangc_hir::def_id::DefId
    ...
    query unsizing_params_for_adt(key: DefId) -> &'tcx crablangc_index::bit_set::BitSet<u32>
    {
        arena_cache
        desc { |tcx|
    ...
    ///
    /// This is an extra query to enable other drivers (like crablangdoc) to
    /// only execute a small subset of the `analysis` query, while allowing
    /// lints to be expected. In crablangc, this query will be executed as part of
    ...
    /// name. This is useful for cases were not all linting code from crablangc
    ...
    query shallow_lint_levels_on(key: hir::OwnerId) -> &'tcx crablangc_middle::lint::ShallowLintLevelMap {
    ...
    query expn_that_defined(key: DefId) -> crablangc_span::ExpnId {
    ...
    query representability(_: LocalDefId) -> crablangc_middle::ty::Representability {
    ...
    query representability_adt_ty(_: Ty<'tcx>) -> crablangc_middle::ty::Representability {
    ...
    query params_in_repr(key: DefId) -> &'tcx crablangc_index::bit_set::BitSet<u32> {
    ...
    /// constructors.
    query mir_keys(_: ()) -> &'tcx crablangc_data_structures::fx::FxIndexSet<LocalDefId> {
        arena_cache
    ...
    query super_predicates_that_define_assoc_type(key: (DefId, Option<crablangc_span::symbol::Ident>)) -> ty::GenericPredicates<'tcx> {
    ...
    query type_param_predicates(key: (LocalDefId, LocalDefId, crablangc_span::symbol::Ident)) -> ty::GenericPredicates<'tcx> {
    ...
    /// constructor, or because it has the `#[crablangc_promotable]` attribute. The attribute should
    ...
    query const_caller_location(key: (crablangc_span::Symbol, u32, u32)) -> ConstValue<'tcx> {
    ...
    query fn_arg_names(def_id: DefId) -> &'tcx [crablangc_span::symbol::Ident] {
    ...
    query all_local_trait_impls(_: ()) -> &'tcx crablangc_data_structures::fx::FxIndexMap<DefId, Vec<LocalDefId>> {
    ...
    /// has significant drop. A type marked with the attribute `crablangc_insignificant_dtor`
    ...
    /// Finds the `crablangc_proc_macro_decls` item of a crate.
    query proc_macro_decls_static(_: ()) -> Option<LocalDefId> {
        desc { "looking up the proc macro declarations for a crate" }
    }

    // The macro which defines `crablangc_metadata::provide_extern` depends on this query's name.
    ...
    /// See `crablangc_resolve::late::lifetimes` for details.
    ...
    query all_diagnostic_items(_: ()) -> &'tcx crablangc_hir::diagnostic_items::DiagnosticItems {
        arena_cache
        eval_always
    ...
    query diagnostic_items(_: CrateNum) -> &'tcx crablangc_hir::diagnostic_items::DiagnosticItems {
        arena_cache
    ...
    query debugger_visualizers(_: CrateNum) -> &'tcx Vec<crablangc_span::DebuggerVisualizerFile> {
        arena_cache
    ...
    /// A list of all traits in a crate, used by crablangdoc and error reporting.
    ...
    query features_query(_: ()) -> &'tcx crablangc_feature::Features {
        feedable
        desc { "looking up enabled feature gates" }
    }

    query metadata_loader((): ()) -> &'tcx Steal<Box<crablangc_session::cstore::MetadataLoaderDyn>> {
        feedable
        no_hash
        desc { "raw operations for metadata file access" }
    }

    query crate_for_resolver((): ()) -> &'tcx Steal<(crablangc_ast::Crate, crablangc_ast::AttrVec)> {
        feedable
        no_hash
