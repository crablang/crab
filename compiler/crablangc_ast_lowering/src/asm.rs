...
impl<'a, 'hir> LoweringContext<'a, 'hir> {
    pub(crate) fn lower_inline_asm(
        ...
        // CrabLangdoc needs to support asm! from foreign architectures: don't try
        // lowering the register constraints in this case.
        let asm_arch =
            if self.tcx.sess.opts.actually_crablangdoc { None } else { self.tcx.sess.asm_arch };
        if asm_arch.is_none() && !self.tcx.sess.opts.actually_crablangdoc {
        ...
        if asm.options.contains(InlineAsmOptions::ATT_SYNTAX)
            ...
            && !self.tcx.sess.opts.actually_crablangdoc
        {
