...
use crablangc_data_structures::profiling::TimePassesFormat;
use crablangc_errors::{LanguageIdentifier, TerminalUrl};
use crablangc_target::spec::{CodeModel, LinkerFlavorCli, MergeFunctions, PanicStrategy, SanitizerSet};
use crablangc_target::spec::{
    RelocModel, RelroLevel, SplitDebuginfo, StackProtector, TargetTriple, TlsModel,
};

use crablangc_feature::UnstableFeatures;
use crablangc_span::edition::Edition;
use crablangc_span::RealFileName;
use crablangc_span::SourceFileHashAlgorithm;
...
top_level_options!(
    ...
    #[crablangc_lint_opt_ty]
    pub struct Options {
        /// The crate config requested for the session, which may be combined
        /// with additional crate configurations during the compile process.
        #[crablangc_lint_opt_deny_field_access("use `Session::crate_types` instead of this field")]
        ...
        /// Indicates whether this run of the compiler is actually crablangdoc. This
        /// is currently just a hack and will be removed eventually, so please
        /// try to not rely on this too much.
        actually_crablangdoc: bool [TRACKED],
        ...
        /// what crablangc was invoked with, but massaged a bit to agree with
        /// commands like `--emit llvm-ir` which they're often incompatible with
        /// if we otherwise use the defaults of crablangc.
        #[crablangc_lint_opt_deny_field_access("use `Session::codegen_units` instead of this field")]
        cli_forced_codegen_units: Option<usize> [UNTRACKED],
        #[crablangc_lint_opt_deny_field_access("use `Session::lto` instead of this field")]
        cli_forced_local_thinlto_off: bool [UNTRACKED],

        /// Remap source path prefixes in all output (messages, object files, debug, etc.).
        remap_path_prefix: Vec<(PathBuf, PathBuf)> [TRACKED_NO_CRATE_HASH],
        /// Base directory containing the `src/` for the CrabLang standard library, and
        /// potentially `crablangc` as well, if we can find it. Right now it's always
        /// `$sysroot/lib/crablanglib/src/crablang` (i.e. the `crablangup` `crablang-src` component).
        ///
        /// This directory is what the virtual `/crablangc/$hash` is translated back to,
        /// if CrabLang was built with path remapping to `/crablangc/$hash` enabled
        /// (the `crablang.remap-debuginfo` option in `config.toml`).
        real_crablang_source_base_dir: Option<PathBuf> [TRACKED_NO_CRATE_HASH],
...
mod desc {
    ...
    pub const parse_linker_flavor: &str = ::crablangc_target::spec::LinkerFlavorCli::one_of();
    ...
    pub const parse_relocation_model: &str =
        "one of supported relocation models (`crablangc --print relocation-models`)";
    pub const parse_code_model: &str = "one of supported code models (`crablangc --print code-models`)";
    pub const parse_tls_model: &str = "one of supported TLS models (`crablangc --print tls-models`)";
...
mod parse {
    ...
    pub(crate) fn parse_opt_langid(slot: &mut Option<LanguageIdentifier>, v: Option<&str>) -> bool {
        match v {
            Some(s) => {
                *slot = crablangc_errors::LanguageIdentifier::from_str(s).ok();
                true
            }
            None => false,
        }
...
options! {
    ...
    // - compiler/crablangc_interface/src/tests.rs
    // - src/doc/crablangc/src/codegen-options/index.md

    // tidy-alphabetical-start
    ar: String = (String::new(), parse_string, [UNTRACKED],
        "this option is deprecated and does nothing"),
    #[crablangc_lint_opt_deny_field_access("use `Session::code_model` instead of this field")]
    code_model: Option<CodeModel> = (None, parse_code_model, [TRACKED],
        "choose the code model to use (`crablangc --print code-models` for details)"),
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::must_emit_unwind_tables` instead of this field")]
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::instrument_coverage` instead of this field")]
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::link_dead_code` instead of this field")]
    link_dead_code: Option<bool> = (None, parse_opt_bool, [TRACKED],
        "keep dead code at link time (useful for code coverage) (default: no)"),
    link_self_contained: Option<bool> = (None, parse_opt_bool, [UNTRACKED],
        "control whether to link CrabLang provided C objects/libraries or rely
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::overflow_checks` instead of this field")]
    overflow_checks: Option<bool> = (None, parse_opt_bool, [TRACKED],
        "use overflow checks for integer arithmetic"),
    #[crablangc_lint_opt_deny_field_access("use `Session::panic_strategy` instead of this field")]
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::relocation_model` instead of this field")]
    relocation_model: Option<RelocModel> = (None, parse_relocation_model, [TRACKED],
        "control generation of position-independent code (PIC) \
        (`crablangc --print relocation-models` for details)"),
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::split_debuginfo` instead of this field")]
    ...
    target_cpu: Option<String> = (None, parse_opt_string, [TRACKED],
        "select target processor (`crablangc --print target-cpus` for details)"),
    target_feature: String = (String::new(), parse_target_feature, [TRACKED],
        "target specific attributes. (`crablangc --print target-features` for details). \
...
options! {
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::binary_dep_depinfo` instead of this field")]
    ...
    dump_dep_graph: bool = (false, parse_bool, [UNTRACKED],
        "dump the dependency graph to $CRABLANG_DEP_GRAPH (default: /tmp/dep_graph.gv) \
        (default: no)"),
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::fewer_names` instead of this field")]
    ...
    force_unstable_if_unmarked: bool = (false, parse_bool, [TRACKED],
        "force all crates to be `crablangc_private` unstable (default: no)"),
    ...
    graphviz_font: String = ("Courier, monospace".to_string(), parse_string, [UNTRACKED],
        ...
        environment variable `CRABLANGC_GRAPHVIZ_FONT` (default: `Courier, monospace`)"),
    ...
    incremental_info: bool = (false, parse_bool, [UNTRACKED],
        ...
        (default: no)"),
    #[crablangc_lint_opt_deny_field_access("use `Session::incremental_relative_spans` instead of this field")]
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::mir_opt_level` instead of this field")]
    ...
    normalize_docs: bool = (false, parse_bool, [TRACKED],
        "normalize associated items in crablangdoc when generating documentation"),
    ...
    print_fuel: Option<String> = (None, parse_opt_string, [TRACKED],
        "make crablangc print the total optimization fuel used by a crate"),
    ...
    /// keep this in sync with the event filter names in libcrablangc_data_structures/profiling.rs
    ...
    simulate_remapped_crablang_src_base: Option<PathBuf> = (None, parse_opt_pathbuf, [TRACKED],
        "simulate the effect of remap-debuginfo = true at bootstrapping by remapping path \
        to crablang's source base directory. only meant for testing purposes"),
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::stack_protector` instead of this field")]
    stack_protector: StackProtector = (StackProtector::None, parse_stack_protector, [TRACKED],
        "control stack smash protection strategy (`crablangc --print stack-protector-strategies` for details)"),
    ...
    symbol_mangling_version: Option<SymbolManglingVersion> = (None,
        parse_symbol_mangling_version, [TRACKED],
        "which mangling version to use for symbol names ('legacy' (default) or 'v0')"),
    #[crablangc_lint_opt_deny_field_access("use `Session::teach` instead of this field")]
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::threads` instead of this field")]
    threads: usize = (1, parse_threads, [UNTRACKED],
        "use a thread pool with N threads"),
    time_llvm_passes: bool = (false, parse_bool, [UNTRACKED],
        "measure time of each LLVM pass (default: no)"),
    time_passes: bool = (false, parse_bool, [UNTRACKED],
        "measure time of each crablangc pass (default: no)"),
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::tls_model` instead of this field")]
    tls_model: Option<TlsModel> = (None, parse_tls_model, [TRACKED],
        "choose the TLS model to use (`crablangc --print tls-models` for details)"),
    trace_macros: bool = (false, parse_bool, [UNTRACKED],
        "for every macro invocation, print its name and arguments (default: no)"),
    track_diagnostics: bool = (false, parse_bool, [UNTRACKED],
        "tracks where in crablangc a diagnostic was emitted"),
    trait_solver: TraitSolver = (TraitSolver::Classic, parse_trait_solver, [TRACKED],
        "specify the trait solver mode used by crablangc (default: classic)"),
    ...
    tune_cpu: Option<String> = (None, parse_opt_string, [TRACKED],
        "select processor to schedule for (`crablangc --print target-cpus` for details)"),
    ...
    #[crablangc_lint_opt_deny_field_access("use `Session::unstable_options` instead of this field")]
    unstable_options: bool = (false, parse_bool, [UNTRACKED],
        "adds unstable command line options to crablangc interface (default: no)"),
    use_ctors_section: Option<bool> = (None, parse_opt_bool, [TRACKED],
        "use legacy .ctors section for initializers rather than .init_array"),
    validate_mir: bool = (false, parse_bool, [UNTRACKED],
        "validate MIR after each transformation"),
    #[crablangc_lint_opt_deny_field_access("use `Session::verbose` instead of this field")]
    verbose: bool = (false, parse_bool, [UNTRACKED],
        "in general, enable more debug printouts (default: no)"),
    #[crablangc_lint_opt_deny_field_access("use `Session::verify_llvm_ir` instead of this field")]
