...
use crablangc_data_structures::stable_hasher::{StableOrd, ToStableHashKey};
use crablangc_target::abi::Align;
use crablangc_target::spec::{PanicStrategy, SanitizerSet, SplitDebuginfo};
use crablangc_target::spec::{Target, TargetTriple, TargetWarnings, TARGETS};

use crate::parse::{CrateCheckConfig, CrateConfig};
use crablangc_feature::UnstableFeatures;
use crablangc_span::edition::{Edition, DEFAULT_EDITION, EDITION_NAME_LIST, LATEST_STABLE_EDITION};
use crablangc_span::source_map::{FileName, FilePathMapping};
use crablangc_span::symbol::{sym, Symbol};
use crablangc_span::RealFileName;
use crablangc_span::SourceFileHashAlgorithm;

use crablangc_errors::emitter::HumanReadableErrorType;
use crablangc_errors::{ColorConfig, DiagnosticArgValue, HandlerFlags, IntoDiagnosticArg};
...
pub enum ErrorOutputType {
    ...
    /// Output that's consumed by other tools such as `crablangfix` or the `RLS`.
    Json {
...
pub enum TraitSolver {
    /// Classic trait solver in `crablangc_trait_selection::traits::select`
    Classic,
    /// Chalk trait solver
    Chalk,
    /// Experimental trait solver in `crablangc_trait_selection::solve`
    Next,
}
...
pub const CRABLANG_CGU_EXT: &str = "rcgu";
    ...
    pub fn temp_path_ext(&self, ext: &str, codegen_unit_name: Option<&str>) -> PathBuf {
        ...
        if !ext.is_empty() {
            if !extension.is_empty() {
                extension.push('.');
                extension.push_str(CRABLANG_CGU_EXT);
                extension.push('.');
            }
...
pub fn host_triple() -> &'static str {
    ...
    // compile time) the target triple that this crablangc is built with and
...
pub enum EntryFnType {
    Main {
        ...
        /// across crablangc and libstd, but we don't want it public in libstd,
...
/// `crablangc_interface::interface::Config` accepts this in the compiler configuration,
        ...
        early_error(
            ...
            &format!(
                "Error loading target specification: {}. \
                 Run `crablangc --print target-list` for a list of built-in targets",
                e
            ),
        )
    });
...
pub struct CrabLangcOptGroup {
    pub apply: Box<dyn Fn(&mut getopts::Options) -> &mut getopts::Options>,
    pub name: &'static str,
    pub stability: OptionStability,
}

impl CrabLangcOptGroup {
    pub fn is_stable(&self) -> bool {
        self.stability == OptionStability::Stable
    }

    pub fn stable<F>(name: &'static str, f: F) -> CrabLangcOptGroup
    where
        F: Fn(&mut getopts::Options) -> &mut getopts::Options + 'static,
    {
        CrabLangcOptGroup { name, apply: Box::new(f), stability: OptionStability::Stable }
    }

    pub fn unstable<F>(name: &'static str, f: F) -> CrabLangcOptGroup
    where
        F: Fn(&mut getopts::Options) -> &mut getopts::Options + 'static,
    {
        CrabLangcOptGroup { name, apply: Box::new(f), stability: OptionStability::Unstable }
    }
}

// The `opt` local module holds wrappers around the `getopts` API that
// adds extra crablangc-specific metadata to each option; such metadata
...
mod opt {
    ...
    use super::CrabLangcOptGroup;

    pub type R = CrabLangcOptGroup;
    ...
    where
        F: Fn(&mut getopts::Options) -> &mut getopts::Options + 'static,
    {
        CrabLangcOptGroup::stable(name, f)
    }
    ...
    where
        F: Fn(&mut getopts::Options) -> &mut getopts::Options + 'static,
    {
        CrabLangcOptGroup::unstable(name, f)
    }
...
/// Returns the "short" subset of the crablangc command line options,
/// including metadata for each option, such as whether the option is
/// part of the stable long-term interface for crablangc.
pub fn crablangc_short_optgroups() -> Vec<CrabLangcOptGroup> {
    vec![
...
/// Returns all crablangc command line options, including metadata for
/// each option, such as whether the option is part of the stable
/// long-term interface for crablangc.
pub fn crablangc_optgroups() -> Vec<CrabLangcOptGroup> {
    let mut opts = crablangc_short_optgroups();
    // FIXME: none of these descriptions are actually used
    opts.extend(vec![
        opt::multi_s(
            "",
            "extern",
            "Specify where an external crablang library is located",
            "NAME[=PATH]",
        ),
        ...
        opt::opt_s(
            "",
            "diagnostic-width",
            "Inform crablangc of the width of the output so that diagnostics can be truncated to fit",
            "WIDTH",
        ),
        opt::multi_s(
            "",
...
pub fn parse_crate_edition(matches: &getopts::Matches) -> Edition {
    ...
    if !edition.is_stable() && !nightly_options::is_unstable_enabled(matches) {
        ...
        let msg = if !is_nightly {
            format!(
                "the crate requires edition {}, but the latest edition supported by this CrabLang version is {}",
...
pub fn build_session_options(matches: &getopts::Matches) -> Options {
    ...
    if let Ok(graphviz_font) = std::env::var("CRABLANGC_GRAPHVIZ_FONT") {
    ...
    // Try to find a directory containing the CrabLang `src`, for more details see
    // the doc comment on the `real_crablang_source_base_dir` field.
    let tmp_buf;
    ...
    let real_crablang_source_base_dir = {
        // This is the location used by the `crablang-src` `crablangup` component.
        let mut candidate = sysroot.join("lib/crablanglib/src/crablang");
        if let Ok(metadata) = candidate.symlink_metadata() {
            // Replace the symlink crablangbuild creates, with its destination.
...
pub fn make_crate_type_option() -> CrabLangcOptGroup {
    opt::multi_s(
        "",
        "crate-type",
...
pub mod nightly_options {
    use super::{ErrorOutputType, OptionStability, CrabLangcOptGroup};
    ...
    pub fn check_nightly_options(matches: &getopts::Matches, flags: &[CrabLangcOptGroup]) {
...
pub(crate) mod dep_tracking {
    ...
    use crablangc_errors::LanguageIdentifier;
    use crablangc_feature::UnstableFeatures;
    use crablangc_span::edition::Edition;
    use crablangc_span::RealFileName;
    use crablangc_target::spec::{CodeModel, MergeFunctions, PanicStrategy, RelocModel};
...
impl OomStrategy {
    pub const SYMBOL: &'static str = "__crablang_alloc_error_handler_should_panic";
