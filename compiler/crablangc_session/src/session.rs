...
pub use crablangc_ast::attr::MarkedAttrs;
pub use crablangc_ast::Attribute;
use crablangc_data_structures::flock;
use crablangc_data_structures::fx::{FxHashMap, FxIndexSet};
use crablangc_data_structures::jobserver::{self, Client};
use crablangc_data_structures::profiling::{duration_to_secs_str, SelfProfiler, SelfProfilerRef};
use crablangc_data_structures::sync::{
    self, AtomicU64, AtomicUsize, Lock, Lrc, OnceCell, OneThread, Ordering, Ordering::SeqCst,
};
use crablangc_errors::annotate_snippet_emitter_writer::AnnotateSnippetEmitterWriter;
use crablangc_errors::emitter::{Emitter, EmitterWriter, HumanReadableErrorType};
use crablangc_errors::json::JsonEmitter;
use crablangc_errors::registry::Registry;
...
pub use crablangc_span::def_id::StableCrateId;
use crablangc_span::edition::Edition;
use crablangc_span::source_map::{FileLoader, RealFileLoader, SourceMap, Span};
use crablangc_span::{SourceFileHashAlgorithm, Symbol};
use crablangc_target::asm::InlineAsmArch;
use crablangc_target::spec::{CodeModel, PanicStrategy, RelocModel, RelroLevel};
...
impl crablangc_errors::IntoDiagnosticArg for Limit {
...
pub struct Session {
    ...
    /// `crablangc_symbol_mangling` crate for more information.
    pub stable_crate_id: OnceCell<StableCrateId>,

    features: OnceCell<crablangc_feature::Features>,
...
impl Session {
    ...
    pub fn needs_crate_hash(&self) -> bool {
        ...
        //   `crablangc_query_system::query::plumbing::execute_job`.
    ...
    ///
    /// [`DUMMY_SP`]: crablangc_span::DUMMY_SP
    #[track_caller]
    ...
    pub fn delay_good_path_bug(&self, msg: impl Into<DiagnosticMessage>) {
        if self.opts.unstable_opts.print_type_sizes
            ...
            || std::env::var_os("CRABLANGC_LOG").is_some()
        {
            return;
        }
    ...
    #[inline]
    pub fn diagnostic(&self) -> &crablangc_errors::Handler {
    ...
    /// `-Zunstable-options` is provided and this isn't crablangdoc (internal lints can trigger errors
    /// to be emitted under crablangdoc).
    pub fn enable_internal_lints(&self) -> bool {
        self.unstable_options() && !self.opts.actually_crablangdoc
    }
    ...
    #[inline]
    pub fn features_untracked(&self) -> &crablangc_feature::Features {
        self.features.get().unwrap()
    }

    pub fn init_features(&self, features: crablangc_feature::Features) {
    ...
    pub fn generate_proc_macro_decls_symbol(&self, stable_crate_id: StableCrateId) -> String {
        format!("__crablangc_proc_macro_decls_{:08x}__", stable_crate_id.to_u64())
    }
    ...
    pub fn get_tools_search_paths(&self, self_contained: bool) -> Vec<PathBuf> {
        let crablanglib_path = crablangc_target::target_crablanglib_path(&self.sysroot, &config::host_triple());
...
impl Session {
    ...
    pub fn verify_llvm_ir(&self) -> bool {
        self.opts.unstable_opts.verify_llvm_ir || option_env!("CRABLANGC_VERIFY_LLVM_IR").is_some()
    }
    ...
    pub fn must_emit_unwind_tables(&self) -> bool {
        ...
        // Historically crablangc always emits `uwtable` attributes by default, so
    ...
    pub fn codegen_units(&self) -> usize {
        ...
        // large number of threads in parallel. The backend of crablangc contains
...
fn default_emitter(
    ...
    registry: crablangc_errors::registry::Registry,
...
pub fn build_session(
    sopts: config::Options,
    io: CompilerIO,
    bundle: Option<Lrc<crablangc_errors::FluentBundle>>,
    registry: crablangc_errors::registry::Registry,
    ...
    let span_diagnostic = crablangc_errors::Handler::with_emitter_and_flags(
        emitter,
    ...
    let ctfe_backtrace = Lock::new(match env::var("CRABLANGC_CTFE_BACKTRACE") {
...
fn validate_commandline_args_with_session_available(sess: &Session) {
    ...
    // dynamically downstream, crablangc generates `__imp_` symbols that help linkers
...
fn early_error_handler(output: config::ErrorOutputType) -> crablangc_errors::Handler {
    ...
    let fallback_bundle =
        fallback_fluent_bundle(vec![crablangc_errors::DEFAULT_LOCALE_RESOURCE], false);
    ...
    crablangc_errors::Handler::with_emitter(true, None, emitter)
}
