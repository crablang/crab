...
metadata_crablangc_lib_required =
    crate `{$crate_name}` required to be available in {$kind} format, but was not found in this form
    .note = only .rmeta files are distributed for `crablangc_private` crates other than `crablangc_driver`
    .help = try adding `extern crate crablangc_driver;` at the top level of this crate
...
metadata_incompatible_crablangc =
    found crate `{$crate_name}` compiled by an incompatible version of crablangc{$add_info}
    .help = please recompile that crate using this compiler ({$crablangc_version}) (consider running `cargo clean` first)
...
metadata_consider_downloading_target =
    consider downloading the target with `crablangup target add {$locator_triple}`
...
metadata_install_missing_components =
    maybe you need to install the missing components with: `crablangup component add crablang-src crablangc-dev llvm-tools-preview`
