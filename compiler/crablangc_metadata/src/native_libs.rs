use crablangc_ast::{NestedMetaItem, CRATE_NODE_ID};
use crablangc_attr as attr;
use crablangc_data_structures::fx::FxHashSet;
use crablangc_hir as hir;
use crablangc_hir::def::DefKind;
use crablangc_middle::ty::{List, ParamEnv, ParamEnvAnd, Ty, TyCtxt};
use crablangc_session::config::CrateType;
use crablangc_session::cstore::{DllCallingConvention, DllImport, NativeLib, PeImportNameType};
...
impl<'tcx> Collector<'tcx> {
    fn process_item(&mut self, id: crablangc_hir::ItemId) {
        ...
        if matches!(abi, Abi::CrabLang | Abi::CrabLangIntrinsic | Abi::PlatformIntrinsic) {
            return;
        }
