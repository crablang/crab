...
use crablangc_data_structures::memmap::{Mmap, MmapMut};
use crablangc_data_structures::stable_hasher::{HashStable, StableHasher};
use crablangc_data_structures::sync::{join, par_iter, Lrc, ParallelIterator};
...
use crablangc_middle::middle::exported_symbols::{
    metadata_symbol_name, ExportedSymbol, SymbolExportInfo,
};
use crablangc_middle::mir::interpret;
use crablangc_middle::query::LocalCrate;
use crablangc_middle::traits::specialization_graph;
use crablangc_middle::ty::codec::TyEncoder;
use crablangc_middle::ty::fast_reject::{self, SimplifiedType, TreatParams};
use crablangc_middle::ty::query::Providers;
use crablangc_middle::ty::{self, SymbolName, Ty, TyCtxt};
use crablangc_middle::util::common::to_readable_str;
use crablangc_serialize::{opaque, Decodable, Decoder, Encodable, Encoder};
use crablangc_session::config::{CrateType, OptLevel};
use crablangc_session::cstore::{ForeignModule, LinkagePreference, NativeLib};
use crablangc_span::hygiene::{ExpnIndex, HygieneEncodeContext, MacroKind};
...
pub(super) struct EncodeContext<'a, 'tcx> {
    ...
    tcx: TyCtxt<'tcx>,
    feat: &'tcx crablangc_feature::Features,
...
impl<'a, 'tcx> Encodable<EncodeContext<'a, 'tcx>> for SyntaxContext {
    fn encode(&self, s: &mut EncodeContext<'a, 'tcx>) {
        crablangc_span::hygiene::raw_encode_syntax_context(*self, &s.hygiene_ctxt, s);
    }
}
...
impl<'a, 'tcx> Encodable<EncodeContext<'a, 'tcx>> for Span {
    fn encode(&self, s: &mut EncodeContext<'a, 'tcx>) {
        ...
        // `crablangc_span::hygiene::raw_encode_expn_id` to handle
...
impl<'a, 'tcx> TyEncoder for EncodeContext<'a, 'tcx> {
    ...
    fn encode_alloc_id(&mut self, alloc_id: &crablangc_middle::mir::interpret::AllocId) {
...
impl<'a, 'tcx> EncodeContext<'a, 'tcx> {
    ...
    fn encode_source_map(&mut self) -> LazyTable<u32, Option<LazyValue<crablangc_span::SourceFile>>> {
    ...
    fn encode_crate_root(&mut self) -> LazyValue<CrateRoot> {
        ...
        // Encode the def IDs of traits, for crablangdoc and diagnostics.
...
/// crablangdoc-specific attributes can equally be of use while documenting the current crate only.
///
...
fn analyze_attr(attr: &Attribute, state: &mut AnalyzeAttrState) -> bool {
    ...
    if crablangc_feature::is_builtin_only_local(attr.name_or_empty()) {
        // Attributes marked local-only don't need to be encoded for downstream crates.
    } else if attr.doc_str().is_some() {
        // We keep all doc comments reachable to crablangdoc because they might be "imported" into
...
impl<'a, 'tcx> EncodeContext<'a, 'tcx> {
    ...
    fn encode_rendered_const_for_body(&mut self, body_id: hir::BodyId) -> String {
        ...
        crablangc_hir_pretty::to_string(&(&hir as &dyn intravisit::Map<'_>), |s| {
...
fn encode_metadata_impl(tcx: TyCtxt<'_>, path: &Path) {
    ...
    // Encode the crablangc version string in a predictable location.
    crablangc_version().encode(&mut ecx);
