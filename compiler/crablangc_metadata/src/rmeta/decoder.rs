...
use crablangc_data_structures::captures::Captures;
use crablangc_data_structures::fx::FxHashMap;
use crablangc_data_structures::svh::Svh;
use crablangc_data_structures::sync::{AppendOnlyVec, Lock, Lrc, OnceCell};
use crablangc_data_structures::unhash::UnhashMap;
use crablangc_expand::base::{SyntaxExtension, SyntaxExtensionKind};
use crablangc_expand::proc_macro::{AttrProcMacro, BangProcMacro, DeriveProcMacro};
use crablangc_hir::def::{CtorKind, DefKind, DocLinkResMap, Res};
use crablangc_hir::def_id::{CrateNum, DefId, DefIndex, CRATE_DEF_INDEX, LOCAL_CRATE};
use crablangc_hir::definitions::{DefKey, DefPath, DefPathData, DefPathHash};
use crablangc_hir::diagnostic_items::DiagnosticItems;
use crablangc_index::vec::{Idx, IndexVec};
use crablangc_middle::metadata::ModChild;
use crablangc_middle::middle::exported_symbols::{ExportedSymbol, SymbolExportInfo};
use crablangc_middle::mir::interpret::{AllocDecodingSession, AllocDecodingState};
use crablangc_middle::ty::codec::TyDecoder;
use crablangc_middle::ty::fast_reject::SimplifiedType;
use crablangc_middle::ty::GeneratorDiagnosticData;
use crablangc_middle::ty::{self, ParameterizedOverTcx, Ty, TyCtxt, Visibility};
use crablangc_serialize::opaque::MemDecoder;
use crablangc_serialize::{Decodable, Decoder};
use crablangc_session::cstore::{
    CrateSource, ExternCrate, ForeignModule, LinkagePreference, NativeLib,
};
use crablangc_session::Session;
use crablangc_span::hygiene::ExpnIndex;
use crablangc_span::symbol::{kw, Ident, Symbol};
use crablangc_span::{self, BytePos, ExpnId, Pos, Span, SyntaxContext, DUMMY_SP};
...
use crablangc_span::hygiene::HygieneDecodeContext;
...
pub(crate) struct CrateMetadata {
    ...
    ///
    /// These can be introduced using either `#![crablangc_coherence_is_core]`
    /// or `#[crablangc_allow_incoherent_impl]`.
...
/// Holds information about a crablangc_span::SourceFile imported from another crate.
/// See `imported_source_file()` for more information.
#[derive(Clone)]
struct ImportedSourceFile {
    /// This SourceFile's byte-offset within the source_map of its original crate
    original_start_pos: crablangc_span::BytePos,
    /// The end of this SourceFile within the source_map of its original crate
    original_end_pos: crablangc_span::BytePos,
    /// The imported SourceFile's representation within the local source_map
    translated_source_file: Lrc<crablangc_span::SourceFile>,
}
...
unsafe impl<'a, 'tcx, T: Decodable<DecodeContext<'a, 'tcx>>> TcrablangedLen
    for DecodeIterator<'a, 'tcx, T>
{
}
...
impl<'a, 'tcx> TyDecoder for DecodeContext<'a, 'tcx> {
    ...
    fn decode_alloc_id(&mut self) -> crablangc_middle::mir::interpret::AllocId {
...
impl<'a, 'tcx> Decodable<DecodeContext<'a, 'tcx>> for SyntaxContext {
    fn decode(decoder: &mut DecodeContext<'a, 'tcx>) -> SyntaxContext {
        ...
        crablangc_span::hygiene::decode_syntax_context(decoder, &cdata.hygiene_context, |_, id| {
...
impl<'a, 'tcx> Decodable<DecodeContext<'a, 'tcx>> for ExpnId {
    fn decode(decoder: &mut DecodeContext<'a, 'tcx>) -> ExpnId {
        ...
        let expn_id = crablangc_span::hygiene::decode_expn_id(cnum, index, |expn_id| {
...
impl MetadataBlob {
    ...
    pub(crate) fn get_crablangc_version(&self) -> String {
...
impl<'a, 'tcx> CrateMetadataRef<'a> {
    ...
    fn get_debugger_visualizers(self) -> Vec<crablangc_span::DebuggerVisualizerFile> {
    ...
    /// Decodes all traits in the crate (for crablangdoc and crablangc diagnostics).
    fn get_traits(self) -> impl Iterator<Item = DefId> + 'a {
        self.root.traits.decode(self).map(move |index| self.local_def_id(index))
    }

    /// Decodes all trait impls in the crate (for crablangdoc).
    ...
    fn expn_hash_to_expn_id(self, sess: &Session, index_guess: u32, hash: ExpnHash) -> ExpnId {
        ...
        crablangc_span::hygiene::register_expn_id(self.cnum, index, data, hash)
    }
    ...
    fn imported_source_file(self, source_file_index: u32, sess: &Session) -> ImportedSourceFile {
        fn filter<'a>(sess: &Session, path: Option<&'a Path>) -> Option<&'a Path> {
            path.filter(|_| {
                ...
                sess.opts.real_crablang_source_base_dir.is_some()
                // Some tests need the translation to be always skipped.
                && sess.opts.unstable_opts.translate_remapped_path_to_local_path
            })
            .filter(|virtual_dir| {
                // Don't translate away `/crablangc/$hash` if we're still remapping to it,
                // since that means we're still building `std`/`crablangc` that need it,
                // and we don't want the real path to leak into codegen/debuginfo.
                !sess.opts.remap_path_prefix.iter().any(|(_from, to)| to == virtual_dir)
            })
        }

        // Translate the virtual `/crablangc/$hash` prefix back to a real directory
        // that should hold actual sources, where possible.
        //
        // NOTE: if you update this, you might need to also update bootstrap's code for generating
        // the `crablang-src` component in `Src::run` in `src/bootstrap/dist.rs`.
        let virtual_crablang_source_base_dir = [
            filter(sess, option_env!("CFG_VIRTUAL_CRABLANG_SOURCE_BASE_DIR").map(Path::new)),
            filter(sess, sess.opts.unstable_opts.simulate_remapped_crablang_src_base.as_deref()),
        ];

        let try_to_translate_virtual_to_real = |name: &mut crablangc_span::FileName| {
            debug!(
                "try_to_translate_virtual_to_real(name={:?}): \
                 virtual_crablang_source_base_dir={:?}, real_crablang_source_base_dir={:?}",
                name, virtual_crablang_source_base_dir, sess.opts.real_crablang_source_base_dir,
            );

            for virtual_dir in virtual_crablang_source_base_dir.iter().flatten() {
                if let Some(real_dir) = &sess.opts.real_crablang_source_base_dir {
                    if let crablangc_span::FileName::Real(old_name) = name {
                        if let crablangc_span::RealFileName::Remapped { local_path: _, virtual_name } =
                            old_name
                        {
                            if let Ok(rest) = virtual_name.strip_prefix(virtual_dir) {
                                ...
                                // `$sysroot/lib/crablanglib/src/crablang/library`, whereas other crates
                                // may be in `$sysroot/lib/crablanglib/src/crablang/` directly. So we
                                ...
                                const STD_LIBS: &[&str] = &[
                                    ...
                                    "profiler_builtins",
                                    "rtstartup",
                                    "crablangc-std-workspace-core",
                                    "crablangc-std-workspace-alloc",
                                    "crablangc-std-workspace-std",
                                    "backtrace",
                                ];
                                ...
                                let new_name = crablangc_span::RealFileName::Remapped {
        ...
        import_info[source_file_index as usize]
            .get_or_insert_with(|| {
                ...
                // If this file is under $sysroot/lib/crablanglib/src/ but has not been remapped
                // during crablang bootstrapping by `remap-debuginfo = true`, and the user
                // wish to simulate that behaviour by -Z simulate-remapped-crablang-src-base,
                // then we change `name` to a similar state as if the crablang was bootstrapped
                // with `remap-debuginfo = true`.
                // This is useful for testing so that tests about the effects of
                // `try_to_translate_virtual_to_real` don't have to worry about how the
                // compiler is bootstrapped.
                if let Some(virtual_dir) = &sess.opts.unstable_opts.simulate_remapped_crablang_src_base
                {
                    if let Some(real_dir) = &sess.opts.real_crablang_source_base_dir {
                        for subdir in ["library", "compiler"] {
                            if let crablangc_span::FileName::Real(ref mut old_name) = name {
                                if let crablangc_span::RealFileName::LocalPath(local) = old_name {
                                    if let Ok(rest) = local.strip_prefix(real_dir.join(subdir)) {
                                        *old_name = crablangc_span::RealFileName::Remapped {
                                            local_path: None,
                ...
                // If this file's path has been remapped to `/crablangc/$hash`,
