...
use crablangc_hir::def::{CtorKind, DefKind, DocLinkResMap};
use crablangc_hir::def_id::{CrateNum, DefId, DefIndex, DefPathHash, StableCrateId};
use crablangc_hir::definitions::DefKey;
...
use crablangc_middle::ty::{self, ReprOptions, Ty, UnusedGenericParams};
use crablangc_middle::ty::{DeducedParamAttrs, GeneratorDiagnosticData, ParameterizedOverTcx, TyCtxt};
use crablangc_serialize::opaque::FileEncoder;
use crablangc_session::config::SymbolManglingVersion;
use crablangc_session::cstore::{CrateDepKind, ForeignModule, LinkagePreference, NativeLib};
use crablangc_span::edition::Edition;
use crablangc_span::hygiene::{ExpnIndex, MacroKind};
use crablangc_span::symbol::{Ident, Symbol};
use crablangc_span::{self, ExpnData, ExpnHash, ExpnId, Span};
use crablangc_target::spec::{PanicStrategy, TargetTriple};
...
use crablangc_span::hygiene::SyntaxContextData;
...
pub(crate) fn crablangc_version() -> String {
    format!("crablangc {}", option_env!("CFG_VERSION").unwrap_or("unknown version"))
}

/// Metadata encoding version.
/// N.B., increment this if you change the format of metadata such that
/// the crablangc version can't be found to compare with `crablangc_version()`.
...
/// unsigned integer, and further followed by the crablangc version string.
...
pub(crate) struct CrateRoot {
    ...
    tables: LazyTables,
    debugger_visualizers: LazyArray<crablangc_span::DebuggerVisualizerFile>,
    ...
    source_map: LazyTable<u32, Option<LazyValue<crablangc_span::SourceFile>>>,
...
- optional:
    ...
    const_param_default: Table<DefIndex, LazyValue<ty::EarlyBinder<crablangc_middle::ty::Const<'static>>>>,
