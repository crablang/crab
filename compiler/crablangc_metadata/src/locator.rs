...
//! is a crablangc-defined file format (currently just an ar archive) while a dylib
...
//! target lib search path ($prefix/lib/crablanglib/$target/lib) to any compilation,
...
//! 3. Is the file an actual crablang library? This is done by loading the metadata
...
//! the file is indeed a crablang library. The same `crate-name` can be specified
...
//!
//! ```text
//! crablangc a.rs --extern b1=path/to/libb1.rlib --extern b2=path/to/libb2.rlib
//! ```
//!
...
//! deal with this in crablangc is to add hashes to the end of each symbol.
//!
...
use crate::errors;
use crate::rmeta::{crablangc_version, MetadataBlob, METADATA_HEADER};
...
use crablangc_errors::{DiagnosticArgValue, FatalError, IntoDiagnosticArg};
use crablangc_fs_util::try_canonicalize;
use crablangc_session::config::{self, CrateType};
use crablangc_session::cstore::{CrateSource, MetadataLoader};
use crablangc_session::filesearch::FileSearch;
use crablangc_session::search_paths::PathKind;
use crablangc_session::utils::CanonicalizedPath;
...
impl<'a> CrateLocator<'a> {
    ...
    // be read, it is assumed that the file isn't a valid crablang library (no
    // errors are emitted).
    fn extract_one(
        &mut self,
        ...
        for (lib, kind) in m {
            ...
            //
            // You can find more info in crablang/crablang#39518 and various linked
    ...
    fn crate_matches(&mut self, metadata: &MetadataBlob, libpath: &Path) -> Option<Svh> {
        let crablangc_version = crablangc_version();
        let found_version = metadata.get_crablangc_version();
        if found_version != crablangc_version {
            info!("Rejecting via version: expected {} got {}", crablangc_version, found_version);
            self.crate_rejections
                .via_version
        ...
        match self {
            ...
            CrateError::LocatorCombined(locator) => {
                ...
                if !locator.crate_rejections.via_hash.is_empty() {
                    ...
                    sess.emit_err(errors::IncompatibleCrabLangc {
                        span,
                        crate_name,
                        add_info,
                        found_crates,
                        crablangc_version: crablangc_version(),
                    });
