...
use crablangc_data_structures::sync::{MappedReadGuard, MappedWriteGuard, ReadGuard, WriteGuard};
use crablangc_expand::base::SyntaxExtension;
use crablangc_hir::def_id::{CrateNum, LocalDefId, StableCrateId, StableCrateIdMap, LOCAL_CRATE};
use crablangc_hir::definitions::Definitions;
use crablangc_index::vec::IndexVec;
use crablangc_middle::ty::TyCtxt;
use crablangc_session::config::{self, CrateType, ExternLocation};
use crablangc_session::cstore::ExternCrateSource;
use crablangc_session::cstore::{CrateDepKind, CrateSource, ExternCrate};
use crablangc_session::lint;
use crablangc_session::output::validate_crate_name;
...
impl CStore {
    ...
    pub fn report_unused_deps(&self, tcx: TyCtxt<'_>) {
        ...
        let level = tcx
            .lint_level_at_node(lint::builtin::UNUSED_CRATE_DEPENDENCIES, crablangc_hir::CRATE_HIR_ID)
            .0;
    ...
    impl<'ast> visit::Visitor<'ast> for Finder {
        fn visit_item(&mut self, item: &'ast ast::Item) {
            if item.ident.name == self.name
                && attr::contains_name(&item.attrs, sym::crablangc_std_internal_symbol)
            {
