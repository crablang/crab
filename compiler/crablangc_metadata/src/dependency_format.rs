...
use crate::errors::{
    ...
    RequiredPanicStrategy, RlibRequired, CrabLangcLibRequired, TwoPanicRuntimes,
};

use crablangc_data_structures::fx::FxHashMap;
use crablangc_hir::def_id::CrateNum;
use crablangc_middle::middle::dependency_format::{Dependencies, DependencyList, Linkage};
use crablangc_middle::ty::TyCtxt;
use crablangc_session::config::CrateType;
use crablangc_session::cstore::CrateDepKind;
use crablangc_session::cstore::LinkagePreference::{self, RequireDynamic, RequireStatic};
    ...
    for (cnum, kind) in ret.iter().enumerate() {
        ...
        match *kind {
            ...
            kind => {
                ...
                if crate_name.as_str().starts_with("crablangc_") {
                    sess.emit_err(CrabLangcLibRequired { crate_name, kind });
                } else {
