...
use crablangc_data_structures::sync::{par_for_each_in, MTLock, MTLockRef};
use crablangc_hir as hir;
use crablangc_hir::def::DefKind;
use crablangc_hir::def_id::{DefId, DefIdMap, LocalDefId};
use crablangc_hir::lang_items::LangItem;
use crablangc_index::bit_set::GrowableBitSet;
use crablangc_middle::mir::interpret::{AllocId, ConstValue};
use crablangc_middle::mir::interpret::{ErrorHandled, GlobalAlloc, Scalar};
use crablangc_middle::mir::mono::{InstantiationMode, MonoItem};
use crablangc_middle::mir::visit::Visitor as MirVisitor;
use crablangc_middle::mir::{self, Local, Location};
use crablangc_middle::ty::adjustment::{CustomCoerceUnsized, PointerCast};
use crablangc_middle::ty::print::with_no_trimmed_paths;
use crablangc_middle::ty::query::TyCtxtAt;
use crablangc_middle::ty::subst::{GenericArgKind, InternalSubsts};
use crablangc_middle::ty::{
    self, GenericParamDefKind, Instance, InstanceDef, Ty, TyCtxt, TypeFoldable, TypeVisitableExt,
    VtblEntry,
};
use crablangc_middle::{middle::codegen_fn_attrs::CodegenFnAttrFlags, mir::visit::TyContext};
use crablangc_session::config::EntryFnType;
use crablangc_session::lint::builtin::LARGE_ASSIGNMENTS;
use crablangc_session::Limit;
use crablangc_span::source_map::{dummy_spanned, respan, Span, Spanned, DUMMY_SP};
...
/// Format instance name that is already known to be too long for crablangc.
...
fn check_type_length_limit<'tcx>(tcx: TyCtxt<'tcx>, instance: Instance<'tcx>) {
    ...
    // CrabLang code can easily create exponentially-long types using only a
    // polynomial recursion depth. Even with the default recursion
    // depth, you can easily get cases that take >2^60 steps to run,
    // which means that crablangc basically hangs.
    //
                ...
                MonoItemCollectionMode::Lazy => {
                    self.entry_fn.and_then(|(id, _)| id.as_local()) == Some(def_id)
                        ...
                        || self
                            ...
                            .flags
                            .contains(CodegenFnAttrFlags::CRABLANGC_STD_INTERNAL_SYMBOL)
                }
            }
    }
