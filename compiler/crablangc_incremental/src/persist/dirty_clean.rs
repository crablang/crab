...
//! `#[crablangc_clean]` we will compare the fingerprint from the current and from the previous
//! compilation session as appropriate:
//!
//! - `#[crablangc_clean(cfg="rev2", except="typeck")]` if we are
//!   in `#[cfg(rev2)]`, then the fingerprints associated with
//!   `DepNode::typeck(X)` must be DIFFERENT (`X` is the `DefId` of the
//!   current node).
//! - `#[crablangc_clean(cfg="rev2")]` same as above, except that the
//!   fingerprints must be the SAME (along with all other fingerprints).
//!
//! - `#[crablangc_clean(cfg="rev2", loaded_from_disk='typeck")]` asserts that
...
use crate::errors;
use crablangc_ast::{self as ast, Attribute, NestedMetaItem};
use crablangc_data_structures::fx::FxHashSet;
use crablangc_hir::def_id::LocalDefId;
use crablangc_hir::intravisit;
use crablangc_hir::Node as HirNode;
use crablangc_hir::{ImplItemKind, ItemKind as HirItem, TraitItemKind};
use crablangc_middle::dep_graph::{label_strs, DepNode, DepNodeExt};
...
/// Represents the requested configuration by crablangc_clean/dirty
struct Assertion {
    clean: Labels,
    dirty: Labels,
...
pub fn check_dirty_clean_annotations(tcx: TyCtxt<'_>) {
    ...
    // can't add `#[crablangc_clean]` etc without opting in to this feature
    ...
    tcx.dep_graph.with_ignore(|| {
        ...
        // all codegen-specific attributes are `AssumedUsed` in crablangc_ast::feature_gate.
...
impl<'tcx> DirtyCleanVisitor<'tcx> {
    ...
    fn assertion_maybe(&mut self, item_id: LocalDefId, attr: &Attribute) -> Option<Assertion> {
        assert!(attr.has_name(sym::crablangc_clean));
    ...
    fn check_item(&mut self, item_id: LocalDefId) {
        ...
        for attr in self.tcx.get_attrs(item_id, sym::crablangc_clean) {
...
/// Given a `#[crablangc_clean]` attribute, scan for a `cfg="foo"` attribute and check whether we have
...
/// A visitor that collects all `#[crablangc_clean]` attributes from
...
impl<'tcx> FindAllAttrs<'tcx> {
    fn is_active_attr(&mut self, attr: &Attribute) -> bool {
        if attr.has_name(sym::crablangc_clean) && check_config(self.tcx, attr) {
            return true;
        }
