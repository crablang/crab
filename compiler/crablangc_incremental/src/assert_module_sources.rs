...
//!
//! `#![crablangc_expected_cgu_reuse(module="spike", cfg="rpass2", kind="post-lto")]`
...
use crablangc_session::cgu_reuse_tracker::*;
...
pub fn assert_module_sources(tcx: TyCtxt<'_>) {
    tcx.dep_graph.with_ignore(|| {
        ...
        for attr in tcx.hir().attrs(crablangc_hir::CRATE_HIR_ID) {
            ams.check_attr(attr);
        }
    })
}
...
impl<'tcx> AssertModuleSource<'tcx> {
    fn check_attr(&self, attr: &ast::Attribute) {
        let (expected_reuse, comp_kind) = if attr.has_name(sym::crablangc_partition_reused) {
            (CguReuse::PreLto, ComparisonKind::AtLeast)
        } else if attr.has_name(sym::crablangc_partition_codegened) {
            (CguReuse::No, ComparisonKind::Exact)
        } else if attr.has_name(sym::crablangc_expected_cgu_reuse) {
