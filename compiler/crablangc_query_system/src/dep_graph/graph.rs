...
use crablangc_data_structures::profiling::{EventId, QueryInvocationId, SelfProfilerRef};
use crablangc_data_structures::sharded::{self, Sharded};
use crablangc_data_structures::stable_hasher::{HashStable, StableHasher};
use crablangc_data_structures::steal::Steal;
use crablangc_data_structures::sync::{AtomicU32, AtomicU64, Lock, Lrc, Ordering};
...
impl<K: DepKind> DepGraphData<K> {
    ...
    /// dep-graph -- see the [crablangc dev guide] for more details on
    ...
    ///   `arg` parameter.
    ///
    /// [crablangc dev guide]: https://crablangc-dev-guide.crablang.org/incremental-compilation.html
    #[inline(always)]
    pub fn with_task<Ctxt: HasDepContext<DepKind = K>, A: Debug, R>(
        &self,
        key: DepNode<K>,
...
impl<K: DepKind> CurrentDepGraph<K> {
    fn new(
        ...
        #[cfg(debug_assertions)]
        let forbidden_edge = match env::var("CRABLANG_FORBID_DEP_GRAPH_EDGE") {
            Ok(s) => match EdgeFilter::new(&s) {
                Ok(f) => Some(f),
                Err(err) => panic!("CRABLANG_FORBID_DEP_GRAPH_EDGE invalid: {}", err),
            },
            Err(_) => None,
        };
