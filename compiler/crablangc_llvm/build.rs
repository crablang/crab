...
/// <https://doc.crablang.org/cargo/reference/environment-variables.html#environment-variables-cargo-sets-for-build-scripts>
...
fn main() {
    for component in REQUIRED_COMPONENTS.iter().chain(OPTIONAL_COMPONENTS.iter()) {
        println!("cargo:crablangc-check-cfg=values(llvm_component,\"{component}\")");
    }

    if tracked_env_var_os("CRABLANG_CHECK").is_some() {
    ...
    // can't tcrablang all the output of llvm-config because it might be targeted
    ...
    for component in components.iter() {
        println!("cargo:crablangc-cfg=llvm_component=\"{component}\"");
    }
    ...
    if tracked_env_var_os("LLVM_CRABLANGLLVM").is_some() {
        cfg.define("LLVM_CRABLANGLLVM", None);
    }
    ...
    cfg.file("llvm-wrapper/PassWrapper.cpp")
        .file("llvm-wrapper/CrabLangWrapper.cpp")
        ...
        || target.starts_with("powerpc-")
    {
        ...
        println!("cargo:crablangc-link-lib=atomic");
    } else if target.contains("windows-gnu") {
        println!("cargo:crablangc-link-lib=shell32");
        println!("cargo:crablangc-link-lib=uuid");
    ...
    for lib in output(&mut cmd).split_whitespace() {
        ...
        println!("cargo:crablangc-link-lib={kind}={name}");
    }

    // LLVM ldflags
    //
    // If we're a cross-compile of LLVM then unfortunately we can't tcrablang these
    ...
    for lib in output(&mut cmd).split_whitespace() {
        if is_crossed {
            if let Some(stripped) = lib.strip_prefix("-LIBPATH:") {
                println!("cargo:crablangc-link-search=native={}", stripped.replace(&host, &target));
            } else if let Some(stripped) = lib.strip_prefix("-L") {
                println!("cargo:crablangc-link-search=native={}", stripped.replace(&host, &target));
            }
        } else if let Some(stripped) = lib.strip_prefix("-LIBPATH:") {
            println!("cargo:crablangc-link-search=native={stripped}");
        } else if let Some(stripped) = lib.strip_prefix("-l") {
            println!("cargo:crablangc-link-lib={stripped}");
        } else if let Some(stripped) = lib.strip_prefix("-L") {
            println!("cargo:crablangc-link-search=native={stripped}");
        }
    }
    ...
    // crablangc_llvm, for example when using static libc++, we may need to
    ...
    if !target.contains("msvc") {
        if let Some(s) = llvm_static_stdcpp {
            ...
            println!("cargo:crablangc-link-search=native={}", path.parent().unwrap().display());
            if target.contains("windows") {
                println!("cargo:crablangc-link-lib=static:-bundle={stdcppname}");
            } else {
                println!("cargo:crablangc-link-lib=static={stdcppname}");
            }
        } else if cxxflags.contains("stdlib=libc++") {
            println!("cargo:crablangc-link-lib=c++");
        } else {
            println!("cargo:crablangc-link-lib={stdcppname}");
        }
    }

    // Libstdc++ depends on pthread which CrabLang doesn't link on MinGW
    // since nothing else requires it.
    if target.ends_with("windows-gnu") {
        println!("cargo:crablangc-link-lib=static:-bundle=pthread");
    }
}
