...
typedef void *(*LLVMCrabLangGetSymbolsCallback)(void *, const char *);
typedef void *(*LLVMCrabLangGetSymbolsErrorCallback)(const char *);

// Note: This is implemented in C++ instead of using the C api from CrabLang as IRObjectFile doesn't
// implement getSymbolName, only printSymbolName, which is inaccessible from the C api.
extern "C" void *LLVMCrabLangGetSymbols(
  char *BufPtr, size_t BufLen, void *State, LLVMCrabLangGetSymbolsCallback Callback,
  LLVMCrabLangGetSymbolsErrorCallback ErrorCallback) {
  std::unique_ptr<MemoryBuffer> Buf =
    MemoryBuffer::getMemBuffer(StringRef(BufPtr, BufLen), StringRef("LLVMCrabLangGetSymbolsObject"),
                               false);
