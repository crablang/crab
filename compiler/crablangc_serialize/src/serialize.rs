...
/// many nested calls that typical CrabLang error handling (via `Result` and `?`)
...
/// (If you need it, revert <https://github.com/crablang/crablang/pull/109984>.)
pub trait Encoder {
    // Primitive types:
...
///   `crablangc_middle`.
/// * `MetadataEncodable` is used in `crablangc_metadata` for types that contain
///   `crablangc_metadata::rmeta::Lazy`.
/// * `TyEncodable` should be used for types that are only serialized in crate
///   metadata or the incremental cache. This is most types in `crablangc_middle`.
...
///   `crablangc_middle`.
/// * `MetadataDecodable` is used in `crablangc_metadata` for types that contain
///   `crablangc_metadata::rmeta::Lazy`.
