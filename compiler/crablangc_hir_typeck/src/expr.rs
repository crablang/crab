...
use crate::fatally_break_crablang;
...
use crablangc_hir::{ExprKind, HirId, QPath};
use crablangc_hir_analysis::astconv::AstConv as _;
use crablangc_hir_analysis::check::ty_kind_suggestion;
use crablangc_infer::infer;
use crablangc_infer::infer::type_variable::{TypeVariableOrigin, TypeVariableOriginKind};
use crablangc_infer::infer::DefineOpaqueTypes;
use crablangc_infer::infer::InferOk;
use crablangc_infer::traits::ObligationCause;
use crablangc_middle::middle::stability;
use crablangc_middle::ty::adjustment::{Adjust, Adjustment, AllowTwoPhase};
use crablangc_middle::ty::error::TypeError::FieldMisMatch;
use crablangc_middle::ty::subst::SubstsRef;
use crablangc_middle::ty::{self, AdtKind, Ty, TypeVisitableExt};
use crablangc_session::errors::ExprParenthesesNeeded;
...
use crablangc_target::spec::abi::Abi::CrabLangIntrinsic;
use crablangc_trait_selection::infer::InferCtxtExt;
use crablangc_trait_selection::traits::{self, ObligationCauseCode};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    /// type with `ty::type_is_bot` will return true (n.b.: since CrabLang is
    ...
    pub(crate) fn check_expr_path(
        ...
        if let ty::FnDef(did, ..) = *ty.kind() {
            ...
            if tcx.fn_sig(did).skip_binder().abi() == CrabLangIntrinsic
            ...
            if !tcx.features().unsized_fn_params {
                ...
                // but don't want to expose the removal to stable CrabLang.
    ...
    fn check_expr_break(
        ...
        if let Ok(target_id) = destination.target_id {
            ...
            if let Some(e) = expr_opt {
                ...
                // ... except when we try to 'break crablang;'.
                // ICE this expression in particular (see #43162).
                if let ExprKind::Path(QPath::Resolved(_, path)) = e.kind {
                    if path.segments.len() == 1 && path.segments[0].ident.name == sym::crablang {
                        fatally_break_crablang(self.tcx.sess);
                    }
                }
            }
    ...
    fn ban_nonexisting_field(
        ...
        if ident.name == kw::Await {
            // We know by construction that `<expr>.await` is either on CrabLang 2015
            // or results in `ExprKind::Await`. Suggest switching the edition to 2018.
            err.note("to `.await` a `Future`, switch to CrabLang 2018 or later");
    ...
    fn ban_take_value_of_method(
        ...
        let after_open = expr.span.lo() + crablangc_span::BytePos(1);
        let before_close = expr.span.hi() - crablangc_span::BytePos(1);
