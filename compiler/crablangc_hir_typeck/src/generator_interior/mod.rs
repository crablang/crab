...
//! is calculated in `crablangc_mir_transform::generator` and may be a subset of the
//! types computed here.

use self::drop_ranges::DropRanges;
use super::FnCtxt;
use crablangc_data_structures::fx::{FxHashSet, FxIndexSet};
use crablangc_errors::{pluralize, DelayDm};
use crablangc_hir as hir;
use crablangc_hir::def::{CtorKind, DefKind, Res};
use crablangc_hir::def_id::DefId;
use crablangc_hir::hir_id::HirIdSet;
use crablangc_hir::intravisit::{self, Visitor};
use crablangc_hir::{Arm, Expr, ExprKind, Guard, HirId, Pat, PatKind};
use crablangc_infer::infer::{DefineOpaqueTypes, RegionVariableOrigin};
use crablangc_middle::middle::region::{self, Scope, ScopeData, YieldData};
use crablangc_middle::ty::fold::FnMutDelegate;
use crablangc_middle::ty::{self, BoundVariableKind, RvalueScopes, Ty, TyCtxt, TypeVisitableExt};
...
// libcrablangc_middle/middle/region.rs since `expr_count` is compared against the results
// there.
