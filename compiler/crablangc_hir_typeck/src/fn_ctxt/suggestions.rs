...
use crablangc_ast::util::parser::{ExprPrecedence, PREC_POSTFIX};
use crablangc_errors::{Applicability, Diagnostic, MultiSpan};
use crablangc_hir as hir;
use crablangc_hir::def::{CtorKind, CtorOf, DefKind};
use crablangc_hir::lang_items::LangItem;
use crablangc_hir::{
    Expr, ExprKind, GenericBound, Node, Path, QPath, Stmt, StmtKind, TyKind, WherePredicate,
};
use crablangc_hir_analysis::astconv::AstConv;
use crablangc_infer::traits::{self, StatementAsExpression};
...
use crablangc_trait_selection::traits::error_reporting::DefIdOrName;
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
        ...
        if let hir::ExprKind::MethodCall(hir::PathSegment { ident: method, .. }, recv_expr, &[], _) = expr.kind &&
            ...
            self.can_coerce(recv_ty, expected) {
                let span = if let Some(recv_span) = recv_expr.span.find_ancestor_inside(expr.span) {
                    ...
                    expr.span.with_lo(method.span.lo() - crablangc_span::BytePos(1))
                };
        ...
        if self.can_coerce(boxed_found, expected) {
            ...
            err.note(
                "for more on the distinction between the stack and the heap, read \
                 https://doc.crablang.org/book/ch15-01-box.html, \
                 https://doc.crablang.org/crablang-by-example/std/box.html, and \
                 https://doc.crablang.org/std/boxed/index.html",
            );
            true
        } else {
            false
        ...
        let mut suggest_copied_or_cloned = || {
                ...
                && self.can_eq(self.param_env, ty, expected_inner_ty)
            {
                ...
                if self.type_is_copy_modulo_regions(self.param_env, ty) {
                    ...
                    && crablangc_trait_selection::traits::type_known_to_meet_bound_modulo_regions(
                        self,
                        self.param_env,
                        ty,
                        clone_did,
                        ...
                        diag.multipart_suggestion_verbose(
                            ...
                            vec![
                                (
                                    blk.span
                                        .shrink_to_lo()
                                        .with_hi(crablangc_span::BytePos(blk.span.lo().0 + 1)),
                                    "[".to_string(),
                                ),
                                (
                                    blk.span
                                        .shrink_to_hi()
                                        .with_lo(crablangc_span::BytePos(blk.span.hi().0 - 1)),
                                    "]".to_string(),
                                ),
                            ],
                            Applicability::MachineApplicable,
    ...
    pub(crate) fn suggest_floating_point_literal(
        ...
        match expr.kind {
            ...
            ExprKind::Lit(Spanned {
                node: crablangc_ast::LitKind::Int(lit, crablangc_ast::LitIntType::Unsuffixed),
                span,
            }) => {
    ...
    pub(crate) fn suggest_null_ptr_for_literal_zero_given_to_ptr_arg(
        ...
        let ExprKind::Lit(Spanned {
            node: crablangc_ast::LitKind::Int(0, _),
            span,
        }) = expr.kind else {
            return false;
        };
