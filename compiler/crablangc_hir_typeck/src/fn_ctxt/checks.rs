...
use crablangc_hir::def::{CtorOf, DefKind, Res};
use crablangc_hir::def_id::DefId;
use crablangc_hir::{ExprKind, Node, QPath};
use crablangc_hir_analysis::astconv::AstConv;
use crablangc_hir_analysis::check::intrinsicck::InlineAsmCtxt;
use crablangc_hir_analysis::check::potentially_plural_count;
use crablangc_hir_analysis::structured_errors::StructuredDiagnostic;
use crablangc_index::vec::IndexVec;
use crablangc_infer::infer::error_reporting::{FailureCode, ObligationCauseExt};
use crablangc_infer::infer::type_variable::{TypeVariableOrigin, TypeVariableOriginKind};
use crablangc_infer::infer::TypeTrace;
use crablangc_infer::infer::{DefineOpaqueTypes, InferOk};
use crablangc_middle::ty::adjustment::AllowTwoPhase;
use crablangc_middle::ty::visit::TypeVisitableExt;
use crablangc_middle::ty::{self, IsSuggestable, Ty};
use crablangc_session::Session;
use crablangc_span::symbol::{kw, Ident};
use crablangc_span::{self, sym, BytePos, Span};
use crablangc_trait_selection::traits::{self, ObligationCauseCode, SelectionContext};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    pub(in super::super) fn check_argument_types(
        ...
        // otherwise, they need to be identical, because crablang doesn't currently support variadic functions
        ...
        for arg in provided_args.iter().skip(minimum_input_count) {
            ...
            if c_variadic {
                fn variadic_error<'tcx>(
                    ...
                    use crablangc_hir_analysis::structured_errors::MissingCastForVariadicArg;
            ...
            if cfg!(debug_assertions) {
                ...
                tcx.sess
                    .struct_span_err(
                        ...
                        but crablangc had trouble determining where",
                    )
                    .note(
                        "we would appreciate a bug report: \
                        https://github.com/crablang/crablang/issues/new",
                    )
                    .emit();
            }
            return;
    ...
    /// ```
    fn get_expr_coercion_span(&self, expr: &hir::Expr<'_>) -> crablangc_span::Span {
