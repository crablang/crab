...
use crablangc_hir::{HirId, Pat, PatKind};
...
use crablangc_middle::ty::{self, Adt, BindingMode, Ty, TypeVisitableExt};
use crablangc_session::lint::builtin::NON_EXHAUSTIVE_OMITTED_PATTERNS;
...
use crablangc_span::{BytePos, DUMMY_SP};
use crablangc_target::abi::FieldIdx;
use crablangc_trait_selection::traits::{ObligationCause, Pattern};
use ty::VariantDef;
...
https://doc.crablang.org/reference/types.html#trait-objects";
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn error_tuple_variant_index_shorthand(
            ...
            (variant.ctor_kind(), &pat.kind)
        {
            ...
            if has_shorthand_field_name {
                let path = crablangc_hir_pretty::to_string(crablangc_hir_pretty::NO_ANN, |s| {
        ...
        fields
            ...
            .map(|field| {
                match self.tcx.sess.source_map().span_to_snippet(field.pat.span) {
                    ...
                    Err(_) => crablangc_hir_pretty::to_string(crablangc_hir_pretty::NO_ANN, |s| {
    ...
    ///
    /// Nb: the partner lint for enums lives in `compiler/crablangc_mir_build/src/thir/pattern/usefulness.rs`.
    fn lint_non_exhaustive_omitted_patterns(
        ...
        lint.span_label(pat.span, format!("field{} {} not listed", crablangc_errors::pluralize!(unmentioned_fields.len()), joined_patterns));
        lint.help(
