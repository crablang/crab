...
use crablangc_middle::ty::adjustment::{Adjust, Adjustment, PointerCast};
...
use crablangc_middle::ty::{self, ClosureSizeProfileData, Ty, TyCtxt};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    pub fn resolve_type_vars_in_body(
        ...
        let crablangc_dump_user_substs = self.tcx.has_attr(item_def_id, sym::crablangc_dump_user_substs);

        let mut wbcx = WritebackCx::new(self, body, crablangc_dump_user_substs);
...
struct WritebackCx<'cx, 'tcx> {
    ...
    crablangc_dump_user_substs: bool,
}
...
impl<'cx, 'tcx> WritebackCx<'cx, 'tcx> {
    ...
    fn visit_user_provided_tys(&mut self) {
        ...
        if self.crablangc_dump_user_substs {
