...
use crablangc_hir::{HirIdMap, Node};
use crablangc_hir_analysis::astconv::AstConv;
use crablangc_hir_analysis::check::check_abi;
...
macro_rules! type_error_struct {
    ($session:expr, $span:expr, $typ:expr, $code:ident, $($message:tt)*) => ({
        let mut err = crablangc_errors::struct_span_err!($session, $span, $code, $($message)*);
...
fn typeck_with_fallback<'tcx>(
    ...
    let param_env = if tcx.has_attr(def_id, sym::crablangc_do_not_const_check) {
    ...
    if let Some(hir::FnSig { header, decl, .. }) = fn_sig {
        let fn_sig = if crablangc_hir_analysis::collect::get_infer_ret_ty(&decl.output).is_some() {
...
fn report_unexpected_variant_res(
    ...
    let path_str = crablangc_hir_pretty::qpath_to_string(qpath);
    ...
    match res {
        Res::Def(DefKind::Fn | DefKind::AssocFn, _) if err_code == "E0164" => {
            let patterns_url = "https://doc.crablang.org/book/ch18-00-patterns.html";
...
fn fatally_break_crablang(sess: &Session) {
    let handler = sess.diagnostic();
    handler.span_bug_no_panic(
        MultiSpan::new(),
        "It looks like you're trying to break crablang; would you like some ICE?",
    );
    handler.note_without_error("the compiler expectedly panicked. this is a feature.");
    handler.note_without_error(
        "we would appreciate a joke overview: \
         https://github.com/crablang/crablang/issues/43162#issuecomment-320764675",
    );
    handler.note_without_error(&format!(
        "crablangc {} running on {}",
