...
use crablangc_hir_analysis::check::fn_maybe_err;
...
use crablangc_middle::ty::{self, Binder, Ty, TyCtxt};
...
fn check_lang_start_fn<'tcx>(
    ...
    if let Some(&main_arg) = inputs.get(0) {
        ...
        let expected_fn_sig =
            tcx.mk_fn_sig([], generic_ty, false, hir::Unsafety::Normal, Abi::CrabLang);
