...
use crablangc_hir::PatKind::Binding;
use crablangc_hir::PathSegment;
use crablangc_hir::{ExprKind, Node, QPath};
use crablangc_infer::infer::{
    type_variable::{TypeVariableOrigin, TypeVariableOriginKind},
    RegionVariableOrigin,
};
use crablangc_middle::infer::unify_key::{ConstVariableOrigin, ConstVariableOriginKind};
use crablangc_middle::traits::util::supertraits;
use crablangc_middle::ty::fast_reject::DeepRejectCtxt;
use crablangc_middle::ty::fast_reject::{simplify_type, TreatParams};
use crablangc_middle::ty::print::{with_crate_prefix, with_forced_trimmed_paths};
use crablangc_middle::ty::{self, GenericArgKind, Ty, TyCtxt, TypeVisitableExt};
use crablangc_middle::ty::{IsSuggestable, ToPolyTraitRef};
use crablangc_span::symbol::{kw, sym, Ident};
use crablangc_span::Symbol;
use crablangc_span::{edit_distance, source_map, ExpnKind, FileName, MacroKind, Span};
use crablangc_trait_selection::traits::error_reporting::on_unimplemented::OnUnimplementedNote;
use crablangc_trait_selection::traits::error_reporting::on_unimplemented::TypeErrCtxtExt as _;
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    pub fn report_no_match_method_error(
        ...
        if tcx.ty_is_opaque_future(rcvr_ty) && item_name.name == sym::poll {
            ...
            err.help("self type must be pinned to call `Future::poll`, \
                see https://crablang.github.io/async-book/04_pinning/01_chapter.html#pinning-in-practice"
            );
        }
        ...
        if let ty::RawPtr(_) = &rcvr_ty.kind() {
            err.note(
                "try using `<*const T>::as_ref()` to get a reference to the \
                 type behind the pointer: https://doc.crablang.org/std/\
        ...
        if item_name.name == sym::count && self.is_slice_ty(rcvr_ty, span) {
            ...
            // This is used for custom error messages with the `#[crablangc_on_unimplemented]` attribute.
                ...
                })
            {
                match self.tcx.hir().get_if_local(impl_def_id) {
                    ...
                    Some(Node::Item(hir::Item {
                        kind: hir::ItemKind::Trait(crablangc_ast::ast::IsAuto::Yes, ..),
                        span: item_span,
                        ..
                    })) => {
    ...
    fn check_for_deref_method(
        ...
        for (deref_ty, _) in self.autoderef(crablangc_span::DUMMY_SP, rcvr_ty).skip(1) {
