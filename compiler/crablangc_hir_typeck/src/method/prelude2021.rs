...
use crablangc_middle::ty::{Adt, Array, Ref, Ty};
use crablangc_session::lint::builtin::CRABLANG_2021_PRELUDE_COLLISIONS;
use crablangc_span::symbol::kw::{Empty, Underscore};
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    pub(super) fn lint_dot_call_from_2018(
        ...
        // CrabLang 2021 and later is already using the new prelude
        if span.crablang_2021() {
            return;
        }

        let prelude_or_array_lint = match segment.ident.name {
            // `try_into` was added to the prelude in CrabLang 2021.
            sym::try_into => CRABLANG_2021_PRELUDE_COLLISIONS,
            // `into_iter` wasn't added to the prelude,
            // but `[T; N].into_iter()` doesn't resolve to IntoIterator::into_iter
            // before CrabLang 2021, which results in the same problem.
            // It is only a problem for arrays.
            sym::into_iter if let Array(..) = self_ty.kind() => {
                // In this case, it wasn't really a prelude addition that was the problem.
                // Instead, the problem is that the array-into_iter hack will no longer apply in CrabLang 2021.
                crablangc_lint::ARRAY_INTO_ITER
            }
            _ => return,
        };
        ...
        if matches!(pick.kind, probe::PickKind::InherentImplPick | probe::PickKind::ObjectPick) {
            ...
            self.tcx.struct_span_lint_hir(
                ...
                self_expr.hir_id,
                self_expr.span,
                format!("trait method `{}` will become ambiguous in CrabLang 2021", segment.ident.name),
                |lint| {
    ...
    pub(super) fn lint_fully_qualified_call_from_2018(
        ...
        // These are the fully qualified methods added to prelude in CrabLang 2021
        ...
        self.tcx.struct_span_lint_hir(
            ...
            span,
            format!(
                "trait-associated function `{}` will become ambiguous in CrabLang 2021",
                method_name.name
            ),
            |lint| {
