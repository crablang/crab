...
use crablangc_hir_analysis::astconv::InferCtxtExt as _;
use crablangc_hir_analysis::autoderef::{self, Autoderef};
use crablangc_infer::infer::canonical::OriginalQueryValues;
use crablangc_infer::infer::canonical::{Canonical, QueryResponse};
use crablangc_infer::infer::DefineOpaqueTypes;
use crablangc_infer::infer::{self, InferOk, TyCtxtInferExt};
use crablangc_middle::middle::stability;
use crablangc_middle::ty::fast_reject::{simplify_type, TreatParams};
use crablangc_middle::ty::AssocItem;
use crablangc_middle::ty::GenericParamDefKind;
use crablangc_middle::ty::ToPredicate;
use crablangc_middle::ty::{self, ParamEnvAnd, Ty, TyCtxt, TypeFoldable, TypeVisitableExt};
...
use crablangc_span::edit_distance::{
    edit_distance_with_substrings, find_best_match_for_name_with_substrings,
};
use crablangc_span::symbol::sym;
use crablangc_span::{symbol::Ident, Span, Symbol, DUMMY_SP};
use crablangc_trait_selection::traits::query::evaluate_obligation::InferCtxtExt;
use crablangc_trait_selection::traits::query::method_autoderef::MethodAutoderefBadTy;
use crablangc_trait_selection::traits::query::method_autoderef::{
    CandidateStep, MethodAutoderefStepsResult,
};
use crablangc_trait_selection::traits::query::CanonicalTyGoal;
use crablangc_trait_selection::traits::NormalizeExt;
use crablangc_trait_selection::traits::{self, ObligationCause};
        ...
        OP: FnOnce(ProbeContext<'_, 'tcx>) -> Result<R, MethodError<'tcx>>,
    {
        ...
        if let Some(bad_ty) = &steps.opt_bad_ty {
            if is_suggestion.0 {
                ...
                // (see https://github.com/crablang/crablang/issues/46906)
                if self.tcx.sess.crablang_2018() {
    ...
    fn assemble_probe(&mut self, self_ty: &Canonical<'tcx, QueryResponse<'tcx, Ty<'tcx>>>) {
        ...
        match *raw_self_ty.kind() {
            ty::Dynamic(data, ..) if let Some(p) = data.principal() => {
                ...
                if self.tcx.has_attr(p.def_id(), sym::crablangc_has_incoherent_inherent_impls) {
            ...
            ty::Adt(def, _) => {
                ...
                if self.tcx.has_attr(def_id, sym::crablangc_has_incoherent_inherent_impls) {
                    self.assemble_inherent_candidates_for_incoherent_ty(raw_self_ty);
                }
            }
            ty::Foreign(did) => {
                self.assemble_inherent_impl_candidates_for_type(did);
                if self.tcx.has_attr(did, sym::crablangc_has_incoherent_inherent_impls) {
                    self.assemble_inherent_candidates_for_incoherent_ty(raw_self_ty);
                }
            }
            ty::Param(p) => {
            ...
            match probe.kind {
                ...
                TraitCandidate(trait_ref) => {
                    if let Some(method_name) = self.method_name {
                        ...
                        if self_ty.is_array() && !method_name.span.crablang_2021() {
