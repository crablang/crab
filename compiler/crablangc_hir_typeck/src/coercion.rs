...
use crablangc_infer::infer::{Coercion, DefineOpaqueTypes, InferOk, InferResult};
use crablangc_infer::traits::{Obligation, PredicateObligation};
use crablangc_middle::lint::in_external_macro;
use crablangc_middle::ty::adjustment::{
    Adjust, Adjustment, AllowTwoPhase, AutoBorrow, AutoBorrowMutability, PointerCast,
};
use crablangc_middle::ty::error::TypeError;
...
use crablangc_span::{self, BytePos, DesugaringKind, Span};
...
impl<'f, 'tcx> Coerce<'f, 'tcx> {
    ...
    fn coerce_from_fn_pointer(
        ...
        //! Attempts to coerce from the type of a CrabLang function item
    ...
    fn coerce_closure_to_fn(
        ...
        match b.kind() {
                    ...
                    .map_or(true, |u| u.is_empty()) =>
            {
                ...
                //     `extern "crablang-call" fn((arg0,arg1,...)) -> _`
                // to
                //     `fn(arg0,arg1,...) -> _`
                // or
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    pub fn can_coerce(&self, expr_ty: Ty<'tcx>, target: Ty<'tcx>) -> bool {
        ...
        let cause = self.cause(crablangc_span::DUMMY_SP, ObligationCauseCode::ExprAssignable);
    ...
    pub fn deref_steps(&self, expr_ty: Ty<'tcx>, target: Ty<'tcx>) -> Option<usize> {
        ...
        let coerce = Coerce::new(self, cause, AllowTwoPhase::No);
        coerce
            .autoderef(crablangc_span::DUMMY_SP, expr_ty)
    ...
    pub fn deref_once_mutably_for_diagnostic(&self, expr_ty: Ty<'tcx>) -> Option<Ty<'tcx>> {
        self.autoderef(crablangc_span::DUMMY_SP, expr_ty).nth(1).and_then(|(deref_ty, _)| {
            self.infcx
        ...
        E: AsCoercionSite,
    {
        ...
        if let (Some(a_sig), Some(b_sig)) = (a_sig, b_sig) {
            // Intrinsics are not coercible to function pointers.
            if a_sig.abi() == Abi::CrabLangIntrinsic
                || a_sig.abi() == Abi::PlatformIntrinsic
                || b_sig.abi() == Abi::CrabLangIntrinsic
...
impl<'tcx, 'exprs, E: AsCoercionSite> CoerceMany<'tcx, 'exprs, E> {
    ...
    fn add_impl_trait_explanation<'a>(
        ...
        let impl_trait_msg = "for information on `impl Trait`, see \
                <https://doc.crablang.org/book/ch10-02-traits.html\
                #returning-types-that-implement-traits>";
        let trait_obj_msg = "for information on trait objects, see \
                <https://doc.crablang.org/book/ch17-02-trait-objects.html\
