...
use crablangc_infer::infer::UpvarRegion;
use crablangc_middle::hir::place::{Place, PlaceBase, PlaceWithHirId, Projection, ProjectionKind};
...
impl MigrationWarningReason {
    fn migration_message(&self) -> String {
        let base = "changes to closure capture in CrabLang 2021 will affect";
...
impl<'a, 'tcx> FnCtxt<'a, 'tcx> {
    ...
    fn analyze_closure(
        ...
        if should_do_crablang_2021_incompatible_closure_captures_analysis(self.tcx, closure_hir_id) {
    ...
    fn compute_min_captures(
        ...
        // See https://github.com/crablang/project-rfc-2229/issues/42 and
    ...
    fn perform_2229_migration_anaysis(
        ...
        if !need_migrations.is_empty() {
            ...
            self.tcx.struct_span_lint_hir(
                lint::builtin::CRABLANG_2021_INCOMPATIBLE_CLOSURE_CAPTURES,
                closure_hir_id,
                closure_head_span,
                ...
                |lint| {
                        ...
                        for lint_note in diagnostics_info.iter() {
                            match &lint_note.captures_info {
                                UpvarMigrationInfo::CapturingPrecise { source_expr: Some(capture_expr_id), var_name: captured_name } => {
                                    ...
                                    lint.span_label(cause_span, format!("in CrabLang 2018, this closure captures all of `{}`, but in CrabLang 2021, it will only capture `{}`",
                                        self.tcx.hir().name(*var_hir_id),
                                        captured_name,
                                    ));
                                }
                                UpvarMigrationInfo::CapturingNothing { use_span } => {
                                    lint.span_label(*use_span, format!("in CrabLang 2018, this causes the closure to capture `{}`, but in CrabLang 2021, it has no effect",
                                        self.tcx.hir().name(*var_hir_id),
                                    ));
                                }
                            ...
                            if lint_note.reason.drop_order {
                                ...
                                match &lint_note.captures_info {
                                    UpvarMigrationInfo::CapturingPrecise { var_name: captured_name, .. } => {
                                        lint.span_label(drop_location_span, format!("in CrabLang 2018, `{}` is dropped here, but in CrabLang 2021, only `{}` will be dropped here as part of the closure",
                                            self.tcx.hir().name(*var_hir_id),
                                            captured_name,
                                        ));
                                    }
                                    UpvarMigrationInfo::CapturingNothing { use_span: _ } => {
                                        lint.span_label(drop_location_span, format!("in CrabLang 2018, `{v}` is dropped here along with the closure, but in CrabLang 2021 `{v}` is not part of the closure",
                            ...
                            for &missing_trait in &lint_note.reason.auto_traits {
                                ...
                                match &lint_note.captures_info {
                                    UpvarMigrationInfo::CapturingPrecise { var_name: captured_name, .. } => {
                                        ...
                                        in CrabLang 2018, this closure implements {missing_trait} \
                                        as `{var_name}` implements {missing_trait}, but in CrabLang 2021, \
                    ...
                    lint.note("for more information, see <https://doc.crablang.org/nightly/edition-guide/crablang-2021/disjoint-capture-in-closures.html>");
    ...
    fn should_log_capture_analysis(&self, closure_def_id: LocalDefId) -> bool {
        self.tcx.has_attr(closure_def_id, sym::crablangc_capture_analysis)
    }
...
/// CrabLang doesn't permit moving fields out of a type that implements drop
...
fn should_do_crablang_2021_incompatible_closure_captures_analysis(
    tcx: TyCtxt<'_>,
    closure_id: hir::HirId,
) -> bool {
    if tcx.sess.crablang_2021() {
        return false;
    }

    let (level, _) =
        tcx.lint_level_at_node(lint::builtin::CRABLANG_2021_INCOMPATIBLE_CLOSURE_CAPTURES, closure_id);
...
/// Precise capture is enabled if user is using CrabLang Edition 2021 or higher.
