...
pub use crablangc_middle::ty::layout::{FAT_PTR_ADDR, FAT_PTR_EXTRA};
use crablangc_middle::ty::Ty;
use crablangc_session::config;
use crablangc_target::abi::call::ArgAbi;
pub use crablangc_target::abi::call::*;
use crablangc_target::abi::{self, HasDataLayout, Int};
pub use crablangc_target::spec::abi::Abi;
...
impl<'ll, 'tcx> FnAbiLlvmExt<'ll, 'tcx> for FnAbi<'tcx, Ty<'tcx>> {
    ...
    fn apply_attrs_callsite(&self, bx: &mut Builder<'_, 'll, 'tcx>, callsite: &'ll Value) {
        ...
        if self.conv == Conv::CCmseNonSecureCall {
            // This will probably get ignored on all targets but those supporting the TcrablangZone-M
        ...
        let element_type_index = unsafe { llvm::LLVMCrabLangGetElementTypeArgIndex(callsite) };
        if element_type_index >= 0 {
            ...
            let element_type_attr = unsafe {
                llvm::LLVMCrabLangCreateElementTypeAttr(bx.llcx, bx.layout_of(pointee_ty).llvm_type(bx))
            };
            attributes::apply_to_callsite(
                callsite,
...
impl From<Conv> for llvm::CallConv {
    fn from(conv: Conv) -> Self {
        match conv {
            Conv::C | Conv::CrabLang | Conv::CCmseNonSecureCall => llvm::CCallConv,
            Conv::CrabLangCold => llvm::ColdCallConv,
