...
use crablangc_middle::mir::mono::MonoItem;
use crablangc_middle::ty::layout::LayoutOf;
use crablangc_middle::ty::{self, Instance, Ty};
use crablangc_middle::{bug, span_bug};
use crablangc_session::config::Lto;
use crablangc_target::abi::{Align, HasDataLayout, Primitive, Scalar, Size, WrappingRange};
use std::ops::Range;
...
fn set_global_alignment<'ll>(cx: &CodegenCx<'ll, '_>, gv: &'ll Value, mut align: Align) {
    ...
    // which can force it to be smaller. CrabLang doesn't support this yet.
...
fn check_and_apply_linkage<'ll, 'tcx>(
    ...
    if let Some(linkage) = attrs.import_linkage {
        ...
        unsafe {
            ...
            llvm::LLVMCrabLangSetLinkage(g1, base::linkage_to_llvm(linkage));
            ...
            // zero.
            let mut real_name = "_crablang_extern_with_linkage_".to_string();
            ...
            llvm::LLVMCrabLangSetLinkage(g2, llvm::Linkage::InternalLinkage);
        ...
        unsafe {
            let gv = match kind {
                Some(kind) if !self.tcx.sess.fewer_names() => {
                    ...
                    llvm::LLVMCrabLangSetLinkage(gv, llvm::Linkage::PrivateLinkage);
                    gv
                }
    ...
    pub(crate) fn get_static(&self, def_id: DefId) -> &'ll Value {
        ...
        let g = if def_id.is_local() && !self.tcx.is_foreign_item(def_id) {
            ...
            if !self.tcx.is_reachable_non_generic(def_id) {
                unsafe {
                    llvm::LLVMCrabLangSetVisibility(g, llvm::Visibility::Hidden);
                }
            }
        ...
        if !def_id.is_local() {
            ...
            if needs_dll_storage_attr {
                // This item is external but not foreign, i.e., it originates from an external CrabLang
    ...
    fn codegen_static(&self, def_id: DefId, is_mutable: bool) {
        unsafe {
            ...
            let g = if val_llty == llty {
                ...
                let linkage = llvm::LLVMCrabLangGetLinkage(g);
                let visibility = llvm::LLVMCrabLangGetVisibility(g);

                let new_g = llvm::LLVMCrabLangGetOrInsertGlobal(
                    self.llmod,
                    name.as_ptr().cast(),
                    name.len(),
                    val_llty,
                ...
                llvm::LLVMCrabLangSetLinkage(new_g, linkage);
                llvm::LLVMCrabLangSetVisibility(new_g, visibility);
            ...
            if attrs.flags.contains(CodegenFnAttrFlags::USED) {
                ...
                // The semantics of #[used] in CrabLang only require the symbol to make it into the
                ...
                // that check happens when assigning the `CodegenFnAttrFlags` in `crablangc_hir_analysis`,
