...
use crablangc_codegen_ssa::common::{IntPredicate, RealPredicate, SynchronizationScope, TypeKind};
...
use crablangc_symbol_mangling::typeid::kcfi_typeid_for_fnabi;
use crablangc_target::abi::{self, call::FnAbi, Align, Size, WrappingRange};
use crablangc_target::spec::{HasTargetSpec, Target};
use std::borrow::Cow;
use std::ffi::CStr;
use std::iter;
use std::ops::Deref;
...
impl<'a, 'll, 'tcx> BuilderMethods<'a, 'tcx> for Builder<'a, 'll, 'tcx> {
    ...
    fn fadd_fast(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe {
            ...
            llvm::LLVMCrabLangSetFastMath(instr);
            instr
        }
    }
    ...
    fn checked_binop(
        ...
        use crablangc_middle::ty::{Int, Uint};
        use crablangc_middle::ty::{IntTy::*, UintTy::*};
    ...
    fn atomic_load(
        ...
        ty: &'ll Type,
        ptr: &'ll Value,
        order: crablangc_codegen_ssa::common::AtomicOrdering,
        size: Size,
    ) -> &'ll Value {
        unsafe {
            let load = llvm::LLVMCrabLangBuildAtomicLoad(
                self.llbuilder,
                ty,
                ptr,
                UNNAMED,
    ...
    fn atomic_store(
        ...
        let ptr = self.check_store(val, ptr);
        unsafe {
            let store = llvm::LLVMCrabLangBuildAtomicStore(
                self.llbuilder,
                val,
                ptr,
                AtomicOrdering::from_generic(order),
    ...
    fn atomic_cmpxchg(
        ...
        src: &'ll Value,
        order: crablangc_codegen_ssa::common::AtomicOrdering,
        failure_order: crablangc_codegen_ssa::common::AtomicOrdering,
        weak: bool,
    ) -> &'ll Value {
    ...
    fn atomic_rmw(
        &mut self,
        op: crablangc_codegen_ssa::common::AtomicRmwBinOp,
        dst: &'ll Value,
        src: &'ll Value,
        order: crablangc_codegen_ssa::common::AtomicOrdering,
    ) -> &'ll Value {
    ...
    fn instrprof_increment(
        ...
        let llfn = unsafe { llvm::LLVMCrabLangGetInstrProfIncrementIntrinsic(self.cx().llmod) };
        ...
        unsafe {
            let _ = llvm::LLVMCrabLangBuildCall(
                self.llbuilder,
                llty,
                llfn,
...
impl<'a, 'll, 'tcx> Builder<'a, 'll, 'tcx> {
    ...
    fn position_at_start(&mut self, llbb: &'ll BasicBlock) {
        unsafe {
            llvm::LLVMCrabLangPositionBuilderAtStart(self.llbuilder, llbb);
        }
    }
    ...
    pub fn minnum(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildMinNum(self.llbuilder, lhs, rhs) }
    }

    pub fn maxnum(&mut self, lhs: &'ll Value, rhs: &'ll Value) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildMaxNum(self.llbuilder, lhs, rhs) }
    }
    ...
    pub fn vector_reduce_fadd(&mut self, acc: &'ll Value, src: &'ll Value) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildVectorReduceFAdd(self.llbuilder, acc, src) }
    }
    pub fn vector_reduce_fmul(&mut self, acc: &'ll Value, src: &'ll Value) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildVectorReduceFMul(self.llbuilder, acc, src) }
    }
    pub fn vector_reduce_fadd_fast(&mut self, acc: &'ll Value, src: &'ll Value) -> &'ll Value {
        unsafe {
            let instr = llvm::LLVMCrabLangBuildVectorReduceFAdd(self.llbuilder, acc, src);
            llvm::LLVMCrabLangSetFastMath(instr);
            instr
        }
    }
    pub fn vector_reduce_fmul_fast(&mut self, acc: &'ll Value, src: &'ll Value) -> &'ll Value {
        unsafe {
            let instr = llvm::LLVMCrabLangBuildVectorReduceFMul(self.llbuilder, acc, src);
            llvm::LLVMCrabLangSetFastMath(instr);
            instr
        }
    }
    pub fn vector_reduce_add(&mut self, src: &'ll Value) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildVectorReduceAdd(self.llbuilder, src) }
    }
    pub fn vector_reduce_mul(&mut self, src: &'ll Value) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildVectorReduceMul(self.llbuilder, src) }
    }
    pub fn vector_reduce_and(&mut self, src: &'ll Value) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildVectorReduceAnd(self.llbuilder, src) }
    }
    pub fn vector_reduce_or(&mut self, src: &'ll Value) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildVectorReduceOr(self.llbuilder, src) }
    }
    pub fn vector_reduce_xor(&mut self, src: &'ll Value) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildVectorReduceXor(self.llbuilder, src) }
    }
    pub fn vector_reduce_fmin(&mut self, src: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMCrabLangBuildVectorReduceFMin(self.llbuilder, src, /*NoNaNs:*/ false)
        }
    }
    pub fn vector_reduce_fmax(&mut self, src: &'ll Value) -> &'ll Value {
        unsafe {
            llvm::LLVMCrabLangBuildVectorReduceFMax(self.llbuilder, src, /*NoNaNs:*/ false)
        }
    }
    pub fn vector_reduce_fmin_fast(&mut self, src: &'ll Value) -> &'ll Value {
        unsafe {
            let instr =
                llvm::LLVMCrabLangBuildVectorReduceFMin(self.llbuilder, src, /*NoNaNs:*/ true);
            llvm::LLVMCrabLangSetFastMath(instr);
            instr
        }
    }
    pub fn vector_reduce_fmax_fast(&mut self, src: &'ll Value) -> &'ll Value {
        unsafe {
            let instr =
                llvm::LLVMCrabLangBuildVectorReduceFMax(self.llbuilder, src, /*NoNaNs:*/ true);
            llvm::LLVMCrabLangSetFastMath(instr);
            instr
        }
    }
    pub fn vector_reduce_min(&mut self, src: &'ll Value, is_signed: bool) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildVectorReduceMin(self.llbuilder, src, is_signed) }
    }
    pub fn vector_reduce_max(&mut self, src: &'ll Value, is_signed: bool) -> &'ll Value {
        unsafe { llvm::LLVMCrabLangBuildVectorReduceMax(self.llbuilder, src, is_signed) }
    }
