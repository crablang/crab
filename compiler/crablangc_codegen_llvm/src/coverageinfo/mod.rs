...
use crablangc_codegen_ssa::coverageinfo::map::{CounterExpression, FunctionCoverage};
...
use crablangc_middle::ty::layout::FnAbiOf;
...
fn create_pgo_func_name_var<'ll, 'tcx>(
    ...
    let llfn = cx.get_fn(instance);
    unsafe { llvm::LLVMCrabLangCoverageCreatePGOFuncNameVar(llfn, mangled_fn_name.as_ptr()) }
}

pub(crate) fn write_filenames_section_to_buffer<'a>(
    filenames: impl IntoIterator<Item = &'a CString>,
    buffer: &CrabLangString,
) {
    let c_str_vec = filenames.into_iter().map(|cstring| cstring.as_ptr()).collect::<Vec<_>>();
    unsafe {
        llvm::LLVMCrabLangCoverageWriteFilenamesSectionToBuffer(
            c_str_vec.as_ptr(),
            c_str_vec.len(),
            buffer,
        );
...
pub(crate) fn write_mapping_to_buffer(
    ...
    unsafe {
        llvm::LLVMCrabLangCoverageWriteMappingToBuffer(
...
pub(crate) fn hash_str(strval: &str) -> u64 {
    ...
    unsafe { llvm::LLVMCrabLangCoverageHashCString(strval.as_ptr()) }
}

pub(crate) fn hash_bytes(bytes: Vec<u8>) -> u64 {
    unsafe { llvm::LLVMCrabLangCoverageHashByteArray(bytes.as_ptr().cast(), bytes.len()) }
}

pub(crate) fn mapping_version() -> u32 {
    unsafe { llvm::LLVMCrabLangCoverageMappingVersion() }
}
...
pub(crate) fn save_cov_data_to_mod<'ll, 'tcx>(
    ...
    let covmap_var_name = llvm::build_string(|s| unsafe {
        llvm::LLVMCrabLangCoverageWriteMappingVarNameToString(s);
    })
    .expect("CrabLang Coverage Mapping var name failed UTF-8 conversion");
    debug!("covmap var name: {:?}", covmap_var_name);

    let covmap_section_name = llvm::build_string(|s| unsafe {
        llvm::LLVMCrabLangCoverageWriteMapSectionNameToString(cx.llmod, s);
    })
    .expect("CrabLang Coverage section name failed UTF-8 conversion");
...
pub(crate) fn save_func_record_to_mod<'ll, 'tcx>(
    ...
    //
    // In LLVM, a "translation unit" (effectively, a `Crate` in CrabLang) can describe functions that
    // are included-but-not-used. If (or when) CrabLang generates functions that are
    ...
    let func_record_section_name = llvm::build_string(|s| unsafe {
        llvm::LLVMCrabLangCoverageWriteFuncSectionNameToString(cx.llmod, s);
    })
    .expect("CrabLang Coverage function record section name failed UTF-8 conversion");
