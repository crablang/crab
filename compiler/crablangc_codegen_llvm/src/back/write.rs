...
use crablangc_codegen_ssa::back::link::ensure_removed;
use crablangc_codegen_ssa::back::write::{
    BitcodeSection, CodegenContext, EmitObj, ModuleConfig, TargetMachineFactoryConfig,
    TargetMachineFactoryFn,
};
use crablangc_codegen_ssa::traits::*;
use crablangc_codegen_ssa::{CompiledModule, ModuleCodegen};
use crablangc_data_structures::profiling::SelfProfilerRef;
use crablangc_data_structures::small_c_str::SmallCStr;
use crablangc_errors::{FatalError, Handler, Level};
use crablangc_fs_util::{link_or_copy, path_to_c_string};
use crablangc_middle::ty::TyCtxt;
use crablangc_session::config::{self, Lto, OutputType, Passes, SplitDwarfKind, SwitchWithOptPath};
use crablangc_session::Session;
use crablangc_span::symbol::sym;
use crablangc_span::InnerSpan;
use crablangc_target::spec::{CodeModel, RelocModel, SanitizerSet, SplitDebuginfo};
...
pub fn llvm_err<'a>(handler: &crablangc_errors::Handler, err: LlvmError<'a>) -> FatalError {
...
pub fn write_output_file<'ll>(
    handler: &crablangc_errors::Handler,
    ...
    unsafe {
        ...
        let result = llvm::LLVMCrabLangWriteOutputFile(
            target,
            pm,
            m,
            output_c.as_ptr(),
        ...
        if result == llvm::LLVMCrabLangResult::Success {
...
pub fn target_machine_factory(
    ...
    Arc::new(move |config: TargetMachineFactoryConfig| {
        ...
        let tm = unsafe {
            llvm::LLVMCrabLangCreateTargetMachine(
                triple.as_ptr(),
                cpu.as_ptr(),
                features.as_ptr(),
                abi.as_ptr(),
...
impl<'a> DiagnosticHandlers<'a> {
    pub fn new(
        ...
        unsafe {
            let old_handler = llvm::LLVMCrabLangContextGetDiagnosticHandler(llcx);
            llvm::LLVMCrabLangContextConfigureDiagnosticHandler(
                llcx,
                diagnostic_handler,
                data.cast(),
                remark_passes_all,
...
impl<'a> Drop for DiagnosticHandlers<'a> {
    fn drop(&mut self) {
        unsafe {
            llvm::LLVMCrabLangContextSetDiagnosticHandler(self.llcx, self.old_handler);
    ...
    match llvm::diagnostic::Diagnostic::unpack(info) {
        ...
        llvm::diagnostic::PGO(diagnostic_ref) | llvm::diagnostic::Linker(diagnostic_ref) => {
            let message = llvm::build_string(|s| {
                llvm::LLVMCrabLangWriteDiagnosticInfoToString(diagnostic_ref, s)
            })
...
pub(crate) unsafe fn llvm_optimize(
    ...
    let result = llvm::LLVMCrabLangOptimize(
...
pub(crate) unsafe fn codegen(
    ...
    let _timer = cgcx.prof.generic_activity_with_arg("LLVM_module_codegen", &*module.name);
    {
            ...
            F: FnOnce(&'ll mut PassManager<'ll>) -> R,
        {
            ...
            llvm::LLVMCrabLangAddLibraryInfo(cpm, llmod, no_builtins);
            f(cpm)
        }
        ...
        if config.emit_ir {
            ...
            extern "C" fn demangle_callback(
                ...
                let Ok(demangled) = crablangc_demangle::try_demangle(input) else { return 0 };
            ...
            let result = llvm::LLVMCrabLangPrintModule(llmod, out_c.as_ptr(), demangle_callback);
...
unsafe fn embed_bitcode(
    ...
    // example crablangc's own LTO will look for LLVM IR inside of the object file
        ...
        || cgcx.opts.target_triple.triple().starts_with("asmjs")
    {
        ...
        let llglobal = llvm::LLVMAddGlobal(
            ...
            "crablangc.embedded.module\0".as_ptr().cast(),
        );
        llvm::LLVMSetInitializer(llglobal, llconst);

        let section = if is_apple { "__LLVM,__bitcode\0" } else { ".llvmbc\0" };
        llvm::LLVMSetSection(llglobal, section.as_ptr().cast());
        llvm::LLVMCrabLangSetLinkage(llglobal, llvm::Linkage::PrivateLinkage);
        ...
        let llglobal = llvm::LLVMAddGlobal(
            llmod,
            common::val_ty(llconst),
            "crablangc.embedded.cmdline\0".as_ptr().cast(),
        );
        llvm::LLVMSetInitializer(llglobal, llconst);
...
fn create_msvc_imps(
    ...
    unsafe {
        ...
        let globals = base::iter_globals(llmod)
            .filter(|&val| {
                llvm::LLVMCrabLangGetLinkage(val) == llvm::Linkage::ExternalLinkage
                    && llvm::LLVMIsDeclaration(val) == 0
            })
            .filter_map(|val| {
                // Exclude some symbols that we know are not CrabLang symbols.
        ...
        for (imp_name, val) in globals {
            ...
            llvm::LLVMCrabLangSetLinkage(imp, llvm::Linkage::ExternalLinkage);
        }
    }
...
fn record_llvm_cgu_instructions_stats(prof: &SelfProfilerRef, llmod: &llvm::Module) {
    ...
    let raw_stats =
        llvm::build_string(|s| unsafe { llvm::LLVMCrabLangModuleInstructionStats(&llmod, s) })
