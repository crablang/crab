...
use crate::llvm::{self, ArchiveKind, LLVMMachineType, LLVMCrabLangCOFFShortExport};
...
impl ArchiveBuilderBuilder for LlvmArchiveBuilderBuilder {
    ...
    fn create_dll_import_lib(
        ...
        if mingw_gnu_toolchain {
            ...
            let output_path_z = crablangc_fs_util::path_to_c_string(&output_path);

            trace!("invoking LLVMCrabLangWriteImportLibrary");
            ...
            // All import names are CrabLang identifiers and therefore cannot contain \0 characters.
            ...
            let ffi_exports: Vec<LLVMCrabLangCOFFShortExport> = cstring_import_name_and_ordinal_vector
                .iter()
                .map(|(name_z, ordinal)| LLVMCrabLangCOFFShortExport::new(name_z.as_ptr(), *ordinal))
                .collect();
            let result = unsafe {
                crate::llvm::LLVMCrabLangWriteImportLibrary(
                    dll_name_z.as_ptr(),
            ...
            if result == crate::llvm::LLVMCrabLangResult::Failure {
...
fn get_llvm_object_symbols(
    ...
    let is_bitcode = unsafe { llvm::LLVMCrabLangIsBitcode(buf.as_ptr(), buf.len()) };
...
impl<'a> LlvmArchiveBuilder<'a> {
    fn build_with_llvm(&mut self, output: &Path) -> io::Result<bool> {
        ...
        unsafe {
            for addition in &mut additions {
                match addition {
                    Addition::File { path, name_in_archive } => {
                        ...
                        members.push(llvm::LLVMCrabLangArchiveMemberNew(
                            path.as_ptr(),
                            name.as_ptr(),
                            None,
                        ));
                    ...
                    Addition::Archive { archive, skip, .. } => {
                        for child in archive.iter() {
                            ...
                            let m = llvm::LLVMCrabLangArchiveMemberNew(
                                ptr::null(),
                                name.as_ptr(),
                                Some(child.raw),
                            );
            ...
            let r = llvm::LLVMCrabLangWriteArchive(
                dst.as_ptr(),
            ...
            let ret = if r.into_result().is_err() {
                let err = llvm::LLVMCrabLangGetLastError();
            ...
            for member in members {
                llvm::LLVMCrabLangArchiveMemberFree(member);
            }
            ret
        }
    }
