...
use crablangc_codegen_ssa::back::lto::{LtoModuleCodegen, SerializedModule, ThinModule, ThinShared};
use crablangc_codegen_ssa::back::symbol_export;
use crablangc_codegen_ssa::back::write::{CodegenContext, FatLTOInput, TargetMachineFactoryConfig};
use crablangc_codegen_ssa::traits::*;
use crablangc_codegen_ssa::{looks_like_crablang_object_file, ModuleCodegen, ModuleKind};
use crablangc_data_structures::fx::FxHashMap;
use crablangc_data_structures::memmap::Mmap;
use crablangc_errors::{FatalError, Handler};
use crablangc_hir::def_id::LOCAL_CRATE;
use crablangc_middle::bug;
use crablangc_middle::dep_graph::WorkProduct;
use crablangc_middle::middle::exported_symbols::{SymbolExportInfo, SymbolExportLevel};
use crablangc_session::cgu_reuse_tracker::CguReuse;
use crablangc_session::config::{self, CrateType, Lto};
...
fn prepare_lto(
    ...
    let export_threshold = match cgcx.lto {
        ...
        Lto::ThinLocal => SymbolExportLevel::CrabLang,
    ...
    if cgcx.lto != Lto::ThinLocal {
        ...
        for &(cnum, ref path) in cgcx.each_linked_rlib_for_lto.iter() {
            ...
            let obj_files = archive
                ...
                .filter(|&(name, _)| looks_like_crablang_object_file(name));
...
fn get_bitcode_slice_from_object_data(obj: &[u8]) -> Result<&[u8], LtoBitcodeFromRlib> {
    let mut len = 0;
    let data =
        unsafe { llvm::LLVMCrabLangGetBitcodeSliceFromObjectData(obj.as_ptr(), obj.len(), &mut len) };
    if !data.is_null() {
        assert!(len != 0);
...
fn fat_lto(
    ...
    let costliest_module = in_memory
        ...
        .map(|(i, module)| {
            let cost = unsafe { llvm::LLVMCrabLangModuleCost(module.module_llvm.llmod()) };
            (cost, i)
        })
        .max();
    ...
    let mut serialized_bitcode = Vec::new();
    {
        ...
        unsafe {
            ...
            llvm::LLVMCrabLangRunRestrictionPass(
                llmod,
...
impl<'a> Linker<'a> {
    pub(crate) fn new(llmod: &'a llvm::Module) -> Self {
        unsafe { Linker(llvm::LLVMCrabLangLinkerNew(llmod)) }
    }

    pub(crate) fn add(&mut self, bytecode: &[u8]) -> Result<(), ()> {
        unsafe {
            if llvm::LLVMCrabLangLinkerAdd(
                self.0,
...
impl Drop for Linker<'_> {
    fn drop(&mut self) {
        unsafe {
            llvm::LLVMCrabLangLinkerFree(&mut *(self.0 as *mut _));
        }
    }
}
...
fn thin_lto(
    ...
    unsafe {
        ...
        // upstream...
        let data = llvm::LLVMCrabLangCreateThinLTOData(
            thin_modules.as_ptr(),
...
pub(crate) fn run_pass_manager(
    ...
    unsafe {
        if !llvm::LLVMCrabLangHasModuleFlag(
            module.module_llvm.llmod(),
            "LTOPostLink".as_ptr().cast(),
            11,
        ) {
            llvm::LLVMCrabLangAddModuleFlag(
                module.module_llvm.llmod(),
...
impl ModuleBuffer {
    pub fn new(m: &llvm::Module) -> ModuleBuffer {
        ModuleBuffer(unsafe { llvm::LLVMCrabLangModuleBufferCreate(m) })
    }
}
...
impl ModuleBufferMethods for ModuleBuffer {
    fn data(&self) -> &[u8] {
        unsafe {
            let ptr = llvm::LLVMCrabLangModuleBufferPtr(self.0);
            let len = llvm::LLVMCrabLangModuleBufferLen(self.0);
...
impl Drop for ModuleBuffer {
    fn drop(&mut self) {
        unsafe {
            llvm::LLVMCrabLangModuleBufferFree(&mut *(self.0 as *mut _));
        }
    }
}
...
impl Drop for ThinData {
    fn drop(&mut self) {
        unsafe {
            llvm::LLVMCrabLangFreeThinLTOData(&mut *(self.0 as *mut _));
        }
    }
}
...
impl ThinBuffer {
    pub fn new(m: &llvm::Module, is_thin: bool) -> ThinBuffer {
        unsafe {
            let buffer = llvm::LLVMCrabLangThinLTOBufferCreate(m, is_thin);
            ThinBuffer(buffer)
        }
    }
}
...
impl ThinBufferMethods for ThinBuffer {
    fn data(&self) -> &[u8] {
        unsafe {
            let ptr = llvm::LLVMCrabLangThinLTOBufferPtr(self.0) as *const _;
            let len = llvm::LLVMCrabLangThinLTOBufferLen(self.0);
            slice::from_raw_parts(ptr, len)
        }
    }
}
...
impl Drop for ThinBuffer {
    fn drop(&mut self) {
        unsafe {
            llvm::LLVMCrabLangThinLTOBufferFree(&mut *(self.0 as *mut _));
        }
    }
}
...
pub unsafe fn optimize_thin_module(
    ...
    let llcx = llvm::LLVMCrabLangContextCreate(cgcx.fewer_names);
    ...
    };
    {
        ...
        // using below. If we find more than one though then crablangc has changed
        // in a way we're not ready for, so generate an ICE by returning
        // an error.
        let mut cu1 = ptr::null_mut();
        let mut cu2 = ptr::null_mut();
        llvm::LLVMCrabLangThinLTOGetDICompileUnit(llmod, &mut cu1, &mut cu2);
        if !cu2.is_null() {
        ...
        // bindings we've got (currently `PassWrapper.cpp`)
        {
            ...
            if !llvm::LLVMCrabLangPrepareThinLTORename(thin_module.shared.data.0, llmod, target) {
...

        {
            ...
            if !llvm::LLVMCrabLangPrepareThinLTOResolveWeak(thin_module.shared.data.0, llmod) {
                return Err(write::llvm_err(&diag_handler, LlvmError::PrepareThinLtoModule));
            }
...

        {
            ...
            if !llvm::LLVMCrabLangPrepareThinLTOInternalize(thin_module.shared.data.0, llmod) {
                return Err(write::llvm_err(&diag_handler, LlvmError::PrepareThinLtoModule));
            }
...

        {
            ...
            if !llvm::LLVMCrabLangPrepareThinLTOImport(thin_module.shared.data.0, llmod, target) {
                return Err(write::llvm_err(&diag_handler, LlvmError::PrepareThinLtoModule));
            }
        ...
        // fixed in LLVM.
        {
            ...
            llvm::LLVMCrabLangThinLTOPatchDICompileUnit(llmod, cu1);
...
impl ThinLTOKeysMap {
    ...
    fn from_thin_lto_modules(
        ...
        let keys = iter::zip(modules, names)
            .map(|(module, name)| {
                let key = build_string(|crablang_str| unsafe {
                    llvm::LLVMCrabLangComputeLTOCacheKey(crablang_str, module.identifier, data.0);
                })
...
pub fn parse_module<'a>(
    ...
    unsafe {
        llvm::LLVMCrabLangParseBitcodeForLTO(cx, data.as_ptr(), data.len(), name.as_ptr())
