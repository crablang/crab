...
use crablangc_target::abi::{Align, Endian, Size, TagEncoding, VariantIdx, Variants};
...
fn build_single_variant_union_fields<'ll, 'tcx>(
    ...
    smallvec![
        ...
        unsafe {
            llvm::LLVMCrabLangDIBuilderCreateStaticMemberType(
                DIB(cx),
                enum_type_di_node,
...
fn build_union_fields_for_direct_tag_enum_or_generator<'ll, 'tcx>(
    ...
    unions_fields.extend(variant_field_infos.into_iter().map(|variant_member_info| {
        ...
        // We use LLVMCrabLangDIBuilderCreateMemberType() member type directly because
