...
use crablangc_target::abi::{Align, Size, VariantIdx};
...
impl<'tcx> UniqueTypeId<'tcx> {
    ...
    /// argument of the various `LLVMCrabLangDIBuilderCreate*Type()` methods.
    ///
...
pub(super) fn stub<'ll, 'tcx>(
    ...
    let metadata = match kind {
        Stub::Struct | Stub::VTableTy { .. } => {
            ...
            unsafe {
                llvm::LLVMCrabLangDIBuilderCreateStructType(
                    DIB(cx),
                    containing_scope,
                    name.as_ptr().cast(),
                    name.len(),
        ...
        Stub::Union => unsafe {
            llvm::LLVMCrabLangDIBuilderCreateUnionType(
                DIB(cx),
                containing_scope,
                name.as_ptr().cast(),
                name.len(),
...
pub(super) fn build_type_with_children<'ll, 'tcx>(
    ...
    if !(members.is_empty() && generics.is_empty()) {
        unsafe {
            ...
            llvm::LLVMCrabLangDICompositeTypeReplaceArrays(
                DIB(cx),
                stub_info.metadata,
                Some(members_array),
                Some(generics_array),
