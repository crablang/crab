...
use crablangc_codegen_ssa::mir::debuginfo::VariableKind::*;
...
use crablangc_codegen_ssa::debuginfo::type_names;
use crablangc_codegen_ssa::mir::debuginfo::{DebugScope, FunctionDebugContext, VariableKind};
use crablangc_codegen_ssa::traits::*;
use crablangc_data_structures::fx::FxHashMap;
use crablangc_data_structures::sync::Lrc;
use crablangc_hir::def_id::{DefId, DefIdMap};
use crablangc_index::vec::IndexVec;
use crablangc_middle::mir;
use crablangc_middle::ty::layout::LayoutOf;
use crablangc_middle::ty::subst::{GenericArgKind, SubstsRef};
use crablangc_middle::ty::{self, Instance, ParamEnv, Ty, TypeVisitableExt};
use crablangc_session::config::{self, DebugInfo};
use crablangc_session::Session;
use crablangc_span::symbol::Symbol;
use crablangc_span::{self, BytePos, Pos, SourceFile, SourceFileAndLine, SourceFileHash, Span};
...
impl Drop for CodegenUnitDebugContext<'_, '_> {
    fn drop(&mut self) {
        unsafe {
            llvm::LLVMCrabLangDIBuilderDispose(&mut *(self.builder as *mut _));
        }
    }
}
...
impl<'ll, 'tcx> CodegenUnitDebugContext<'ll, 'tcx> {
    pub fn new(llmod: &'ll llvm::Module) -> Self {
        ...
        let builder = unsafe { llvm::LLVMCrabLangDIBuilderCreate(llmod) };
    ...
    pub fn finalize(&self, sess: &Session) {
        unsafe {
            llvm::LLVMCrabLangDIBuilderFinalize(self.builder);
            ...
            llvm::LLVMCrabLangAddModuleFlag(
                ...
                llvm::LLVMModFlagBehavior::Warning,
                ptr.cast(),
                llvm::LLVMCrabLangDebugMetadataVersion(),
            );
        }
    }
}
...
pub fn finalize(cx: &CodegenCx<'_, '_>) {
    if let Some(dbg_cx) = &cx.dbg_cx {
        ...
        if gdb::needs_gdb_debug_scripts_section(cx) {
            ...
            // cause GDB to try and load the gdb_load_crablang_pretty_printers.py file,
            // which activates the CrabLang pretty printers for binary this section is
            // contained in.
...
impl<'ll> DebugInfoBuilderMethods for Builder<'_, 'll, '_> {
    ...
    fn dbg_var_addr(
        ...
        let op_deref = || unsafe { llvm::LLVMCrabLangDIBuilderCreateOpDeref() };
        let op_plus_uconst = || unsafe { llvm::LLVMCrabLangDIBuilderCreateOpPlusUconst() };
        let op_llvm_fragment = || unsafe { llvm::LLVMCrabLangDIBuilderCreateOpLLVMFragment() };
        ...
        unsafe {
            ...
            llvm::LLVMCrabLangDIBuilderInsertDeclareAtEnd(
                DIB(self.cx()),
                variable_alloca,
                dbg_var,
                addr_ops.as_ptr(),
...
// `crablangc_span::Loc` rather than `DILocation`, perhaps by making
...
impl CodegenCx<'_, '_> {
    ...
    pub fn lookup_debug_loc(&self, pos: BytePos) -> DebugLoc {
        ...
        // See discussion in https://github.com/crablang/crablang/issues/42921
...
impl<'ll, 'tcx> DebugInfoMethods<'tcx> for CodegenCx<'ll, 'tcx> {
    ...
    fn dbg_scope_fn(
        ...
        let function_type_metadata = unsafe {
            ...
            llvm::LLVMCrabLangDIBuilderCreateSubroutineType(DIB(self), fn_signature)
        };
        ...
        unsafe {
            return llvm::LLVMCrabLangDIBuilderCreateFunction(
                DIB(self),
                containing_scope,
                name.as_ptr().cast(),
                name.len(),
                ...
                iter::zip(substs, names)
                    .filter_map(|(kind, name)| {
                        if let GenericArgKind::Type(ty) = kind.unpack() {
                            ...
                            Some(unsafe {
                                Some(llvm::LLVMCrabLangDIBuilderCreateTemplateTypeParameter(
                                    DIB(cx),
                                    None,
                                    name.as_ptr().cast(),
                                    name.len(),
    ...
    fn dbg_loc(
        ...
        unsafe { llvm::LLVMCrabLangDIBuilderCreateDebugLocation(line, col, scope, inlined_at) }
    }
    ...
    fn extend_scope_to_file(
        ...
        file: &crablangc_span::SourceFile,
    ) -> &'ll DILexicalBlock {
    ...
    fn create_dbg_var(
        ...
        unsafe {
            llvm::LLVMCrabLangDIBuilderCreateVariable(
                DIB(self),
                dwarf_tag,
                scope_metadata,
                name.as_ptr().cast(),
