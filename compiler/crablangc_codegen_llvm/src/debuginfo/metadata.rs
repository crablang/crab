...
use cstr::cstr;
use crablangc_codegen_ssa::debuginfo::type_names::cpp_like_debuginfo;
use crablangc_codegen_ssa::debuginfo::type_names::VTableNameKind;
use crablangc_codegen_ssa::traits::*;
use crablangc_fs_util::path_to_c_string;
use crablangc_hir::def::CtorKind;
use crablangc_hir::def_id::{DefId, LOCAL_CRATE};
use crablangc_middle::bug;
use crablangc_middle::ty::layout::{LayoutOf, TyAndLayout};
...
use crablangc_session::config::{self, DebugInfo, Lto};
use crablangc_span::symbol::Symbol;
use crablangc_span::FileName;
use crablangc_span::{self, FileNameDisplayPreference, SourceFile};
use crablangc_symbol_mangling::typeid_for_trait_ref;
...
const DW_LANG_CRABLANG: c_uint = 0x1c;
...
fn build_fixed_size_array_di_node<'ll, 'tcx>(
    ...
    let subrange =
        unsafe { Some(llvm::LLVMCrabLangDIBuilderGetOrCreateSubrange(DIB(cx), 0, upper_bound)) };

    let subscripts = create_DIArray(DIB(cx), &[subrange]);
    let di_node = unsafe {
        llvm::LLVMCrabLangDIBuilderCreateArrayType(
            DIB(cx),
            size.bits(),
            align.bits() as u32,
...
fn build_pointer_or_reference_di_node<'ll, 'tcx>(
    ...
    match fat_pointer_kind(cx, pointee_type) {
        None => {
            ...
            let di_node = unsafe {
                llvm::LLVMCrabLangDIBuilderCreatePointerType(
                    DIB(cx),
...
fn build_subroutine_type_di_node<'ll, 'tcx>(
    ...
    // type in CrabLang by using 'impl trait':
    //
    ...
    let fn_di_node = unsafe {
        llvm::LLVMCrabLangDIBuilderCreateSubroutineType(
            DIB(cx),
...
fn recursion_marker_type_di_node<'ll, 'tcx>(cx: &CodegenCx<'ll, 'tcx>) -> &'ll DIType {
    *debug_context(cx).recursion_marker_type.get_or_init(move || {
        unsafe {
            ...
            let name = "<recur_type>";
            llvm::LLVMCrabLangDIBuilderCreateBasicType(
                DIB(cx),
                name.as_ptr().cast(),
                name.len(),
                cx.tcx.data_layout.pointer_size.bits(),
...
pub fn file_metadata<'ll>(cx: &CodegenCx<'ll, '_>, source_file: &SourceFile) -> &'ll DIFile {
    ...
    fn alloc_new_file_metadata<'ll>(
        ...
        let hash_kind = match source_file.src_hash.kind {
            crablangc_span::SourceFileHashAlgorithm::Md5 => llvm::ChecksumKind::MD5,
            crablangc_span::SourceFileHashAlgorithm::Sha1 => llvm::ChecksumKind::SHA1,
            crablangc_span::SourceFileHashAlgorithm::Sha256 => llvm::ChecksumKind::SHA256,
        };
        let hash_value = hex_encode(source_file.src_hash.hash_bytes());

        unsafe {
            llvm::LLVMCrabLangDIBuilderCreateFile(
                DIB(cx),
...
fn build_basic_type_di_node<'ll, 'tcx>(
    ...
    let typedef_di_node = unsafe {
        llvm::LLVMCrabLangDIBuilderCreateTypedef(
            DIB(cx),
            ty_di_node,
...
pub fn build_compile_unit_di_node<'ll, 'tcx>(
    ...
    let crablangc_producer =
        format!("crablangc version {}", option_env!("CFG_VERSION").expect("CFG_VERSION"),);
    // FIXME(#41252) Remove "clang LLVM" if we can get GDB and LLVM to play nice.
    let producer = format!("clang LLVM ({})", crablangc_producer);
    ...
    unsafe {
        let compile_unit_file = llvm::LLVMCrabLangDIBuilderCreateFile(
        ...
        let unit_metadata = llvm::LLVMCrabLangDIBuilderCreateCompileUnit(
            debug_context.builder,
            DW_LANG_CRABLANG,
            compile_unit_file,
        ...
        if tcx.sess.target.is_like_wasm {
            let name_metadata = llvm::LLVMMDStringInContext(
                debug_context.llcontext,
                crablangc_producer.as_ptr().cast(),
                crablangc_producer.as_bytes().len() as c_uint,
            );
...
fn build_field_di_node<'ll, 'tcx>(
    ...
    unsafe {
        llvm::LLVMCrabLangDIBuilderCreateMemberType(
            DIB(cx),
            owner,
            name.as_ptr().cast(),
            name.len(),
...
/// Creates the debuginfo node for a CrabLang struct type. Maybe be a regular struct or a tuple-struct.
...
/// Builds the DW_TAG_structure_type debuginfo node for a CrabLang tuple type.
...
/// Build the debuginfo node for a CrabLang `union` type.
            ...
            let template_params: SmallVec<_> = iter::zip(substs, names)
                .filter_map(|(kind, name)| {
                    if let GenericArgKind::Type(ty) = kind.unpack() {
                        ...
                        Some(unsafe {
                            llvm::LLVMCrabLangDIBuilderCreateTemplateTypeParameter(
                                DIB(cx),
                                None,
                                name.as_ptr().cast(),
                                name.len(),
...
pub fn build_global_var_di_node<'ll>(cx: &CodegenCx<'ll, '_>, def_id: DefId, global: &'ll Value) {
    ...
    // https://github.com/crablang/crablang/pull/46457#issuecomment-351750952).
    ...
    unsafe {
        llvm::LLVMCrabLangDIBuilderCreateStaticVariable(
            DIB(cx),
            Some(var_scope),
...
fn vcall_visibility_metadata<'ll, 'tcx>(
    ...
    unsafe {
        ...
        llvm::LLVMCrabLangGlobalAddMetadata(
            vtable,
...
pub fn extend_scope_to_file<'ll>(
    ...
    unsafe { llvm::LLVMCrabLangDIBuilderCreateLexicalBlockFile(DIB(cx), scope_metadata, file_metadata) }
}
