...
use crablangc_codegen_ssa::common::TypeKind;
...
use crablangc_target::abi::{AddressSpace, Align, Integer, Size};
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(
            &llvm::build_string(|s| unsafe {
                llvm::LLVMCrabLangWriteTypeToString(self, s);
            })
...
impl<'ll, 'tcx> BaseTypeMethods<'tcx> for CodegenCx<'ll, 'tcx> {
    ...
    fn type_kind(&self, ty: &'ll Type) -> TypeKind {
        unsafe { llvm::LLVMCrabLangGetTypeKind(ty).to_generic() }
    }
    ...
    fn type_array(&self, ty: &'ll Type, len: u64) -> &'ll Type {
        unsafe { llvm::LLVMCrabLangArrayType(ty, len) }
    }
}
