        ...
        let pass_name = super::build_string(|pass_name| {
            message = super::build_string(|message| {
                filename = super::build_string(|filename| {
                    super::LLVMCrabLangUnpackOptimizationDiagnostic(
                        di,
                        pass_name,
                        &mut function,
                        &mut line,
    ...
    pub unsafe fn unpack(diag: &SMDiagnostic) -> SrcMgrDiagnostic {
        ...
        let message = super::build_string(|message| {
            buffer = super::build_string(|buffer| {
                have_source = super::LLVMCrabLangUnpackSMDiagnostic(
                    diag,
                    message,
                    buffer,
                    &mut level,
...
impl InlineAsmDiagnostic {
    unsafe fn unpackInlineAsm(di: &DiagnosticInfo) -> Self {
        ...
        super::LLVMCrabLangUnpackInlineAsmDiagnostic(di, &mut level, &mut cookie, &mut message);
    ...
    unsafe fn unpackSrcMgr(di: &DiagnosticInfo) -> Self {
        let mut cookie = 0;
        let smdiag = SrcMgrDiagnostic::unpack(super::LLVMCrabLangGetSMDiagnostic(di, &mut cookie));
        InlineAsmDiagnostic {
            level: smdiag.level,
            cookie,
...
impl<'ll> Diagnostic<'ll> {
    pub unsafe fn unpack(di: &'ll DiagnosticInfo) -> Self {
        ...
        let kind = super::LLVMCrabLangGetDiagInfoKind(di);
