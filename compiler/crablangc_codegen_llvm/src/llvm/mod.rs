...
use crablangc_llvm::CrabLangString;
...
impl LLVMCrabLangResult {
    pub fn into_result(self) -> Result<(), ()> {
        match self {
            LLVMCrabLangResult::Success => Ok(()),
            LLVMCrabLangResult::Failure => Err(()),
        }
    }
}
...
pub fn AddFunctionAttributes<'ll>(llfn: &'ll Value, idx: AttributePlace, attrs: &[&'ll Attribute]) {
    unsafe {
        LLVMCrabLangAddFunctionAttributes(llfn, idx.as_uint(), attrs.as_ptr(), attrs.len());
    }
}
...
pub fn AddCallSiteAttributes<'ll>(
    ...
    unsafe {
        LLVMCrabLangAddCallSiteAttributes(callsite, idx.as_uint(), attrs.as_ptr(), attrs.len());
    }
}
...
pub fn SetUniqueComdat(llmod: &Module, val: &Value) {
    unsafe {
        ...
        LLVMCrabLangSetComdat(llmod, val, name.as_ptr().cast(), name.len());
    }
}
...
impl AttributeKind {
    ...
    pub fn create_attr(self, llcx: &Context) -> &Attribute {
        unsafe { LLVMCrabLangCreateAttrNoValue(llcx, self) }
    }
}
...
impl MemoryEffects {
    ...
    pub fn create_attr(self, llcx: &Context) -> &Attribute {
        unsafe { LLVMCrabLangCreateMemoryEffectsAttr(llcx, self) }
    }
}
...
pub fn set_linkage(llglobal: &Value, linkage: Linkage) {
    unsafe {
        LLVMCrabLangSetLinkage(llglobal, linkage);
    }
}

pub fn set_visibility(llglobal: &Value, visibility: Visibility) {
    unsafe {
        LLVMCrabLangSetVisibility(llglobal, visibility);
    }
}
...
pub fn set_comdat(llmod: &Module, llglobal: &Value, name: &str) {
    unsafe {
        LLVMCrabLangSetComdat(llmod, llglobal, name.as_ptr().cast(), name.len());
    }
}
...
pub fn build_string(f: impl FnOnce(&CrabLangString)) -> Result<String, FromUtf8Error> {
    let sr = CrabLangString { bytes: RefCell::new(Vec::new()) };
    f(&sr);
    String::from_utf8(sr.bytes.into_inner())
}

pub fn build_byte_buffer(f: impl FnOnce(&CrabLangString)) -> Vec<u8> {
    let sr = CrabLangString { bytes: RefCell::new(Vec::new()) };
    f(&sr);
    sr.bytes.into_inner()
}
...
pub fn twine_to_string(tr: &Twine) -> String {
    unsafe {
        build_string(|s| LLVMCrabLangWriteTwineToString(tr, s)).expect("got a non-UTF8 Twine from LLVM")
    }
}

pub fn last_error() -> Option<String> {
    unsafe {
        let cstr = LLVMCrabLangGetLastError();
        if cstr.is_null() {
            None
        } else {
...
impl<'a> OperandBundleDef<'a> {
    pub fn new(name: &str, vals: &[&'a Value]) -> Self {
        ...
        let def = unsafe {
            LLVMCrabLangBuildOperandBundleDef(name.as_ptr(), vals.as_ptr(), vals.len() as c_uint)
        };
...
impl Drop for OperandBundleDef<'_> {
    fn drop(&mut self) {
        unsafe {
            LLVMCrabLangFreeOperandBundleDef(&mut *(self.raw as *mut _));
        }
    }
}
