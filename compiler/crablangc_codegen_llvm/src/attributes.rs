...
use crablangc_data_structures::small_str::SmallStr;
...
use crablangc_target::spec::{FramePointer, SanitizerSet, StackProbeType, StackProtector};
...
use crate::llvm_util;
pub use crablangc_attr::{InlineAttr, InstructionSetAttr, OptimizeAttr};
...
fn probestack_attr<'ll>(cx: &CodegenCx<'ll, '_>) -> Option<&'ll Attribute> {
    ...
    let attr_value = match cx.sess().target.stack_probes {
        ...
        // Flag our internal `__crablang_probestack` function as the stack probe symbol.
        // This is defined in the `compiler-builtins` crate for each architecture.
        StackProbeType::Call => "__crablang_probestack",
        // Pick from the two above based on the LLVM version.
        StackProbeType::InlineOrCall { min_llvm_version_for_inline } => {
            if llvm_util::get_version() < min_llvm_version_for_inline {
                "__crablang_probestack"
            } else {
                "inline-asm"
            }
        }
...
fn create_alloc_family_attr(llcx: &llvm::Context) -> &llvm::Attribute {
    llvm::CreateAttrStringValue(llcx, "alloc-family", "__crablang_alloc")
}
...
pub fn from_fn_attrs<'ll, 'tcx>(
    ...
    if codegen_fn_attrs.flags.contains(CodegenFnAttrFlags::NAKED) {
        ...
        // Otherwise LLVM's "naked" functions come with endbr prefixes per https://github.com/crablang/crablang/issues/98768
    ...
    // Without this, ThinLTO won't inline CrabLang functions into Clang generated
