...
use crablangc_codegen_ssa::ModuleCodegen;
use crablangc_codegen_ssa::{CodegenResults, CompiledModule};
use crablangc_data_structures::fx::FxHashMap;
use crablangc_errors::{DiagnosticMessage, ErrorGuaranteed, FatalError, Handler, SubdiagnosticMessage};
...
use crablangc_session::config::{OptLevel, OutputFilenames, PrintRequest};
...
impl WriteBackendMethods for LlvmCodegenBackend {
    ...
    fn print_pass_timings(&self) {
        unsafe {
            llvm::LLVMCrabLangPrintPassTimings();
        }
    }
    fn run_link(
...
"#
    ...
    fn codegen_crate<'tcx>(
        ...
        Box::new(crablangc_codegen_ssa::base::codegen_crate(
            LlvmCodegenBackend(()),
            tcx,
    ...
    fn join_codegen(
        ...
        let (codegen_results, work_products) = ongoing_codegen
            .downcast::<crablangc_codegen_ssa::back::write::OngoingCodegen<LlvmCodegenBackend>>()
...
impl ModuleLlvm {
    fn new(tcx: TyCtxt<'_>, mod_name: &str) -> Self {
        unsafe {
            let llcx = llvm::LLVMCrabLangContextCreate(tcx.sess.fewer_names());
...
impl Drop for ModuleLlvm {
    fn drop(&mut self) {
        unsafe {
            llvm::LLVMCrabLangDisposeTargetMachine(&mut *(self.tm as *mut _));
