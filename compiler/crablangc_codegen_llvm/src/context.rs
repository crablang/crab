...
use crablangc_data_structures::base_n;
...
use crablangc_middle::mir::mono::CodegenUnit;
...
use crablangc_session::config::{BranchProtection, CFGuard, CFProtection};
use crablangc_session::config::{CrateType, DebugInfo, PAuthKey, PacRet};
...
use crablangc_target::spec::{HasTargetSpec, RelocModel, Target, TlsModel};
...
pub struct CodegenCx<'ll, 'tcx> {
    ...
    pub crablang_try_fn: Cell<Option<(&'ll Type, &'ll Value)>>,
...
pub unsafe fn create_module<'ll>(
    ...
    if sess.target.is_builtin {
        ...
        llvm::LLVMCrabLangSetDataLayoutFromTargetMachine(llmod, tm);
        llvm::LLVMCrabLangDisposeTargetMachine(tm);
        ...
        // `data_layout` than the one that is in the crablang/crablang repo. If
        ...
        //
        // Unsure if this will actually cause breakage when crablangc is configured
        // as such.
        //
        // FIXME(#34960)
        ...
        if !custom_llvm_used && target_data_layout != llvm_data_layout {
            bug!(
                "data-layout for target `{crablangc_target}`, `{crablangc_layout}`, \
                  differs from LLVM target's `{llvm_target}` default layout, `{llvm_layout}`",
                crablangc_target = sess.opts.target_triple,
                crablangc_layout = target_data_layout,
    ...
    llvm::LLVMCrabLangSetModuleCodeModel(llmod, to_llvm_code_model(sess.code_model()));

    // If skipping the PLT is enabled, we need to add some module metadata
    // to ensure intrinsic calls don't use it.
    if !sess.needs_plt() {
        let avoid_plt = "RtLibUseGOT\0".as_ptr().cast();
        llvm::LLVMCrabLangAddModuleFlag(llmod, llvm::LLVMModFlagBehavior::Warning, avoid_plt, 1);
    }
    ...
    if sess.is_sanitizer_kcfi_enabled() {
        ...
        llvm::LLVMCrabLangAddModuleFlag(llmod, llvm::LLVMModFlagBehavior::Override, kcfi, 1);
    }
        ...
        if sess.target.arch == "aarch64" {
            ...
            bug!(
                "branch-protection used on non-AArch64 target; \
                  this should be checked in crablangc_session."
            );
        }
    }
...
impl<'ll, 'tcx> CodegenCx<'ll, 'tcx> {
    pub(crate) fn new(
        ...
        //   files in CrabLang are not destined to become part of a dll as binaries
        ...
        // require adding a few attributes to CrabLang itself (feature gated at the
    ...
    pub(crate) fn create_used_variable_impl(&self, name: &'static CStr, values: &[&'ll Value]) {
        ...
        unsafe {
            ...
            llvm::LLVMCrabLangSetLinkage(g, llvm::Linkage::AppendingLinkage);
...
impl<'ll, 'tcx> MiscMethods<'tcx> for CodegenCx<'ll, 'tcx> {
    ...
    fn eh_personality(&self) -> &'ll Value {
        ...
        // `crablang_eh_personality` function, but rather we wired it up to the
    ...
    pub(crate) fn eh_catch_typeinfo(&self) -> &'ll Value {
        ...
        let eh_catch_typeinfo = match tcx.lang_items().eh_catch_typeinfo() {
            ...
            _ => {
                ...
                self.declare_global("crablang_eh_catch_typeinfo", ty)
            }
        };
