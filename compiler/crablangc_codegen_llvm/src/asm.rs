...
use crablangc_middle::{bug, span_bug, ty::Instance};
use crablangc_span::{Pos, Span};
                    ...
                    match operands[operand_idx] {
                        ...
                        GlobalAsmOperandRef::SymFn { instance } => {
                            ...
                            let symbol = llvm::build_string(|s| unsafe {
                                llvm::LLVMCrabLangGetMangledName(llval, s);
                            })
...
pub(crate) fn inline_asm_call<'ll>(
    ...
    unsafe {
        ...
        let constraints_ok = llvm::LLVMCrabLangInlineAsmVerify(fty, cons.as_ptr().cast(), cons.len());
        debug!("constraint verification result: {:?}", constraints_ok);
        if constraints_ok {
            let v = llvm::LLVMCrabLangInlineAsm(
                fty,
                asm.as_ptr().cast(),
                asm.len(),
                cons.as_ptr().cast(),
