...
use crablangc_codegen_ssa::base::{compare_simd_types, wants_msvc_seh};
use crablangc_codegen_ssa::common::{IntPredicate, TypeKind};
use crablangc_codegen_ssa::errors::{ExpectedPointerMutability, InvalidMonomorphization};
use crablangc_codegen_ssa::mir::operand::OperandRef;
use crablangc_codegen_ssa::mir::place::PlaceRef;
use crablangc_codegen_ssa::traits::*;
use crablangc_hir as hir;
use crablangc_middle::ty::layout::{FnAbiOf, HasTyCtxt, LayoutOf};
use crablangc_middle::ty::{self, Ty};
use crablangc_middle::{bug, span_bug};
use crablangc_span::{sym, symbol::kw, Span, Symbol};
use crablangc_target::abi::{self, Align, HasDataLayout, Primitive};
use crablangc_target::spec::{HasTargetSpec, PanicStrategy};
        ...
        let llval = match name {
            ...
            sym::raw_eq => {
                ...
                let use_integer_compare = match layout.abi() {
                    ...
                    Aggregate { .. } => {
                        // For crablangy ABIs, small aggregates are actually passed
...
// MSVC's definition of the `crablang_try` function.
//
...
fn codegen_msvc_try<'ll>(
    ...
    let (llty, llfn) = get_crablang_try_fn(bx, &mut |mut bx| {
        bx.set_personality_fn(bx.eh_personality());

        let normal = bx.append_sibling_block("normal");
        let catchswitch = bx.append_sibling_block("catchswitch");
        let catchpad_crablang = bx.append_sibling_block("catchpad_crablang");
        ...
        //
        //   declare i32 @crablang_try(%try_func, %data, %catch_func) {
        //      %slot = alloca i8*
        ...
        //
        //   catchswitch:
        //      %cs = catchswitch within none [%catchpad_crablang, %catchpad_foreign] unwind to caller
        //
        //   catchpad_crablang:
        ...
        //          crablang_panic(const crablang_panic&);
        //          ~crablang_panic();
        //
        //          void* x[2];
        //      };
        ...
        let cs = bx.catch_switch(None, None, &[catchpad_crablang, catchpad_foreign]);
        ...
        let type_name = bx.const_bytes(b"crablang_panic\0");
        let type_info =
            bx.const_struct(&[type_info_vtable, bx.const_null(bx.type_i8p()), type_name], false);
        let tydesc = bx.declare_global("__crablang_panic_type_info", bx.val_ty(type_info));
        unsafe {
            llvm::LLVMCrabLangSetLinkage(tydesc, llvm::Linkage::LinkOnceODRLinkage);
        ...
        bx.switch_to_block(catchpad_crablang);
...
// Definition of the standard `try` function for CrabLang using the GNU-like model
...
fn codegen_gnu_try<'ll>(
    ...
    let (llty, llfn) = get_crablang_try_fn(bx, &mut |mut bx| {
        ...
        // crablang_try ignores the selector.
...
// Variant of codegen_gnu_try used for emscripten where CrabLang panics are
// implemented using C++ exceptions. Here we use exceptions of a specific type
// (`struct crablang_panic`) to represent CrabLang panics.
fn codegen_emcc_try<'ll>(
    ...
    let (llty, llfn) = get_crablang_try_fn(bx, &mut |mut bx| {
        ...
        //      %crablang_typeid = @llvm.eh.typeid.for(@_ZTI10crablang_panic)
        //      %is_crablang_panic = %selector == %crablang_typeid
        //      %catch_data = alloca { i8*, i8 }
        //      %catch_data[0] = %ptr
        //      %catch_data[1] = %is_crablang_panic
        ...
        // Check if the typeid we got is the one for a CrabLang panic.
        let crablang_typeid = bx.call_intrinsic("llvm.eh.typeid.for", &[tydesc]);
        let is_crablang_panic = bx.icmp(IntPredicate::IntEQ, selector, crablang_typeid);
        let is_crablang_panic = bx.zext(is_crablang_panic, bx.type_bool());

        // We need to pass two values to catch_func (ptr and is_crablang_panic), so
        ...
        bx.store(is_crablang_panic, catch_data_1, i8_align);
...
fn gen_fn<'ll, 'tcx>(
    cx: &CodegenCx<'ll, 'tcx>,
    name: &str,
    crablang_fn_sig: ty::PolyFnSig<'tcx>,
    codegen: &mut dyn FnMut(Builder<'_, 'll, 'tcx>),
) -> (&'ll Type, &'ll Value) {
    let fn_abi = cx.fn_abi_of_fn_ptr(crablang_fn_sig, ty::List::empty());
    let llty = fn_abi.llvm_type(cx);
    let llfn = cx.declare_fn(name, fn_abi);
    cx.set_frame_pointer_type(llfn);
    cx.apply_target_cpu_attr(llfn);
    // FIXME(eddyb) find a nicer way to do this.
    unsafe { llvm::LLVMCrabLangSetLinkage(llfn, llvm::Linkage::InternalLinkage) };
...
// Helper function used to get a handle to the `__crablang_try` function used to
// catch exceptions.
//
// This function is only generated once and is then cached.
fn get_crablang_try_fn<'ll, 'tcx>(
    cx: &CodegenCx<'ll, 'tcx>,
    codegen: &mut dyn FnMut(Builder<'_, 'll, 'tcx>),
) -> (&'ll Type, &'ll Value) {
    if let Some(llfn) = cx.crablang_try_fn.get() {
        return llfn;
    }

    // Define the type up front for the signature of the crablang_try function.
    let tcx = cx.tcx;
    ...
    let crablang_fn_sig = ty::Binder::dummy(cx.tcx.mk_fn_sig(
    ...
    let crablang_try = gen_fn(cx, "__crablang_try", crablang_fn_sig, codegen);
    cx.crablang_try_fn.set(Some(crablang_try));
    crablang_try
}
