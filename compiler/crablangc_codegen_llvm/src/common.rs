...
use crablangc_session::cstore::{DllCallingConvention, DllImport, PeImportNameType};
use crablangc_target::abi::{self, AddressSpace, HasDataLayout, Pointer, Size};
...
* C code found in a C library, or crablang code found in a crablang crate.
*
* Most "externs" are implicitly declared (automatically) as a result of a
* user declaring an extern _module_ dependency; this causes the crablang driver
* to locate an extern crate, scan its compilation metadata, and emit extern
* declarations for any symbols used by the declaring crate.
*
* A "foreign" is an extern that references C (or other non-crablang ABI) code.
* There is no metadata to scan for extern references so in these cases either
* a header-digester like bindgen, or manual function prototypes, have to
* serve as declarators. So these are usually given explicitly as prototype
* declarations, in crablang code, with ABI attributes on them noting which ABI to
* link via.
*
...
impl<'ll, 'tcx> ConstMethods<'tcx> for CodegenCx<'ll, 'tcx> {
        ...
        let str_global = *self
            ...
            .or_insert_with(|| {
                ...
                unsafe {
                    ...
                    llvm::LLVMCrabLangSetLinkage(g, llvm::Linkage::InternalLinkage);
                }
                (s.to_owned(), g)
            })
            .1;
    ...
    fn const_to_opt_uint(&self, v: &'ll Value) -> Option<u64> {
        try_as_const_integral(v).and_then(|v| unsafe {
            let mut i = 0u64;
            let success = llvm::LLVMCrabLangConstIntGetZExtValue(v, &mut i);
            success.then_some(i)
        })
    }
    ...
    fn const_to_opt_u128(&self, v: &'ll Value, sign_ext: bool) -> Option<u128> {
        try_as_const_integral(v).and_then(|v| unsafe {
            ...
            let success = llvm::LLVMCrabLangConstInt128Get(v, sign_ext, &mut hi, &mut lo);
    ...
    fn scalar_to_backend(&self, cv: Scalar, layout: abi::Scalar, llty: &'ll Type) -> &'ll Value {
        ...
        match cv {
            ...
            Scalar::Ptr(ptr, _size) => {
                ...
                let llval = unsafe {
                    llvm::LLVMCrabLangConstInBoundsGEP2(
                        self.type_i8(),
