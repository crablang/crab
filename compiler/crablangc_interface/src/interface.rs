...
use crablangc_ast::{self as ast, LitKind, MetaItemKind};
...
use crablangc_data_structures::OnDrop;
...
use crablangc_query_impl::QueryCtxt;
use crablangc_session::config::{self, CheckCfg, ErrorOutputType, Input, OutputFilenames};
use crablangc_session::lint;
use crablangc_session::parse::{CrateConfig, ParseSess};
use crablangc_session::Session;
use crablangc_session::{early_error, CompilerIO};
...
///
/// Can be used to run `crablangc_interface` queries.
...
pub fn parse_cfgspecs(cfgspecs: Vec<String>) -> FxHashSet<(String, Option<String>)> {
    crablangc_span::create_default_session_if_not_set_then(move |_| {
        let cfg = cfgspecs
            .into_iter()
            .map(|s| {
...
pub fn run_compiler<R: Send>(config: Config, f: impl FnOnce(&Compiler) -> R + Send) -> R {
    ...
    util::run_in_thread_pool_with_globals(
        ...
        || {
            ...
            crablangc_span::set_source_map(compiler.sess.parse_sess.clone_source_map(), move || {
                let r = {
