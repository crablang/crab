...
use crablangc_errors::{emitter::HumanReadableErrorType, registry, ColorConfig};
use crablangc_session::config::crablangc_optgroups;
use crablangc_session::config::DebugInfo;
use crablangc_session::config::Input;
use crablangc_session::config::InstrumentXRay;
use crablangc_session::config::TraitSolver;
use crablangc_session::config::{build_configuration, build_session_options, to_crate_config};
use crablangc_session::config::{
    BranchProtection, Externs, OomStrategy, OutputType, OutputTypes, PAuthKey, PacRet,
    ProcMacroExecutionStrategy, SymbolManglingVersion, WasiExecModel,
};
use crablangc_session::config::{CFGuard, ExternEntry, LinkerPluginLto, LtoCli, SwitchWithOptPath};
use crablangc_session::config::{DumpMonoStatsFormat, MirSpanview};
use crablangc_session::config::{ErrorOutputType, ExternLocation, LocationDetail, Options, Strip};
use crablangc_session::config::{InstrumentCoverage, Passes};
use crablangc_session::lint::Level;
use crablangc_session::search_paths::SearchPath;
use crablangc_session::utils::{CanonicalizedPath, NativeLib, NativeLibKind};
use crablangc_session::CompilerIO;
use crablangc_session::{build_session, getopts, Session};
...
use crablangc_target::spec::{CodeModel, LinkerFlavorCli, MergeFunctions, PanicStrategy, RelocModel};
use crablangc_target::spec::{RelroLevel, SanitizerSet, SplitDebuginfo, StackProtector, TlsModel};
...
fn optgroups() -> getopts::Options {
    ...
    for group in crablangc_optgroups() {
...
fn test_switch_implies_cfg_test() {
    crablangc_span::create_default_session_globals_then(|| {
...
fn test_top_level_options_tracked_no_crate() {
    ...
    tracked!(
        ...
        Some("/home/bors/crablang/.crablangup/toolchains/nightly/lib/crablanglib/src/crablang".into())
    );
    tracked!(remap_path_prefix, vec![("/home/bors/crablang".into(), "src".into())]);
    // tidy-alphabetical-end
}
...
fn test_unstable_options_tracking_hash() {
    ...
    tracked!(simulate_remapped_crablang_src_base, Some(PathBuf::from("/crablangc/abc")));
