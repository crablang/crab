...
use crablangc_parse::validate_attr;
use crablangc_session as session;
use crablangc_session::config::CheckCfg;
use crablangc_session::config::{self, CrateType};
use crablangc_session::config::{ErrorOutputType, OutputFilenames};
use crablangc_session::filesearch::sysroot_candidates;
use crablangc_session::lint::{self, BuiltinLintDiagnostics, LintBuffer};
use crablangc_session::parse::CrateConfig;
use crablangc_session::{early_error, filesearch, output, Session};
use crablangc_span::edit_distance::find_best_match_for_name;
use crablangc_span::edition::Edition;
use crablangc_span::source_map::FileLoader;
...
pub fn create_session(
    ...
    let bundle = match crablangc_errors::fluent_bundle(
...
fn get_stack_size() -> Option<usize> {
    ...
    env::var_os("CRABLANG_MIN_STACK").is_none().then_some(STACK_SIZE)
}
...
pub(crate) fn run_in_thread_pool_with_globals<F: FnOnce() -> R + Send, R: Send>(
    ...
    let mut builder = thread::Builder::new().name("crablangc".to_string());
    ...
    thread::scope(|s| {
        ...
        let r = builder
            .spawn_scoped(s, move || crablangc_span::create_session_globals_then(edition, f))
            .unwrap()
            .join();
...
pub(crate) fn run_in_thread_pool_with_globals<F: FnOnce() -> R + Send, R: Send>(
    ...
    use crablangc_data_structures::jobserver;
    use crablangc_middle::ty::tls;
    use crablangc_query_impl::{deadlock, QueryContext, QueryCtxt};

    let mut builder = rayon::ThreadPoolBuilder::new()
        .thread_name(|_| "crablangc".to_string())
    ...
    crablangc_span::create_session_globals_then(edition, || {
        crablangc_span::with_session_globals(|session_globals| {
            builder
                .build_scoped(
                    // Initialize each new worker thread when created.
                    move |thread: rayon::ThreadBuilder| {
                        crablangc_span::set_session_globals_then(session_globals, || thread.run())
                    },
...
fn load_backend_from_dylib(path: &Path) -> MakeBackendFn {
    ...
    let backend_sym = unsafe { lib.get::<MakeBackendFn>(b"__crablangc_codegen_backend") }
        .unwrap_or_else(|e| {
...
pub fn get_codegen_backend(
    ...
    let load = LOAD.get_or_init(|| {
        ...
        match backend_name.unwrap_or(default_codegen_backend) {
            ...
            #[cfg(feature = "llvm")]
            "llvm" => crablangc_codegen_llvm::LlvmCodegenBackend::new,
    ...
    // backend we hope that the backend links against the same crablangc_driver version. If this is not
    // the case, we get UB.
    unsafe { load() }
}

// This is used for crablangdoc, but it uses similar machinery to codegen backend
// loading, so we leave the code here. It is potentially useful for other tools
// that want to invoke the crablangc binary while linking to crablangc as well.
pub fn crablangc_path<'a>() -> Option<&'a Path> {
    static CRABLANGC_PATH: OnceLock<Option<PathBuf>> = OnceLock::new();

    const BIN_PATH: &str = env!("CRABLANGC_INSTALL_BINDIR");

    CRABLANGC_PATH.get_or_init(|| get_crablangc_path_inner(BIN_PATH)).as_deref()
}

fn get_crablangc_path_inner(bin_path: &str) -> Option<PathBuf> {
...
fn get_codegen_sysroot(maybe_sysroot: &Option<PathBuf>, backend_name: &str) -> MakeBackendFn {
    ...
    let expected_names = &[
        format!("crablangc_codegen_{}-{}", backend_name, env!("CFG_RELEASE")),
        format!("crablangc_codegen_{backend_name}"),
    ];
    ...
    match sess.io.output_file {
        None => {
            ...
            let stem = sess
                ...
                .crate_name
                .clone()
                .or_else(|| crablangc_attr::find_crate_name(attrs).map(|n| n.to_string()))
...
/// Returns the version string for `crablangc` itself (which may be different from a tool version).
pub fn crablangc_version_str() -> Option<&'static str> {
    version_str!()
}
