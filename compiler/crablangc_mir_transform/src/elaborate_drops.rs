...
use crablangc_mir_dataflow::elaborate_drops::{elaborate_drop, DropFlagState, Unwind};
use crablangc_mir_dataflow::elaborate_drops::{DropElaborator, DropFlagMode, DropStyle};
use crablangc_mir_dataflow::impls::{MaybeInitializedPlaces, MaybeUninitializedPlaces};
use crablangc_mir_dataflow::move_paths::{LookupResult, MoveData, MovePathIndex};
use crablangc_mir_dataflow::on_lookup_result_bits;
use crablangc_mir_dataflow::un_derefer::UnDerefer;
use crablangc_mir_dataflow::MoveDataParamEnv;
use crablangc_mir_dataflow::{on_all_children_bits, on_all_drop_children_bits};
use crablangc_mir_dataflow::{Analysis, ResultsCursor};
use crablangc_span::{DesugaringKind, Span};
...
impl<'a, 'tcx> DropElaborator<'a, 'tcx> for Elaborator<'a, '_, 'tcx> {
    ...
    fn field_subpath(&self, path: Self::Path, field: FieldIdx) -> Option<Self::Path> {
        crablangc_mir_dataflow::move_path_children_matching(self.ctxt.move_data(), path, |e| match e {
    ...
    fn deref_subpath(&self, path: Self::Path) -> Option<Self::Path> {
        crablangc_mir_dataflow::move_path_children_matching(self.ctxt.move_data(), path, |e| {
...
impl<'b, 'tcx> ElaborateDropsCtxt<'b, 'tcx> {
    ...
    fn drop_flags_for_args(&mut self) {
        let loc = Location::START;
        crablangc_mir_dataflow::drop_flag_effects_for_function_entry(
            self.tcx,
            self.body,
            self.env,
            |path, ds| {
    ...
    fn drop_flags_for_locs(&mut self) {
        ...
        for (bb, data) in self.body.basic_blocks.iter_enumerated() {
            ...
            for i in 0..(data.statements.len() + 1) {
                ...
                crablangc_mir_dataflow::drop_flag_effects_for_location(
                    self.tcx,
                    self.body,
                    self.env,
                    loc,
