...
//! # Motivation
//!
//! MIR building can insert a lot of redundant copies, and CrabLang code in general often tends to move
...
//! <https://github.com/crablang/crablang/issues/32966>), so this leaves some performance on the table
//! that we can regain by implementing an optimization for removing these assign statements in crablangc
//! itself. When this optimization runs fast enough, it can also speed up the constant evaluation
//! and code generation phases of crablangc due to the reduced number of statements and locals.
//!
//! # The Optimization
//!
...
//! - crablangc now has a powerful dataflow analysis framework that can handle forwards and backwards
...
//!
//! Also, crablangc now has a simple NRVO pass (see `nrvo.rs`), which handles a subset of the cases that
...
//! [attempt 1]: https://github.com/crablang/crablang/pull/47954
//! [attempt 2]: https://github.com/crablang/crablang/pull/71003
//! [attempt 3]: https://github.com/crablang/crablang/pull/72632
...
use crablangc_middle::mir::visit::{MutVisitor, PlaceContext, Visitor};
use crablangc_middle::mir::{dump_mir, PassWhere};
...
use crablangc_mir_dataflow::impls::MaybeLiveLocals;
use crablangc_mir_dataflow::{Analysis, ResultsCursor};
...
impl<'tcx> MirPass<'tcx> for DestinationPropagation {
    ...
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        let borrowed = crablangc_mir_dataflow::impls::borrowed_locals(body);
        ...
        // expect to see MIR like that. To verify this, a test was run against `[crablang/regex]` -
        ...
        //
        // [crablang/regex]:
        //     https://github.com/crablang/regex/tree/b5372864e2df6a2f5e543a556a62197f50ca3650
