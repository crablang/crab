use crate::crablangc_middle::ty::util::IntTypeExt;
use crate::MirPass;
use crablangc_data_structures::fx::FxHashMap;
use crablangc_middle::mir::interpret::AllocId;
use crablangc_middle::mir::*;
use crablangc_middle::ty::{self, AdtDef, ParamEnv, Ty, TyCtxt};
use crablangc_session::Session;
use crablangc_target::abi::{HasDataLayout, Size, TagEncoding, Variants};
...
impl EnumSizeOpt {
    fn candidate<'tcx>(
        ...
        macro_rules! encode_store {
            ($curr_idx: expr, $endian: expr, $bytes: expr) => {
                let bytes = match $endian {
                    crablangc_target::abi::Endian::Little => $bytes.to_le_bytes(),
                    crablangc_target::abi::Endian::Big => $bytes.to_be_bytes(),
                };
        ...
        for (var_idx, layout) in variants.iter_enumerated() {
            ...
            let sz = layout.size;
            match ptr_sized_int {
                crablangc_target::abi::Integer::I32 => {
                    encode_store!(curr_idx, data_layout.endian, sz.bytes() as u32);
                }
                crablangc_target::abi::Integer::I64 => {
