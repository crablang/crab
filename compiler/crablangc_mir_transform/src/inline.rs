...
use crablangc_middle::ty::{self, Instance, InstanceDef, ParamEnv, Ty, TyCtxt};
use crablangc_session::config::OptLevel;
use crablangc_span::{hygiene::ExpnKind, ExpnData, LocalExpnId, Span};
...
impl<'tcx> Inliner<'tcx> {
    ...
    fn try_inlining(
        ...
        if callsite.fn_sig.abi() == Abi::CrabLangCall {
            let (arg_tuple, skipped_args) = match &args[..] {
                [arg_tuple] => (arg_tuple, 0),
                [_, arg_tuple] => (arg_tuple, 1),
                _ => bug!("Expected `crablang-call` to have 1 or 2 args"),
            };
    ...
    fn make_call_args(
        ...
        // as distinct arguments. (This is the "crablang-call" ABI hack.) Normally, codegen has
        ...
        if callsite.fn_sig.abi() == Abi::CrabLangCall && callee_body.spread_arg.is_none() {
