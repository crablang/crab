...
use crablangc_const_eval::interpret::{ConstValue, ImmTy, Immediate, InterpCx, Scalar};
use crablangc_data_structures::fx::FxHashMap;
use crablangc_hir::def::DefKind;
use crablangc_middle::mir::visit::{MutVisitor, Visitor};
use crablangc_middle::mir::*;
use crablangc_middle::ty::layout::TyAndLayout;
use crablangc_middle::ty::{self, Ty, TyCtxt};
use crablangc_mir_dataflow::value_analysis::{Map, State, TrackElem, ValueAnalysis, ValueOrPlace};
use crablangc_mir_dataflow::{lattice::FlatSet, Analysis, ResultsVisitor, SwitchIntEdgeEffects};
use crablangc_span::DUMMY_SP;
use crablangc_target::abi::{Align, FieldIdx, VariantIdx};
...
impl<'mir, 'tcx> crablangc_const_eval::interpret::Machine<'mir, 'tcx> for DummyMachine {
    crablangc_const_eval::interpret::compile_time_machine!(<'mir, 'tcx>);
    type MemoryKind = !;
    ...
    fn find_mir_or_eval_fn(
        ...
        _abi: crablangc_target::spec::abi::Abi,
        _args: &[crablangc_const_eval::interpret::OpTy<'tcx, Self::Provenance>],
        _destination: &crablangc_const_eval::interpret::PlaceTy<'tcx, Self::Provenance>,
    ...
    fn assert_panic(
        _ecx: &mut InterpCx<'mir, 'tcx, Self>,
        _msg: &crablangc_middle::mir::AssertMessage<'tcx>,
        _unwind: UnwindAction,
    ) -> interpret::InterpResult<'tcx> {
        unimplemented!()
    }
    ...
    fn binary_ptr_op(
        _ecx: &InterpCx<'mir, 'tcx, Self>,
        _bin_op: BinOp,
        _left: &crablangc_const_eval::interpret::ImmTy<'tcx, Self::Provenance>,
        _right: &crablangc_const_eval::interpret::ImmTy<'tcx, Self::Provenance>,
    ...
    fn init_frame_extra(
        _ecx: &mut InterpCx<'mir, 'tcx, Self>,
        _frame: crablangc_const_eval::interpret::Frame<'mir, 'tcx, Self::Provenance>,
    ) -> interpret::InterpResult<
        'tcx,
        crablangc_const_eval::interpret::Frame<'mir, 'tcx, Self::Provenance, Self::FrameExtra>,
    > {
        unimplemented!()
    }
    ...
    ) -> &'a [crablangc_const_eval::interpret::Frame<'mir, 'tcx, Self::Provenance, Self::FrameExtra>]
    {
        unimplemented!()
    }
