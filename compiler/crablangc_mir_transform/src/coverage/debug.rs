...
//!
//! To enable coverage, include the crablangc command line option:
//!
...
//!
//! ```shell
//! CRABLANGC_LOG=crablangc_mir_transform::transform::coverage=debug
//! ```
//!
//! Other module paths with coverage-related debug logs may also be of interest, particularly for
//! debugging the coverage map data, injected as global variables in the LLVM IR (during crablangc's
//! code generation pass). For example:
//!
//! ```shell
//! CRABLANGC_LOG=crablangc_mir_transform::transform::coverage,crablangc_codegen_ssa::coverageinfo,crablangc_codegen_llvm::coverageinfo=debug
//! ```
//!
...
//!
//! ```shell
//! CRABLANGC_COVERAGE_DEBUG_OPTIONS=<options>
//! ```
//!
//! These options are comma-separated, and specified in the format `option-name=value`. For example:
//!
//! ```shell
//! $ CRABLANGC_COVERAGE_DEBUG_OPTIONS=counter-format=id+operation,allow-unused-expressions=yes cargo build
//! ```
//!
...
//!     and expressions.
//!
//!     When this kind of bug is encountered, the crablangc compiler will panic by default. Setting:
...
use crablangc_middle::mir::create_dump_file;
use crablangc_middle::mir::generic_graphviz::GraphvizWriter;
use crablangc_middle::mir::spanview::{self, SpanViewable};

use crablangc_data_structures::fx::FxHashMap;
use crablangc_middle::mir::coverage::*;
use crablangc_middle::mir::{self, BasicBlock, TerminatorKind};
...
const CRABLANGC_COVERAGE_DEBUG_OPTIONS: &str = "CRABLANGC_COVERAGE_DEBUG_OPTIONS";
...
/// "CRABLANGC_COVERAGE_DEBUG_OPTIONS", if set.
#[derive(Debug, Clone)]
...
impl DebugOptions {
    fn from_env() -> Self {
        ...
        if let Ok(env_debug_options) = std::env::var(CRABLANGC_COVERAGE_DEBUG_OPTIONS) {
                ...
                match option {
                    "allow_unused_expressions" => {
                        ...
                        debug!(
                            ...
                            CRABLANGC_COVERAGE_DEBUG_OPTIONS, allow_unused_expressions
                        );
                    }
                    "counter_format" => {
                        match value {
                            None => {
                                bug!(
                                    ...
                                    `id+block+operation`)",
                                    option,
                                    CRABLANGC_COVERAGE_DEBUG_OPTIONS
                                );
                            }
                            Some(val) => {
                                ...
                                debug!(
                                    ...
                                    CRABLANGC_COVERAGE_DEBUG_OPTIONS, counter_format
                                );
                            }
                        };
                    }
