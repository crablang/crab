...
//!
//! ```shell
//! ./x.py test --keep-stage 1 compiler/crablangc_mir --test-args '--show-output coverage'
//! ```
//!
...
//! limitations) and deep dependencies on other elements of the full CrabLang compiler (which is
...
//! to: `crablangc_span::create_default_session_globals_then(|| { test_here(); })`.
...
use crablangc_data_structures::graph::WithNumNodes;
use crablangc_data_structures::graph::WithSuccessors;
use crablangc_index::vec::{Idx, IndexVec};
use crablangc_middle::mir::coverage::CoverageKind;
use crablangc_middle::mir::*;
use crablangc_middle::ty;
use crablangc_span::{self, BytePos, Pos, Span, DUMMY_SP};
