...
impl SubdiagnosticDeriveBuilder {
    ...
    pub(crate) fn into_tokens(self, mut structure: Structure<'_>) -> TokenStream {
        ...
        let ret = structure.gen_impl(quote! {
            gen impl crablangc_errors::AddToDiagnostic for @Self {
                fn add_to_diagnostic_with<__F>(self, #diag: &mut crablangc_errors::Diagnostic, #f: __F)
                where
                    __F: core::ops::Fn(
                ...
                {
                    use crablangc_errors::{Applicability, IntoDiagnosticArg};
                    #implementation
                }
            }
        });
