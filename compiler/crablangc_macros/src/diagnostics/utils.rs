...
pub(crate) fn report_error_if_not_applied_to_applicability(
    ...
    report_error_if_not_applied_to_ty(
        attr,
        info,
        &["crablangc_errors", "Applicability"],
        "`Applicability`",
    )
}
...
pub(crate) fn report_error_if_not_applied_to_span(
    ...
    if !type_matches_path(info.ty.inner_type(), &["crablangc_span", "Span"])
        && !type_matches_path(info.ty.inner_type(), &["crablangc_errors", "MultiSpan"])
    {
...
/// `Applicability` of a suggestion - mirrors `crablangc_errors::Applicability` - and used to represent
    ...
    fn to_tokens(&self, tokens: &mut TokenStream) {
        tokens.extend(match self {
            Applicability::MachineApplicable => {
                quote! { crablangc_errors::Applicability::MachineApplicable }
            }
            Applicability::MaybeIncorrect => {
                quote! { crablangc_errors::Applicability::MaybeIncorrect }
            }
            Applicability::HasPlaceholders => {
                quote! { crablangc_errors::Applicability::HasPlaceholders }
            }
            Applicability::Unspecified => {
                quote! { crablangc_errors::Applicability::Unspecified }
            }
        });
    }
}
    ...
    pub fn to_suggestion_style(&self) -> TokenStream {
        match self {
            SuggestionKind::Normal => {
                quote! { crablangc_errors::SuggestionStyle::ShowCode }
            }
            SuggestionKind::Short => {
                quote! { crablangc_errors::SuggestionStyle::HideCodeInline }
            }
            SuggestionKind::Hidden => {
                quote! { crablangc_errors::SuggestionStyle::HideCodeAlways }
            }
            SuggestionKind::Verbose => {
                quote! { crablangc_errors::SuggestionStyle::ShowAlways }
            }
            SuggestionKind::ToolOnly => {
                quote! { crablangc_errors::SuggestionStyle::CompletelyHidden }
            }
        }
    }
