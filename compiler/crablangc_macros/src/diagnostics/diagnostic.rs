    ...
    pub(crate) fn into_tokens(self) -> TokenStream {
        ...
        structure.gen_impl(quote! {
            gen impl<'__diagnostic_handler_sess, G>
                    crablangc_errors::IntoDiagnostic<'__diagnostic_handler_sess, G>
                    for @Self
                where G: crablangc_errors::EmissionGuarantee
            {

                #[track_caller]
                fn into_diagnostic(
                    self,
                    #handler: &'__diagnostic_handler_sess crablangc_errors::Handler
                ) -> crablangc_errors::DiagnosticBuilder<'__diagnostic_handler_sess, G> {
                    use crablangc_errors::IntoDiagnosticArg;
                    #implementation
                }
            }
        })
...
impl<'a> LintDiagnosticDerive<'a> {
    ...
    pub(crate) fn into_tokens(self) -> TokenStream {
        ...
        structure.gen_impl(quote! {
            gen impl<'__a> crablangc_errors::DecorateLint<'__a, ()> for @Self {
                #[track_caller]
                fn decorate_lint<'__b>(
                    self,
                    #diag: &'__b mut crablangc_errors::DiagnosticBuilder<'__a, ()>
                ) -> &'__b mut crablangc_errors::DiagnosticBuilder<'__a, ()> {
                    use crablangc_errors::IntoDiagnosticArg;
                    #implementation
                }

                fn msg(&self) -> crablangc_errors::DiagnosticMessage {
                    #msg
                }
            }
        })
...
impl Mismatch {
    ...
    fn check(slug: &syn::Path) -> Option<Mismatch> {
        ...
        // If we're not in a "crablangc_" crate, bail.
        let Some(("crablangc", slug_prefix)) = crate_name.split_once('_') else { return None };
