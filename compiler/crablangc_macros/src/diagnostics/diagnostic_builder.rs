...
impl<'a> DiagnosticDeriveVariantBuilder<'a> {
        ...
        if name == "diag" {
            ...
            attr.parse_nested_meta(|nested| {
                ...
                if path.is_ident("code") {
                    ...
                    tokens.extend(quote! {
                        #diag.code(crablangc_errors::DiagnosticId::Error(#code.to_string()));
                    });
                } else {
    ...
    fn generate_inner_field_code(
        ...
        match subdiag {
            ...
            SubdiagnosticKind::Note | SubdiagnosticKind::Help | SubdiagnosticKind::Warn => {
                ...
                if type_matches_path(inner, &["crablangc_span", "Span"])
                    || type_matches_path(inner, &["crablangc_span", "MultiSpan"])
                {
                    Ok(self.add_spanned_subdiagnostic(binding, &fn_ident, slug))
            ...
            SubdiagnosticKind::Suggestion {
                ...
                let applicability = applicability
                    .value()
                    .unwrap_or_else(|| quote! { crablangc_errors::Applicability::Unspecified });
    ...
    fn span_and_applicability_of_ty(
        ...
        match &info.ty.inner_type() {
            ...
            ty @ Type::Path(..) if type_matches_path(ty, &["crablangc_span", "Span"]) => {
            ...
            Type::Tuple(tup) => {
                ...
                for (idx, elem) in tup.elems.iter().enumerate() {
                    if type_matches_path(elem, &["crablangc_span", "Span"]) {
                        span_idx.set_once(syn::Index::from(idx), elem.span().unwrap());
                    } else if type_matches_path(elem, &["crablangc_errors", "Applicability"]) {
