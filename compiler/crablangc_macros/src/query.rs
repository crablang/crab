...
fn add_query_desc_cached_impl(
    ...
    let cache = if let Some((args, expr)) = modifiers.cache.as_ref() {
        ...
        // we're taking `key` by reference, but some crablangc types usually prefer being passed by value
        quote! {
            #[allow(unused_variables, unused_braces, crablangc::pass_by_value)]
            #[inline]
    ...
    let desc = quote! {
        ...
        pub fn #name<'tcx>(tcx: TyCtxt<'tcx>, key: crate::ty::query::query_keys::#name<'tcx>) -> String {
            let (#tcx, #key) = (tcx, key);
            ::crablangc_middle::ty::print::with_no_trimmed_paths!(
                format!(#desc)
            )
        }
    };
...
pub fn crablangc_queries(input: TokenStream) -> TokenStream {
    ...
    for query in queries.0 {
        ...
        // at the entire `crablangc_queries!` invocation, which wouldn't
        // be very useful.
        let span = name.span();
    ...
    TokenStream::from(quote! {
        #[macro_export]
        macro_rules! crablangc_query_append {
        ...
        macro_rules! crablangc_feedable_queries {
            ( $macro:ident! ) => {
