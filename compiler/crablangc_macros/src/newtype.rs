...
impl Parse for Newtype {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        ...
        let encodable_impls = if encodable {
            quote! {
                impl<D: ::crablangc_serialize::Decoder> ::crablangc_serialize::Decodable<D> for #name {
                    fn decode(d: &mut D) -> Self {
                        Self::from_u32(d.read_u32())
                    }
                }
                impl<E: ::crablangc_serialize::Encoder> ::crablangc_serialize::Encodable<E> for #name {
        ...
        let step = if ord {
            quote! {
                ...
                unsafe impl ::std::iter::TcrablangedStep for #name {}
            }
        } else {
            quote! {}
        };
        ...
        Ok(Self(quote! {
            ...
            #[crablangc_layout_scalar_valid_range_end(#max)]
            ...
            impl crablangc_index::vec::Idx for #name {
                #[inline]
