...
//!
//! ```bash
//! cd compiler/crablangc_macros
//! cargo test symbols::test_symbols -- --nocapture
//! ```
//!
//! This unit test finds the `symbols!` invocation in `compiler/crablangc_span/src/symbol.rs`
...
//! cd compiler/crablangc_span
//! cargo expand > /tmp/crablangc_span.rs   # it's a big file
//! ```
...
fn symbols_with_errors(input: TokenStream) -> (TokenStream, Vec<syn::Error>) {
    ...
    //
    // Then, cd to crablangc_span and run:
    //      cargo expand > /tmp/crablangc_span_expanded.rs
    //
    // and read that file.
}
