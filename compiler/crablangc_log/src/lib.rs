//! This crate allows tools to enable crablang logging without having to magically
//! match crablangc's tracing crate version.
//!
//! For example if someone is working on crablangc_ast and wants to write some
//! minimal code against it to run in a debugger, with access to the `debug!`
//! logs emitted by crablangc_ast, that can be done by writing:
//!
//! ```toml
//! [dependencies]
//! crablangc_ast = { path = "../crablang/compiler/crablangc_ast" }
//! crablangc_log = { path = "../crablang/compiler/crablangc_log" }
//! crablangc_span = { path = "../crablang/compiler/crablangc_span" }
//! ```
//!
//! ```
//! fn main() {
//!     crablangc_log::init_env_logger("LOG").unwrap();
//!
//!     let edition = crablangc_span::edition::Edition::Edition2021;
...
//! crablangc's debug logging. In a workflow like this, one might also add
//! `std::env::set_var("LOG", "debug")` to the top of main so that `cargo
//! run` by itself is sufficient to get logs.
//!
//! The reason crablangc_log is a tiny separate crate, as opposed to exposing the
//! same things in crablangc_driver only, is to enable the above workflow. If you
//! had to depend on crablangc_driver in order to turn on crablangc's debug logs, that's
//! an enormously bigger dependency tree; every change you make to crablangc_ast (or
//! whichever piece of the compiler you are interested in) would involve
//! rebuilding all the rest of crablangc up to crablangc_driver in order to run your
//! main.rs. Whereas by depending only on crablangc_log and the few crates you are
