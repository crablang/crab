...
use crablangc_ast::ast::{InlineAsmOptions, InlineAsmTemplatePiece};
use crablangc_codegen_ssa::mir::operand::OperandValue;
use crablangc_codegen_ssa::mir::place::PlaceRef;
use crablangc_codegen_ssa::traits::{AsmBuilderMethods, AsmMethods, BaseTypeMethods, BuilderMethods, GlobalAsmOperandRef, InlineAsmOperandRef};

use crablangc_middle::{bug, ty::Instance};
use crablangc_span::Span;
use crablangc_target::asm::*;
...
// CrabLang asm! and GCC Extended Asm semantics differ substantially.
//
// 1. CrabLang asm operands go along as one list of operands. Operands themselves indicate
...
//    cannot interleave.
//
// 2. Operand lists in both CrabLang and GCC are indexed. Index starts from 0. Indexes are important
//    because the asm template refers to operands by index.
//
//    Mapping from CrabLang to GCC index would be 1-1 if it wasn't for...
//
// 3. Clobbers. GCC has a separate list of clobbers, and clobbers don't have indexes.
//    Contrary, CrabLang expresses clobbers through "out" operands that aren't tied to
...
//
// With that in mind, let's see how we translate CrabLang syntax to GCC
...
impl<'a, 'gcc, 'tcx> AsmBuilderMethods<'tcx> for Builder<'a, 'gcc, 'tcx> {
    fn codegen_inline_asm(&mut self, template: &[InlineAsmTemplatePiece], crablang_operands: &[InlineAsmOperandRef<'tcx, Self>], options: InlineAsmOptions, span: &[Span], _instance: Instance<'_>, _dest_catch_funclet: Option<(Self::BasicBlock, Self::BasicBlock, Option<&Self::Funclet>)>) {
        ...
        for (crablang_idx, op) in crablang_operands.iter().enumerate() {
            match *op {
                ...
                InlineAsmOperandRef::InOut { reg, late, in_value, out_place } => {
                    ...
                    // CrabLangc frontend guarantees that input and output types are "compatible",
            ...
            match *piece {
                InlineAsmTemplatePiece::String(ref string) => {
                    for char in string.chars() {
                        // TODO(antoyo): might also need to escape | if crablangc doesn't do it.
                        let escaped_char =
                            match char {
                                '%' => "%%",
                                '{' => "%{",
                ...
                InlineAsmTemplatePiece::Placeholder { operand_idx, modifier, span: _ } => {
                    ...
                    match crablang_operands[operand_idx] {
                        InlineAsmOperandRef::Out { reg, ..  } => {
                            let modifier = modifier_to_gcc(asm_arch, reg.reg_class(), modifier);
                            let gcc_index = outputs.iter()
                                .position(|op| operand_idx == op.crablang_idx)
                                .expect("wrong crablang index");
        ...
        //     (especially with current `crablangc_backend_ssa` API).
            ...
            match reg {
                InlineAsmReg::X86(_) => {
                    ...
                    return ConstraintOrRegister::Register(match reg.name() {
                        // Some of registers' names does not map 1-1 from crablang to gcc
                        "st(0)" => "st",
