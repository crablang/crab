...
use crablangc_codegen_ssa::traits::{BackendTypes, BaseTypeMethods, BuilderMethods, OverflowOp};
...
impl<'a, 'gcc, 'tcx> Builder<'a, 'gcc, 'tcx> {
    ...
    pub fn gcc_lshr(&mut self, a: RValue<'gcc>, b: RValue<'gcc>) -> RValue<'gcc> {
        ...
        if a_native && b_native {
            ...
            // NOTE: since jumps were added in a place crablangc does not expect, the current block in the
        ...
        if self.is_native_int_type_or_bool(a_type) && self.is_native_int_type_or_bool(b_type) {
            ...
            let func_name =
                match (operation, signed) {
                    (BinaryOp::Plus, true) => "__crablang_i128_add",
                    (BinaryOp::Plus, false) => "__crablang_u128_add",
                    (BinaryOp::Minus, true) => "__crablang_i128_sub",
                    (BinaryOp::Minus, false) => "__crablang_u128_sub",
    ...
    pub fn gcc_checked_binop(&self, oop: OverflowOp, typ: Ty<'_>, lhs: <Self as BackendTypes>::Value, rhs: <Self as BackendTypes>::Value) -> (<Self as BackendTypes>::Value, <Self as BackendTypes>::Value) {
        use crablangc_middle::ty::{Int, IntTy::*, Uint, UintTy::*};
                    ...
                    Int(I128) | Uint(U128) => {
                        let func_name =
                            match oop {
                                OverflowOp::Add =>
                                    match new_kind {
                                        Int(I128) => "__crablang_i128_addo",
                                        Uint(U128) => "__crablang_u128_addo",
                                        _ => unreachable!(),
                                    },
                                OverflowOp::Sub =>
                                    match new_kind {
                                        Int(I128) => "__crablang_i128_subo",
                                        Uint(U128) => "__crablang_u128_subo",
                                        _ => unreachable!(),
                                    },
                                OverflowOp::Mul =>
                                    match new_kind {
                                        Int(I128) => "__crablang_i128_mulo", // TODO(antoyo): use __muloti4d instead?
                                        Uint(U128) => "__crablang_u128_mulo",
                                        _ => unreachable!(),
                                    },
                            };
    ...
    pub fn gcc_icmp(&self, op: IntPredicate, mut lhs: RValue<'gcc>, mut rhs: RValue<'gcc>) -> RValue<'gcc> {
        ...
        if self.is_non_native_int_type(a_type) || self.is_non_native_int_type(b_type) {
            ...
            // NOTE: gcc cannot compare pointers to different objects, but crablangc does that, so cast them to usize.
...
impl<'gcc, 'tcx> CodegenCx<'gcc, 'tcx> {
    ...
    // TODO(antoyo): can we use https://github.com/crablang/compiler-builtins/blob/master/src/int/mod.rs#L379 instead?
