...
use crate::crablangc_codegen_ssa::traits::{BaseTypeMethods, DerivedTypeMethods, LayoutTypeMethods};
use crablangc_middle::bug;
use crablangc_middle::ty::{self, Ty, TypeVisitableExt};
use crablangc_middle::ty::layout::{FnAbiOf, LayoutOf, TyAndLayout};
use crablangc_middle::ty::print::with_no_trimmed_paths;
use crablangc_target::abi::{self, Abi, Align, F32, F64, FieldsShape, Int, Integer, Pointer, PointeeInfo, Size, TyAbiInterface, Variants};
use crablangc_target::abi::call::{CastTarget, FnAbi, Reg};
...
fn uncached_gcc_type<'gcc, 'tcx>(cx: &CodegenCx<'gcc, 'tcx>, layout: TyAndLayout<'tcx>, defer: &mut Option<(Struct<'gcc>, TyAndLayout<'tcx>)>) -> Type<'gcc> {
    ...
    let name = match layout.ty.kind() {
        ...
        ty::Adt(..) => {
            ...
            // avoided by LLVM (with increasing suffixes). If crablangc doesn't generate names then that
            // can improve perf.
...
impl<'tcx> LayoutGccExt<'tcx> for TyAndLayout<'tcx> {
    ...
    /// Gets the GCC type corresponding to a CrabLang type, i.e., `crablangc_middle::ty::Ty`.
    ...
    fn gcc_type<'gcc>(&self, cx: &CodegenCx<'gcc, 'tcx>) -> Type<'gcc> {
        ...
        // types for CrabLang types that only differ in the choice of lifetimes.
