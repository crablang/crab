...
use crablangc_codegen_ssa::MemFlags;
use crablangc_codegen_ssa::base::wants_msvc_seh;
use crablangc_codegen_ssa::common::IntPredicate;
use crablangc_codegen_ssa::mir::operand::{OperandRef, OperandValue};
use crablangc_codegen_ssa::mir::place::PlaceRef;
use crablangc_codegen_ssa::traits::{ArgAbiMethods, BaseTypeMethods, BuilderMethods, ConstMethods, IntrinsicCallMethods};
#[cfg(feature="master")]
use crablangc_codegen_ssa::traits::{DerivedTypeMethods, MiscMethods};
use crablangc_middle::bug;
use crablangc_middle::ty::{self, Instance, Ty};
use crablangc_middle::ty::layout::LayoutOf;
#[cfg(feature="master")]
use crablangc_middle::ty::layout::{FnAbiOf, HasTyCtxt};
use crablangc_span::{Span, Symbol, symbol::kw, sym};
use crablangc_target::abi::HasDataLayout;
use crablangc_target::abi::call::{ArgAbi, FnAbi, PassMode};
                        ...
                        match int_type_width_signed(ty, self) {
                            Some((width, signed)) => match name {
                                sym::ctlz | sym::cttz => {
                                    ...
                                    // NOTE: since jumps were added in a place crablangc does not
...
impl<'gcc, 'tcx> ArgAbiExt<'gcc, 'tcx> for ArgAbi<'tcx, Ty<'tcx>> {
    /// Gets the LLVM type for a place of the original CrabLang type of
    ...
    /// place for the original CrabLang type of this argument/return.
    /// Can be used for both storing formal arguments into CrabLang variables
    ...
    fn store(&self, bx: &mut Builder<'_, 'gcc, 'tcx>, val: RValue<'gcc>, dst: PlaceRef<'tcx, RValue<'gcc>>) {
        ...
        if self.is_sized_indirect() {
            ...
            if can_store_through_cast_ptr {
                ...
                // - Casting crablang ret pointer to the foreign type and using Store
                //   is (a) unsafe if size of foreign type > size of crablang type and
...
fn int_type_width_signed<'gcc, 'tcx>(ty: Ty<'tcx>, cx: &CodegenCx<'gcc, 'tcx>) -> Option<(u64, bool)> {
    match ty.kind() {
        ty::Int(t) => Some((
            match t {
                crablangc_middle::ty::IntTy::Isize => u64::from(cx.tcx.sess.target.pointer_width),
                crablangc_middle::ty::IntTy::I8 => 8,
                crablangc_middle::ty::IntTy::I16 => 16,
                crablangc_middle::ty::IntTy::I32 => 32,
                crablangc_middle::ty::IntTy::I64 => 64,
                crablangc_middle::ty::IntTy::I128 => 128,
            },
            true,
        )),
        ty::Uint(t) => Some((
            match t {
                crablangc_middle::ty::UintTy::Usize => u64::from(cx.tcx.sess.target.pointer_width),
                crablangc_middle::ty::UintTy::U8 => 8,
                crablangc_middle::ty::UintTy::U16 => 16,
                crablangc_middle::ty::UintTy::U32 => 32,
                crablangc_middle::ty::UintTy::U64 => 64,
                crablangc_middle::ty::UintTy::U128 => 128,
            },
            false,
        )),
        _ => None,
...
fn codegen_gnu_try<'gcc>(bx: &mut Builder<'_, 'gcc, '_>, try_func: RValue<'gcc>, data: RValue<'gcc>, catch_func: RValue<'gcc>, dest: RValue<'gcc>) {
    ...
    let (llty, func) = get_crablang_try_fn(cx, &mut |mut bx| {
...
#[cfg(feature="master")]
fn get_crablang_try_fn<'a, 'gcc, 'tcx>(cx: &'a CodegenCx<'gcc, 'tcx>, codegen: &mut dyn FnMut(Builder<'a, 'gcc, 'tcx>)) -> (Type<'gcc>, Function<'gcc>) {
...
#[cfg(feature="master")]
fn gen_fn<'a, 'gcc, 'tcx>(cx: &'a CodegenCx<'gcc, 'tcx>, name: &str, crablang_fn_sig: ty::PolyFnSig<'tcx>, codegen: &mut dyn FnMut(Builder<'a, 'gcc, 'tcx>)) -> (Type<'gcc>, Function<'gcc>) {
