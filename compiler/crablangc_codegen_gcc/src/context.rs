...
use crablangc_middle::ty::{self, Instance, ParamEnv, PolyExistentialTraitRef, Ty, TyCtxt};
use crablangc_middle::ty::layout::{FnAbiError, FnAbiOf, FnAbiOfHelpers, FnAbiRequest, HasParamEnv, HasTyCtxt, LayoutError, TyAndLayout, LayoutOfHelpers};
use crablangc_session::Session;
use crablangc_span::{Span, source_map::respan};
use crablangc_target::abi::{call::FnAbi, HasDataLayout, PointeeInfo, Size, TargetDataLayout, VariantIdx};
use crablangc_target::spec::{HasTargetSpec, Target, TlsModel};
...
pub struct CodegenCx<'gcc, 'tcx> {
    ...
    /// TODO(antoyo): remove when the crablangc API is fixed.
    ...
    pub crablang_try_fn: Cell<Option<(Type<'gcc>, Function<'gcc>)>>,

    pub pointee_infos: RefCell<FxHashMap<(Ty<'tcx>, Size), Option<PointeeInfo>>>,

    /// NOTE: a hack is used because the crablangc API is not suitable to libgccjit and as such,
    /// `const_undef()` returns struct as pointer so that they can later be assigned a value.
    /// As such, this set remembers which of these pointers were returned by this function so that
    /// they can be dereferenced later.
    /// FIXME(antoyo): fix the crablangc API to avoid having this hack.
...
impl<'gcc, 'tcx> MiscMethods<'tcx> for CodegenCx<'gcc, 'tcx> {
    ...
    fn get_fn_addr(&self, instance: Instance<'tcx>) -> RValue<'gcc> {
        ...
        // FIXME(antoyo): the crablangc API seems to call get_fn_addr() when not needed (e.g. for FFI).
