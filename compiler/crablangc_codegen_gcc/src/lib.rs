...
use crablangc_codegen_ssa::{CodegenResults, CompiledModule, ModuleCodegen};
use crablangc_codegen_ssa::base::codegen_crate;
use crablangc_codegen_ssa::back::write::{CodegenContext, FatLTOInput, ModuleConfig, TargetMachineFactoryFn};
use crablangc_codegen_ssa::back::lto::{LtoModuleCodegen, SerializedModule, ThinModule};
use crablangc_codegen_ssa::target_features::supported_target_features;
use crablangc_codegen_ssa::traits::{CodegenBackend, ExtraBackendMethods, ModuleBufferMethods, ThinBufferMethods, WriteBackendMethods};
use crablangc_data_structures::fx::FxHashMap;
use crablangc_errors::{DiagnosticMessage, ErrorGuaranteed, Handler, SubdiagnosticMessage};
...
use crablangc_session::config::{Lto, OptLevel, OutputFilenames};
use crablangc_session::Session;
use crablangc_span::Symbol;
use crablangc_span::fatal_error::FatalError;
...
impl CodegenBackend for GccCodegenBackend {
    ...
    fn join_codegen(&self, ongoing_codegen: Box<dyn Any>, sess: &Session, _outputs: &OutputFilenames) -> Result<(CodegenResults, FxHashMap<WorkProductId, WorkProduct>), ErrorGuaranteed> {
        let (codegen_results, work_products) = ongoing_codegen
            .downcast::<crablangc_codegen_ssa::back::write::OngoingCodegen<GccCodegenBackend>>()
...
/// This is the entrypoint for a hot plugged crablangc_codegen_gccjit
#[no_mangle]
