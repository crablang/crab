...
use crablangc_apfloat::{ieee, Float, Round, Status};
use crablangc_codegen_ssa::MemFlags;
use crablangc_codegen_ssa::common::{
...
use crablangc_middle::ty::layout::{FnAbiError, FnAbiOfHelpers, FnAbiRequest, HasParamEnv, HasTyCtxt, LayoutError, LayoutOfHelpers, TyAndLayout};
...
impl<'a, 'gcc, 'tcx> BuilderMethods<'a, 'tcx> for Builder<'a, 'gcc, 'tcx> {
    ...
    fn ret(&mut self, value: RValue<'gcc>) {
        let value =
            if self.structs_as_pointer.borrow().contains(&value) {
                // NOTE: hack to workaround a limitation of the crablangc API: see comment on
    ...
    fn exactsdiv(&mut self, a: RValue<'gcc>, b: RValue<'gcc>) -> RValue<'gcc> {
        // TODO(antoyo): poison if not exact.
        // FIXME(antoyo): crablangc_codegen_ssa::mir::intrinsic uses different types for a and b but they
        // should be the same.
    ...
    fn alloca(&mut self, ty: Type<'gcc>, align: Align) -> RValue<'gcc> {
        ...
        // TODO(antoyo): It might be better to return a LValue, but fixing the crablangc API is non-trivial.
    ...
    fn insert_value(&mut self, aggregate_value: RValue<'gcc>, value: RValue<'gcc>, idx: u64) -> RValue<'gcc> {
        ...
        let value =
            // NOTE: sometimes, crablangc will create a value with the wrong type.
