...
impl<'tcx> CheckConstVisitor<'tcx> {
    ...
    fn const_check_violated(&self, expr: NonConstExpr, span: Span) {
        ...
        let is_feature_allowed = |feature_gate| {
            ...
            // even if the function has `#[crablangc_allow_const_fn_unstable(the_gate)]`.
            ...
            if !tcx.features().staged_api || tcx.has_attr(def_id, sym::crablangc_const_unstable) {
                return true;
            }

            // However, we cannot allow stable `const fn`s to use unstable features without an explicit
            // opt-in via `crablangc_allow_const_fn_unstable`.
            let attrs = tcx.hir().attrs(tcx.hir().local_def_id_to_hir_id(def_id));
            attr::crablangc_allow_const_fn_unstable(&tcx.sess, attrs).any(|name| name == feature_gate)
        };
