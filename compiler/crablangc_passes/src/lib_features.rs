...
use crablangc_attr::{crablang_version_symbol, VERSION_PLACEHOLDER};
use crablangc_hir::intravisit::Visitor;
use crablangc_middle::hir::nested_filter;
use crablangc_middle::middle::lib_features::LibFeatures;
...
impl<'tcx> LibFeatureCollector<'tcx> {
    ...
    fn extract(&self, attr: &Attribute) -> Option<(Symbol, Option<Symbol>, Span)> {
        let stab_attrs = [
            ...
            sym::crablangc_default_body_unstable,
        ];

        // Find a stability attribute: one of #[stable(…)], #[unstable(…)],
        // #[crablangc_const_stable(…)], #[crablangc_const_unstable(…)] or #[crablangc_default_body_unstable].
        if let Some(stab_attr) = stab_attrs.iter().find(|stab_attr| attr.has_name(**stab_attr)) {
            if let Some(metas) = attr.meta_item_list() {
                ...
                // `crablangc_const_unstable` is not mutually exclusive with
