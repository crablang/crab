...
use crate::errors;
use crablangc_attr::{
    self as attr, crablang_version_symbol, ConstStability, Stability, StabilityLevel, Unstable,
...
use crablangc_hir::def_id::{LocalDefId, CRATE_DEF_ID};
use crablangc_hir::hir_id::CRATE_HIR_ID;
use crablangc_hir::intravisit::{self, Visitor};
use crablangc_hir::{FieldDef, Item, ItemKind, TraitRef, Ty, TyKind, Variant};
use crablangc_middle::hir::nested_filter;
use crablangc_middle::middle::privacy::EffectiveVisibilities;
use crablangc_middle::middle::stability::{AllowUnstable, DeprecationEntry, Index};
use crablangc_middle::ty::{query::Providers, TyCtxt};
use crablangc_session::lint;
use crablangc_session::lint::builtin::{INEFFECTIVE_UNSTABLE_TRAIT_IMPL, USELESS_DEPRECATED};
...
impl<'a, 'tcx> Annotator<'a, 'tcx> {
        ...
        F: FnOnce(&mut Self),
    {
        ...
        if let (Some(const_span), Some(fn_sig)) = (const_span, fn_sig) {
            if fn_sig.header.abi != Abi::CrabLangIntrinsic
        ...
        if let Some((crablangc_attr::Deprecation { is_since_crablangc_version: true, .. }, span)) = &depr {
            if stab.is_none() {
...
fn stability_index(tcx: TyCtxt<'_>, (): ()) -> Index {
...

    {
        ...
        // with the `crablangc_private` feature. This is intended for use when
        // compiling `libcrablangc_*` crates themselves so we can leverage crates.io
        ...
        match item.kind {
            ...
            hir::ItemKind::Impl(hir::Impl {
                ...
                if features.staged_api {
                    ...
                    // See: https://github.com/crablang/crablang/issues/55436
                    if let Some((Stability { level: attr::Unstable { .. }, .. }, span)) = stab {
                        ...
                        if t.path.res != Res::Err && c.fully_stable {
                            self.tcx.struct_span_lint_hir(
                                ...
                                |lint| lint.note("see issue #55436 <https://github.com/crablang/crablang/issues/55436> for more information")
                            );
                        }
                    }
    ...
    fn visit_path(&mut self, path: &hir::Path<'tcx>, id: hir::HirId) {
        if let Some(def_id) = path.res.opt_def_id() {
            ...
            if item_is_allowed && !is_allowed_through_unstable_modules(def_id) {
                ...
                // path segment twice
                //
                // We include special cases via #[crablangc_allowed_through_unstable_modules] for items
...
impl<'tcx> Visitor<'tcx> for CheckTraitImplStable<'tcx> {
    ...
    fn visit_ty(&mut self, t: &'tcx Ty<'tcx>) {
        ...
        if let TyKind::BareFn(f) = t.kind {
            if crablangc_target::spec::abi::is_stable(f.abi.name()).is_err() {
                self.fully_stable = false;
            }
        }
...
pub fn check_unused_or_stable_features(tcx: TyCtxt<'_>) {
    ...
    let mut remaining_implications =
        tcx.stability_implications(crablangc_hir::def_id::LOCAL_CRATE).clone();
