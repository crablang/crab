...
use crablangc_hir::{Expr, HirId, HirIdMap, HirIdSet};
use crablangc_index::vec::IndexVec;
use crablangc_middle::ty::query::Providers;
use crablangc_middle::ty::{self, RootVariableMinCaptureList, Ty, TyCtxt};
    ...
    fn visit_param(&mut self, param: &'tcx hir::Param<'tcx>) {
        ...
        param.pat.each_binding(|_bm, hir_id, _x, ident| {
            let var = match param.pat.kind {
                crablangc_hir::PatKind::Struct(..) => Local(LocalInfo {
                    id: hir_id,
                    name: ident.name,
                    is_shorthand: shorthand_field_ids.contains(&hir_id),
                }),
...
impl<'tcx> Liveness<'_, 'tcx> {
    ...
    fn has_added_lit_match_name_span(
        ...
        for lit_expr in visitor.lit_exprs {
            ...
            let crablangc_ast::LitKind::Str(syb, _) = litx.node else{ continue; };
