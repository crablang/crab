...
use crablangc_middle::middle::privacy::{self, Level};
...
impl<'tcx> ReachableContext<'tcx> {
    ...
    fn propagate_node(&mut self, node: &Node<'tcx>, search_item: LocalDefId) {
        if !self.any_library {
            ...
            let std_internal =
                codegen_attrs.flags.contains(CodegenFnAttrFlags::CRABLANGC_STD_INTERNAL_SYMBOL);
...
fn has_custom_linkage(tcx: TyCtxt<'_>, def_id: LocalDefId) -> bool {
    ...
    codegen_attrs.contains_extern_indicator()
        ...
        // `SymbolExportLevel::CrabLang` export level but may end up being exported in dylibs.
