...
/// Extract the first `crablangc_diagnostic_item = "$name"` out of a list of attributes.
fn extract(attrs: &[ast::Attribute]) -> Option<Symbol> {
    attrs.iter().find_map(|attr| {
        if attr.has_name(sym::crablangc_diagnostic_item) { attr.value_str() } else { None }
    })
}
