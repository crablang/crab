...
use crablangc_ast::{ast, AttrStyle, Attribute, LitKind, MetaItemKind, MetaItemLit, NestedMetaItem};
use crablangc_data_structures::fx::FxHashMap;
use crablangc_errors::{Applicability, IntoDiagnosticArg, MultiSpan};
use crablangc_expand::base::resolve_path;
use crablangc_feature::{AttributeDuplicates, AttributeType, BuiltinAttribute, BUILTIN_ATTRIBUTE_MAP};
...
use crablangc_hir::{MethodKind, Target, Unsafety};
use crablangc_middle::hir::nested_filter;
use crablangc_middle::middle::resolve_bound_vars::ObjectLifetimeDefault;
...
impl CheckAttrVisitor<'_> {
    ...
    fn check_attributes(
        ...
        for attr in attrs {
            let attr_is_valid = match attr.name_or_empty() {
                ...
                sym::crablangc_must_implement_one_of => {
                    self.check_crablangc_must_implement_one_of(attr, span, target)
                }
                ...
                sym::crablangc_layout_scalar_valid_range_start
                | sym::crablangc_layout_scalar_valid_range_end => {
                    self.check_crablangc_layout_scalar_valid_range(&attr, span, target)
                }
                sym::allow_internal_unstable => {
                    self.check_allow_internal_unstable(hir_id, &attr, span, target, &attrs)
                }
                sym::debugger_visualizer => self.check_debugger_visualizer(&attr, target),
                sym::crablangc_allow_const_fn_unstable => {
                    self.check_crablangc_allow_const_fn_unstable(hir_id, &attr, span, target)
                }
                sym::crablangc_std_internal_symbol => {
                    self.check_crablangc_std_internal_symbol(&attr, span, target)
                }
                sym::naked => self.check_naked(hir_id, attr, span, target),
                sym::crablangc_legacy_const_generics => {
                    self.check_crablangc_legacy_const_generics(hir_id, &attr, span, target, item)
                }
                sym::crablangc_lint_query_instability => {
                    self.check_crablangc_lint_query_instability(hir_id, &attr, span, target)
                }
                sym::crablangc_lint_diagnostics => {
                    self.check_crablangc_lint_diagnostics(hir_id, &attr, span, target)
                }
                sym::crablangc_lint_opt_ty => self.check_crablangc_lint_opt_ty(&attr, span, target),
                sym::crablangc_lint_opt_deny_field_access => {
                    self.check_crablangc_lint_opt_deny_field_access(&attr, span, target)
                }
                sym::crablangc_clean
                | sym::crablangc_dirty
                | sym::crablangc_if_this_changed
                | sym::crablangc_then_this_would_need => self.check_crablangc_dirty_clean(&attr),
                sym::crablangc_coinductive => self.check_crablangc_coinductive(&attr, span, target),
                ...
                sym::crablangc_pass_by_value => self.check_pass_by_value(&attr, span, target),
                sym::crablangc_allow_incoherent_impl => {
                    self.check_allow_incoherent_impl(&attr, span, target)
                }
                sym::crablangc_has_incoherent_inherent_impls => {
                    self.check_has_incoherent_inherent_impls(&attr, span, target)
                }
                sym::ffi_pure => self.check_ffi_pure(attr.span, attrs, target),
                sym::ffi_const => self.check_ffi_const(attr.span, target),
                sym::ffi_returns_twice => self.check_ffi_returns_twice(attr.span, target),
                sym::crablangc_const_unstable
                | sym::crablangc_const_stable
                | sym::unstable
                | sym::stable
                | sym::crablangc_allowed_through_unstable_modules
                | sym::crablangc_promotable => self.check_stability_promotable(&attr, span, target),
            ...
            match attr.name_or_empty() {
                ...
                sym::crablangc_object_lifetime_default => self.check_object_lifetime_default(hir_id),
                sym::proc_macro => {
    ...
    /// Checks if the `#[crablangc_must_implement_one_of]` attribute on a `target` is valid. Returns `true` if valid.
    fn check_crablangc_must_implement_one_of(
        &self,
        attr: &Attribute,
        span: Span,
        target: Target,
    ...
    fn check_doc_keyword(&self, meta: &NestedMetaItem, hir_id: HirId) -> bool {
        ...
        if !crablangc_lexer::is_ident(doc_keyword.as_str()) {
                    ...
                    _ => {
                        self.tcx.emit_spanned_lint(
                            ...
                            errors::DocTestUnknown {
                                path: crablangc_ast_pretty::ppcrablang::path_to_string(
            ...
            for meta in list {
                if let Some(i_meta) = meta.meta_item() {
                    match i_meta.name_or_empty() {
                        ...
                        // passes: deprecated
                        // plugins: removed, but crablangdoc warns about it itself
                        sym::alias
                        | sym::cfg
                        | sym::cfg_hide
                        | sym::hidden
                        ...
                        _ => {
                            let path = crablangc_ast_pretty::ppcrablang::path_to_string(&i_meta.path);
    ...
    fn check_link(&self, hir_id: HirId, attr: &Attribute, span: Span, target: Target) {
        if target == Target::ForeignMod
            ...
            && !matches!(abi, Abi::CrabLang | Abi::CrabLangIntrinsic | Abi::PlatformIntrinsic)
        {
            return;
        }
    ...
    fn check_crablangc_layout_scalar_valid_range(
        &self,
        attr: &Attribute,
        span: Span,
        target: Target,
        ...
        if target != Target::Struct {
            self.tcx.sess.emit_err(errors::CrabLangcLayoutScalarValidRangeNotStruct {
                attr_span: attr.span,
                span,
            });
            return false;
        ...
        if matches!(&list[..], &[NestedMetaItem::Lit(MetaItemLit { kind: LitKind::Int(..), .. })]) {
            ...
            self.tcx.sess.emit_err(errors::CrabLangcLayoutScalarValidRangeArg { attr_span: attr.span });
            false
        }
    }

    /// Checks if `#[crablangc_legacy_const_generics]` is applied to a function and has a valid argument.
    fn check_crablangc_legacy_const_generics(
        &self,
        hir_id: HirId,
        attr: &Attribute,
        span: Span,
        ...
        for param in generics.params {
            match param.kind {
                ...
                _ => {
                    self.tcx.sess.emit_err(errors::CrabLangcLegacyConstGenericsOnly {
                        attr_span: attr.span,
        ...
        if list.len() != generics.params.len() {
            self.tcx.sess.emit_err(errors::CrabLangcLegacyConstGenericsIndex {
                attr_span: attr.span,
        ...
        for meta in list {
            if let Some(LitKind::Int(val, _)) = meta.lit().map(|lit| &lit.kind) {
                if *val >= arg_count {
                    let span = meta.span();
                    self.tcx.sess.emit_err(errors::CrabLangcLegacyConstGenericsIndexExceed {
                        span,
        ...
        if !invalid_args.is_empty() {
            self.tcx.sess.emit_err(errors::CrabLangcLegacyConstGenericsIndexNegative { invalid_args });
            false
        } else {
            true
        }
    ...
    /// Checks that the `#[crablangc_lint_query_instability]` attribute is only applied to a function
    /// or method.
    fn check_crablangc_lint_query_instability(
        &self,
        hir_id: HirId,
        attr: &Attribute,
        span: Span,
    ...
    /// Checks that the `#[crablangc_lint_diagnostics]` attribute is only applied to a function or
    /// method.
    fn check_crablangc_lint_diagnostics(
        &self,
        hir_id: HirId,
        attr: &Attribute,
        span: Span,
    ...
    /// Checks that the `#[crablangc_lint_opt_ty]` attribute is only applied to a struct.
    fn check_crablangc_lint_opt_ty(&self, attr: &Attribute, span: Span, target: Target) -> bool {
        match target {
            Target::Struct => true,
            _ => {
                self.tcx.sess.emit_err(errors::CrabLangcLintOptTy { attr_span: attr.span, span });
                false
            }
        }
    }

    /// Checks that the `#[crablangc_lint_opt_deny_field_access]` attribute is only applied to a field.
    fn check_crablangc_lint_opt_deny_field_access(
        &self,
        attr: &Attribute,
        span: Span,
        target: Target,
        ...
        match target {
            ...
            _ => {
                self.tcx
                    .sess
                    .emit_err(errors::CrabLangcLintOptDenyFieldAccess { attr_span: attr.span, span });
                false
            }
        }
    }
    ...
    fn check_crablangc_dirty_clean(&self, attr: &Attribute) -> bool {
        if self.tcx.sess.opts.unstable_opts.query_dep_graph {
            true
        } else {
            self.tcx.sess.emit_err(errors::CrabLangcDirtyClean { span: attr.span });
            false
        }
    }

    /// Checks if the `#[crablangc_coinductive]` attribute is applied to a trait.
    fn check_crablangc_coinductive(&self, attr: &Attribute, span: Span, target: Target) -> bool {
        match target {
            Target::Trait => true,
            _ => {
                self.tcx.sess.emit_err(errors::AttrShouldBeAppliedToTrait {
    ...
    fn check_used(&self, attrs: &[Attribute], target: Target) {
        ...
        for attr in attrs.iter().filter(|attr| attr.has_name(sym::used)) {
            ...
            match inner.as_deref() {
                ...
                Some(_) => {
                    // This error case is handled in crablangc_hir_analysis::collect.
                }
                None => {
    ...
    /// Outputs an error for `#[allow_internal_unstable]` which can only be applied to macros.
    /// (Allows proc_macro functions)
    fn check_crablangc_allow_const_fn_unstable(
        &self,
        hir_id: HirId,
        attr: &Attribute,
        span: Span,
        ...
        match target {
            ...
            _ => {
                self.tcx
                    .sess
                    .emit_err(errors::CrabLangcAllowConstFnUnstable { attr_span: attr.span, span });
                false
            }
        }
    }

    fn check_crablangc_std_internal_symbol(
        &self,
        attr: &Attribute,
        span: Span,
        target: Target,
        ...
        match target {
            ...
            _ => {
                self.tcx
                    .sess
                    .emit_err(errors::CrabLangcStdInternalSymbol { attr_span: attr.span, span });
                false
            }
        }
    }
