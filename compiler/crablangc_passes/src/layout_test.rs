...
use crablangc_middle::ty::layout::{HasParamEnv, HasTyCtxt, LayoutError, LayoutOfHelpers, TyAndLayout};
use crablangc_middle::ty::{ParamEnv, Ty, TyCtxt};
use crablangc_span::source_map::Spanned;
use crablangc_span::symbol::sym;
use crablangc_span::Span;
use crablangc_target::abi::{HasDataLayout, TargetDataLayout};

use crate::errors::{Abi, Align, HomogeneousAggregate, LayoutOf, Size, UnrecognizedField};

pub fn test_layout(tcx: TyCtxt<'_>) {
    if tcx.features().crablangc_attrs {
        // if the `crablangc_attrs` feature is not enabled, don't bother testing layout
        for id in tcx.hir().items() {
            if matches!(
                tcx.def_kind(id.owner_id),
                DefKind::TyAlias | DefKind::Enum | DefKind::Struct | DefKind::Union
            ) {
                for attr in tcx.get_attrs(id.owner_id, sym::crablangc_layout) {
...
fn dump_layout_of(tcx: TyCtxt<'_>, item_def_id: LocalDefId, attr: &Attribute) {
    ...
    match tcx.layout_of(param_env.and(ty)) {
        Ok(ty_layout) => {
            // Check out the `#[crablangc_layout(..)]` attribute to tell what to dump.
...
impl<'tcx> LayoutOfHelpers<'tcx> for UnwrapLayoutCx<'tcx> {
    ...
    fn handle_layout_err(&self, err: LayoutError<'tcx>, span: Span, ty: Ty<'tcx>) -> ! {
        span_bug!(
            span,
            "`#[crablangc_layout(..)]` test resulted in `layout_of({}) = Err({})`",
            ty,
            err
        );
    }
