...
use crablangc_hir::{ItemId, Node, CRATE_HIR_ID};
use crablangc_middle::ty::query::Providers;
use crablangc_middle::ty::TyCtxt;
use crablangc_session::config::{sigpipe, CrateType, EntryFnType};
use crablangc_session::parse::feature_err;
use crablangc_span::symbol::sym;
use crablangc_span::{Span, Symbol};

use crate::errors::{
    AttrOnlyInFunctions, AttrOnlyOnMain, AttrOnlyOnRootMain, ExternMain, MultipleCrabLangcMain,
...
// Beware, this is duplicated in `libcrablangc_builtin_macros/test_harness.rs`
...
fn entry_point_type(ctxt: &EntryContext<'_>, id: ItemId, at_root: bool) -> EntryPointType {
    ...
    } else if attr::contains_name(attrs, sym::crablangc_main) {
...
fn find_item(id: ItemId, ctxt: &mut EntryContext<'_>) {
    ...
    match entry_point_type(ctxt, id, at_root) {
        ...
        _ if !matches!(ctxt.tcx.def_kind(id.owner_id), DefKind::Fn) => {
            for attr in [sym::start, sym::crablangc_main] {
        ...
        EntryPointType::CrabLangcMainAttr => {
            if ctxt.attr_main_fn.is_none() {
                ...
                ctxt.tcx.sess.emit_err(MultipleCrabLangcMain {
...
fn no_main_err(tcx: TyCtxt<'_>, visitor: &EntryContext<'_>) {
    ...
    if tcx.sess.parse_sess.reached_eof.load(crablangc_data_structures::sync::Ordering::Relaxed) {
