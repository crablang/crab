...
use crablangc_hir::{Node, PatKind, TyKind};
use crablangc_middle::middle::codegen_fn_attrs::CodegenFnAttrFlags;
use crablangc_middle::middle::privacy::Level;
...
impl<'tcx> MarkSymbolVisitor<'tcx> {
    ...
    /// Automatically generated items marked with `crablangc_trivial_field_reads`
    ...
    fn should_ignore_item(&mut self, def_id: DefId) -> bool {
        if let Some(impl_of) = self.tcx.impl_of_method(def_id) {
            ...
            if let Some(trait_of) = self.tcx.trait_id_of_impl(impl_of)
                && self.tcx.has_attr(trait_of, sym::crablangc_trivial_field_reads)
            {
