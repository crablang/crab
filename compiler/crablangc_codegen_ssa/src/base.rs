use crate::back::link::are_upstream_crablang_objects_already_included;
...
use crablangc_data_structures::profiling::{get_resident_set_size, print_time_passes_entry};

use crablangc_data_structures::sync::par_iter;
#[cfg(parallel_compiler)]
use crablangc_data_structures::sync::ParallelIterator;
...
use crablangc_middle::middle::codegen_fn_attrs::CodegenFnAttrs;
use crablangc_middle::middle::exported_symbols;
use crablangc_middle::middle::exported_symbols::SymbolExportKind;
use crablangc_middle::middle::lang_items;
use crablangc_middle::mir::mono::{CodegenUnit, CodegenUnitNameBuilder, MonoItem};
use crablangc_middle::ty::layout::{HasTyCtxt, LayoutOf, TyAndLayout};
use crablangc_middle::ty::query::Providers;
use crablangc_middle::ty::{self, Instance, Ty, TyCtxt};
use crablangc_session::cgu_reuse_tracker::CguReuse;
use crablangc_session::config::{self, CrateType, EntryFnType, OutputType};
use crablangc_session::Session;
use crablangc_span::symbol::sym;
use crablangc_span::Symbol;
use crablangc_span::{DebuggerVisualizerFile, DebuggerVisualizerType};
use crablangc_target::abi::{Align, FIRST_VARIANT};
...
/// Creates the `main` function which will initialize the crablang runtime and call
/// users main function.
pub fn maybe_create_entry_wrapper<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>>(
    ...
    if main_is_local {
        // We want to create the wrapper in the same codegen unit as CrabLang's main
        // function.
    ...
    fn create_entry_fn<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>>(
        cx: &'a Bx::CodegenCx,
        crablang_main: Bx::Value,
        crablang_main_def_id: DefId,
        ...
        let main_ret_ty = cx.tcx().fn_sig(crablang_main_def_id).no_bound_vars().unwrap().output();
        ...
        let Some(llfn) = cx.declare_c_main(llfty) else {
            ...
            let span = cx.tcx().def_span(crablang_main_def_id);
        ...
        let (start_fn, start_ty, args) = if let EntryFnType::Main { sigpipe } = entry_type {
            ...
            let start_ty =
                cx.type_func(&[cx.val_ty(crablang_main), isize_ty, i8pp_ty, i8_ty], isize_ty);
            (start_fn, start_ty, vec![crablang_main, arg_argc, arg_argv, arg_sigpipe])
        } else {
            debug!("using user-defined start fn");
            let start_ty = cx.type_func(&[isize_ty, i8pp_ty], isize_ty);
            (crablang_main, start_ty, vec![arg_argc, arg_argv])
        };
...
/// Obtain the `argc` and `argv` values to pass to the crablang start function.
fn get_argc_argv<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>>(
    cx: &'a Bx::CodegenCx,
    bx: &mut Bx,
) -> (Bx::Value, Bx::Value) {
    if cx.sess().target.main_needs_argc_argv {
        // Params from native `main()` used as args for crablang start function
        ...
        // The CrabLang start function doesn't need `argc` and `argv`, so just pass zeros.
...
impl CrateInfo {
    pub fn new(tcx: TyCtxt<'_>, target_cpu: String) -> CrateInfo {
        ...
        let crate_attrs = tcx.hir().attrs(crablangc_hir::CRATE_HIR_ID);
        ...
        if !are_upstream_crablang_objects_already_included(tcx.sess) {
