...
use crablangc_middle::mir::{self, AssertKind, SwitchTargets};
use crablangc_middle::ty::layout::{HasTyCtxt, LayoutOf, ValidityRequirement};
...
use crablangc_symbol_mangling::typeid::typeid_for_fnabi;
use crablangc_target::abi::call::{ArgAbi, FnAbi, PassMode, Reg};
use crablangc_target::abi::{self, HasDataLayout, WrappingRange};
...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn codegen_assert_terminator(
        ...
        // with #[crablangc_inherit_overflow_checks] and inlined from
    ...
    fn codegen_call_terminator(
        ...
        // Split the crablang-call tupled arguments off.
        let (first_args, untuple) = if abi == Abi::CrabLangCall && !args.is_empty() {
        ...
        let (mut llval, align, by_ref) = match op.val {
            ...
            Ref(llval, _, align) => {
                if arg.is_indirect() && align < arg.layout.align.abi {
                    ...
                    // think that ATM (CrabLang 1.16) we only pass temporaries, but we shouldn't
