...
use crablangc_middle::mir::interpret::{ConstValue, Pointer, Scalar};
use crablangc_middle::ty::layout::{LayoutOf, TyAndLayout};
use crablangc_middle::ty::Ty;
use crablangc_target::abi::{Abi, Align, Size};

use std::fmt;

/// The representation of a CrabLang value. The enum variant is in fact
...
/// An `OperandRef` is an "SSA" reference to a CrabLang value, along with
/// its type.
///
...
pub struct OperandRef<'tcx, V> {
    ...
    /// The layout of value, based on its CrabLang type.
