...
use crablangc_attr::InstructionSetAttr;
...
/// Features that control behaviour of crablangc, rather than the codegen.
pub const CRABLANGC_SPECIFIC_FEATURES: &[&str] = &["crt-static"];

// When adding features to the below lists
// check whether they're named already elsewhere in crablang
// e.g. in stdarch and whether the given name matches LLVM's
// if it doesn't, to_llvm_feature in llvm_util in crablangc_codegen_llvm needs to be adapted
...
/// When crablangdoc is running, provide a list of all known features so that all their respective
...
pub fn from_target_feature(
    ...
    let crablang_features = tcx.features();
    for item in list {
        ...
        target_features.extend(value.as_str().split(',').filter_map(|feature| {
            ...
            let allowed = match feature_gate.as_ref().copied() {
                Some(sym::arm_target_feature) => crablang_features.arm_target_feature,
                Some(sym::hexagon_target_feature) => crablang_features.hexagon_target_feature,
                Some(sym::powerpc_target_feature) => crablang_features.powerpc_target_feature,
                Some(sym::mips_target_feature) => crablang_features.mips_target_feature,
                Some(sym::riscv_target_feature) => crablang_features.riscv_target_feature,
                Some(sym::avx512_target_feature) => crablang_features.avx512_target_feature,
                Some(sym::sse4a_target_feature) => crablang_features.sse4a_target_feature,
                Some(sym::tbm_target_feature) => crablang_features.tbm_target_feature,
                Some(sym::wasm_target_feature) => crablang_features.wasm_target_feature,
                Some(sym::rtm_target_feature) => crablang_features.rtm_target_feature,
                Some(sym::ermsb_target_feature) => crablang_features.ermsb_target_feature,
                Some(sym::bpf_target_feature) => crablang_features.bpf_target_feature,
                Some(sym::aarch64_ver_target_feature) => crablang_features.aarch64_ver_target_feature,
...
pub(crate) fn provide(providers: &mut Providers) {
    *providers = Providers {
        supported_target_features: |tcx, cnum| {
            ...
            if tcx.sess.opts.actually_crablangdoc {
                // crablangdoc needs to be able to document functions that use all the features, so
                // whitelist them all
