...
use crablangc_hir::definitions::{DefPathData, DefPathDataName, DisambiguatedDefPathData};
use crablangc_hir::{AsyncGeneratorKind, GeneratorKind, Mutability};
use crablangc_middle::ty::layout::{IntegerExt, TyAndLayout};
use crablangc_middle::ty::subst::{GenericArgKind, SubstsRef};
use crablangc_middle::ty::{self, ExistentialProjection, ParamEnv, Ty, TyCtxt};
...
fn push_debuginfo_type_name<'tcx>(
    ...
    match *t.kind() {
            ...
            let layout_for_cpp_like_fallback = if cpp_like_debuginfo && def.is_enum() {
                match tcx.layout_of(ParamEnv::reveal_all().and(t)) {
                    ...
                    Err(e) => {
                        ...
                        // cannot represent the type. See https://github.com/crablang/crablang/issues/94961.
                        // FIXME: migrate once `crablangc_middle::mir::interpret::InterpError` is translatable.
        ...
        ty::FnDef(..) | ty::FnPtr(_) => {
            ...
            if cpp_like_debuginfo {
                ...
                if sig.abi != crablangc_target::spec::abi::Abi::CrabLang {
    ...
    // crablangc_codegen_llvm/src/debuginfo/metadata/enums/cpp_like.rs.
