...
use crablangc_ast::expand::allocator::ALLOCATOR_METHODS;
use crablangc_data_structures::fx::FxHashMap;
use crablangc_hir::def::DefKind;
use crablangc_hir::def_id::{CrateNum, DefId, DefIdMap, LocalDefId, LOCAL_CRATE};
...
use crablangc_middle::ty::{self, SymbolName, TyCtxt};
use crablangc_session::config::{CrateType, OomStrategy};
use crablangc_target::spec::SanitizerSet;
...
fn crate_export_threshold(crate_type: CrateType) -> SymbolExportLevel {
    match crate_type {
        ...
        CrateType::Rlib | CrateType::Dylib => SymbolExportLevel::CrabLang,
    }
}
...
pub fn crates_export_threshold(crate_types: &[CrateType]) -> SymbolExportLevel {
    if crate_types
        .iter()
        .any(|&crate_type| crate_export_threshold(crate_type) == SymbolExportLevel::CrabLang)
    {
        SymbolExportLevel::CrabLang
    } else {
        SymbolExportLevel::C
    }
}
...
fn reachable_non_generics_provider(tcx: TyCtxt<'_>, _: LocalCrate) -> DefIdMap<SymbolExportInfo> {
    ...
    // Down below we'll hardwire all of the symbols to the `CrabLang` export
    // level instead.
    ...
    let mut reachable_non_generics: DefIdMap<_> = tcx
        ...
        .map(|def_id| {
            ...
            let used = name == "crablang_eh_personality";
...
fn exported_symbols_provider_local(
    ...
    if allocator_kind_for_codegen(tcx).is_some() {
        for symbol_name in ALLOCATOR_METHODS
            .iter()
            .map(|method| format!("__crablang_{}", method.name))
            .chain(["__crablang_alloc_error_handler".to_string(), OomStrategy::SYMBOL.to_string()])
        {
            ...
            symbols.push((
                exported_symbol,
                SymbolExportInfo {
                    level: SymbolExportLevel::CrabLang,
                    kind: SymbolExportKind::Text,
                    used: false,
                },
            ));
    ...
    if tcx.sess.opts.share_generics() && tcx.local_crate_exports_generics() {
        use crablangc_middle::mir::mono::{Linkage, MonoItem, Visibility};
        use crablangc_middle::ty::InstanceDef;

        // Normally, we require that shared monomorphizations are not hidden,
        // because if we want to re-use a monomorphization from a CrabLang dylib, it
        // needs to be exported.
        // However, on platforms that don't allow for CrabLang dylibs, having
        ...
        for (mono_item, &(linkage, visibility)) in cgus.iter().flat_map(|cgu| cgu.items().iter()) {
            ...
            if need_visibility && visibility == Visibility::Hidden {
                // If we potentially share things from CrabLang dylibs, they must
                // not be hidden
                continue;
            }
...
fn symbol_export_level(tcx: TyCtxt<'_>, sym_def_id: DefId) -> SymbolExportLevel {
    ...
    let std_internal =
        codegen_fn_attrs.flags.contains(CodegenFnAttrFlags::CRABLANGC_STD_INTERNAL_SYMBOL);
    ...
    if is_extern && !std_internal {
        ...
        if target.contains("emscripten") {
            if let DefKind::Static(_) = tcx.def_kind(sym_def_id) {
                return SymbolExportLevel::CrabLang;
            }
        }
...
pub fn symbol_name_for_instance_in_crate<'tcx>(
    ...
    match symbol {
        ExportedSymbol::NonGeneric(def_id) => {
            crablangc_symbol_mangling::symbol_name_for_instance_in_crate(
                tcx,
        ...
        ExportedSymbol::DropGlue(ty) => crablangc_symbol_mangling::symbol_name_for_instance_in_crate(
            tcx,
...
pub fn linking_symbol_name_for_instance_in_crate<'tcx>(
    ...
    use crablangc_target::abi::call::Conv;
    ...
    let instance = match symbol {
        ...
        // DropGlue always use the CrabLang calling convention and thus follow the target's default
