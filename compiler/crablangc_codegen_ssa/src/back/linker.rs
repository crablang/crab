...
use crablangc_middle::middle::exported_symbols::{ExportedSymbol, SymbolExportInfo, SymbolExportKind};
use crablangc_middle::ty::TyCtxt;
use crablangc_session::config::{self, CrateType, DebugInfo, LinkerPluginLto, Lto, OptLevel, Strip};
use crablangc_session::Session;
use crablangc_target::spec::{Cc, LinkOutputKind, LinkerFlavor, Lld};
...
/// This should be acceptable because other messages from crablangc are in English anyway,
...
pub trait Linker {
    ...
    fn link_crablang_dylib(&mut self, lib: &str, path: &Path);
...
impl<'a> GccLinker<'a> {
    ...
    fn build_dylib(&mut self, out_filename: &Path) {
        ...
        if self.sess.target.is_like_osx {
            ...
            // purely to support crablangbuild right now, we should get a more
...
impl<'a> Linker for GccLinker<'a> {
    ...
    fn set_output_kind(&mut self, output_kind: LinkOutputKind, out_filename: &Path) {
        ...
        // VxWorks compiler driver introduced `--static-crt` flag specifically for crablangc,
    ...
    fn link_crablang_dylib(&mut self, lib: &str, _path: &Path) {
        self.hint_dynamic();
    ...
    fn gc_sections(&mut self, keep_metadata: bool) {
        ...
        // with CrabLang's compilation model of compiling libraries at a time into
...
impl<'a> Linker for MsvcLinker<'a> {
    ...
    fn gc_sections(&mut self, _keep_metadata: bool) {
        ...
        // slow for CrabLang and thus we disable it by default when not in
        // optimization build.
    ...
    fn debuginfo(&mut self, strip: Strip, natvis_debugger_visualizers: &[PathBuf]) {
        match strip {
            Strip::None => {
                ...
                let natvis_dir_path = self.sess.sysroot.join("lib\\crablanglib\\etc");
    ...
    fn subsystem(&mut self, subsystem: &str) {
        ...
        //
        // In CrabLang we just always generate a `main` function so we want control
...
impl<'a> Linker for L4Bender<'a> {
    ...
    fn link_crablang_dylib(&mut self, _: &str, _: &Path) {
        panic!("CrabLang dylibs not supported");
    }
...
impl<'a> Linker for AixLinker<'a> {
    ...
    fn link_crablang_dylib(&mut self, lib: &str, _: &Path) {
        self.hint_dynamic();
        self.cmd.arg(format!("-l{}", lib));
    }
...
impl<'a> Linker for PtxLinker<'a> {
    ...
    fn link_crablang_dylib(&mut self, _lib: &str, _path: &Path) {
        panic!("external dylibs not supported")
    }
