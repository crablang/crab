...
use crablangc_ast::CRATE_NODE_ID;
...
use crablangc_data_structures::temp_dir::MaybeTempDir;
use crablangc_errors::{ErrorGuaranteed, Handler};
use crablangc_fs_util::fix_windows_verbatim_for_gcc;
use crablangc_hir::def_id::{CrateNum, LOCAL_CRATE};
use crablangc_metadata::find_native_static_library;
use crablangc_metadata::fs::{emit_wrapper_file, METADATA_FILENAME};
use crablangc_middle::middle::dependency_format::Linkage;
use crablangc_middle::middle::exported_symbols::SymbolExportKind;
use crablangc_session::config::{self, CFGuard, CrateType, DebugInfo, LdImpl, Strip};
use crablangc_session::config::{OutputFilenames, OutputType, PrintRequest, SplitDwarfKind};
use crablangc_session::cstore::DllImport;
use crablangc_session::output::{check_file_is_writeable, invalid_output_for_target, out_filename};
use crablangc_session::search_paths::PathKind;
use crablangc_session::utils::NativeLibKind;
/// For all the linkers we support, and information they might
/// need out of the shared crate context before we get rid of it.
use crablangc_session::{filesearch, Session};
use crablangc_span::symbol::Symbol;
use crablangc_span::DebuggerVisualizerFile;
use crablangc_target::spec::crt_objects::{CrtObjects, LinkSelfContainedDefault};
use crablangc_target::spec::{Cc, LinkOutputKind, LinkerFlavor, LinkerFlavorCli, Lld, PanicStrategy};
use crablangc_target::spec::{RelocModel, RelroLevel, SanitizerSet, SplitDebuginfo};
...
use crate::{
    errors, looks_like_crablang_object_file, CodegenResults, CompiledModule, CrateInfo, NativeLib,
};
...
fn link_staticlib<'a>(
    ...
    let res = each_linked_rlib(
        ...
        &mut |cnum, path| {
            let lto = are_upstream_crablang_objects_already_included(sess)
            ...
            ab.add_archive(
                ...
                Box::new(move |fname: &str| {
                    ...
                    // Don't include CrabLang objects if LTO is enabled
                    if lto && looks_like_crablang_object_file(fname) {
                        return true;
                    }
...
/// Use `thorin` (crablang implementation of a dwarf packaging utility) to link DWARF objects into a
/// DWARF package.
...
fn link_natively<'a>(
    ...
    info!("{:?}", &cmd);
    let retry_on_segfault = env::var("CRABLANGC_RETRY_LINKER_ON_SEGFAULT").is_ok();
    ...
    loop {
        ...
        // about!  Unfortunately though in crablang/crablang#38878 we're
...
fn link_sanitizer_runtime(sess: &Session, linker: &mut dyn Linker, name: &str) {
    ...
    if sess.target.is_like_osx {
        ...
        let filename = format!("crablangc{}_rt.{}", channel, name);
        let path = find_sanitizer_runtime(&sess, &filename);
        let rpath = path.to_str().expect("non-utf8 component in path");
        linker.args(&["-Wl,-rpath", "-Xlinker", rpath]);
        linker.link_dylib(&filename, false, true);
    } else {
        let filename = format!("libcrablangc{}_rt.{}.a", channel, name);
        ...
        match (linker, flavor) {
            ...
            (None, Some(flavor)) => Some((
                PathBuf::from(match flavor {
                    ...
                    LinkerFlavor::Ptx => "crablang-ptx-linker",
                }),
                flavor,
            )),
            (Some(linker), None) => {
                ...
                } else if stem == "lld" || stem == "crablang-lld" {
...
fn detect_self_contained_mingw(sess: &Session) -> bool {
    ...
    // Assume `-C linker=crablang-lld` as self-contained mode
    if linker == Path::new("crablang-lld") {
        return true;
    }
...
/// Various toolchain components used during linking are used from crablangc distribution
...
fn self_contained(sess: &Session, crate_type: CrateType) -> bool {
    ...
    match sess.target.link_self_contained {
        ...
        // (https://github.com/crablang/crablang/pull/71769#issuecomment-626330237).
...
fn add_link_script(cmd: &mut dyn Linker, sess: &Session, tmpdir: &Path, crate_type: CrateType) {
    match (crate_type, &sess.target.link_script) {
        (CrateType::Cdylib | CrateType::Executable, Some(script)) => {
            ...
            let file_name = ["crablangc", &sess.target.llvm_target, "linkfile.ld"].join("-");
...
/// like `crablang_begin_unwind` but libstd ends up defining it. To get this
...
fn linker_with_args<'a>(
    ...
    // 3. Upstream crablang libraries
    // 4. Upstream native libraries
    //
    // The rationale behind this ordering is that those items lower down in the
    // list can't depend on items higher up in the list. For example nothing can
    // depend on what we just generated (e.g., that'd be a circular dependency).
    // Upstream crablang libraries are not supposed to depend on our local native
    // libraries as that would violate the structure of the DAG, in that
    // scenario they are required to link to them as well in a shared fashion.
    //
    // Note that upstream crablang libraries may contain native dependencies as
    ...
    // Upstream crablang crates and their non-dynamic native libraries.
    add_upstream_crablang_crates(
        cmd,
        sess,
        archive_builder_builder,
        codegen_results,
    ...
    // Add order-independent options determined by crablangc from its compiler options,
...
fn add_local_native_libraries(
    ...
    if sess.opts.unstable_opts.link_native_libraries {
        ...
        // used to find CrabLang crates, so some of them may have been added already by the previous
...
fn add_upstream_crablang_crates<'a>(
    cmd: &mut dyn Linker,
    sess: &'a Session,
    archive_builder_builder: &dyn ArchiveBuilderBuilder,
    codegen_results: &CodegenResults,
...
fn add_upstream_native_libraries(
    ...
    for &cnum in &codegen_results.crate_info.used_crates {
        // Static libraries are not linked here, they are linked in `add_upstream_crablang_crates`.
        // FIXME: Merge this function to `add_upstream_crablang_crates` so that all native libraries
        ...
        // results in https://github.com/crablang/crablang/pull/102832#issuecomment-1279772306).
...
fn add_static_crate<'a>(
    ...
    if !are_upstream_crablang_objects_already_included(sess)
        || ignored_for_lto(sess, &codegen_results.crate_info, cnum)
    {
    ...
    sess.prof.generic_activity_with_arg("link_altering_rlib", name).run(|| {
        ...
        let upstream_crablang_objects_already_included =
            are_upstream_crablang_objects_already_included(sess);
        let is_builtins =
        ...
        if let Err(error) = archive.add_archive(
            ...
            Box::new(move |f| {
                ...
                let is_crablang_object =
                    canonical.starts_with(&canonical_name) && looks_like_crablang_object_file(&f);

                // If we're performing LTO and this is a crablang-generated object
                // file, then we don't need the object file as it's part of the
                // LTO module. Note that `#![no_builtins]` is excluded from LTO,
                // though, so we let that object file slide.
                if upstream_crablang_objects_already_included && is_crablang_object && is_builtins {
                    return true;
                }
...
fn add_dynamic_crate(cmd: &mut dyn Linker, sess: &Session, cratepath: &Path) {
    ...
    cmd.link_crablang_dylib(&stem[prefix..], parent.unwrap_or_else(|| Path::new("")));
}

fn relevant_lib(sess: &Session, lib: &NativeLib) -> bool {
    match lib.cfg {
        Some(ref cfg) => crablangc_attr::cfg_matches(cfg, &sess.parse_sess, CRATE_NODE_ID, None),
        None => true,
    }
}

pub(crate) fn are_upstream_crablang_objects_already_included(sess: &Session) -> bool {
    match sess.lto() {
        config::Lto::Fat => true,
        config::Lto::Thin => {
...
fn get_apple_sdk_root(sdk_name: &str) -> Result<String, errors::AppleSdkRootError<'_>> {
    ...
    // SDKROOT; for crablangc, the user or build system can set it, or we
        ...
        | LinkerFlavor::WasmLld(Cc::Yes) = flavor
        {
            match ld_impl {
                LdImpl::Lld => {
                    ...
                    // by adding crablangc distribution directories to the tool search path.
