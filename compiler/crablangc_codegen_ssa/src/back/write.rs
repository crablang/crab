...
use crablangc_data_structures::profiling::TimingGuard;
use crablangc_data_structures::profiling::VerboseTimingGuard;
use crablangc_data_structures::sync::Lrc;
use crablangc_errors::emitter::Emitter;
use crablangc_errors::{translation::Translate, DiagnosticId, FatalError, Handler, Level};
use crablangc_errors::{DiagnosticMessage, Style};
use crablangc_fs_util::link_or_copy;
use crablangc_hir::def_id::{CrateNum, LOCAL_CRATE};
use crablangc_incremental::{
    copy_cgu_workproduct_to_incr_comp_cache_dir, in_incr_comp_dir, in_incr_comp_dir_sess,
};
use crablangc_metadata::EncodedMetadata;
use crablangc_middle::dep_graph::{WorkProduct, WorkProductId};
use crablangc_middle::middle::exported_symbols::SymbolExportInfo;
use crablangc_middle::ty::TyCtxt;
use crablangc_session::cgu_reuse_tracker::CguReuseTracker;
use crablangc_session::config::{self, CrateType, Lto, OutputFilenames, OutputType};
use crablangc_session::config::{Passes, SwitchWithOptPath};
use crablangc_session::Session;
use crablangc_span::source_map::SourceMap;
use crablangc_span::symbol::sym;
use crablangc_span::{BytePos, FileName, InnerSpan, Pos, Span};
use crablangc_target::spec::{MergeFunctions, SanitizerSet};
...
impl ModuleConfig {
    fn new(
            ...
            || (sess.opts.cg.linker_plugin_lto.enabled() && !no_builtins)
        {
            ...
            // native linker or crablangc itself.
            //
            // Note, however, that the linker-plugin-lto requested here is
            // explicitly ignored for `#![no_builtins]` crates. These crates are
            // specifically ignored by crablangc's LTO passes and wouldn't work if
...
pub struct CodegenContext<B: WriteBackendMethods> {
    ...
    pub split_debuginfo: crablangc_target::spec::SplitDebuginfo,
    pub split_dwarf_kind: crablangc_session::config::SplitDwarfKind,
...
struct Diagnostic {
    ...
    args: FxHashMap<DiagnosticArgName<'static>, crablangc_errors::DiagnosticArgValue<'static>>,
...
fn start_executing_work<B: ExtraBackendMethods>(
    ...
    fn queue_full_enough(items_in_queue: usize, workers_running: usize) -> bool {
        ...
        // give tokens back to the job server. That enables some other crablangc to
        ...
        // crablangc can make use of the concurrency, then we've squandered it.
        //
...
impl Translate for SharedEmitter {
    fn fluent_bundle(&self) -> Option<&Lrc<crablangc_errors::FluentBundle>> {
        None
    }

    fn fallback_fluent_bundle(&self) -> &crablangc_errors::FluentBundle {
        panic!("shared emitter attempted to translate a diagnostic");
    }
}

impl Emitter for SharedEmitter {
    fn emit_diagnostic(&mut self, diag: &crablangc_errors::Diagnostic) {
        let args: FxHashMap<Cow<'_, str>, crablangc_errors::DiagnosticArgValue<'_>> =
        ...
        loop {
            ...
            match message {
                Ok(SharedEmitterMessage::Diagnostic(diag)) => {
                    ...
                    let mut d = crablangc_errors::Diagnostic::new_with_messages(diag.lvl, diag.msg);
