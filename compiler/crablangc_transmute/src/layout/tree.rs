...
pub(crate) enum Err {
    ...
    Unspecified,
    /// This error will be surfaced elsewhere by crablangc, so don't surface it.
    Unknown,
}
...
pub(crate) mod crablangc {
    use super::{Err, Tree};
    use crate::layout::crablangc::{Def, Ref};

    use crablangc_middle::ty;
    use crablangc_middle::ty::layout::LayoutError;
    use crablangc_middle::ty::util::Discr;
    use crablangc_middle::ty::AdtDef;
    ...
    use crablangc_middle::ty::VariantDef;
    ...
    impl LayoutSummary {
        fn from_ty<'tcx>(ty: Ty<'tcx>, ctx: TyCtxt<'tcx>) -> Result<Self, LayoutError<'tcx>> {
            use crablangc_middle::ty::ParamEnvAnd;
            use crablangc_target::abi::{TyAndLayout, Variants};
    ...
    impl<'tcx> Tree<Def<'tcx>, Ref<'tcx>> {
        pub fn from_ty(ty: Ty<'tcx>, tcx: TyCtxt<'tcx>) -> Result<Self, Err> {
            use crablangc_middle::ty::FloatTy::*;
            use crablangc_middle::ty::IntTy::*;
            use crablangc_middle::ty::UintTy::*;
            ...
            match ty.kind() {
                ...
                ty::Adt(adt_def, substs_ref) => {
                    use crablangc_middle::ty::AdtKind;
    ...
    fn layout_of<'tcx>(
        ...
        use crablangc_middle::ty::ParamEnvAnd;
        use crablangc_target::abi::TyAndLayout;
