...
use crablangc_span::{BytePos, CharPos, FileName, Pos, Symbol};
...
/// Used by crablangdoc and perhaps other tools, but not by crablangc.
...
// it appears this function is called only from ppcrablang... that's
...
pub fn gather_comments(sm: &SourceMap, path: FileName, src: String) -> Vec<Comment> {
    ...
    for token in crablangc_lexer::tokenize(&text[pos..]) {
        ...
        match token.kind {
            ...
            crablangc_lexer::TokenKind::BlockComment { doc_style, .. } => {
