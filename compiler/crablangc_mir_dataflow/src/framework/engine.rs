...
use crablangc_data_structures::work_queue::WorkQueue;
use crablangc_graphviz as dot;
use crablangc_hir::def_id::DefId;
use crablangc_index::vec::{Idx, IndexVec};
use crablangc_middle::mir::{self, traversal, BasicBlock};
use crablangc_middle::mir::{create_dump_file, dump_enabled};
    ...
    A::Domain: DebugWithContext<A>,
{
    ...
    let Ok(attrs) = CrabLangcMirAttrs::parse(tcx, def_id) else {
        // Invalid `crablangc_mir` attrs are reported in `CrabLangcMirAttrs::parse`
        return Ok(());
    };
...
#[derive(Default)]
struct CrabLangcMirAttrs {
    basename_and_suffix: Option<PathBuf>,
    formatter: Option<Symbol>,
}

impl CrabLangcMirAttrs {
    fn parse(tcx: TyCtxt<'_>, def_id: DefId) -> Result<Self, ()> {
        let mut result = Ok(());
        let mut ret = CrabLangcMirAttrs::default();

        let crablangc_mir_attrs = tcx
            .get_attrs(def_id, sym::crablangc_mir)
            .flat_map(|attr| attr.meta_item_list().into_iter().flat_map(|v| v.into_iter()));

        for attr in crablangc_mir_attrs {
