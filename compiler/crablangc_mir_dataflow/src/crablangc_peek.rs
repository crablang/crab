...
use crablangc_index::bit_set::ChunkedBitSet;
use crablangc_middle::mir::MirPass;
use crablangc_middle::mir::{self, Body, Local, Location};
...
// FIXME: This should be a `MirLint`, but it needs to be moved back to `crablangc_mir_transform` first.
impl<'tcx> MirPass<'tcx> for SanityCheck {
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        use crate::has_crablangc_mir_with;
        let def_id = body.source.def_id();
        if !tcx.has_attr(def_id, sym::crablangc_mir) {
            debug!("skipping crablangc_peek::SanityCheck on {}", tcx.def_path_str(def_id));
            return;
        } else {
            debug!("running crablangc_peek::SanityCheck on {}", tcx.def_path_str(def_id));
        }
        ...
        if has_crablangc_mir_with(tcx, def_id, sym::crablangc_peek_maybe_init).is_some() {
            let flow_inits = MaybeInitializedPlaces::new(tcx, body, &mdpe)
                .into_engine(tcx, body)
                .iterate_to_fixpoint();

            sanity_check_via_crablangc_peek(tcx, body, &flow_inits);
        }

        if has_crablangc_mir_with(tcx, def_id, sym::crablangc_peek_maybe_uninit).is_some() {
            let flow_uninits = MaybeUninitializedPlaces::new(tcx, body, &mdpe)
                .into_engine(tcx, body)
                .iterate_to_fixpoint();

            sanity_check_via_crablangc_peek(tcx, body, &flow_uninits);
        }

        if has_crablangc_mir_with(tcx, def_id, sym::crablangc_peek_definite_init).is_some() {
            let flow_def_inits = DefinitelyInitializedPlaces::new(tcx, body, &mdpe)
                .into_engine(tcx, body)
                .iterate_to_fixpoint();

            sanity_check_via_crablangc_peek(tcx, body, &flow_def_inits);
        }

        if has_crablangc_mir_with(tcx, def_id, sym::crablangc_peek_liveness).is_some() {
            let flow_liveness = MaybeLiveLocals.into_engine(tcx, body).iterate_to_fixpoint();

            sanity_check_via_crablangc_peek(tcx, body, &flow_liveness);
        }

        if has_crablangc_mir_with(tcx, def_id, sym::stop_after_dataflow).is_some() {
...
/// `crablangc_peek` that have the expression form `crablangc_peek(&expr)`.
///
...
/// that call to `crablangc_peek` is ignored by the sanity check. If the
/// bit-state is a 0, then this pass emits an error message saying
/// "crablangc_peek: bit not set".
///
/// The intention is that one can write unit tests for dataflow by
/// putting code into a UI test and using `crablangc_peek` to
/// make observations about the results of dataflow static analyses.
///
/// (If there are any calls to `crablangc_peek` that do not match the
/// expression form above, then that emits an error as well, but those
/// errors are not intended to be used for unit tests.)
pub fn sanity_check_via_crablangc_peek<'tcx, A>(
    tcx: TyCtxt<'tcx>,
    body: &Body<'tcx>,
    results: &Results<'tcx, A>,
) where
    A: CrabLangcPeekAt<'tcx>,
{
    let def_id = body.source.def_id();
    debug!("sanity_check_via_crablangc_peek def_id: {:?}", def_id);
    ...
    for (bb, block_data, call) in peek_calls {
        ...
        let (statement_index, peek_rval) = block_data
            ...
            .expect(
                "call to crablangc_peek should be preceded by \
    ...
    fn from_terminator<'tcx>(
            ...
            &terminator.kind
        {
            if let ty::FnDef(def_id, substs) = *func.literal.ty().kind() {
                ...
                if !tcx.is_intrinsic(def_id) || name != sym::crablangc_peek {
                    return None;
                }
...
pub trait CrabLangcPeekAt<'tcx>: Analysis<'tcx> {
    fn peek_at(
        &self,
        tcx: TyCtxt<'tcx>,
        place: mir::Place<'tcx>,
...
impl<'tcx, A, D> CrabLangcPeekAt<'tcx> for A
where
    ...
    fn peek_at(
        ...
        match self.move_data().rev_lookup.find(place.as_ref()) {
            LookupResult::Exact(peek_mpi) => {
                ...
                debug!("crablangc_peek({:?} = &{:?}) bit_state: {}", call.arg, place, bit_state);
                if !bit_state {
...
impl<'tcx> CrabLangcPeekAt<'tcx> for MaybeLiveLocals {
    fn peek_at(
        &self,
        tcx: TyCtxt<'tcx>,
        place: mir::Place<'tcx>,
