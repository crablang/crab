...
use crablangc_const_eval::util::{call_kind, CallDesugaringKind};
...
use crablangc_infer::infer::{LateBoundRegionConversionTime, TyCtxtInferExt};
use crablangc_middle::mir::tcx::PlaceTy;
...
use crablangc_middle::ty::print::Print;
use crablangc_middle::ty::{self, Instance, Ty, TyCtxt};
use crablangc_mir_dataflow::move_paths::{InitLocation, LookupResult};
use crablangc_span::def_id::LocalDefId;
use crablangc_span::{symbol::sym, Span, Symbol, DUMMY_SP};
...
pub(crate) use crablangc_const_eval::util::CallKind;
        ...
        if ty.is_box() {
            ...
            match *ty.kind() {
                ...
                _ => {
                    ...
                    // (https://github.com/crablang/rfcs/pull/1546)
...
impl UseSpans<'_> {
    ...
    pub(super) fn var_subdiag(
        self,
        err: &mut Diagnostic,
        kind: Option<crablangc_middle::mir::BorrowKind>,
        ...
        if let UseSpans::ClosureUse { generator_kind, capture_kind_span, path_span, .. } = self {
            if capture_kind_span != path_span {
                err.subdiagnostic(match kind {
                    Some(kd) => match kd {
                        crablangc_middle::mir::BorrowKind::Shared
                        | crablangc_middle::mir::BorrowKind::Shallow
                        | crablangc_middle::mir::BorrowKind::Unique => {
                            Immute { kind_span: capture_kind_span }
                        }

                        crablangc_middle::mir::BorrowKind::Mut { .. } => {
    ...
    pub(super) fn move_spans(
        ...
        }) = &self.body[location.block].terminator
        {
            let Some((method_did, method_substs)) =
                crablangc_const_eval::util::find_self_call(
                    self.infcx.tcx,
                    &self.body,
                    target_temp,
                    location.block,
        ...
        if let UseSpans::FnSelfUse { var_span, fn_call_span, fn_span, kind } = move_spans {
            ...
            match kind {
                ...
                CallKind::Normal { self_arg, desugaring, method_did, method_substs } => {
                    ...
                    let parent_self_ty =
                        matches!(tcx.def_kind(parent_did), crablangc_hir::def::DefKind::Impl { .. })
