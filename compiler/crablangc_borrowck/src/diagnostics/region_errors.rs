...
use crablangc_errors::{Applicability, Diagnostic, DiagnosticBuilder, ErrorGuaranteed, MultiSpan};
use crablangc_hir as hir;
use crablangc_hir::def::Res::Def;
use crablangc_hir::def_id::DefId;
use crablangc_hir::intravisit::Visitor;
use crablangc_hir::GenericBound::Trait;
use crablangc_hir::QPath::Resolved;
use crablangc_hir::WherePredicate::BoundPredicate;
use crablangc_hir::{PolyTraitRef, TyKind, WhereBoundPredicate};
...
use crablangc_middle::mir::{ConstraintCategory, ReturnConstraint};
use crablangc_middle::ty::subst::InternalSubsts;
use crablangc_middle::ty::TypeVisitor;
use crablangc_middle::ty::{self, RegionVid, Ty};
use crablangc_middle::ty::{Region, TyCtxt};
...
impl<'a, 'tcx> MirBorrowckCtxt<'a, 'tcx> {
        ...
        match variance_info {
            ...
            ty::VarianceDiagInfo::Invariant { ty, param_index } => {
                let (desc, note) = match ty.kind() {
                    ty::RawPtr(ty_mut) => {
                        assert_eq!(ty_mut.mutbl, crablangc_hir::Mutability::Mut);
                        (
                    ...
                    ty::Ref(_, inner_ty, mutbl) => {
                        assert_eq!(*mutbl, crablangc_hir::Mutability::Mut);
                        (
                ...
                diag.note(&note);
                diag.help("see <https://doc.crablang.org/nomicon/subtyping.html> for more information about variance");
            }
        }
    ...
    fn suggest_move_on_borrowing_closure(&self, diag: &mut Diagnostic) {
        ...
        let mut closure_span = None::<crablangc_span::Span>;
        match expr.kind {
