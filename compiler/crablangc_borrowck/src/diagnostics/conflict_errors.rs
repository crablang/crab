use either::Either;
use crablangc_const_eval::util::CallKind;
...
use crablangc_hir::{AsyncGeneratorKind, GeneratorKind, LangItem};
...
use crablangc_middle::ty::{self, suggest_constraining_type_params, PredicateKind, Ty};
use crablangc_mir_dataflow::move_paths::{InitKind, MoveOutIndex, MovePathIndex};
...
impl<'cx, 'tcx> MirBorrowckCtxt<'cx, 'tcx> {
    ...
    fn suggest_binding_for_closure_capture_self(
        ...
        struct ExpressionFinder<'hir> {
            ...
            in_closure: bool,
            suggest_arg: String,
            hir: crablangc_middle::hir::map::Map<'hir>,
    ...
    fn report_local_value_does_not_live_long_enough(
        ...
        if let Some(annotation) = self.annotate_argument_and_return_for_borrow(borrow) {
            ...
            err.note(
                "to learn more, visit <https://doc.crablang.org/book/ch04-02-\
                ...
                for (index, argument) in sig.inputs().skip_binder().iter().enumerate() {
                    if let ty::Ref(argument_region, _, _) = argument.kind() {
                        if argument_region == return_region {
                            // Need to use the `crablangc_middle::ty` types to compare against the
                            // `return_region`. Then use the `crablangc_hir` type to get only
                            // the lifetime span.
    ...
    pub(crate) fn emit(&self, cx: &mut MirBorrowckCtxt<'_, 'tcx>, diag: &mut Diagnostic) -> String {
        match self {
            ...
            &AnnotatedBorrowFnSignature::AnonymousFunction {
                ...
                diag.note(
                    "to learn more, visit <https://doc.crablang.org/book/ch10-03-\
