...
use crablangc_middle::mir::{create_dump_file, dump_enabled, dump_mir, PassWhere};
use crablangc_middle::mir::{
    BasicBlock, Body, ClosureOutlivesSubject, ClosureRegionRequirements, LocalKind, Location,
    Promoted,
};
use crablangc_middle::ty::{self, OpaqueHiddenType, Region, RegionVid, TyCtxt};
...
use crablangc_mir_dataflow::move_paths::{InitKind, InitLocation, MoveData};
...
use crate::{
    ...
    facts::{AllFacts, AllFactsExt, CrabLangcFacts},
    invalidation,
...
pub type PoloniusOutput = Output<CrabLangcFacts>;
...
pub(super) fn dump_annotation<'tcx>(
    ...
    if !tcx.has_attr(base_def_id, sym::crablangc_regions) {
        return;
    }

    // When the enclosing function is tagged with `#[crablangc_regions]`,
