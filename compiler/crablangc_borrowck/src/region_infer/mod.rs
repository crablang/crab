...
use crablangc_data_structures::binary_search_util;
use crablangc_data_structures::frozen::Frozen;
use crablangc_data_structures::fx::{FxIndexMap, FxIndexSet};
use crablangc_data_structures::graph::scc::Sccs;
use crablangc_errors::Diagnostic;
use crablangc_hir::def_id::CRATE_DEF_ID;
use crablangc_index::vec::{IndexSlice, IndexVec};
use crablangc_infer::infer::outlives::test_type_match;
use crablangc_infer::infer::region_constraints::{GenericKind, VarInfos, VerifyBound, VerifyIfEq};
use crablangc_infer::infer::{InferCtxt, NllRegionVariableOrigin, RegionVariableOrigin};
...
use crablangc_middle::traits::ObligationCauseCode;
use crablangc_middle::ty::{self, RegionVid, Ty, TyCtxt, TypeFoldable, TypeVisitableExt};
...
/// `InferCtxt::type_must_outlive` in `crablangc_infer::infer::InferCtxt`.
#[derive(Clone, Debug)]
...
impl<'tcx> RegionInferenceContext<'tcx> {
    ...
    /// Adds annotations for `#[crablangc_regions]`; see `UniversalRegions::annotate`.
    ...
    fn check_polonius_subset_errors(
        ...
        // the outlives suggestions or the debug output from `#[crablangc_regions]` would be
        ...
        // Handle the placeholder errors as usual, until the chalk-crablangc-polonius triumvirate has
    ...
    pub(crate) fn best_blame_constraint(
        ...
        //
        // An example might be this bit of CrabLang code:
        //
        // ```crablang
        // let x: fn(&'static ()) = |_| {};
        // let y: for<'a> fn(&'a ()) = x;
