...
use crablangc_middle::mir::{self, BasicBlock, Body, Location, Place};
use crablangc_middle::ty::RegionVid;
use crablangc_middle::ty::TyCtxt;
use crablangc_mir_dataflow::impls::{EverInitializedPlaces, MaybeUninitializedPlaces};
use crablangc_mir_dataflow::ResultsVisitable;
use crablangc_mir_dataflow::{self, fmt::DebugWithContext, CallReturnPlaces, GenKill};
use crablangc_mir_dataflow::{Analysis, Direction, Results};
use std::fmt;
...
impl<'tcx> crablangc_mir_dataflow::AnalysisDomain<'tcx> for Borrows<'_, 'tcx> {
...
impl<'tcx> crablangc_mir_dataflow::GenKillAnalysis<'tcx> for Borrows<'_, 'tcx> {
