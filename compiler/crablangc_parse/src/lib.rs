...
use crablangc_ast::{AttrItem, Attribute, MetaItem};
use crablangc_ast_pretty::ppcrablang;
use crablangc_data_structures::sync::Lrc;
use crablangc_errors::{Applicability, Diagnostic, FatalError, Level, PResult};
...
use crablangc_span::{FileName, SourceFile, Span};
...
macro_rules! panictry_buffer {
    ($handler:expr, $e:expr) => {{
        use crablangc_errors::FatalError;
...
pub fn fake_token_stream_for_item(sess: &ParseSess, item: &ast::Item) -> TokenStream {
    let source = ppcrablang::item_to_string(item);
...
pub fn fake_token_stream_for_crate(sess: &ParseSess, krate: &ast::Crate) -> TokenStream {
    let source = ppcrablang::crate_to_string_for_macros(krate);
    let filename = FileName::macro_expansion_source_code(&source);
...
const CFG_ATTR_NOTE_REF: &str = "for more information, visit \
    <https://doc.crablang.org/reference/conditional-compilation.html\
