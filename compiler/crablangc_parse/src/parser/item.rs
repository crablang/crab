...
use crablangc_ast::token::{self, Delimiter, TokenKind};
use crablangc_ast::tokenstream::{DelimSpan, TokenStream, TokenTree};
use crablangc_ast::util::case::Case;
use crablangc_ast::{self as ast, AttrVec, Attribute, DUMMY_NODE_ID};
use crablangc_ast::{Async, Const, Defaultness, IsAuto, Mutability, Unsafe, UseTree, UseTreeKind};
use crablangc_ast::{BindingAnnotation, Block, FnDecl, FnSig, Param, SelfKind};
use crablangc_ast::{EnumDef, FieldDef, Generics, TraitRef, Ty, TyKind, Variant, VariantData};
use crablangc_ast::{FnHeader, ForeignItem, Path, PathSegment, Visibility, VisibilityKind};
use crablangc_ast::{MacCall, MacDelimiter};
...
use crablangc_span::edit_distance::edit_distance;
use crablangc_span::edition::Edition;
use crablangc_span::source_map::{self, Span};
...
impl<'a> Parser<'a> {
            ...
            let err = if self.check_fn_front_matter(false, Case::Sensitive) {
                ...
                match self.parse_fn(
                    ...
                    Ok(_) => {
                        ...
                        err.help("see https://doc.crablang.org/book/ch05-03-method-syntax.html for more information");
                        err
                    }
                    Err(err) => {
                        err.cancel();
                    ...
                    && self.token.kind == TokenKind::Colon
                {
                    ...
                    err.note("see <https://doc.crablang.org/book/ch05-01-defining-structs.html> for more information");
                    err.emit();
                    return Ok(ident);
                } else {
                    self.restore_snapshot(snapshot);
    ...
    fn complain_if_pub_macro(&self, vis: &Visibility, macro_rules: bool) {
        ...
        let vstr = ppcrablang::vis_to_string(vis);
    ...
    fn recover_nested_adt_item(&mut self, keyword: Symbol) -> PResult<'a, bool> {
            ...
            && self.look_ahead(1, |t| t.is_ident())
        {
            ...
            let kw_str = ppcrablang::token_to_string(&kw_token);
                ...
                Err(mut err) => {
                    ...
                    // Recover incorrect visibility order such as `async pub`
                        ...
                        if let Ok(snippet) = self.span_to_snippet(sp) {
                            ...
                            let vs = ppcrablang::vis_to_string(&current_vis);
                            let vs = vs.trim_end();
