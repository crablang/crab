...
use crablangc_ast::token::{self, Delimiter, Lit, LitKind, TokenKind};
...
use crablangc_span::{Span, SpanSnippetError, Symbol, DUMMY_SP};
use std::mem::take;
...
impl<'a> Parser<'a> {
    ...
    pub(super) fn is_lit_bad_ident(&mut self) -> Option<(usize, Symbol)> {
        ...
        if let token::Literal(Lit {
            ...
            && crablangc_ast::MetaItemLit::from_token(&self.token).is_none()
        {
    ...
    pub(super) fn expected_one_of_not_found(
        ...
        if expected.contains(&TokenType::Token(token::Semi)) {
                ...
                && self.prev_token.kind == token::CloseDelim(Delimiter::Parenthesis)
            {
                ...
                //         ,arg("2")
                //         ^
                // https://github.com/crablang/crablang/issues/72253
        ...
        if let Some((sp, likely_path)) = self.last_type_ascription.take() {
            ...
            if allow_unstable {
                ...
                if !likely_path {
                    ...
                    err.note(
                        "see issue #23416 <https://github.com/crablang/crablang/issues/23416> \
                ...
                (BinOpKind::Ge, AssocOp::GreaterEqual | AssocOp::Greater) => {
                    let expr_to_str = |e: &Expr| {
                        self.span_to_snippet(e.span)
                            .unwrap_or_else(|_| ppcrablang::expr_to_string(&e))
                    };
    ...
    pub(super) fn maybe_report_ambiguous_plus(&mut self, impl_dyn_multi: bool, ty: &Ty) {
        if impl_dyn_multi {
            self.sess.emit_err(AmbiguousPlus { sum_ty: ppcrablang::ty_to_string(&ty), span: ty.span });
        }
    }
    ...
    pub(super) fn maybe_recover_from_bad_type_plus(&mut self, ty: &Ty) -> PResult<'a, ()> {
        ...
        let sub = match &ty.kind {
            TyKind::Ref(lifetime, mut_ty) => {
                let sum_with_parens = ppcrablang::to_string(|s| {
                    s.s.word("&");
        ...
        self.sess.emit_err(BadTypePlus { ty: ppcrablang::ty_to_string(ty), span: sum_span, sub });
    ...
    fn recover_from_inc_dec(
        ...
        let mut err = self.struct_span_err(
            op_span,
            &format!("CrabLang has no {} {} operator", kind.fixity, kind.op.name()),
        );
    ...
    pub(super) fn maybe_recover_from_bad_qpath_stage_2<T: RecoverQPath>(
        ...
        let ty_str = self.span_to_snippet(ty_span).unwrap_or_else(|_| ppcrablang::ty_to_string(&ty));
        self.sess.emit_err(BadQPathStage2 {
            span: path.span,
            ty: format!("<{}>::{}", ty_str, ppcrablang::path_to_string(&path)),
        });
    ...
    fn error_on_incorrect_await(&self, lo: Span, hi: Span, expr: &Expr, is_question: bool) -> Span {
        ...
        self.sess.emit_err(IncorrectAwait {
            ...
            expr: self.span_to_snippet(expr.span).unwrap_or_else(|_| ppcrablang::expr_to_string(&expr)),
                    ...
                    _ => {
                        ...
                        if let Some(ty) = pat.to_ty() {
                            err.span_suggestion_verbose(
                                ...
                                format!("_: {}", ppcrablang::ty_to_string(&ty)),
                                Applicability::MachineApplicable,
                            );
                            err.note(rfc_note);
                        }
