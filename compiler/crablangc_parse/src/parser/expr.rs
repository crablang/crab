...
use crablangc_ast::token::{self, Delimiter, Token, TokenKind};
use crablangc_ast::tokenstream::Spacing;
use crablangc_ast::util::case::Case;
use crablangc_ast::util::classify;
use crablangc_ast::util::parser::{prec_let_scrutinee_needs_par, AssocOp, Fixity};
use crablangc_ast::visit::Visitor;
use crablangc_ast::{self as ast, AttrStyle, AttrVec, CaptureBy, ExprField, UnOp, DUMMY_NODE_ID};
use crablangc_ast::{AnonConst, BinOp, BinOpKind, FnDecl, FnRetTy, MacCall, Param, Ty, TyKind};
use crablangc_ast::{Arm, Async, BlockCheckMode, Expr, ExprKind, Label, Movability, RangeLimits};
use crablangc_ast::{ClosureBinder, MetaItemLit, StmtKind};
...
use crablangc_session::errors::{report_lit_error, ExprParenthesesNeeded};
use crablangc_session::lint::builtin::BREAK_WITH_LABEL_AND_LOOP;
use crablangc_session::lint::BuiltinLintDiagnostics;
use crablangc_span::source_map::{self, Span, Spanned};
use crablangc_span::symbol::kw::PathRoot;
...
impl<'a> Parser<'a> {
    ...
    fn should_continue_as_assoc_expr(&mut self, lhs: &Expr) -> bool {
        match (self.expr_is_complete(lhs), AssocOp::from_token(&self.token)) {
            ...
            // See https://github.com/crablang/crablang/issues/29071
            (true, None) => false,
    ...
    fn error_unexpected_after_dot(&self) {
        ...
        let actual = ppcrablang::token_to_string(&self.token);
    ...
    fn parse_dot_suffix(&mut self, self_arg: P<Expr>, lo: Span) -> PResult<'a, P<Expr>> {
        if self.token.uninterpolated_span().crablang_2018() && self.eat_keyword(kw::Await) {
    ...
    fn parse_expr_bottom(&mut self) -> PResult<'a, P<Expr>> {
        ...
        } else if self.token.uninterpolated_span().crablang_2018() {
            // `Span::crablang_2018()` is somewhat expensive; don't get it repeatedly.
        ...
        if self.token == token::Dot {
            ...
            if let Some(token) = &recovered {
                ...
                self.sess.emit_err(errors::FloatLiteralRequiresIntegerPart {
                    span: token.span,
                    correct: ppcrablang::token_to_string(token).into_owned(),
                });
            }
        }
    ...
    fn parse_expr_closure(&mut self) -> PResult<'a, P<Expr>> {
        ...
        let asyncness = if self.token.uninterpolated_span().crablang_2018() {
    ...
    fn is_try_block(&self) -> bool {
        self.token.is_keyword(kw::Try)
            ...
            && self.token.uninterpolated_span().crablang_2018()
    }
