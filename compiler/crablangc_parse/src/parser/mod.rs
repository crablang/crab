...
use crablangc_ast::token::{self, Delimiter, Nonterminal, Token, TokenKind};
use crablangc_ast::tokenstream::{AttributesData, DelimSpan, Spacing};
use crablangc_ast::tokenstream::{TokenStream, TokenTree, TokenTreeCursor};
use crablangc_ast::util::case::Case;
use crablangc_ast::AttrId;
use crablangc_ast::DUMMY_NODE_ID;
use crablangc_ast::{self as ast, AnonConst, AttrStyle, Const, DelimArgs, Extern};
use crablangc_ast::{Async, AttrArgs, AttrArgsEq, Expr, ExprKind, MacDelimiter, Mutability, StrLit};
use crablangc_ast::{HasAttrs, HasTokens, Unsafe, Visibility, VisibilityKind};
use crablangc_ast_pretty::ppcrablang;
use crablangc_data_structures::fx::FxHashMap;
use crablangc_data_structures::sync::Ordering;
...
pub struct Parser<'a> {
    ...
    /// If present, this `Parser` is not parsing CrabLang code but rather a macro call.
...
crablangc_data_structures::static_assert_size!(Parser<'_>, 288);
...
impl TokenType {
    fn to_string(&self) -> String {
        match self {
            TokenType::Token(t) => format!("`{}`", ppcrablang::token_kind_to_string(t)),
...
pub(super) fn token_descr(token: &Token) -> String {
    let name = ppcrablang::token_to_string(token).to_string();
                ...
                if first {
                    ...
                    match self.expect(t) {
                        ...
                        Err(mut expect_err) => {
                            ...
                            let token_str = ppcrablang::token_kind_to_string(t);
                            ...
                            // https://github.com/crablang/crablang/issues/72373
                            if self.prev_token.is_ident() && self.token.kind == token::DotDot {
                                let msg = format!(
                                    "if you meant to bind the contents of \
                                    the rest of the array pattern into `{}`, use `@`",
                                    ppcrablang::token_to_string(&self.prev_token)
                                );
                                expect_err
...
pub(crate) fn make_unclosed_delims_error(
    ...
    let err = MismatchedClosingDelimiter {
        spans,
        delimiter: ppcrablang::token_kind_to_string(&token::CloseDelim(found_delim)).to_string(),
