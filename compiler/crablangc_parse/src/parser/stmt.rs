...
use crablangc_ast::{AttrStyle, AttrVec, LocalKind, MacCall, MacCallStmt, MacStmtStyle};
use crablangc_ast::{Block, BlockCheckMode, Expr, ExprKind, HasAttrs, Local, Stmt};
use crablangc_ast::{StmtKind, DUMMY_NODE_ID};
use crablangc_errors::{Applicability, DiagnosticBuilder, ErrorGuaranteed, PResult};
...
impl<'a> Parser<'a> {
    ...
    fn parse_stmt_path_start(&mut self, lo: Span, attrs: AttrWrapper) -> PResult<'a, Stmt> {
        let stmt = self.collect_tokens_trailing_token(attrs, ForceCollect::No, |this, attrs| {
            ...
            Ok((this.mk_stmt(crablangc_span::DUMMY_SP, StmtKind::Expr(expr)), TrailingToken::None))
        })?;
    ...
    fn error_block_no_opening_brace_msg(
        ...
        //      bar;
        //
        // which is valid in other languages, but not CrabLang.
