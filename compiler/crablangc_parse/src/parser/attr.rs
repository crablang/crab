...
use crablangc_ast::token::{self, Delimiter, Nonterminal};
use crablangc_errors::{error_code, Diagnostic, IntoDiagnostic, PResult};
...
pub fn maybe_needs_tokens(attrs: &[ast::Attribute]) -> bool {
    ...
    attrs.iter().any(|attr| {
        ...
        attr.ident().map_or(true, |ident| {
            ident.name == sym::cfg_attr || !crablangc_feature::is_builtin_attr_name(ident.name)
        })
    })
}
