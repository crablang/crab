...
use crablangc_ast::MetaItemKind;
use crablangc_ast::{self as ast, AttrArgs, AttrArgsEq, Attribute, DelimArgs, MacDelimiter, MetaItem};
use crablangc_ast_pretty::ppcrablang;
use crablangc_errors::{Applicability, FatalError, PResult};
use crablangc_feature::{AttributeTemplate, BuiltinAttribute, BUILTIN_ATTRIBUTE_MAP};
use crablangc_session::lint::builtin::ILL_FORMED_ATTRIBUTE_INPUT;
...
pub fn check_attr(sess: &ParseSess, attr: &Attribute) {
    ...
    match attr_info {
        // `crablangc_dummy` doesn't have any restrictions specific to built-in attributes.
        Some(BuiltinAttribute { name, template, .. }) if *name != sym::crablangc_dummy => {
        ...
        kind: match &item.args {
            ...
            AttrArgs::Eq(_, AttrArgsEq::Ast(expr)) => {
                    ...
                    && let Ok(lit) = ast::MetaItemLit::from_token_lit(token_lit, expr.span)
                {
                    ...
                    // reported.
                    let msg = format!("unexpected expression: `{}`", ppcrablang::expr_to_string(expr));
