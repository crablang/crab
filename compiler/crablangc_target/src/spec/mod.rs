//! [Flexible target specification.](https://github.com/crablang/rfcs/pull/131)
//!
//! CrabLang targets a wide variety of usecases, and in the interest of flexibility,
//! allows new target triples to be defined in configuration files. Most users
//! will not need to care about these, but this is invaluable when porting CrabLang
//! to a new platform, and allows for an unprecedented level of control over how
//! the compiler works.
//!
//! # Using custom targets
//!
//! A target triple, as passed via `crablangc --target=TRIPLE`, will first be
//! compared against the list of built-in targets. This is to ease distributing
//! crablangc (no need for configuration files) and also to hold these built-in
//! targets as immutable and sacred. If `TRIPLE` is not one of the built-in
//! targets, crablangc will check if a file named `TRIPLE` exists. If it does, it
//! will be loaded as the target configuration. If the file does not exist,
//! crablangc will search each directory in the environment variable
//! `CRABLANG_TARGET_PATH` for a file named `TRIPLE.json`. The first one found will
//! be loaded. If no file is found in any of those directories, a fatal error
//! will be given.
//!
//! Projects defining their own targets should use
//! `--target=path/to/my-awesome-platform.json` instead of adding to
//! `CRABLANG_TARGET_PATH`.
//!
...
//! `arch`, and `os`. In general, options passed to crablangc with `-C` override
...
/// All linkers have some kinds of command line interfaces and crablangc needs to know which commands
...
/// `-flavor LLD_FLAVOR` argument to choose which logic to use. Our shipped `crablang-lld` in
...
pub enum StackProbeType {
    ...
    /// the first place.
    Inline,
    /// Call `__crablang_probestack` whenever stack needs to be probed.
    Call,
    /// Use inline option for LLVM versions later than specified in `min_llvm_version_for_inline`
    /// and call `__crablang_probestack` otherwise.
...
impl StackProbeType {
    ...
    // Notable past issues were crablang#83139 (fixed in 14) and crablang#84667 (fixed in 16).
...
/// Everything `crablangc` knows about how to compile for a specific target.
///
...
pub struct TargetOptions {
    ...
    /// CrabLang dependency is dynamically linked.
    pub late_link_args_dynamic: LinkArgs,
    late_link_args_dynamic_json: LinkArgsCli,
    /// Linker arguments used in addition to `late_link_args` if all CrabLang
    ...
    ///
    /// See <https://doc.crablang.org/reference/conditional-compilation.html#target_family>.
    ...
    /// Whether or not SIMD types are passed by reference in the CrabLang ABI,
    ...
    ///
    /// Workaround for: <https://github.com/crablang/crablang/issues/57356>
...
impl Target {
    ...
    pub fn is_abi_supported(&self, abi: Abi) -> Option<bool> {
        ...
        Some(match abi {
            ...
            //
            // This is well documented across MSDN. Support for this in CrabLang has been added in
            // #54576. This makes much more sense in context of Microsoft's C++ than it does in
            // CrabLang, but there isn't much leeway remaining here to change it back at the time this
                ...
                obj.remove(&name).and_then(|o| o.as_str().and_then(|s| {
                    match s.parse::<RelocModel>() {
                        ...
                        _ => return Some(Err(format!("'{}' is not a valid relocation model. \
                                                      Run `crablangc --print relocation-models` to \
                ...
                obj.remove(&name).and_then(|o| o.as_str().and_then(|s| {
                    match s.parse::<CodeModel>() {
                        ...
                        _ => return Some(Err(format!("'{}' is not a valid code model. \
                                                      Run `crablangc --print code-models` to \
                                                      see the list of supported values.", s))),
                    }
                    Some(Ok(()))
                })).unwrap_or(Ok(()))
                ...
                obj.remove(&name).and_then(|o| o.as_str().and_then(|s| {
                    match s.parse::<TlsModel>() {
                        ...
                        _ => return Some(Err(format!("'{}' is not a valid TLS model. \
                                                      Run `crablangc --print tls-models` to \
                                                      see the list of supported values.", s))),
                    }
                    Some(Ok(()))
                })).unwrap_or(Ok(()))
    ...
    ///
    /// If none is found in `$CRABLANG_TARGET_PATH`, look for a file called `target.json` inside the
    /// sysroot under the target-triple's `crablanglib` directory. Note that it could also just be a
    ...
    pub fn search(
        ...
        match *target_triple {
            TargetTriple::TargetTriple(ref target_triple) => {
                ...
                // search for a file named `target_triple`.json in CRABLANG_TARGET_PATH
                let path = {
                ...
                let target_path = env::var_os("CRABLANG_TARGET_PATH").unwrap_or_default();
                ...
                // Additionally look in the sysroot under `lib/crablanglib/<triple>/target.json`
                // as a fallback.
                let crablanglib_path = crate::target_crablanglib_path(sysroot, target_triple);
                let p = PathBuf::from_iter([
                    Path::new(sysroot),
                    Path::new(&crablanglib_path),
...
pub enum TargetTriple {
    TargetTriple(String),
    TargetJson {
        /// Warning: This field may only be used by crablangdoc. Using it anywhere else will lead to
        /// inconsistencies as it is discarded during serialization.
        path_for_crablangdoc: PathBuf,
        triple: String,
        contents: String,
    },
}
...
impl PartialEq for TargetTriple {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (
                Self::TargetJson { path_for_crablangdoc: _, triple: l_triple, contents: l_contents },
                Self::TargetJson { path_for_crablangdoc: _, triple: r_triple, contents: r_contents },
...
impl Hash for TargetTriple {
    fn hash<H: Hasher>(&self, state: &mut H) -> () {
        match self {
            ...
            TargetTriple::TargetJson { path_for_crablangdoc: _, triple, contents } => {
                1u8.hash(state);
                triple.hash(state);
                contents.hash(state)
            }
...
impl<S: Encoder> Encodable<S> for TargetTriple {
    fn encode(&self, s: &mut S) {
        match self {
            ...
            TargetTriple::TargetJson { path_for_crablangdoc: _, triple, contents } => s
    ...
    fn decode(d: &mut D) -> Self {
        match d.read_usize() {
            ...
            1 => TargetTriple::TargetJson {
                path_for_crablangdoc: PathBuf::new(),
...
impl TargetTriple {
    ...
    pub fn from_path(path: &Path) -> Result<Self, io::Error> {
        ...
        let triple = canonicalized_path
            ...
            .to_owned();
        Ok(TargetTriple::TargetJson { path_for_crablangdoc: canonicalized_path, triple, contents })
    }
    ...
    pub fn debug_triple(&self) -> String {
        ...
        match self {
            ...
            TargetTriple::TargetJson { path_for_crablangdoc: _, triple, contents: content } => {
