...
pub fn options() -> TargetOptions {
    macro_rules! args {
        ($prefix:literal) => {
            &[
                ...
                // hide some bugs and fcrablangrate someone at some point, so we should ideally
                // work towards a world where we can explicitly list symbols that are
                // supposed to be imported and have all other symbols generate errors if
                // they remain undefined.
                concat!($prefix, "--allow-undefined"),
                // CrabLang code should never have warnings, and warnings are often
                // indicative of bugs, let's prevent them.
                concat!($prefix, "--fatal-warnings"),
                // LLD only implements C++-like demangling, which doesn't match our own
                // mangling scheme. Tell LLD to not demangle anything and leave it up to
                // us to demangle these symbols later. Currently crablangc does not perform
    ...
    TargetOptions {
        ...
        // we use the LLD shipped with the CrabLang toolchain by default
        ...
        //
        // crablang/crablang#104137: cannot blindly remove this without putting in
