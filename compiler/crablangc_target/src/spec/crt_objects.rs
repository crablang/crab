...
//!
//! Use cases for crablangc linking the CRT objects explicitly:
//!     - crablangc needs to add its own CrabLang-specific objects (mingw is the example)
...
//! However, for some targets (musl, mingw) crablangc historically provides a more self-contained
//! installation not requiring users to install the native target's toolchain.
//! In that case crablangc distributes the objects as a part of the target's CrabLang toolchain
//! and falls back to linking with them manually.
//! Unlike native toolchains, crablangc only currently adds the libc's objects during linking,
//! but not gcc's. As a result crablangc cannot link with C++ static libraries (#36710)
