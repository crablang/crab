...
pub use self::Level::*;
use crablangc_ast::node_id::NodeId;
use crablangc_ast::{AttrId, Attribute};
...
use crablangc_error_messages::{DiagnosticMessage, MultiSpan};
use crablangc_hir::HashStableContext;
use crablangc_hir::HirId;
use crablangc_span::edition::Edition;
use crablangc_span::{sym, symbol::Ident, Span, Symbol};
...
pub enum Applicability {
    ...
    /// result in valid CrabLang code if it is applied.
    MaybeIncorrect,

    /// The suggestion contains placeholders like `(...)` or `{ /* fields */ }`. The suggestion
    /// cannot be applied automatically because it will not result in valid CrabLang code. The user
...
impl<HCX: crablangc_hir::HashStableContext> HashStable<HCX> for LintExpectationId {
    #[inline]
...
impl<HCX: crablangc_hir::HashStableContext> ToStableHashKey<HCX> for LintExpectationId {
...
///
/// See: <https://doc.crablang.org/crablangc/lints/levels.html>
...
pub struct Lint {
    ...
    ///
    /// See <https://crablangc-dev-guide.crablang.org/diagnostics.html#lint-naming>
    /// for naming guidelines.
    pub name: &'static str,

    /// Default level for the lint.
    ///
    /// See <https://crablangc-dev-guide.crablang.org/diagnostics.html#diagnostic-levels>
...
pub struct BufferedEarlyLint {
    ...
    /// `crablangc_lint::early::EarlyContextAndPass::check_id`.
    pub lint_id: LintId,
...
/// ///
/// /// [issue #xxxxx]: https://github.com/crablang/crablang/issues/xxxxx
/// ```
///
...
///
/// If this is a crablangdoc-only lint, then only include a brief introduction
/// with a link with the text `[crablangdoc book]` so that the validator knows
/// that this is for crablangdoc only (see BROKEN_INTRA_DOC_LINKS as an example).
///
/// Commands to view and test the documentation:
///
/// * `./x.py doc --stage=1 src/doc/crablangc --open`: Builds the crablangc book and opens it.
