//! Support code for crablangc's built in unit-test and micro-benchmarking
//! framework.
//!
...
// Module to be used by crablangc to compile tests in libtest
pub mod test {
    pub use crate::{
        assert_test_result,
        bench::Bencher,
...
const SECONDARY_TEST_INVOKER_VAR: &str = "__CRABLANG_TEST_INVOKE";
...
///
/// This is the entry point for the main function generated by `crablangc --test`
/// when panic=unwind.
...
impl FilteredTests {
    ...
    fn add_bench_as_test(
        ...
        let testfn = DynTestFn(Box::new(move || {
            bench::run_once(|b| __crablang_begin_short_backtrace(|| benchfn(b)))
        }));
    ...
    match testfn {
        ...
        DynTestFn(f) => {
            ...
            run_test_inner(
                ...
                desc,
                monitor_ch,
                Box::new(move || __crablang_begin_short_backtrace(f)),
                test_run_opts,
            )
        }
...
/// Fixed frame used to clean the backtrace with `CRABLANG_BACKTRACE=1`.
#[inline(never)]
fn __crablang_begin_short_backtrace<T, F: FnOnce() -> T>(f: F) -> T {
    let result = f();
