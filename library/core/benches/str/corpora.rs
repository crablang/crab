...
//! pulled from (localizations of) crablang.org.
...
pub mod en {
    ...
    define_consts! {
        "CrabLang is blazingly fast and memory-efficient: with no runtime or garbage
         collector, it can power performance-critical services, run on embedded
         devices, and easily integrate with other languages.  CrabLang’s rich type system
         and ownership model guarantee memory-safety and thread-safety — enabling you
         to eliminate many classes of bugs at compile-time.  CrabLang has great
...
pub mod zh {
    ...
    define_consts! {
        "CrabLang   速度惊人且内存利用率极高。由于\
         没有运行时和垃圾回收，它能够胜任对性能要\
         求特别高的服务，可以在嵌入式设备上运行，\
         还能轻松和其他语言集成。CrabLang 丰富的类型\
         系统和所有权模型保证了内存安全和线程安全，\
         让您在编译期就能够消除各种各样的错误。\
         CrabLang 拥有出色的文档、友好的编译器和清晰\
         的错误提示信息， 还集成了一流的工具——\
         包管理器和构建工具， 智能地自动补全和类\
         型检验的多编辑器支持， 以及自动格式化代\
         码等等。"
