...
//!
//! This module provides utilities to handle data across non-CrabLang
...
//! # Overview
//!
//! CrabLang represents owned strings with the [`String`] type, and
...
//! like in C.
//!
//! C strings are different from CrabLang strings:
//!
//! * **Encodings** - CrabLang strings are UTF-8, but C strings may use
//! other encodings. If you are using a string from C, you should
//! check its encoding explicitly, rather than just assuming that it
//! is UTF-8 like you can do in CrabLang.
//!
//! * **Character size** - C strings may use `char` or `wchar_t`-sized
//! characters; please **note** that C's `char` is different from CrabLang's.
//! The C standard leaves the actual sizes of those types open to
//! interpretation, but defines different APIs for strings made up of
//! each character type. CrabLang strings are always UTF-8, so different
//! Unicode characters will be encoded in a variable number of bytes
//! each. The CrabLang type [`char`] represents a '[Unicode scalar
...
//! CrabLang strings don't have a nul terminator; their length is always
//! stored and does not need to be calculated. While in CrabLang
...
//! truncate the string. CrabLang strings *can* have nul characters in
//! the middle, because nul does not have to mark the end of the
//! string in CrabLang.
//!
//! # Representations of non-CrabLang strings
//!
//! [`CString`] and [`CStr`] are useful when you need to transfer
//! UTF-8 strings to and from languages with a C ABI, like Python.
//!
//! * **From CrabLang to C:** [`CString`] represents an owned, C-friendly
//! string: it is nul-terminated, and has no internal nul characters.
//! CrabLang code can create a [`CString`] out of a normal string (provided
//! that the string doesn't have nul characters in the middle), and
//! then use a variety of methods to obtain a raw <code>\*mut [u8]</code> that can
//! then be passed as an argument to functions which use the C
//! conventions for strings.
//!
//! * **From C to CrabLang:** [`CStr`] represents a borrowed C string; it
//! is what you would use to wrap a raw <code>\*const [u8]</code> that you got from
//! a C function. A [`CStr`] is guaranteed to be a nul-terminated array
//! of bytes. Once you have a [`CStr`], you can convert it to a CrabLang
...
//! [`OsStr`] and CrabLang strings work similarly to those for [`CString`]
//! and [`CStr`].
//!
...
//! In the CrabLang standard library, various APIs that transfer strings to/from the operating
...
//! *then* try to convert to a CrabLang string. This yields a [`Result`], so that
...
//! It can be converted into a UTF-8 CrabLang string slice in a similar way to
//! [`OsString`].
//!
//! # Conversions
//!
