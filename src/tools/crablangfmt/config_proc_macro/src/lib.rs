...
pub fn config_type(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    {
        utils::debug_with_crablangfmt(&output);
    }
...
/// #[nightly_only_test]
/// #[test]
/// fn test_needs_nightly_crablangfmt() {
///   assert!(true);
/// }
/// ```
#[proc_macro_attribute]
...
///
/// ```crablang
/// # use crablangfmt_config_proc_macro::stable_only_test;
///
/// #[stable_only_test]
/// #[test]
/// fn test_needs_stable_crablangfmt() {
///   assert!(true);
/// }
/// ```
#[proc_macro_attribute]
...
/// Used to conditionally output the TokenStream for tests that should be run as part of crablangfmts
/// test suite, but should be ignored when running in the crablang/crablang test suite.
#[proc_macro_attribute]
pub fn crablangfmt_only_ci_test(_args: TokenStream, input: TokenStream) -> TokenStream {
    if option_env!("CRABLANGFMT_CI").is_some() {
        input
    } else {
