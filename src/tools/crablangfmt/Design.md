# Some thoughts on the design of crablangfmt
...
* running on a whole repo before check-in
  - in particular, to replace the `make tidy` pass on the CrabLang distro
...
* running on CrabLang-like code, specifically code which doesn't parse
...
crablangfmt and, as well as warning, can either fix problems or emit refactoring
scripts to do so.
...
least a config file which matches the CrabLang style guidelines.
...
A reformatting tool can be based on either the AST or a token stream (in CrabLang
...
I want crablangfmt to be useful as soon as possible and to always be useful. I
...
Until crablangfmt is perfect, there will always be a trade-off between doing more and
doing existing things well. I want to err on the side of the latter.
Specifically, crablangfmt should never take OK code and make it look worse. If we
...
We use the AST from [syntex_syntax], an export of crablangc's libsyntax. We use
...
We read in formatting configuration from a `crablangfmt.toml` file if there is one.
