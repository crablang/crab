...
use crablangc_ast::{ast, token::Delimiter, visit};
use crablangc_data_structures::sync::Lrc;
use crablangc_span::{symbol, BytePos, Pos, Span};
...
impl<'b, 'a: 'b> FmtVisitor<'a> {
    ...
    fn visit_stmt(&mut self, stmt: &Stmt<'_>, include_empty_semi: bool) {
        ...
        if stmt.is_empty() {
            ...
            if !snippet.is_empty() {
                ...
                // formatting where crablangfmt would preserve redundant semicolons on Items in a
                // statement position.
    ...
    fn visit_mac(&mut self, mac: &ast::MacCall, ident: Option<symbol::Ident>, pos: MacroPosition) {
        ...
        // As of v638 of the crablangc-ap-* crates, the associated span no longer includes
            ...
            if attr.has_name(depr_skip_annotation()) {
                ...
                match &attr.kind {
                    ast::AttrKind::Normal(ref normal)
                        if self.is_unknown_crablangfmt_attr(&normal.item.path.segments) =>
                    {
                        let file_name = self.parse_sess.span_to_filename(attr.span);
                        self.report.append(
                            file_name,
    ...
    fn is_unknown_crablangfmt_attr(&self, segments: &[ast::PathSegment]) -> bool {
        if segments[0].ident.to_string() != "crablangfmt" {
            return false;
        }
        !is_skip_attr(segments)
    }

    fn walk_mod_items(&mut self, items: &[crablangc_ast::ptr::P<ast::Item>]) {
    ...
    fn walk_stmts(&mut self, stmts: &[Stmt<'_>], include_current_empty_semi: bool) {
        ...
        if items.is_empty() {
            ...
            // statement position.
            //
            // Starting in crablangc-ap-* v692 (~2020-12-01) the crablangc parser now parses this as
            ...
            //
            // crablangfmt typically tosses unnecessary/redundant semicolons, and eventually we
