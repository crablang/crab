...
use crablangc_ast::token::{BinOpToken, Delimiter, Token, TokenKind};
use crablangc_ast::tokenstream::{TokenStream, TokenTree, TokenTreeCursor};
use crablangc_ast::{ast, ptr};
...
fn rewrite_macro_name(
    ...
    let name = if path.segments.len() == 1 {
        ...
        format!("{}!", ppcrablang::path_to_string(path))
    };
    match extra_ident {
    ...
    fn rewrite(
        ...
        match *self {
            ...
            MacroArgKind::Repeat(delim_tok, ref args, ref another, ref tok) => {
                ...
                let repeat_tok = ppcrablang::token_to_string(tok);
...
impl MacroArgParser {
        ...
        for tok in iter {
            ...
            match tok {
                ...
                TokenTree::Token(ref t, _) => {
                    buffer.push_str(&ppcrablang::token_to_string(t));
                }
                _ => return None,
            }
        }
    ...
    fn update_buffer(&mut self, t: &Token) {
        ...
        self.buf.push_str(&ppcrablang::token_to_string(t));
    }
...
// meaning of some tokens is a bit different here from usual CrabLang, e.g., `*`
...
pub(crate) fn convert_try_mac(
    ...
    let path = &ppcrablang::path_to_string(&mac.path);
...
impl MacroBranch {
    fn rewrite(
        ...
        // parsed as regular CrabLang code (and note that these can be escaped using
