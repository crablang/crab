...
#[clap(
    ...
    about = "This utility formats all bin and lib files of \
             the current crate using crablangfmt."
)]
pub struct Opts {
    ...
    /// Print crablangfmt version and exit
    #[clap(long = "version")]
    version: bool,
    ...
    /// Options passed to crablangfmt
    // 'raw = true' to make `--` explicit.
    #[clap(name = "crablangfmt_options", raw(true))]
    crablangfmt_options: Vec<String>,

    /// Format all packages, and also their local path-based dependencies
    #[clap(long = "all")]
    format_all: bool,

    /// Run crablangfmt in check mode
    #[clap(long = "check")]
    check: bool,
}
...
fn execute() -> i32 {
    ...
    if opts.version {
        return handle_command_status(get_crablangfmt_info(&[String::from("--version")]));
    }
    if opts.crablangfmt_options.iter().any(|s| {
        ["--print-config", "-h", "--help", "-V", "--version"].contains(&s.as_str())
            || s.starts_with("--help=")
            || s.starts_with("--print-config=")
    }) {
        return handle_command_status(get_crablangfmt_info(&opts.crablangfmt_options));
    }

    let strategy = CargoFmtStrategy::from_opts(&opts);
    let mut crablangfmt_args = opts.crablangfmt_options;
    if opts.check {
        let check_flag = "--check";
        if !crablangfmt_args.iter().any(|o| o == check_flag) {
            crablangfmt_args.push(check_flag.to_owned());
        }
    }
    if let Some(message_format) = opts.message_format {
        if let Err(msg) = convert_message_format_to_crablangfmt_args(&message_format, &mut crablangfmt_args)
        {
    ...
    if let Some(specified_manifest_path) = opts.manifest_path {
        ...
        handle_command_status(format_crate(verbosity, &strategy, crablangfmt_args, None))
    }
}

fn crablangfmt_command() -> Command {
    ...
    Command::new(crablangfmt)
}

fn convert_message_format_to_crablangfmt_args(
    message_format: &str,
    crablangfmt_args: &mut Vec<String>,
) -> Result<(), String> {
    let mut contains_emit_mode = false;
    let mut contains_check = false;
    let mut contains_list_files = false;
    for arg in crablangfmt_args.iter() {
    ...
    match message_format {
        "short" => {
            if !contains_list_files {
                crablangfmt_args.push(String::from("-l"));
            }
            Ok(())
        }
        "json" => {
            ...
            crablangfmt_args.push(String::from("--emit"));
            crablangfmt_args.push(String::from("json"));
            Ok(())
        }
        "human" => Ok(()),
        _ => Err(format!(
...
fn get_crablangfmt_info(args: &[String]) -> Result<i32, io::Error> {
    let mut command = crablangfmt_command()
        ...
        .map_err(|e| match e.kind() {
            io::ErrorKind::NotFound => io::Error::new(
                io::ErrorKind::Other,
                "Could not run crablangfmt, please make sure it is in your PATH.",
            ),
            _ => e,
        })?;
...
fn format_crate(
    ...
    crablangfmt_args: Vec<String>,
    manifest_path: Option<&Path>,
) -> Result<i32, io::Error> {
    let targets = get_targets(strategy, manifest_path)?;

    // Currently only bin and lib files get formatted.
    run_crablangfmt(&targets, &crablangfmt_args, verbosity)
}
...
pub struct Target {
    ...
    kind: String,
    /// CrabLang edition for this target.
    edition: String,
}
...
fn get_targets_recursive(
    ...
    for package in &metadata.packages {
        ...
        // It's theoretically possible someone could use a newer version of crablangfmt with
        ...
        // https://github.com/crablang/cargo/pull/8994
