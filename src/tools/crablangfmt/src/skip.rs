...
static CRABLANGFMT: &str = "crablangfmt";
static SKIP: &str = "skip";

/// Say if you're playing with `crablangfmt`'s skip attribute
pub(crate) fn is_skip_attr(segments: &[ast::PathSegment]) -> bool {
    if segments.len() < 2 || segments[0].ident.to_string() != CRABLANGFMT {
        return false;
    }
    match segments.len() {
        ...
        3 => {
            segments[1].ident.to_string() == SKIP
                && ["macros", "attributes"]
                    .iter()
                    .any(|&n| n == ppcrablang::path_segment_to_string(&segments[2]))
        }
        _ => false,
    }
}
...
fn get_skip_names(kind: &str, attrs: &[ast::Attribute]) -> Vec<String> {
    ...
    let path = format!("{}::{}::{}", CRABLANGFMT, SKIP, kind);
    for attr in attrs {
        // crablangc_ast::ast::Path is implemented partialEq
        // but it is designed for segments.len() == 1
        if let ast::AttrKind::Normal(normal) = &attr.kind {
            if ppcrablang::path_to_string(&normal.item.path) != path {
                continue;
            }
        }
