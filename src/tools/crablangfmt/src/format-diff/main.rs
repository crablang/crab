...
///
/// We only want to format crablang files by default.
...
fn run(opts: Opts) -> Result<(), FormatDiffError> {
    ...
    run_crablangfmt(&files, &ranges)
}

fn run_crablangfmt(files: &HashSet<String>, ranges: &[Range]) -> Result<(), FormatDiffError> {
    ...
    let exit_status = process::Command::new(crablangfmt)
        .args(files)
        .arg("--file-lines")
        .arg(ranges_as_json)
        .status()?;
    ...
    if !exit_status.success() {
        return Err(FormatDiffError::IoError(io::Error::new(
            io::ErrorKind::Other,
            format!("crablangfmt failed with {}", exit_status),
        )));
    }
    Ok(())
}
