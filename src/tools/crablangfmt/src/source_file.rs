    ...
    T: Write,
{
    ...
    impl From<&FileName> for crablangc_span::FileName {
        fn from(filename: &FileName) -> crablangc_span::FileName {
            match filename {
                FileName::Real(path) => {
                    crablangc_span::FileName::Real(crablangc_span::RealFileName::LocalPath(path.to_owned()))
                }
                FileName::Stdin => crablangc_span::FileName::Custom("stdin".to_owned()),
            }
        }
    }

    // SourceFile's in the SourceMap will always have Unix-style line endings
    // See: https://github.com/crablang/crablangfmt/issues/3850
    // So if the user has explicitly overridden the crablangfmt `newline_style`
