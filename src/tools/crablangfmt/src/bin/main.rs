...
use crablangfmt_nightly as crablangfmt;
...
use crate::crablangfmt::{
    load_config, CliOptions, Color, Config, Edition, EmitMode, FileLines, FileName,
    FormatReportFormatterBuilder, Input, Session, Verbosity,
};

fn main() {
    env_logger::Builder::from_env("CRABLANGFMT_LOG").init();
    let opts = make_opts();
...
fn make_opts() -> Options {
    ...
    opts.optopt(
        "",
        "config-path",
        "Recursively searches the given path for the crablangfmt.toml config file. If not \
         found reverts to the input file path",
        "[Path for the configuration file]",
    );
    opts.optopt("", "edition", "CrabLang edition to use", "[2015|2018|2021]");
    opts.optopt(
        "",
        "color",
    ...
    opts.optmulti(
        "",
        "config",
        "Set options from command line. These settings take priority over .crablangfmt.toml",
        "[key1=val1,key2=val2...]",
    );
...
fn format(
    ...
    if config.verbose() == Verbosity::Verbose {
        if let Some(path) = config_path.as_ref() {
            println!("Using crablangfmt config file {}", path.display());
        }
    }
            ...
            if config_path.is_none() {
                ...
                if local_config.verbose() == Verbosity::Verbose {
                    if let Some(path) = config_path {
                        println!(
                            "Using crablangfmt config file {} for {}",
                            path.display(),
                            file.display()
                        );
                    }
...
fn print_version() {
    ...
    println!("crablangfmt {}", version_info);
}
...
impl GetOptsOptions {
    pub fn from_matches(matches: &Matches) -> Result<GetOptsOptions> {
        ...
        let crablang_nightly = is_nightly();
