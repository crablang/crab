...
use crablangc_ast::ptr;
use crablangc_ast_pretty::ppcrablang;
use crablangc_span::{sym, symbol, BytePos, LocalExpnId, Span, Symbol, SyntaxContext};
...
pub(crate) fn depr_skip_annotation() -> Symbol {
    Symbol::intern("crablangfmt_skip")
}

#[inline]
pub(crate) fn skip_annotation() -> Symbol {
    Symbol::intern("crablangfmt::skip")
}
...
pub(crate) fn is_same_visibility(a: &Visibility, b: &Visibility) -> bool {
    match (&a.kind, &b.kind) {
        ...
        ) => ppcrablang::path_to_string(p) == ppcrablang::path_to_string(q),
...
#[inline]
// Transform `Vec<crablangc_ast::ptr::P<T>>` into `Vec<&T>`
...
fn is_skip(meta_item: &MetaItem) -> bool {
    match meta_item.kind {
        MetaItemKind::Word => {
            let path_str = ppcrablang::path_to_string(&meta_item.path);
