...
use crablangfmt_config_proc_macro::config_type;
...
pub enum ControlBraceStyle {
    /// K&R style, CrabLang community default
    AlwaysSameLine,
    /// Stroustrup style
    ClosingNextLine,
    /// Allman style
...
/// Controls how crablangfmt should handle case in hexadecimal literals.
#[config_type]
...
/// What CrabLangfmt should emit. Mostly corresponds to the `--emit` command line
/// option.
#[config_type]
pub enum EmitMode {
    /// Emits to files.
    ...
    ModifiedLines,
    /// Checks if a diff can be generated. If so, crablangfmt outputs a diff and
...
#[config_type]
/// crablangfmt format style version.
pub enum Version {
    /// 1.x.y. When specified, crablangfmt will format in the same style as 1.0.0.
    One,
    /// 2.x.y. When specified, crablangfmt will format in the the latest style.
    Two,
}
...
/// How chatty should CrabLangfmt be?
#[config_type]
pub enum Verbosity {
    /// Emit more.
    Verbose,
...
/// A set of directories, files and modules that crablangfmt should ignore.
#[derive(Default, Clone, Debug, PartialEq)]
pub struct IgnoreList {
    /// A set of path specified in crablangfmt.toml.
    path_set: HashSet<PathBuf>,
    /// A path to crablangfmt.toml.
    crablangfmt_toml_path: PathBuf,
}
...
impl<'de> Deserialize<'de> for IgnoreList {
        ...
        D: Deserializer<'de>,
    {
        ...
        Ok(IgnoreList {
            ...
            crablangfmt_toml_path: PathBuf::new(),
        })
    }
}
...
impl IgnoreList {
    pub fn add_prefix(&mut self, dir: &Path) {
        self.crablangfmt_toml_path = dir.to_path_buf();
    }

    pub fn crablangfmt_toml_path(&self) -> &Path {
        &self.crablangfmt_toml_path
    }
}
...
/// Maps client-supplied options to CrabLangfmt's internals, mostly overriding
...
impl From<Edition> for crablangc_span::edition::Edition {
...
impl PartialOrd for Edition {
    fn partial_cmp(&self, other: &Edition) -> Option<std::cmp::Ordering> {
        crablangc_span::edition::Edition::partial_cmp(&(*self).into(), &(*other).into())
    }
}

/// Controls how crablangfmt should handle leading pipes on match arms.
#[config_type]
