...
// This macro defines configuration options used in crablangfmt. Each option
...
create_config! {
    ...
    // Control options (changes the operation of crablangfmt, rather than the formatting)
    color: Color, Color::Auto, false,
        "What Color option to use when none is supplied: Always, Never, Auto";
    required_version: String, env!("CARGO_PKG_VERSION").to_owned(), false,
        "Require a specific version of crablangfmt";
    ...
    file_lines: FileLines, FileLines::all(), false,
        "Lines to format; this is not supported in crablangfmt.toml, and can only be specified \
...
impl Config {
    pub(crate) fn version_meets_requirement(&self) -> bool {
        if self.was_set().required_version() {
            ...
            if version != required_version {
                println!(
                    "Error: crablangfmt version ({}) doesn't match the required version ({})",
    ...
    /// searches parents for a `crablangfmt.toml` see `from_resolved_toml_path`.
    ///
    ...
    ///
    /// Searches for `crablangfmt.toml` beginning with `dir`, and
...
// Check for the presence of known config file names (`crablangfmt.toml, `.crablangfmt.toml`) in `dir`
//
// Return the path if a config file exists, empty if no file exists, and Error for IO errors
fn get_toml_path(dir: &Path) -> Result<Option<PathBuf>, Error> {
    const CONFIG_FILE_NAMES: [&str; 2] = [".crablangfmt.toml", "crablangfmt.toml"];
...
mod test {
    ...
    use crablangfmt_config_proc_macro::{nightly_only_test, stable_only_test};
