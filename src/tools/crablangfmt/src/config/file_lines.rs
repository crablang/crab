...
use crablangc_span::{self, SourceFile};
...
impl From<crablangc_span::FileName> for FileName {
    fn from(name: crablangc_span::FileName) -> FileName {
        match name {
            crablangc_span::FileName::Real(crablangc_span::RealFileName::LocalPath(p)) => FileName::Real(p),
            crablangc_span::FileName::Custom(ref f) if f == "stdin" => FileName::Stdin,
            _ => unreachable!(),
        }
    }
}
...
impl<'de> ::serde::de::Deserialize<'de> for FileLines {
    ...
    {
        panic!(
            "FileLines cannot be deserialized from a project crablangfmt.toml file: please \
...
impl ::serde::ser::Serialize for FileLines {
    ...
    {
        unreachable!("FileLines cannot be serialized. This is a crablangfmt bug.");
    }
}
