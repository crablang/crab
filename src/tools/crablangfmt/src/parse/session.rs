...
use crablangc_data_structures::sync::{Lrc, Send};
use crablangc_errors::emitter::{Emitter, EmitterWriter};
use crablangc_errors::translation::Translate;
use crablangc_errors::{ColorConfig, Diagnostic, Handler, Level as DiagnosticLevel, TerminalUrl};
use crablangc_session::parse::ParseSess as RawParseSess;
use crablangc_span::{
...
impl Emitter for SilentOnIgnoredFilesEmitter {
    ...
    fn emit_diagnostic(&mut self, db: &Diagnostic) {
        ...
        if let Some(primary_span) = &db.span.primary_span() {
            ...
            if let crablangc_span::FileName::Real(crablangc_span::RealFileName::LocalPath(ref path)) =
                file_name
            {
                if self
                    .ignore_path_set
...
impl ParseSess {
    ...
    pub(crate) fn default_submod_path(
        ...
        crablangc_expand::module::default_submod_path(&self.parse_sess, id, relative, dir_path).or_else(
            |e| {
                ...
                if matches!(e, ModError::FileNotFound(..)) && relative.is_some() {
                    crablangc_expand::module::default_submod_path(&self.parse_sess, id, None, dir_path)
                        .map_err(|_| e)
                } else {
                    Err(e)
                }
    ...
    pub(crate) fn is_file_parsed(&self, path: &Path) -> bool {
        self.parse_sess
            .source_map()
            .get_source_file(&crablangc_span::FileName::Real(
                crablangc_span::RealFileName::LocalPath(path.to_path_buf()),
            ))
            .is_some()
    }
    ...
    pub(crate) fn span_to_file_contents(&self, span: Span) -> Lrc<crablangc_span::SourceFile> {
        self.parse_sess
            .source_map()
...
mod tests {
    ...
    mod emitter {
        ...
        use crablangc_span::{FileName as SourceMapFileName, RealFileName};
