...
use crablangc_ast::{ast, attr, ptr};
use crablangc_errors::Diagnostic;
use crablangc_parse::{new_parser_from_file, parser::Parser as RawParser};
...
pub(crate) type DirectoryOwnership = crablangc_expand::module::DirOwnership;
pub(crate) type ModulePathSuccess = crablangc_expand::module::ModulePathSuccess;
pub(crate) type ModError<'a> = crablangc_expand::module::ModError<'a>;
...
/// A parser for CrabLang source code.
...
impl<'a> ParserBuilder<'a> {
    ...
    fn parser(
        sess: &'a crablangc_session::parse::ParseSess,
        input: Input,
    ) -> Result<crablangc_parse::parser::Parser<'a>, Option<Vec<Diagnostic>>> {
        match input {
            Input::File(ref file) => catch_unwind(AssertUnwindSafe(move || {
                new_parser_from_file(sess, file, None)
            }))
            .map_err(|_| None),
            Input::Text(text) => crablangc_parse::maybe_new_parser_from_source_str(
                sess,
                crablangc_span::FileName::Custom("stdin".to_owned()),
                text,
            )
            .map_err(Some),
        }
