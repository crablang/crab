...
use crablangc_ast::token::{Delimiter, LitKind};
use crablangc_ast::{ast, ptr, token};
...
pub(crate) fn format_expr(
    ...
    let expr_rw = match expr.kind {
        ...
        // https://github.com/crablang-dev-tools/fmt-rfcs/issues/152
...
pub(crate) fn can_be_overflowed_expr(
    ...
    match expr.kind {
        ...
        ast::ExprKind::MacCall(ref mac) => {
            match (
                crablangc_ast::ast::MacDelimiter::from_token(mac.args.delim.to_token()),
                context.config.overflow_delimited_expr(),
            ) {
