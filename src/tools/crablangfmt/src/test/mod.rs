...
use crate::crablangfmt_diff::{make_diff, print_diff, DiffLine, Mismatch, ModifiedChunk, OutputWriter};
use crate::source_file;
use crate::{is_nightly_channel, FormatReport, FormatReportFormatterBuilder, Input, Session};

use crablangfmt_config_proc_macro::nightly_only_test;
...
fn verify_config_used(path: &Path, config_name: &str) {
    for entry in fs::read_dir(path).expect(&format!(
        ...
        if path.extension().map_or(false, |f| f == "rs") {
            // check if "// crablangfmt-<config_name>:" appears in the file.
            ...
            assert!(
                filebuf
                    ...
                    .any(|l| l.starts_with(&format!("// crablangfmt-{}", config_name))),
...
// `println!`) that is used by `crablangfmt::crablangfmt_diff::print_diff`. Writing
...
// Helper function for comparing the results of crablangfmt
...
// Run crablangfmt on itself. This operation must be idempotent. We also check that
...
fn self_tests() {
    ...
    let bin_directories = vec!["cargo-fmt", "git-crablangfmt", "bin", "format-diff"];
    ...
    let (reports, count, fails) = check_files(files, &Some(PathBuf::from("crablangfmt.toml")));
    let mut warnings = 0;
    ...
    assert_eq!(
        warnings, 0,
        "CrabLangfmt's code generated {} warnings",
        warnings
    );
}
...
/// when embedding CrabLangfmt (e.g. inside RLS).
#[test]
...
fn stdin_disable_all_formatting_test() {
    ...
    let mut child = Command::new(crablangfmt().to_str().unwrap())
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
...
fn stdin_handles_mod_inner_ignore_attr() {
    // see https://github.com/crablang/crablangfmt/issues/5368
    init_log();
    let input = String::from("#![crablangfmt::skip]\n\nfn    main() {  }");
    let mut child = Command::new(crablangfmt().to_str().unwrap())
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
...
// For each file, run crablangfmt and collect the output.
...
fn idempotent_check(
    ...
    let config = if let Some(ref config_file_path) = opt_config {
        Config::from_toml_path(config_file_path).expect("`crablangfmt.toml` not found")
    } else {
...
// Reads significant comments of the form: `// crablangfmt-key: value` into a hash map.
fn read_significant_comments(file_name: &Path) -> HashMap<String, String> {
    ...
    let pattern = r"^\s*//\s*crablangfmt-([^:]+):\s*(\S+)";
    let regex = regex::Regex::new(pattern).expect("failed creating pattern 1");

    // Matches lines containing significant comments or whitespace.
    let line_regex = regex::Regex::new(r"(^\s*$)|(^\s*//\s*crablangfmt-[^:]+:\s*\S+)")
...
#[test]
fn crablangfmt_diff_make_diff_tests() {
    init_log();
...
#[test]
fn crablangfmt_diff_no_diff_test() {
    init_log();
    let diff = make_diff("a\nb\nc\nd", "a\nb\nc\nd", 3);
    assert_eq!(diff, vec![]);
}
...
fn make_temp_file(file_name: &'static str) -> TempFile {
    ...
    // Used in the CrabLang build system.
    let target_dir = var("CRABLANGFMT_TEST_DIR").unwrap_or_else(|_| ".".to_owned());
...
fn crablangfmt() -> PathBuf {
    ...
    assert!(
        ...
        "{}",
        "no crablangfmt bin, try running `cargo build` or `cargo build --release` before testing"
    );
    me
}
...
fn verify_check_works() {
    ...
    Command::new(crablangfmt().to_str().unwrap())
        .arg("--check")
...
fn verify_check_works_with_stdin() {
...

    {
        ...
        stdin
            .write_all("fn main() {}\n".as_bytes())
            .expect("Failed to write to crablangfmt --check");
    }
    let output = child
        .wait_with_output()
        .expect("Failed to wait on crablangfmt child");
    assert!(output.status.success());
}
