...
use crate::crablangfmt_diff::{make_diff, Mismatch};
use crate::{Input, Session};

const CONFIGURATIONS_FILE_NAME: &str = "Configurations.md";

// This enum is used to represent one of three text features in Configurations.md: a block of code
// with its starting line number, the name of a crablangfmt configuration option, or the value of a
// crablangfmt configuration option.
...
impl ConfigCodeBlock {
    ...
    fn code_block_valid(&self) -> bool {
        ...
        // See if code block begins with #![crablangfmt::skip].
    ...
    /// True if the code block starts with #![crablangfmt::skip]
    ...
    // - CrabLang code blocks are identifed by lines beginning with "```crablang".
    // - One explicit configuration setting is supported per code block.
    // - CrabLang code blocks with no configuration setting are illegal and cause an
    //   assertion failure, unless the snippet begins with #![crablangfmt::skip].
...
// entry for each CrabLang code block found.
