...
extern crate crablangc_builtin_macros;
...
pub use crate::crablangfmt_diff::{ModifiedChunk, ModifiedLines};
...
mod reorder;
mod rewrite;
pub(crate) mod crablangfmt_diff;
mod shape;
mod skip;
...
pub enum ErrorKind {
    ...
    #[error("`crablangfmt_skip` is deprecated; use `crablangfmt::skip`")]
    DeprecatedAttr,
    /// Used a crablangfmt:: attribute other than skip or skip::macros.
    ...
    #[error("parse error")]
    ParseError,
    /// The user mandated a version and the current version of CrabLangfmt does not
...
/// Reports on any issues that occurred during a run of CrabLangfmt.
///
...
// https://github.com/crablang/crablang/issues/78625
// https://github.com/crablang/crablang/issues/39935
...
fn format_code_block(
    ...
    // not directly outputted by crablangfmt command, but used by the comment formatter's input.
...
/// A session is a run of crablangfmt across a single or multiple inputs.
...
impl<'b, T: Write + 'b> Session<'b, T> {
    ...
    /// The main entry point for CrabLangfmt. Formats the given input according to the
