...
/// Enum indicating if the code block contains crablang based on attributes
...
impl CodeBlockAttribute {
    /// Parse comma separated attributes list. Return crablang only if all
    /// attributes are valid crablang attributes
    /// See <https://doc.crablang.org/crablangdoc/print.html#attributes>
    fn new(attributes: &str) -> CodeBlockAttribute {
        for attribute in attributes.split(',') {
            match attribute.trim() {
                "" | "crablang" | "should_panic" | "no_run" | "edition2015" | "edition2018"
                | "edition2021" => (),
                "ignore" | "compile_fail" | "text" => return CodeBlockAttribute::NotCrabLang,
                _ => return CodeBlockAttribute::NotCrabLang,
            }
        }
        CodeBlockAttribute::CrabLang
    }
}
...
const CRABLANGFMT_CUSTOM_COMMENT_PREFIX: &str = "//#### ";

fn hide_sharp_behind_comment(s: &str) -> Cow<'_, str> {
    let s_trimmed = s.trim();
    if s_trimmed.starts_with("# ") || s_trimmed == "#" {
        Cow::from(format!("{}{}", CRABLANGFMT_CUSTOM_COMMENT_PREFIX, s))
    } else {
        Cow::from(s)
    }
}
...
fn trim_custom_comment_prefix(s: &str) -> String {
    s.lines()
        .map(|line| {
            ...
            if left_trimmed.starts_with(CRABLANGFMT_CUSTOM_COMMENT_PREFIX) {
                left_trimmed.trim_start_matches(CRABLANGFMT_CUSTOM_COMMENT_PREFIX)
            } else {
                line
            }
        })
