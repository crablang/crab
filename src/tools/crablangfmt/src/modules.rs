...
pub(crate) struct Module<'a> {
    ...
    pub(crate) items: Cow<'a, ThinVec<crablangc_ast::ptr::P<ast::Item>>>,
...
impl<'a> Module<'a> {
    pub(crate) fn new(
        mod_span: Span,
        ast_mod_kind: Option<Cow<'a, ast::ModKind>>,
        mod_items: Cow<'a, ThinVec<crablangc_ast::ptr::P<ast::Item>>>,
...
/// Defines variants similar to those of [crablangc_expand::module::ModError]
#[derive(Debug, Error)]
...
impl<'ast, 'sess, 'c> ModResolver<'ast, 'sess> {
    ...
    fn visit_mod_outside_ast(
        &mut self,
        items: ThinVec<crablangc_ast::ptr::P<ast::Item>>,
    ...
    fn visit_mod_from_ast(
        &mut self,
        items: &'ast [crablangc_ast::ptr::P<ast::Item>],
    ) -> Result<(), ModuleResolutionError> {
        for item in items {
            if is_cfg_if(item) {
...
// examine the first one, since crablangc ignores the second and the subsequent ones
