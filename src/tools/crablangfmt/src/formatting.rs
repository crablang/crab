...
impl<'b, T: Write + 'b> Session<'b, T> {
    pub(crate) fn format_input_inner(
        ...
        crablangc_span::create_session_if_not_set_then(self.config.edition().into(), |_| {
    ...
    let krate = match Parser::parse_crate(input, &parse_session) {
        ...
        Err(e) => {
            ...
            should_emit_verbose(forbid_verbose, config, || {
                eprintln!("The CrabLang parser panicked");
            });
...
impl<'a> FormatLines<'a> {
    ...
    /// Returns `true` if the line with the given line number was skipped by `#[crablangfmt::skip]`.
