...
#
# `which crablangfmt` fails if crablangfmt is not found. Since we don't install
# `crablangfmt` via `crablangup`, this is the case unless we manually install it. Once
# that happens, `cargo install --force` will be called, which installs
# `crablangfmt`, `cargo-fmt`, etc to `~/.cargo/bin`. This directory is cached by
# travis (see `.travis.yml`'s "cache" key), such that build-bots that arrive
# here after the first installation will find `crablangfmt` and won't need to build
# it again.
#
...
function check_fmt_base {
    ...
    touch crablangfmt.toml
    cargo fmt --all -v |& tee crablangfmt_output
    if [[ ${PIPESTATUS[0]} != 0 ]]; then
        cat crablangfmt_output
        return 1
    fi
    cat crablangfmt_output
    ! cat crablangfmt_output | grep -q "internal error"
    if [[ $? != 0 ]]; then
        return 1
    fi
    ! cat crablangfmt_output | grep -q "warning"
    if [[ $? != 0 ]]; then
        return 1
    fi
    ! cat crablangfmt_output | grep -q "Warning"
    ...
    cargo fmt --all -- --check |& tee crablangfmt_check_output
    if [[ ${PIPESTATUS[0]} != 0 ]]; then
        cat crablangfmt_check_output
        return 1
    fi
...
case ${INTEGRATION} in
    cargo)
        git clone --depth=1 https://github.com/crablang/${INTEGRATION}.git
        cd ${INTEGRATION}
        show_head
    ...
    bitflags)
        ...
        ;;
    error-chain | tempdir)
        git clone --depth=1 https://github.com/crablang-deprecated/${INTEGRATION}.git
        cd ${INTEGRATION}
        show_head
        check_fmt_with_all_tests
        cd -
