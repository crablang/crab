...
fn cargo_fmt(args: &[&str]) -> (String, String) {
    ...
    // Ensure cargo-fmt runs the crablangfmt binary from the local target dir.
...
#[test]
fn version() {
    assert_that!(&["--version"], starts_with("crablangfmt "));
    assert_that!(&["--version"], starts_with("crablangfmt "));
    assert_that!(&["--", "-V"], starts_with("crablangfmt "));
    assert_that!(&["--", "--version"], starts_with("crablangfmt "));
}
...
#[test]
fn crablangfmt_help() {
    assert_that!(&["--", "--help"], contains("Format CrabLang code"));
    assert_that!(&["--", "-h"], contains("Format CrabLang code"));
...
fn cargo_fmt_out_of_line_test_modules() {
    // See also https://github.com/crablang/crablangfmt/issues/5119
