...
tarball_with_package_name() {
	...
	--work-dir="$WORK_DIR" \
	--output-dir="$OUT_DIR" \
	--package-name=crablangc-nightly
    try "$WORK_DIR/crablangc-nightly/install.sh" --prefix="$PREFIX_DIR"
    try test -e "$OUT_DIR/crablangc-nightly.tar.gz"
    try test -e "$OUT_DIR/crablangc-nightly.tar.xz"
}
...
combine_installers() {
	...
	--work-dir="$WORK_DIR" \
	--output-dir="$OUT_DIR" \
	--package-name=crablang \
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
    try test -e "$PREFIX_DIR/something-to-install"
    try test -e "$PREFIX_DIR/dir-to-install/foo"
    try test -e "$PREFIX_DIR/bin/program"
    try test -e "$PREFIX_DIR/bin/program2"
    try test -e "$PREFIX_DIR/bin/bad-bin"
    try test -e "$PREFIX_DIR/bin/cargo"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR"
    try test ! -e "$PREFIX_DIR/something-to-install"
    try test ! -e "$PREFIX_DIR/dir-to-install/foo"
    try test ! -e "$PREFIX_DIR/bin/program"
    try test ! -e "$PREFIX_DIR/bin/program2"
...
combine_three_installers() {
	...
	--work-dir="$WORK_DIR" \
	--output-dir="$OUT_DIR" \
	--package-name=crablang-docs \
	--component-name=crablang-docs
    try sh "$S/combine-installers.sh" \
	--work-dir="$WORK_DIR" \
	--output-dir="$OUT_DIR" \
	--package-name=crablang \
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz,$OUT_DIR/crablang-docs.tar.gz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
    try test -e "$PREFIX_DIR/something-to-install"
    try test -e "$PREFIX_DIR/dir-to-install/foo"
    try test -e "$PREFIX_DIR/bin/program"
...
combine_installers_with_overlay() {
	...
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz" \
	--non-installed-overlay="$WORK_DIR/overlay"
    try test -e "$WORK_DIR/crablang/README"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
...
combine_install_with_separate_uninstall() {
    ...
    try test -e "$PREFIX_DIR/bin/bad-bin"
    try test -e "$PREFIX_DIR/bin/cargo"
    try "$WORK_DIR/crablangc/install.sh --uninstall" --prefix="$PREFIX_DIR"
    try test ! -e "$PREFIX_DIR/something-to-install"
    try test ! -e "$PREFIX_DIR/dir-to-install/foo"
    try test ! -e "$PREFIX_DIR/bin/program"
    try test ! -e "$PREFIX_DIR/bin/program2"
...
select_components_to_install() {
	...
	--package-name=crablang \
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz,$OUT_DIR/crablang-docs.tar.gz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" --components=crablangc
    try test -e "$PREFIX_DIR/bin/program"
    try test ! -e "$PREFIX_DIR/bin/cargo"
    try test ! -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" --components=cargo
    try test ! -e "$PREFIX_DIR/bin/program"
    try test -e "$PREFIX_DIR/bin/cargo"
    try test ! -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" --components=crablang-docs
    try test ! -e "$PREFIX_DIR/bin/program"
    try test ! -e "$PREFIX_DIR/bin/cargo"
    try test -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" --components=crablangc,cargo
    try test -e "$PREFIX_DIR/bin/program"
    try test -e "$PREFIX_DIR/bin/cargo"
    try test ! -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR" --components=crablangc,cargo,crablang-docs
    try test ! -e "$PREFIX_DIR/bin/program"
    try test ! -e "$PREFIX_DIR/bin/cargo"
    try test ! -e "$PREFIX_DIR/baz"
    try test ! -e "$PREFIX_DIR/lib/packagelib"
...
select_components_to_uninstall() {
	...
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz,$OUT_DIR/crablang-docs.tar.gz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR" --components=crablangc
    try test ! -e "$PREFIX_DIR/bin/program"
    try test -e "$PREFIX_DIR/bin/cargo"
    try test -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR" --components=cargo
    try test -e "$PREFIX_DIR/bin/program"
    try test ! -e "$PREFIX_DIR/bin/cargo"
    try test -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR" --components=crablang-docs
    try test -e "$PREFIX_DIR/bin/program"
    try test -e "$PREFIX_DIR/bin/cargo"
    try test ! -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR" --components=crablangc,cargo
    try test ! -e "$PREFIX_DIR/bin/program"
    try test ! -e "$PREFIX_DIR/bin/cargo"
    try test -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
...
invalid_component() {
	...
	--package-name=crablang \
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz,$OUT_DIR/crablang-docs.tar.gz"
    expect_output_fail "unknown component" "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" --components=foo
}
...
without_components() {
	...
	--package-name=crablang \
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz,$OUT_DIR/crablang-docs.tar.gz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" --without=crablang-docs
    try test -e "$PREFIX_DIR/bin/program"
    try test -e "$PREFIX_DIR/bin/cargo"
    try test ! -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" --without=crablang-docs,cargo
    try test -e "$PREFIX_DIR/bin/program"
    try test ! -e "$PREFIX_DIR/bin/cargo"
    try test ! -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" --without=crablang-docs,crablangc
    try test ! -e "$PREFIX_DIR/bin/program"
    try test -e "$PREFIX_DIR/bin/cargo"
    try test ! -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR"
...
uninstall_without_components() {
	...
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz,$OUT_DIR/crablang-docs.tar.gz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR" --without=crablang-docs
    try test ! -e "$PREFIX_DIR/bin/program"
    try test ! -e "$PREFIX_DIR/bin/cargo"
    try test -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR" --without=crablang-docs,cargo
    try test ! -e "$PREFIX_DIR/bin/program"
    try test -e "$PREFIX_DIR/bin/cargo"
    try test -e "$PREFIX_DIR/baz"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR"
    try "$WORK_DIR/crablang/install.sh --uninstall" --prefix="$PREFIX_DIR" --without=crablang-docs,crablangc
    try test -e "$PREFIX_DIR/bin/program"
    try test ! -e "$PREFIX_DIR/bin/cargo"
    try test -e "$PREFIX_DIR/baz"
}
...
without_any_components() {
	...
	"$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" --without=crablang-docs,crablangc,cargo
}
...
uninstall_without_any_components() {
	...
	"$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" \
	--uninstall --without=crablang-docs,crablangc,cargo
}
...
list_components() {
	...
	--package-name=crablang \
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz,$OUT_DIR/crablang-docs.tar.gz"
    expect_output_ok "crablangc" "$WORK_DIR/crablang/install.sh" --list-components
    expect_output_ok "cargo" "$WORK_DIR/crablang/install.sh" --list-components
    expect_output_ok "crablang-docs" "$WORK_DIR/crablang/install.sh" --list-components
}
...
combined_remains() {
	...
	--package-name=crablang \
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz,$OUT_DIR/crablang-docs.tar.gz"
    for component in crablangc cargo crablang-docs; do
	# crablangbuild wants the original extracted package intact too
	try test -d "$WORK_DIR/$component/$component"
	try test -d "$WORK_DIR/crablang/$component"
    done
}
...
upgrade_from_future_installer_error() {
	...
	--rel-manifest-dir=crablanglib
    try "$WORK_DIR/package/install.sh" --prefix="$PREFIX_DIR"
    echo 100 > "$PREFIX_DIR/lib/crablanglib/crablang-installer-version"
    expect_fail "$WORK_DIR/package/install.sh" --prefix="$PREFIX_DIR"
}
...
# https://github.com/crablang/crablang-installer/issues/22
help() {
    try sh "$S/gen-installer.sh" \
	--image-dir="$TEST_DIR/image1" \
	--work-dir="$WORK_DIR" \
...
# https://github.com/crablang/crablang-installer/issues/31
...
docdir_default() {
	...
	--output-dir="$OUT_DIR"
    try "$WORK_DIR/package/install.sh" --prefix="$PREFIX_DIR"
    try test -e "$PREFIX_DIR/share/doc/crablang/README"
    try test -e "$PREFIX_DIR/share/doc/crablang/crablangdocs.txt"
}
...
docdir() {
    ...
    try test -e "$WORK_DIR/docdir/crablangdocs.txt"
}
runtest docdir
...
docdir_combined() {
	...
	--input-tarballs="$OUT_DIR/crablangc.tar.gz,$OUT_DIR/cargo.tar.gz"
    try mkdir "$WORK_DIR/docdir"
    try "$WORK_DIR/crablang/install.sh" --prefix="$PREFIX_DIR" --docdir="$WORK_DIR/docdir"
    try test -e "$WORK_DIR/docdir/README"
    try test -e "$WORK_DIR/docdir/crablangdocs.txt"
    try test -e "$WORK_DIR/docdir/README"
...
combine_installers_different_input_compression_formats() {
    ...
    try sh "$S/combine-installers.sh" \
        ...
        --output-dir="$OUT_DIR" \
        --package-name=crablang \
        --input-tarballs="$OUT_DIR/crablangc.tar.xz,$OUT_DIR/cargo.tar.gz"

    try test -e "${OUT_DIR}/crablang.tar.gz"
    try test -e "${OUT_DIR}/crablang.tar.xz"
}
...
generate_compression_formats_one() {
    ...
    try test ! -e "${OUT_DIR}/crablangc.tar.gz"
    try test -e "${OUT_DIR}/crablangc.tar.xz"
}
