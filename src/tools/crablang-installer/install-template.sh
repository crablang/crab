...
uninstall_components() {
    ...
    if [ -f "$abs_libdir/$TEMPLATE_REL_MANIFEST_DIR/crablang-installer-version" ]; then
	_installed_version=`cat "$_abs_libdir/$TEMPLATE_REL_MANIFEST_DIR/crablang-installer-version"`

	# Sanity check
	if [ ! -n "$_installed_version" ]; then critical_err "crablang installer version is empty"; fi
    fi
	...
	case "$_installed_version" in
	    ...
	    "$TEMPLATE_CRABLANG_INSTALLER_VERSION")
		;;
	    ...
	    * )
		...
		echo "unknown version ($_installed_version) of crablang-installer."
...
install_components() {
	...
	while read _directive; do
		...
		_file_install_path="$CFG_MANDIR/$_f"
            ...
            # component. The problem here is that the combined crablang installer
            # contains two "products": crablang and cargo; so the contents of those
...
maybe_configure_ld() {
	...
	# /etc/ld.so.conf.d/crablang-installer-v1 so ldconfig picks it up.
	# Issue #30.
	#
	# This will get rm'd when the last component is uninstalled in
	# maybe_unconfigure_ld.
	if [ "$_abs_libdir" = "/usr/local/lib" -a -d "/etc/ld.so.conf.d" ]; then
	    echo "$_abs_libdir" > "/etc/ld.so.conf.d/crablang-installer-v1-$TEMPLATE_REL_MANIFEST_DIR.conf"
	    if [ $? -ne 0 ]; then
...
maybe_unconfigure_ld() {
    ...
    rm "/etc/ld.so.conf.d/crablang-installer-v1-$TEMPLATE_REL_MANIFEST_DIR.conf" 2> /dev/null
...
do_preflight_sanity_checks() {
    ...
    touch "$CFG_LIBDIR/crablang-install-probe" > /dev/null
    if [ $? -ne 0 ]
    then
	err "can't write to destination. consider \`sudo\`."
    fi
    rm "$CFG_LIBDIR/crablang-install-probe"
...
# crablanglib or cargo
TEMPLATE_REL_MANIFEST_DIR=%%TEMPLATE_REL_MANIFEST_DIR%%
# 'CrabLang is ready to roll.' or 'Cargo is cool to cruise.'
...
TEMPLATE_CRABLANG_INSTALLER_VERSION=%%TEMPLATE_CRABLANG_INSTALLER_VERSION%%
...
if [ -n "$CFG_WITHOUT" ]; then
    ...
    # with multiple CrabLang versions (which may change the exact list of
...
write_to_file "$TEMPLATE_CRABLANG_INSTALLER_VERSION" "$abs_libdir/$TEMPLATE_REL_MANIFEST_DIR/crablang-installer-version"
