...
function showHelp() {
    console.log("crablangdoc-js options:");
...
async function main(argv) {
    ...
    if (files.length === 0) {
        console.error("crablangdoc-gui: No test selected");
        process.exit(2);
    }
    ...
    console.log(`Running ${files.length} crablangdoc-gui (${opts["jobs"]} concurrently) ...`);
...
--no-sandbox` at the end. For example: `x.py test tests/crablangdoc-gui --test-args --no-sandbox`");
            console.log("");
        }
    };
