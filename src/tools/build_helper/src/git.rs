...
/// Finds the remote for crablang/crablang.
/// For example for these remotes it will return `upstream`.
/// ```text
/// origin  https://github.com/Nilstrieb/crablang.git (fetch)
/// origin  https://github.com/Nilstrieb/crablang.git (push)
/// upstream        https://github.com/crablang/crablang (fetch)
/// upstream        https://github.com/crablang/crablang (push)
/// ```
pub fn get_crablang_lang_crablang_remote(git_dir: Option<&Path>) -> Result<String, String> {
    ...
    let crablang_lang_remote = stdout
        .lines()
        .find(|remote| remote.contains("crablang"))
        .ok_or_else(|| "crablang/crablang remote not found".to_owned())?;

    let remote_name =
        crablang_lang_remote.split('.').nth(1).ok_or_else(|| "remote name not found".to_owned())?;
    Ok(remote_name.into())
}
...
/// This will usually be crablang/crablang master, but sometimes this might not exist.
...
pub fn updated_master_branch(git_dir: Option<&Path>) -> Result<String, String> {
    let upstream_remote = get_crablang_lang_crablang_remote(git_dir)?;
