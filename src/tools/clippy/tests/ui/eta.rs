...
mod type_param_bound {
    ...
    fn test<X: Trait>() {
        // don't lint, but it's questionable that crablang requires a cast
        take(|| X::fun());
        take(X::fun as fn());
    }
}
...
// https://github.com/crablang/crablang-clippy/issues/7861
fn box_dyn() {
    fn f(_: impl Fn(usize) -> Box<dyn std::any::Any>) {}
    f(|x| Box::new(x));
}

// https://github.com/crablang/crablang-clippy/issues/5939
fn not_general_enough() {
    fn f(_: impl FnMut(&Path) -> std::io::Result<()>) {}
    f(|path| std::fs::remove_file(path));
}

// https://github.com/crablang/crablang-clippy/issues/9369
...
// https://github.com/crablang/crablang-clippy/issues/7746
