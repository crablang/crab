...
macro_rules! get_version_info {
    () => {{
        ...
        let host_compiler = std::option_env!("CRABLANGC_RELEASE_CHANNEL").map(str::to_string);
...
/// `CRABLANGC_RELEASE_CHANNEL`
#[macro_export]
macro_rules! setup_version_info {
    () => {{
        println!(
            "cargo:crablangc-env=GIT_HASH={}",
            $crate::get_commit_hash().unwrap_or_default()
        );
        println!(
            "cargo:crablangc-env=COMMIT_DATE={}",
            $crate::get_commit_date().unwrap_or_default()
        );
        println!("cargo:crablangc-env=CRABLANGC_RELEASE_CHANNEL={}", $crate::get_channel());
    }};
}
...
pub fn get_channel() -> String {
    match std::env::var("CFG_RELEASE_CHANNEL") {
        Ok(channel) => channel,
        Err(_) => {
            // if that failed, try to ask crablangc -V, do some parsing and find out
            match std::process::Command::new("crablangc")
                .arg("-V")
                .output()
                .ok()
                .and_then(|r| String::from_utf8(r.stdout).ok())
            {
                Some(crablangc_output) => {
                    if crablangc_output.contains("beta") {
                        String::from("beta")
                    } else if crablangc_output.contains("stable") {
                        String::from("stable")
                    } else {
...
mod test {
    ...
    fn test_struct_local() {
        ...
        assert_eq!(vi.crate_name, "crablangc_tools_util");
    ...
    fn test_display_local() {
        let vi = get_version_info!();
        assert_eq!(vi.to_string(), "crablangc_tools_util 0.3.0");
    }
    ...
    fn test_debug_local() {
        ...
        assert_eq!(
            s,
            "VersionInfo { crate_name: \"crablangc_tools_util\", major: 0, minor: 3, patch: 0 }"
        );
    }
}
