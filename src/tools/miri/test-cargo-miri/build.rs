...
fn main() {
    ...
    println!("cargo:crablangc-env=MIRITESTVAR=testval");

    // Test that autocfg works. This invokes CRABLANGC.
    let a = autocfg::new();
    assert!(a.probe_sysroot_crate("std"));
    assert!(!a.probe_sysroot_crate("doesnotexist"));
    assert!(a.probe_crablangc_version(1, 0));
    assert!(!a.probe_crablangc_version(2, 0));
