...
pub use version::{read_dylib_info, read_version, CrabLangCInfo};
...
impl MacroDylib {
    ...
    pub fn new(path: AbsPathBuf) -> io::Result<MacroDylib> {
        ...
        if info.version.0 < 1 || info.version.1 < 47 {
            let msg = format!("proc-macro {} built by {info:#?} is not supported by crablang-analyzer, please update your CrabLang version.", path.display());
