//! crablang-analyzer relies heavily on source code generation.
//!
//! Things like feature documentation or assist tests are implemented by
//! processing crablang-analyzer's own source code and generating the appropriate
...
pub fn list_crablang_files(dir: &Path) -> Vec<PathBuf> {
...
impl fmt::Display for Location {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(
            f,
            "https://github.com/crablang/crablang-analyzer/blob/master/{}#L{}[{}]",
            path,
            self.line,
            name.to_str().unwrap()
        )
...
fn ensure_crablangfmt(sh: &Shell) {
    let version = cmd!(sh, "crablangup run stable crablangfmt --version").read().unwrap_or_default();
    if !version.contains("stable") {
        panic!(
            "Failed to run crablangfmt from toolchain 'stable'. \
                 Please run `crablangup component add crablangfmt --toolchain stable` to install it.",
        );
    }
}
...
pub fn reformat(text: String) -> String {
    ...
    ensure_crablangfmt(&sh);
    let crablangfmt_toml = project_root().join("crablangfmt.toml");
    let mut stdout = cmd!(
        sh,
        "crablangup run stable crablangfmt --config-path {crablangfmt_toml} --config fn_single_line=true"
    )
    .stdin(text)
    .read()
    .unwrap();
