...
fn asi() {
    // Thanks, Christopher!
    //
    // https://internals.crablang.org/t/understanding-decisions-behind-semicolons/15181/29
    check(
        r#"
...
#[test]
fn stmt_boundaries() {
    // FIXME: this actually works OK under crablangc.
    check(
        r#"
macro_rules! m {
    ($($s:stmt)*) => (stringify!($($s |)*);)
...
#[test]
fn range_patterns() {
    // FIXME: crablangc thinks there are three patterns here, not one.
    check(
        r#"
macro_rules! m {
    ($($p:pat)*) => (stringify!($($p |)*);)
...
// For this test and the one below, see crablang/crablang#86730.
#[test]
