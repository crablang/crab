...
fn token_mapping_floats() {
    // Regression test for https://github.com/crablang/crablang-analyzer/issues/12216
    // (and related issues)
    check(
        r#"
// +tokenids
...
fn test_tt_with_composite_without_space() {
    ...
    // See https://github.com/crablang/crablang-analyzer/issues/6692
    check(
        r#"
...
fn test_underscore_not_greedily() {
    check(
        r#"
// `_` overlaps with `$a:ident` but crablangc matches it under the `_` token.
macro_rules! m1 {
    ($($a:ident)* _) => { ok!(); }
}
m1![a b c d _];

// `_ => ou` overlaps with `$a:expr => $b:ident` but crablangc matches it under `_ => $c:expr`.
macro_rules! m2 {
