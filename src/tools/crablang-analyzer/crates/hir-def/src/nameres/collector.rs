...
impl DefCollector<'_> {
    fn seed_with_top_level(&mut self) {
        ...
        if attrs.cfg().map_or(true, |cfg| self.cfg_options.check(&cfg) != Some(false)) {
            ...
            for attr in &*attrs {
                ...
                if attr_name.as_text().as_deref() == Some("crablangc_coherence_is_core") {
                    self.def_map.crablangc_coherence_is_core = true;
                    continue;
                }
    ...
    fn inject_prelude(&mut self, crate_attrs: &Attrs) {
        // See compiler/crablangc_builtin_macros/src/standard_library_imports.rs
        ...
        let edition = match self.def_map.edition {
            Edition::Edition2015 => name![crablang_2015],
            Edition::Edition2018 => name![crablang_2018],
            Edition::Edition2021 => name![crablang_2021],
        };
        ...
        // Fall back to the older `std::prelude::v1` for compatibility with CrabLang <1.52.0
    ...
    /// macro and pass the resulting dynamic library to crablang-analyzer.
    ///
    ...
    fn define_macro_rules(
        ...
        // Module scoping
        // In CrabLang, `#[macro_export]` macros are unconditionally visible at the
    ...
    fn record_resolved_import(&mut self, directive: &ImportDirective) {
        ...
        match import.kind {
            ImportKind::Plain | ImportKind::TypeOnly => {
                ...
                // extern crates in the crate root are special-cased to insert entries into the extern prelude: crablang/crablang#54658
                if import.is_extern_crate
                ...
                match def.take_types() {
                    Some(ModuleDefId::ModuleId(m)) => {
                        if import.is_prelude {
                            // Note: This dodgily overrides the injected prelude. The crablangc
...
impl ModCollector<'_, '_> {
    ...
    fn collect_macro_rules(&mut self, id: FileItemTreeId<MacroRules>, module: ModuleId) {
        ...
        let expander = if attrs.by_key("crablangc_builtin_macro").exists() {
            // `#[crablangc_builtin_macro = "builtin_name"]` overrides the `macro_rules!` name.
            let name;
            let name = match attrs.by_key("crablangc_builtin_macro").string_value() {
                Some(it) => {
                    // FIXME: a hacky way to create a Name from string.
                    name =
                ...
                None => {
                    let explicit_name =
                        attrs.by_key("crablangc_builtin_macro").tt_values().next().and_then(|tt| {
    ...
    fn collect_macro_def(&mut self, id: FileItemTreeId<MacroDef>, module: ModuleId) {
        ...
        let expander = if attrs.by_key("crablangc_builtin_macro").exists() {
            if let Some(expander) = find_builtin_macro(&mac.name) {
                ...
                if let Some(attr) = attrs.by_key("crablangc_builtin_macro").tt_values().next() {
                    // NOTE: The item *may* have both `#[crablangc_builtin_macro]` and `#[proc_macro_derive]`,
                    // in which case crablangc ignores the helper attributes from the latter, but it
                    // "doesn't make sense in practice" (see crablang/crablang#87027).
                    if let Some((name, helpers)) =
                        parse_macro_name_and_helper_attrs(&attr.token_trees)
                    {
                        // NOTE: crablangc overrides the name if the macro name if it's different from the
                        // macro name, but we assume it isn't as there's no such case yet. FIXME if
                        // the following assertion fails.
                        stdx::always!(
                            name == mac.name,
                            "built-in macro {} has #[crablangc_builtin_macro] which declares different name {}",
                            mac.name,
                            name
                        );
