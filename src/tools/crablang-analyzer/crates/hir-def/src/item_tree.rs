...
//! "crablang-analyzer: Debug ItemTree".
//!
//! Compared to crablangc's architecture, `ItemTree` has properties from both crablangc's AST and HIR: many
//! syntax-level CrabLang features are already desugared to simpler forms in the `ItemTree`, but name
//! resolution has not yet been performed. `ItemTree`s are per-file, while crablangc's AST and HIR are
