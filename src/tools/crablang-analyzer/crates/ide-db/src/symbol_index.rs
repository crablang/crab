...
//! for each library (which is assumed to never change) and an FST for each CrabLang
...
// the filtering via the `crablang-analyzer.workspace.symbol.search.scope` and
// `crablang-analyzer.workspace.symbol.search.kind` settings.
//
// |===
// | Editor  | Shortcut
//
