...
/// This clones crablangc repo, and so is not worth to keep up-to-date. We update
...
fn sourcegen_lint_completions() {
    ...
    let crablang_repo = project_root().join("./target/crablang");
    if !crablang_repo.exists() {
        cmd!(sh, "git clone --depth=1 https://github.com/crablang/crablang {crablang_repo}")
            .run()
            .unwrap();
    }
...
",
    ...
    generate_feature_descriptor(&mut contents, &crablang_repo.join("src/doc/unstable-book/src"));
    contents.push('\n');

    let lints_json = project_root().join("./target/clippy_lints.json");
    cmd!(
        sh,
        "curl https://crablang.github.io/crablang-clippy/master/lints.json --output {lints_json}"
    )
    .run()
    .unwrap();
...
fn generate_lint_descriptor(sh: &Shell, buf: &mut String) {
    // FIXME: crablangdoc currently requires an input file for -Whelp cc https://github.com/crablang/crablang/pull/88831
    let file = project_root().join(file!());
    let stdout = cmd!(sh, "crablangdoc -W help {file}").read().unwrap();
    let start_lints = stdout.find("----  -------  -------").unwrap();
    let start_lint_groups = stdout.find("----  ---------").unwrap();
    let start_lints_crablangdoc =
        stdout.find("Lint checks provided by plugins loaded by this crate:").unwrap();
    let start_lint_groups_crablangdoc =
    ...
    buf.push('\n');
    buf.push_str(r#"pub const CRABLANGDOC_LINTS: &[Lint] = &["#);
    buf.push('\n');

    let lints_crablangdoc =
        stdout[start_lints_crablangdoc..].lines().skip(2).take_while(|l| !l.is_empty()).map(|line| {
            let (name, rest) = line.trim().split_once(char::is_whitespace).unwrap();
            let (_default_level, description) =
                rest.trim().split_once(char::is_whitespace).unwrap();
            (name.trim(), Cow::Borrowed(description.trim()), vec![])
        });
    let lint_groups_crablangdoc =
        stdout[start_lint_groups_crablangdoc..].lines().skip(2).take_while(|l| !l.is_empty()).map(
            |line| {
                let (name, lints) = line.trim().split_once(char::is_whitespace).unwrap();
                (
                    name.trim(),
    ...
    let lints_crablangdoc = lints_crablangdoc
        .chain(lint_groups_crablangdoc)
        .sorted_by(|(ident, ..), (ident2, ..)| ident.cmp(ident2))
        .collect::<Vec<_>>();

    for (name, description, ..) in &lints_crablangdoc {
        push_lint_completion(buf, &name.replace('-', "_"), description)
    }
    buf.push_str("];\n");

    buf.push_str(r#"pub const CRABLANGDOC_LINT_GROUPS: &[LintGroup] = &["#);
    for (name, description, children) in &lints_crablangdoc {
        if !children.is_empty() {
            push_lint_group(buf, &name.replace('-', "_"), description, children);
        }
    }
