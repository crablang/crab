...
use ide_db::{
    active_parameter::ActiveParameter, base_db::FileId, defs::Definition, crablang_doc::is_crablang_fence,
    SymbolKind,
};
use syntax::{
...
const CRABLANGDOC_FENCE_LENGTH: usize = 3;
...
pub(super) fn doc_comment(
    ...
    for attr in attributes.by_key("doc").attrs() {
        ...
        for line in line.split('\n') {
            ...
            match CRABLANGDOC_FENCES.into_iter().find_map(|fence| line.find(fence)) {
                Some(idx) => {
                    is_codeblock = !is_codeblock;
                    // Check whether code is crablang by inspecting fence guards
                    let guards = &line[idx + CRABLANGDOC_FENCE_LENGTH..];
                    let is_crablang = is_crablang_fence(guards);
                    is_doctest = is_codeblock && is_crablang;
                    continue;
                }
