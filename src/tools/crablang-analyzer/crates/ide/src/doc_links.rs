...
// | Editor  | Action Name
//
// | VS Code | **crablang-analyzer: Open Docs**
// |===
...
// FIXME:
// BUG: For Option::Some
// Returns https://doc.crablang.org/nightly/core/prelude/v1/enum.Option.html#variant.Some
// Instead of https://doc.crablang.org/nightly/core/option/enum.Option.html
//
// This should cease to be a problem if RFC2988 (Stable CrabLangdoc URLs) is implemented
// https://github.com/crablang/rfcs/pull/2988
...
///
/// ```ignore
/// https://doc.crablang.org/std/iter/trait.Iterator.html#tymethod.next
/// ^^^^^^^^^^^^^^^^^^^^^^^^^^
/// ```
fn get_doc_base_url(db: &RootDatabase, def: Definition) -> Option<Url> {
    // special case base url of `BuiltinType` to core
    // https://github.com/crablang/crablang-analyzer/issues/12250
    if let Definition::BuiltinType(..) = def {
        return Url::parse("https://doc.crablang.org/nightly/core/").ok();
    };
    ...
    let base = match db.crate_graph()[krate.into()].origin {
        ...
        CrateOrigin::Lang(
            ...
            format!("https://doc.crablang.org/nightly/{origin}")
        }
        _ => {
...
/// Get the filename and extension generated for a symbol by crablangdoc.
///
/// ```ignore
/// https://doc.crablang.org/std/iter/trait.Iterator.html#tymethod.next
///                                    ^^^^^^^^^^^^^^^^^^^
...
fn filename_and_frag_for_def(
    ...
    let res = match def {
        ...
        Definition::Module(m) => match m.name(db) {
            // `#[doc(keyword = "...")]` is internal used only by crablang compiler
...
fn get_assoc_item_fragment(db: &dyn HirDatabase, assoc_item: hir::AssocItem) -> Option<String> {
    Some(match assoc_item {
        AssocItem::Function(function) => {
            ...
            // CrabLangdoc makes this decision based on whether a method 'has defaultness'.
