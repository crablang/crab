...
// | Editor  | Action Name
//
// | VS Code | **crablang-analyzer: Expand macro recursively**
// |===
//
...
pub(crate) fn expand_macro(db: &RootDatabase, position: FilePosition) -> Option<ExpandedMacro> {
    ...
    // due to how crablang-analyzer works internally, we need to special case derive attributes,
...
fn _format(
    ...
    let mut cmd = std::process::Command::new(toolchain::crablangfmt());
    cmd.arg("--edition");
    cmd.arg(edition.to_string());

    let mut crablangfmt = cmd
    ...
    std::io::Write::write_all(&mut crablangfmt.stdin.as_mut()?, expansion.as_bytes()).ok()?;

    let output = crablangfmt.wait_with_output().ok()?;
