...
// | Editor  | Action Name
//
// | VS Code | **crablang-analyzer: Peek Related Tests**
// |===
...
const CRABLANGDOC_CODE_BLOCK_ATTRIBUTES_RUNNABLE: &[&str] =
    &["", "crablang", "should_panic", "edition2015", "edition2018", "edition2021"];
...
fn has_runnable_doc_test(attrs: &hir::Attrs) -> bool {
    attrs.docs().map_or(false, |doc| {
        ...
        for line in String::from(doc).lines() {
            if let Some(header) =
                CRABLANGDOC_FENCES.into_iter().find_map(|fence| line.strip_prefix(fence))
            {
                ...
                if in_code_block
                    && header
                        .split(',')
                        .all(|sub| CRABLANGDOC_CODE_BLOCK_ATTRIBUTES_RUNNABLE.contains(&sub.trim()))
                {
                    return true;
                }
            }
