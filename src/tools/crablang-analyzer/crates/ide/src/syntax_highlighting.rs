...
//
// crablang-analyzer highlights the code semantically.
// For example, `Bar` in `foo::Bar` might be colored differently depending on whether `Bar` is an enum or a trait.
// crablang-analyzer does not specify colors directly, instead it assigns a tag (like `struct`) and a set of modifiers (like `declaration`) to each token.
...
// .Token Tags
//
// CrabLang-analyzer currently emits the following token tags:
//
// - For items:
// +
// [horizontal]
...
// unresolvedReference:: Emitted for unresolved references, names that crablang-analyzer can't find the definition of.
...
//
// CrabLang-analyzer currently emits the following token modifiers:
//
// [horizontal]
...
// injected:: Emitted for doc-string injected highlighting like crablang source blocks in documentation.
...
fn traverse(
    ...
    for event in root.preorder_with_tokens() {
        ...
        // FIXME: do proper macro def highlighting https://github.com/crablang/crablang-analyzer/issues/6232
