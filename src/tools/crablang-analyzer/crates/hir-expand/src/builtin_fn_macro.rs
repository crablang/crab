...
fn concat_expand(
    ...
    for (i, mut t) in tt.token_trees.iter().enumerate() {
        ...
        // implement crablangc's model. cc https://github.com/crablang/crablang-analyzer/pull/10623
...
fn include_str_expand(
    ...
    // it's unusual to `include_str!` a CrabLang file), but we can return an empty string.
...
fn env_expand(
    ...
    let s = get_env_inner(db, arg_id, &key).unwrap_or_else(|| {
        // The only variable crablang-analyzer ever sets is `OUT_DIR`, so only diagnose that to avoid
