...
/// CrabLang are macros: each macro can be thought of as producing a "temporary
...
impl MacroCallKind {
    ...
    /// Returns the original file range that best describes the location of this macro call.
    ///
    /// Here we try to roughly match what crablangc does to improve diagnostics: fn-like macros
...
/// In CrabLang, macros expand token trees to token trees. When we want to turn a
