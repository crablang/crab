//! Discovery of `cargo` & `crablangc` executables.
...
pub fn crablangc() -> PathBuf {
    get_path_for_executable("crablangc")
}

pub fn crablangup() -> PathBuf {
    get_path_for_executable("crablangup")
}

pub fn crablangfmt() -> PathBuf {
    get_path_for_executable("crablangfmt")
}
...
fn get_path_for_executable(executable_name: &'static str) -> PathBuf {
    ...
    //      example: for cargo, this checks $CARGO environment variable; for crablangc, $CRABLANGC; etc
    // 2) `<executable_name>`
    //      example: for cargo, this tries just `cargo`, which will succeed if `cargo` is on the $PATH
    // 3) `$CARGO_HOME/bin/<executable_name>`
    //      where $CARGO_HOME defaults to ~/.cargo (see https://doc.crablang.org/cargo/guide/cargo-home.html)
    //      example: for cargo, this tries $CARGO_HOME/bin/cargo, or ~/.cargo/bin/cargo if $CARGO_HOME is unset.
    //      It seems that this is a reasonable place to try for cargo, crablangc, and crablangup
