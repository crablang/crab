...
use chalk_solve::{logging_db::LoggingCrabLangIrDatabase, Solver};
...
fn solve(
    ...
    let mut solve = || {
        ...
        let solution = if is_chalk_print() {
            let logging_db =
                LoggingCrabLangIrDatabaseLoggingOnDrop(LoggingCrabLangIrDatabase::new(context));
...
struct LoggingCrabLangIrDatabaseLoggingOnDrop<'a>(LoggingCrabLangIrDatabase<Interner, ChalkContext<'a>>);

impl<'a> Drop for LoggingCrabLangIrDatabaseLoggingOnDrop<'a> {
    fn drop(&mut self) {
