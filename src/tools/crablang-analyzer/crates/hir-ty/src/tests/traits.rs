...
fn associated_type_placeholder() {
    // inside the generic function, the associated type gets normalized to a placeholder `ApplL::Out<T>` [https://crablang.github.io/crablangc-guide/traits/associated-types.html#placeholder-associated-types].
    check_types(
        r#"
pub trait ApplyL {
    type Out;
...
struct SyntaxNode<L> {}
fn foo() -> impl Iterator<Item = SyntaxNode<CrabLanguage>> {}
...
fn unselected_projection_in_trait_env_cycle_3() {
    // this is a cycle for crablangc; we currently accept it
    check_types(
        r#"
//- /main.rs
trait Trait {
...
struct G {}
struct T {}
struct CrabLangAnalyzer;
impl Types for CrabLangAnalyzer {
    type TokenStream = T;
    type Group = G;
}

fn make<T>() -> T { loop {} }
impl TokenStream for CrabLangAnalyzer {
...
fn gats_with_impl_trait() {
    // FIXME: the last function (`fn i()`) is not valid CrabLang as of this writing because you cannot
