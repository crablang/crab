...
fn infer_nested_generics_crash() {
    // another crash found typechecking crablangc
    check_infer(
        r#"
        struct Canonical<V> {
            value: V,
...
fn issue_3999_struct() {
    // crablang-analyzer should not panic on seeing this malformed
    // record pattern.
    check_infer(
        r#"
        struct Bar {
...
#[test]
fn cfg_tail() {
    // https://github.com/crablang/crablang-analyzer/issues/8378
    check_infer(
        r#"
        fn fake_tail(){
            { "first" }
