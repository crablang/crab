//! The implementation of `CrabLangIrDatabase` for Chalk, which provides information
...
use chalk_solve::crablang_ir::{self, OpaqueTyDatumBound, WellKnownTrait};
...
pub(crate) type AssociatedTyDatum = chalk_solve::crablang_ir::AssociatedTyDatum<Interner>;
pub(crate) type TraitDatum = chalk_solve::crablang_ir::TraitDatum<Interner>;
pub(crate) type StructDatum = chalk_solve::crablang_ir::AdtDatum<Interner>;
pub(crate) type ImplDatum = chalk_solve::crablang_ir::ImplDatum<Interner>;
pub(crate) type OpaqueTyDatum = chalk_solve::crablang_ir::OpaqueTyDatum<Interner>;

pub(crate) type AssocTypeId = chalk_ir::AssocTypeId<Interner>;
pub(crate) type TraitId = chalk_ir::TraitId<Interner>;
pub(crate) type AdtId = chalk_ir::AdtId<Interner>;
pub(crate) type ImplId = chalk_ir::ImplId<Interner>;
pub(crate) type AssociatedTyValueId = chalk_solve::crablang_ir::AssociatedTyValueId<Interner>;
pub(crate) type AssociatedTyValue = chalk_solve::crablang_ir::AssociatedTyValue<Interner>;
pub(crate) type FnDefDatum = chalk_solve::crablang_ir::FnDefDatum<Interner>;
pub(crate) type Variances = chalk_ir::Variances<Interner>;

impl<'a> chalk_solve::CrabLangIrDatabase<Interner> for ChalkContext<'a> {
    ...
    fn adt_repr(&self, _struct_id: AdtId) -> Arc<crablang_ir::AdtRepr<Interner>> {
        // FIXME: keep track of these
        Arc::new(crablang_ir::AdtRepr { c: false, packed: false, int: None })
    }
    ...
    ) -> Arc<crablang_ir::FnDefDatum<Interner>> {
    ...
    fn well_known_trait_id(
        &self,
        well_known_trait: crablang_ir::WellKnownTrait,
    ...
    ) -> crablang_ir::ClosureKind {
        // Fn is the closure kind that implements all three traits
        crablang_ir::ClosureKind::Fn
    }
    fn closure_inputs_and_output(
        &self,
        _closure_id: chalk_ir::ClosureId<Interner>,
        substs: &chalk_ir::Substitution<Interner>,
    ) -> chalk_ir::Binders<crablang_ir::FnDefInputsAndOutputDatum<Interner>> {
        ...
        let io = crablang_ir::FnDefInputsAndOutputDatum {
    ...
    fn adt_size_align(&self, _id: chalk_ir::AdtId<Interner>) -> Arc<crablang_ir::AdtSizeAlign> {
        // FIXME
        Arc::new(crablang_ir::AdtSizeAlign::from_one_zst(false))
    }
    ...
    fn generator_datum(
    ...
    ) -> std::sync::Arc<chalk_solve::crablang_ir::GeneratorDatum<Interner>> {
        ...
        let input_output = crablang_ir::GeneratorInputOutputDatum {
        ...
        let movability = match movability {
            Movability::Static => crablang_ir::Movability::Static,
            Movability::Movable => crablang_ir::Movability::Movable,
        };

        Arc::new(crablang_ir::GeneratorDatum { movability, input_output })
    }
    fn generator_witness_datum(
        &self,
        id: chalk_ir::GeneratorId<Interner>,
    ) -> std::sync::Arc<chalk_solve::crablang_ir::GeneratorWitnessDatum<Interner>> {
        // FIXME: calculate inner types
        let inner_types =
            crablang_ir::GeneratorWitnessExistential { types: wrap_empty_binders(vec![]) };
        ...
        Arc::new(crablang_ir::GeneratorWitnessDatum { inner_types })
    }
...
pub(crate) fn associated_ty_data_query(
    ...
    if !ctx.unsized_types.borrow().contains(&self_ty) {
        ...
        let sized_bound = sized_trait.into_iter().map(|sized_trait| {
            let trait_bound =
                crablang_ir::TraitBound { trait_id: sized_trait, args_no_self: Default::default() };
            let inline_bound = crablang_ir::InlineBound::TraitBound(trait_bound);
    ...
    //        (crablang-analyzer#9052)
    // let where_clauses = convert_where_clauses(db, type_alias.into(), &bound_vars);
    let bound_data = crablang_ir::AssociatedTyDatumBound { bounds, where_clauses: vec![] };
...
pub(crate) fn trait_datum_query(
    ...
    let flags = crablang_ir::TraitFlags {
    ...
    let trait_datum_bound = crablang_ir::TraitDatumBound { where_clauses };
...
pub(crate) fn struct_datum_query(
    ...
    let flags = crablang_ir::AdtFlags {
        upstream,
    ...
    let variant = crablang_ir::AdtVariantDatum {
        fields: Vec::new(), // FIXME add fields (only relevant for auto traits),
    };
    let struct_datum_bound = crablang_ir::AdtDatumBound { variants: vec![variant], where_clauses };
    let struct_datum = StructDatum {
        // FIXME set ADT kind
        kind: crablang_ir::AdtKind::Struct,
        id: struct_id,
...
fn impl_def_datum(
    ...
    let impl_type = if impl_id.lookup(db.upcast()).container.krate() == krate {
        crablang_ir::ImplType::Local
    } else {
        crablang_ir::ImplType::External
    };
    ...
    let polarity = if negative { crablang_ir::Polarity::Negative } else { crablang_ir::Polarity::Positive };

    let impl_datum_bound = crablang_ir::ImplDatumBound { trait_ref, where_clauses };
    let trait_data = db.trait_data(trait_);
...
fn type_alias_associated_ty_value(
    ...
    let value_bound = crablang_ir::AssociatedTyValueBound { ty };
    let value = crablang_ir::AssociatedTyValue {
...
pub(crate) fn fn_def_datum_query(
    ...
    let bound = crablang_ir::FnDefDatumBound {
        // Note: Chalk doesn't actually use this information yet as far as I am aware, but we provide it anyway
        inputs_and_output: chalk_ir::Binders::empty(
            Interner,
            crablang_ir::FnDefInputsAndOutputDatum {
                argument_types: sig.params().to_vec(),
                return_type: sig.ret().clone(),
            }
            .shifted_in(Interner),
...
pub(super) fn generic_predicate_to_inline_bound(
    ...
    self_ty: &Ty,
) -> Option<chalk_ir::Binders<crablang_ir::InlineBound<Interner>>> {
    ...
    match pred {
        WhereClause::Implemented(trait_ref) => {
            ...
            let args_no_self = trait_ref.substitution.as_slice(Interner)[1..]
                ...
                .collect();
            let trait_bound = crablang_ir::TraitBound { trait_id: trait_ref.trait_id, args_no_self };
            Some(chalk_ir::Binders::new(binders, crablang_ir::InlineBound::TraitBound(trait_bound)))
        }
        WhereClause::AliasEq(AliasEq { alias: AliasTy::Projection(projection_ty), ty }) => {
            ...
            let args_no_self = projection_ty.substitution.as_slice(Interner)[1..]
                ...
                .map(|ty| ty.clone().cast(Interner))
                .collect();
            let alias_eq_bound = crablang_ir::AliasEqBound {
                value: ty.clone(),
                trait_bound: crablang_ir::TraitBound {
                    trait_id: to_chalk_trait_id(trait_),
                    args_no_self,
                },
            ...
            Some(chalk_ir::Binders::new(
                binders,
                crablang_ir::InlineBound::AliasEqBound(alias_eq_bound),
            ))
        }
        _ => None,
    }
