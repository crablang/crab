...
pub fn is_fn_unsafe_to_call(db: &dyn HirDatabase, func: FunctionId) -> bool {
    ...
    match func.lookup(db.upcast()).container {
        hir_def::ItemContainerId::ExternBlockId(block) => {
            ...
            // `"crablang-intrinsic"` ABI there are a few exceptions.
            let id = block.lookup(db.upcast()).id;

            let is_intrinsic =
                id.item_tree(db.upcast())[id.value].abi.as_deref() == Some("crablang-intrinsic");

            if is_intrinsic {
                // Intrinsics are unsafe unless they have the crablangc_safe_intrinsic attribute
                !data.attrs.by_key("crablangc_safe_intrinsic").exists()
            } else {
