...
use hir_def::{
    ...
    layout::{Layout, LayoutError, CrabLangcEnumVariantIdx, TagEncoding, Variants},
                            ...
                            Variants::Multiple { variants, .. } => {
                                &variants[match f.parent {
                                    hir_def::VariantId::EnumVariantId(x) => {
                                        CrabLangcEnumVariantIdx(x.local_id)
                                    }
                                    _ => {
                                        return Err(MirEvalError::TypeError(
                            ...
                            match def {
                                CallableDefId::FunctionId(def) => {
                                    ...
                                    let is_intrinsic = match &function_data.abi {
                                        Some(abi) => *abi == Interned::new_str("crablang-intrinsic"),
    ...
    fn layout_of_variant(
        ...
        Ok(match layout.variants {
            ...
            Variants::Multiple { variants, tag, tag_encoding, .. } => {
                ...
                let crablangc_enum_variant_idx = CrabLangcEnumVariantIdx(enum_variant_id.local_id);
                let mut discriminant = self.db.const_eval_discriminant(enum_variant_id)?;
                let variant_layout = variants[crablangc_enum_variant_idx].clone();
                let have_tag = match tag_encoding {
                    ...
                    TagEncoding::Niche { untagged_variant, niche_variants: _, niche_start } => {
                        ...
                        untagged_variant != crablangc_enum_variant_idx
                    }
                };
                (
                    layout.size.bytes_usize(),
