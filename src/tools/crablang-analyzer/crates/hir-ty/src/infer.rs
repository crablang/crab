...
//!
//! For type inference, compare the implementations in crablangc (the various
//! check_* methods in crablangc_hir_analysis/check/mod.rs are a good entry point) and
//! IntelliJ-CrabLang (org.crablang.lang.core.types.infer). Our entry point for
...
//! the `ena` crate, which is extracted from crablangc.
...
//
// https://github.com/crablang/crablang/issues/57411
...
/// <https://doc.crablang.org/reference/patterns.html#binding-modes>
...
impl<'a> InferenceContext<'a> {
    ...
    pub(crate) fn resolve_all(self) -> InferenceResult {
        ...
        result.diagnostics.retain_mut(|diagnostic| {
            ...
            {
                *ty = table.resolve_completely(ty.clone());
                // FIXME: Remove this when we are on par with crablangc in terms of inference
    ...
    fn resolve_variant(&mut self, path: Option<&Path>, value_ns: bool) -> (Ty, Option<VariantId>) {
        ...
        // https://play.crablang.org/?gist=087992e9e22495446c01c0d4e2d69521
...
impl Expectation {
    ...
    /// The following explanation is copied straight from crablangc:
