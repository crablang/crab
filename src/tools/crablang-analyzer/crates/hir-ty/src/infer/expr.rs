...
impl<'a> InferenceContext<'a> {
        ...
        let (trait_, func) = match trait_func {
            ...
            None => {
                ...
                let ret_ty = if self.is_builtin_binop(&lhs_ty, &lhs_ty, op) {
                    ...
                    // on the correctness, crablangc would complain as necessary lang items don't seem
                    // to exist anyway.
        ...
        let (break_ty, ty) =
            self.with_breakable_ctx(BreakableKind::Block, Some(coerce_ty.clone()), label, |this| {
                ...
                if let Some(expr) = tail {
                    ...
                    // Citing crablangc: if there is no explicit tail expression,
    ...
    fn check_call_arguments(
        ...
        // Quoting https://github.com/crablang/crablang/blob/6ef275e6c3cb1384ec78128eceeb4963ff788dca/src/libcrablangc_typeck/check/mod.rs#L3325 --
    ...
    fn enforce_builtin_binop_types(&mut self, lhs: &Ty, rhs: &Ty, op: BinaryOp) -> Ty {
        // Special-case a single layer of referencing, so that things like `5.0 + &6.0f32` work (See crablang/crablang#57447).
