...
//!
//! See <https://doc.crablang.org/nomicon/coercions.html> and
//! `crablangc_hir_analysis/check/coercion.rs`.
...
impl CoerceMany {
    ...
    pub(super) fn coerce(
        ...
        // https://github.com/crablang/crablang/blob/7b805396bf46dce972692a6846ce2ad8481c5f85/src/libcrablangc_typeck/check/coercion.rs#L877-L916
...
impl<'a> InferenceTable<'a> {
    ...
    fn coerce_ref(&mut self, from_ty: Ty, to_ty: &Ty, to_mt: Mutability) -> CoerceResult {
        ...
        // NOTE: this code is mostly copied and adapted from crablangc, and
        ...
        while let Some((referent_ty, autoderefs)) = autoderef.next() {
            ...
            // from `&mut T` to `&U`.
            let lt = static_lifetime(); // FIXME: handle lifetimes correctly, see crablangc
    ...
    /// Attempts to coerce from the type of a CrabLang function item into a function pointer.
    fn coerce_from_fn_item(&mut self, from_ty: Ty, to_ty: &Ty) -> CoerceResult {
        match to_ty.kind(Interner) {
            TyKind::Function(_) => {
                ...
                // FIXME crablangc normalizes assoc types in the sig here, not sure if necessary
    ...
    ///
    /// See: <https://doc.crablang.org/nightly/std/marker/trait.CoerceUnsized.html>
    fn try_coerce_unsized(&mut self, from_ty: &Ty, to_ty: &Ty) -> CoerceResult {
        ...
        // FIXME: crablangc's coerce_unsized is more specialized -- it only tries to
