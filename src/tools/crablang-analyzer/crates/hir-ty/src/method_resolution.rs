...
//! For details about how this works in crablangc, see the method lookup page in the
//! [crablangc guide](https://crablang.github.io/crablangc-guide/method-lookup.html)
//! and the corresponding code mostly in crablangc_hir_analysis/check/method/probe.rs.
...
pub fn def_crates(
    ...
    match ty.kind(Interner) {
        &TyKind::Adt(AdtId(def_id), _) => {
            let crablangc_has_incoherent_inherent_impls = match def_id {
                hir_def::AdtId::StructId(id) => {
                    db.struct_data(id).crablangc_has_incoherent_inherent_impls
                }
                hir_def::AdtId::UnionId(id) => {
                    db.union_data(id).crablangc_has_incoherent_inherent_impls
                }
                hir_def::AdtId::EnumId(id) => db.enum_data(id).crablangc_has_incoherent_inherent_impls,
            };
            Some(if crablangc_has_incoherent_inherent_impls {
        ...
        &TyKind::Foreign(id) => {
            ...
            Some(if db.type_alias_data(alias).crablangc_has_incoherent_inherent_impls {
        ...
        TyKind::Dyn(_) => {
            ...
            Some(if db.trait_data(trait_id).crablangc_has_incoherent_inherent_impls {
...
fn is_inherent_impl_coherent(
    ...
    let impl_allowed = match self_ty {
        ...
        | TyKind::Slice(_)
        | TyKind::Str
        | TyKind::Scalar(_) => def_map.is_crablangc_coherence_is_core(),
    ...
    impl_allowed || {
        let crablangc_has_incoherent_inherent_impls = match self_ty {
            TyKind::Tuple(_, _)
            | TyKind::FnDef(_, _)
            | TyKind::Array(_, _)
            | TyKind::Never
            ...
            &TyKind::Adt(AdtId(adt), _) => match adt {
                hir_def::AdtId::StructId(it) => {
                    db.struct_data(it).crablangc_has_incoherent_inherent_impls
                }
                hir_def::AdtId::UnionId(it) => {
                    db.union_data(it).crablangc_has_incoherent_inherent_impls
                }
                hir_def::AdtId::EnumId(it) => db.enum_data(it).crablangc_has_incoherent_inherent_impls,
            },
            TyKind::Dyn(it) => it.principal().map_or(false, |trait_ref| {
        ...
        crablangc_has_incoherent_inherent_impls
            ...
            && impl_data.items.iter().copied().all(|assoc| match assoc {
                AssocItemId::FunctionId(it) => db.function_data(it).crablangc_allow_incoherent_impl,
                AssocItemId::ConstId(it) => db.const_data(it).crablangc_allow_incoherent_impl,
                AssocItemId::TypeAliasId(it) => db.type_alias_data(it).crablangc_allow_incoherent_impl,
            })
    }
}
...
pub fn iterate_method_candidates_dyn(
    ...
    match mode {
        LookupMode::MethodCall => {
            // For method calls, crablang first does any number of autoderef, and
            ...
            // (just as crablangc does an autoderef and then autoref again).
...
fn iterate_trait_method_candidates(
    ...
    'traits: for &t in traits_in_scope {
        ...
        // Traits annotated with `#[crablangc_skip_array_during_method_dispatch]` are skipped during
...
fn autoderef_method_receiver(
    ...
    // As a last step, we can do array unsizing (that's the only unsizing that crablangc does for method receivers!)
