...
impl GlobalState {
    ...
    /// crablang-analyzer is resilient -- if it fails, this doesn't usually affect
    /// the user experience. Part of that is that we deliberately hide panics
    /// from the user.
    ///
    /// We do however want to pester crablang-analyzer developers with panics and
    ...
    pub(crate) fn poke_crablang_analyzer_developer(&mut self, message: String) {
    ...
    pub(crate) fn report_progress(
        ...
        let token = lsp_types::ProgressToken::String(
            cancel_token.unwrap_or_else(|| format!("crablangAnalyzer/{title}")),
        );
