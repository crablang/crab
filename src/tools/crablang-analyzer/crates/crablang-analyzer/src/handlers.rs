...
use crate::{
    ...
    config::{CrabLangfmtConfig, WorkspaceSymbolConfig},
    diff::diff,
    from_proto,
...
pub(crate) fn handle_formatting(
    ...
    run_crablangfmt(&snap, params.text_document, None)
}
...
pub(crate) fn handle_range_formatting(
    ...
    run_crablangfmt(&snap, params.text_document, Some(params.range))
}
        ...
        .map(|d| Diagnostic {
            ...
            code_description: Some(lsp_types::CodeDescription {
                href: lsp_types::Url::parse(&format!(
                    "https://crablang-analyzer.github.io/manual.html#{}",
                    d.code.as_str()
                ))
                .unwrap(),
            }),
            source: Some("crablang-analyzer".to_string()),
            message: d.message,
...
fn run_crablangfmt(
    ...
    let mut command = match snap.config.crablangfmt() {
        CrabLangfmtConfig::CrabLangfmt { extra_args, enable_range_formatting } => {
            let mut cmd = process::Command::new(toolchain::crablangfmt());
            cmd.envs(snap.config.extra_env());
            cmd.args(extra_args);
            // try to chdir to the file so we can respect `crablangfmt.toml`
            // FIXME: use `crablangfmt --config-path` once
            // https://github.com/crablang/crablangfmt/issues/4660 gets fixed
            match text_document.uri.to_file_path() {
                ...
                Err(_) => {
                    tracing::error!(
                        "Unable to get file path for {}, crablangfmt.toml might be ignored",
                        text_document.uri
                    );
                }
            }
            ...
            if let Some(range) = range {
                if !enable_range_formatting {
                    return Err(LspError::new(
                        ...
                        String::from(
                            "crablangfmt range formatting is unstable. \
                            Opt-in by using a nightly build of crablangfmt and setting \
                            `crablangfmt.rangeFormatting.enable` to true in your LSP configuration",
                        ),
                    )
                    .into());
                }
    ...
    crablangfmt.stdin.as_mut().unwrap().write_all(file.as_bytes())?;

    let output = crablangfmt.wait_with_output()?;
    ...
    if !output.status.success() {
        ...
        return match output.status.code() {
            Some(1) if !crablangfmt_not_installed => {
                // While `crablangfmt` doesn't have a specific exit code for parse errors this is the
                ...
                tracing::warn!(
                    ?command,
                    %captured_stderr,
                    "crablangfmt exited with status 1"
                );
                Ok(None)
            }
            _ => {
                // Something else happened - e.g. `crablangfmt` is missing or caught a signal
                Err(LspError::new(
                    -32900,
                    format!(
                        r#"crablangfmt exited with:
                           Status: {}
