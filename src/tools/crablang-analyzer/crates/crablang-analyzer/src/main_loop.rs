//! The main loop of `crablang-analyzer` responsible for dispatching LSP
...
pub fn main_loop(config: Config, connection: Connection) -> Result<()> {
    ...
    // https://github.com/crablang/crablang-analyzer/issues/2835
    #[cfg(windows)]
    unsafe {
    ...
    fn handle_event(&mut self, event: Event) -> Result<()> {
        ...
        if let Some(diagnostic_changes) = self.diagnostics.take_changes() {
            for file_id in diagnostic_changes {
                ...
                // See https://github.com/crablang/crablang-analyzer/issues/11404
                // See https://github.com/crablang/crablang-analyzer/issues/13130
        ...
        if loop_duration > Duration::from_millis(100) && was_quiescent {
            ...
            self.poke_crablang_analyzer_developer(format!("overly long loop turn: {loop_duration:?}"));
        }
        Ok(())
    }
    ...
    fn handle_flycheck_msg(&mut self, message: flycheck::Message) {
        match message {
            flycheck::Message::AddDiagnostic { id, workspace_root, diagnostic } => {
                ...
                let diagnostics = crate::diagnostics::to_proto::map_crablang_diagnostic_to_lsp(
            ...
            flycheck::Message::Progress { id, progress } => {
                ...
                self.report_progress(
                    ...
                    message,
                    None,
                    Some(format!("crablang-analyzer/flycheck/{id}")),
                );
            }
        }
    }
        ...
        NotificationDispatcher { not: Some(not), global_state: self }
            ...
            .on::<lsp_types::notification::WorkDoneProgressCancel>(|this, params| {
                if let lsp_types::NumberOrString::String(s) = &params.token {
                    if let Some(id) = s.strip_prefix("crablang-analyzer/flycheck/") {
                ...
                this.send_request::<lsp_types::request::WorkspaceConfiguration>(
                    lsp_types::ConfigurationParams {
                        items: vec![lsp_types::ConfigurationItem {
                            scope_uri: None,
                            section: Some("crablang-analyzer".to_string()),
                        }],
                    },
                    |this, resp| {
