...
use project_model::{
    CargoConfig, CargoFeatures, ProjectJson, ProjectJsonData, ProjectManifest, CrabLangLibSource,
    UnsetTestCrates,
};
...
// Defines the server-side configuration of the crablang-analyzer. We generate
...
config_data! {
    struct ConfigData {
        ...
        cargo_buildScripts_useCrabLangcWrapper: bool = "true",
        ...
        ///
        /// This option does not take effect until crablang-analyzer is restarted.
        crablangc_source: Option<String> = "null",

        /// Additional arguments to `crablangfmt`.
        crablangfmt_extraArgs: Vec<String>               = "[]",
        ...
        /// standard output.
        crablangfmt_overrideCommand: Option<Vec<String>> = "null",
        ...
        crablangfmt_rangeFormatting_enable: bool = "false",
...
#[derive(Debug, Clone)]
pub enum CrabLangfmtConfig {
    CrabLangfmt { extra_args: Vec<String>, enable_range_formatting: bool },
...
impl fmt::Display for ConfigUpdateError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(
            f,
            "crablang-analyzer found {} invalid config value{}:\n{}",
            self.errors.len(),
...
impl Config {
    ...
    pub fn cargo(&self) -> CargoConfig {
        let crablangc_source = self.data.crablangc_source.as_ref().map(|crablangc_src| {
            if crablangc_src == "discover" {
                CrabLangLibSource::Discover
            } else {
                CrabLangLibSource::Path(self.root_path.join(crablangc_src))
            }
        });
        let sysroot = self.data.cargo_sysroot.as_ref().map(|sysroot| {
            if sysroot == "discover" {
                ...
                CrabLangLibSource::Path(self.root_path.join(sysroot))
            }
        });
        let sysroot_src =
        ...
        CargoConfig {
            ...
            wrap_crablangc_in_build_scripts: self.data.cargo_buildScripts_useCrabLangcWrapper,
    ...
    pub fn crablangfmt(&self) -> CrabLangfmtConfig {
        match &self.data.crablangfmt_overrideCommand {
            Some(args) if !args.is_empty() => {
                let mut args = args.clone();
                let command = args.remove(0);
                CrabLangfmtConfig::CustomCommand { command, args }
            }
            Some(_) | None => CrabLangfmtConfig::CrabLangfmt {
                extra_args: self.data.crablangfmt_extraArgs.clone(),
                enable_range_formatting: self.data.crablangfmt_rangeFormatting_enable,
            },
        }
    }
    ...
    pub fn client_commands(&self) -> ClientCommandsConfig {
        ...
        ClientCommandsConfig {
            run_single: get("crablang-analyzer.runSingle"),
            debug_single: get("crablang-analyzer.debugSingle"),
            show_reference: get("crablang-analyzer.showReferences"),
            goto_location: get("crablang-analyzer.gotoLocation"),
...
fn schema(fields: &[(&'static str, &'static str, &[&str], &str)]) -> serde_json::Value {
    let map = fields
        ...
        .map(|(field, ty, doc, default)| {
            ...
            let name = format!("crablang-analyzer.{name}");
...
fn manual(fields: &[(&'static str, &'static str, &[&str], &str)]) -> String {
    fields
        ...
        .map(|(field, _ty, doc, default)| {
            let name = format!("crablang-analyzer.{}", field.replace('_', "."));
            let doc = doc_comment_to_string(doc);
            if default.contains('\n') {
                format!(
                    r#"[[{name}]]{name}::
