...
//! fixed "project" crablang-analyzer is working with, "current project" is itself
...
//! macros) `cargo check`.
//!
//! The main guiding principle here is, as elsewhere in crablang-analyzer,
...
impl GlobalState {
    ...
    pub(crate) fn current_status(&self) -> lsp_ext::ServerStatusParams {
        ...
        for ws in self.workspaces.iter() {
            ...
            if let ProjectWorkspace::Cargo { crablangc: Err(Some(e)), .. } = ws {
                status.health = lsp_ext::Health::Warning;
                message.push_str(e);
                message.push_str("\n\n");
            }
    ...
    pub(super) fn fetch_workspace_error(&self) -> Result<(), String> {
        ...
        if last_op_result.is_empty() {
            stdx::format_to!(buf, "crablang-analyzer failed to discover workspace");
        } else {
            for ws in last_op_result {
                if let Err(err) = ws {
                    stdx::format_to!(buf, "crablang-analyzer failed to load workspace: {:#}\n", err);
                }
            }
        }
...
impl ProjectFolders {
    pub(crate) fn new(
        ...
        // the case of the crablangc workspace, we can end up with two source roots that are almost the
        // same but not quite, like:
        // PackageRoot { is_local: false, include: [AbsPathBuf(".../crablang/src/tools/miri/cargo-miri")], exclude: [] }
        // PackageRoot {
        //     is_local: true,
        //     include: [AbsPathBuf(".../crablang/src/tools/miri/cargo-miri"), AbsPathBuf(".../crablang/build/x86_64-pc-windows-msvc/stage0-tools/x86_64-pc-windows-msvc/release/build/cargo-miri-85801cd3d2d1dae4/out")],
        //     exclude: [AbsPathBuf(".../crablang/src/tools/miri/cargo-miri/.git"), AbsPathBuf(".../crablang/src/tools/miri/cargo-miri/target")]
        // }
        //
        // The first one comes from the explicit crablangc workspace which points to the crablangc workspace itself
        // The second comes from the crablangc workspace that we load as the actual project workspace
        ...
        // So we need to deduplicate these, usually it would be enough to deduplicate by `include`, but as the crablangc example shows here that doesn't work,
