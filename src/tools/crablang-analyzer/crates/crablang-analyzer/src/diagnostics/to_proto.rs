...
fn diagnostic_severity(
    ...
    let res = match level {
        ...
        DiagnosticLevel::Warning => match &code {
            // HACK: special case for `warnings` crablangc lint.
            Some(code)
...
fn is_dummy_macro_file(file_name: &str) -> bool {
    // FIXME: current crablangc does not seem to emit `<macro file>` files anymore?
    file_name.starts_with('<') && file_name.ends_with('>')
}

/// Converts a CrabLang span to a LSP location
fn location(
    config: &DiagnosticsMapConfig,
...
/// Extracts a suitable "primary" location from a crablangc diagnostic.
///
...
/// Converts a secondary CrabLang span to a LSP related information
///
...
enum MappedCrabLangChildDiagnostic {
    SubDiagnostic(SubDiagnostic),
    MessageLine(String),
}

fn map_crablang_child_diagnostic(
    config: &DiagnosticsMapConfig,
    workspace_root: &AbsPath,
    rd: &flycheck::Diagnostic,
    snap: &GlobalStateSnapshot,
) -> MappedCrabLangChildDiagnostic {
    let spans: Vec<&DiagnosticSpan> = rd.spans.iter().filter(|s| s.is_primary).collect();
    if spans.is_empty() {
        // `crablangc` uses these spanless children as a way to print multi-line
        // messages
        return MappedCrabLangChildDiagnostic::MessageLine(rd.message.clone());
    }
    ...
    // crablangc renders suggestion diagnostics by appending the suggested replacement, so do the same
    ...
    if edit_map.is_empty() {
        MappedCrabLangChildDiagnostic::SubDiagnostic(SubDiagnostic {
            related: lsp_types::DiagnosticRelatedInformation {
...
#[derive(Debug)]
pub(crate) struct MappedCrabLangDiagnostic {
    pub(crate) url: lsp_types::Url,
    pub(crate) diagnostic: lsp_types::Diagnostic,
    pub(crate) fix: Option<Fix>,
}

/// Converts a CrabLang root diagnostic to LSP form
///
/// This flattens the CrabLang diagnostic by:
///
...
pub(crate) fn map_crablang_diagnostic_to_lsp(
    config: &DiagnosticsMapConfig,
    rd: &flycheck::Diagnostic,
    workspace_root: &AbsPath,
    snap: &GlobalStateSnapshot,
) -> Vec<MappedCrabLangDiagnostic> {
    ...
    let mut source = String::from("crablangc");
    ...
    for child in &rd.children {
        let child = map_crablang_child_diagnostic(config, workspace_root, child, snap);
        match child {
            MappedCrabLangChildDiagnostic::SubDiagnostic(sub) => {
                subdiagnostics.push(sub);
            }
            MappedCrabLangChildDiagnostic::MessageLine(message_line) => {
                format_to!(message, "\n{}", message_line);
    ...
    let code_description = match source.as_str() {
        "crablangc" => crablangc_code_description(code.as_deref()),
    ...
    primary_spans
        ...
        .flat_map(|primary_span| {
            ...
            for (i, span) in span_stack.enumerate() {
                ...
                diagnostics.push(MappedCrabLangDiagnostic {
...
fn crablangc_code_description(code: Option<&str>) -> Option<lsp_types::CodeDescription> {
    code.filter(|code| {
    ...
    .and_then(|code| {
        lsp_types::Url::parse(&format!("https://doc.crablang.org/error-index.html#{code}"))
            .ok()
...
fn clippy_code_description(code: Option<&str>) -> Option<lsp_types::CodeDescription> {
    code.and_then(|code| {
        lsp_types::Url::parse(&format!(
            "https://crablang.github.io/crablang-clippy/master/index.html#{code}"
        ))
        .ok()
        .map(|href| lsp_types::CodeDescription { href })
    })
...
mod tests {
    ...
    fn check_with_config(config: DiagnosticsMapConfig, diagnostics_json: &str, expect: ExpectFile) {
        ...
        let mut actual = map_crablang_diagnostic_to_lsp(&config, &diagnostic, workspace_root, &snap);
        actual.iter_mut().for_each(|diag| diag.diagnostic.data = None);
        expect.assert_debug_eq(&actual)
    }

    #[test]
    fn crablangc_incompatible_type_for_trait() {
        check(
            r##"{
            ...
            "##,
            expect_file!["./test_data/crablangc_incompatible_type_for_trait.txt"],
        );
    }

    #[test]
    fn crablangc_unused_variable() {
        check(
            r##"{
    ...
    "rendered": "warning: unused variable: `foo`\n   --> driver/subcommand/repl.rs:291:9\n    |\n291 |     let foo = 42;\n    |         ^^^ help: consider prefixing with an underscore: `_foo`\n    |\n    = note: #[warn(unused_variables)] on by default\n\n"
            ...
            expect_file!["./test_data/crablangc_unused_variable.txt"],
        );
    }

    #[test]
    #[cfg(not(windows))]
    fn crablangc_unused_variable_as_info() {
        check_with_config(
    ...
    "rendered": "warning: unused variable: `foo`\n   --> driver/subcommand/repl.rs:291:9\n    |\n291 |     let foo = 42;\n    |         ^^^ help: consider prefixing with an underscore: `_foo`\n    |\n    = note: #[warn(unused_variables)] on by default\n\n"
            ...
            expect_file!["./test_data/crablangc_unused_variable_as_info.txt"],
        );
    }

    #[test]
    #[cfg(not(windows))]
    fn crablangc_unused_variable_as_hint() {
        check_with_config(
            DiagnosticsMapConfig {
    ...
    "rendered": "warning: unused variable: `foo`\n   --> driver/subcommand/repl.rs:291:9\n    |\n291 |     let foo = 42;\n    |         ^^^ help: consider prefixing with an underscore: `_foo`\n    |\n    = note: #[warn(unused_variables)] on by default\n\n"
            ...
            expect_file!["./test_data/crablangc_unused_variable_as_hint.txt"],
        );
    }

    #[test]
    fn crablangc_wrong_number_of_parameters() {
        check(
            r##"{
    ...
    "rendered": "error[E0061]: this function takes 2 parameters but 3 parameters were supplied\n   --> compiler/ty/select.rs:104:18\n    |\n104 |               self.add_evidence(target_fixed, evidence_fixed, false);\n    |                    ^^^^^^^^^^^^ expected 2 parameters\n...\n219 | /     pub fn add_evidence(\n220 | |         &mut self,\n221 | |         target_poly: &ty::Ref<ty::Poly>,\n222 | |         evidence_poly: &ty::Ref<ty::Poly>,\n...   |\n230 | |         }\n231 | |     }\n    | |_____- defined here\n\n"
            ...
            expect_file!["./test_data/crablangc_wrong_number_of_parameters.txt"],
        );
    }
    ...
    "children": [
        ...
        {
            "message": "for further information visit https://crablang.github.io/crablang-clippy/master/index.html#trivially_copy_pass_by_ref",
            "code": null,
            "level": "help",
            "spans": [],
            "children": [],
    ...
    #[test]
    fn crablangc_range_map_lsp_position() {
        check(
            r##"{
            ...
            expect_file!("./test_data/crablangc_range_map_lsp_position.txt"),
        )
    }

    #[test]
    fn crablangc_mismatched_type() {
        check(
            r##"{
    "message": "mismatched types",
    "code": {
    ...
    "rendered": "error[E0308]: mismatched types\n  --> runtime/compiler_support.rs:48:65\n   |\n48 |     let layout = alloc::Layout::from_size_align_unchecked(size, align);\n   |                                                                 ^^^^^ expected usize, found u32\n\n"
            ...
            expect_file!["./test_data/crablangc_mismatched_type.txt"],
        );
    }
    ...
    fn snap_multi_line_fix() {
        check(
            r##"{
                "rendered": "warning: returning the result of a let binding from a block\n --> src/main.rs:4:5\n  |\n3 |     let a = (0..10).collect();\n  |     -------------------------- unnecessary let binding\n4 |     a\n  |     ^\n  |\n  = note: `#[warn(clippy::let_and_return)]` on by default\n  = help: for further information visit https://crablang.github.io/crablang-clippy/master/index.html#let_and_return\nhelp: return the expression directly\n  |\n3 |     \n4 |     (0..10).collect()\n  |\n\n",
                "children": [
                    {
                    "children": [],
                    "code": null,
                    ...
                    "code": null,
                    "level": "help",
                    "message": "for further information visit https://crablang.github.io/crablang-clippy/master/index.html#let_and_return",
                    "rendered": null,
                    "spans": []
                    },
                    {
