...
fn check_code_formatting() {
    ...
    let out = cmd!(sh, "crablangup run stable crablangfmt --version").read().unwrap();
    ...
    let res = cmd!(sh, "crablangup run stable cargo fmt -- --check").run();
    if res.is_err() {
        let _ = cmd!(sh, "crablangup run stable cargo fmt").run();
    }
    res.unwrap()
}
...
fn check_lsp_extensions_docs() {
    ...
    let expected_hash = {
        let lsp_ext_rs = sh
            .read_file(sourcegen::project_root().join("crates/crablang-analyzer/src/lsp_ext.rs"))
            .unwrap();
...
fn check_test_attrs(path: &Path, text: &str) {
    let ignore_rule =
        "https://github.com/crablang/crablang-analyzer/blob/master/docs/dev/style.md#ignore";
    let need_ignore: &[&str] = &[
        // This file.
        "slow-tests/tidy.rs",
    ...
    let panic_rule =
        "https://github.com/crablang/crablang-analyzer/blob/master/docs/dev/style.md#should_panic";
    let need_panic: &[&str] = &[
        // This file.
        "slow-tests/tidy.rs",
