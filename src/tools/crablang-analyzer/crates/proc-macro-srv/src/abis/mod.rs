...
//! to version of the compiler. To support this we copy the ABI from the crablang
...
//! interface the rest of crablang-analyzer can use to talk to the macro
//! provider.
//!
//! # Adding a new ABI
//!
//! To add a new ABI you'll need to copy the source of the target proc_macro
//! crate from the source tree of the CrabLang compiler into this directory tree.
...
//! for the relevant versions of the crablang compiler
//!
...
// see `build.rs`
include!(concat!(env!("OUT_DIR"), "/crablangc_version.rs"));
...
use proc_macro_api::{ProcMacroKind, CrabLangCInfo};
...
impl Abi {
    ...
    /// *`info` - CrabLangCInfo about the compiler that was used to compile the
    ...
    pub fn from_lib(
        ...
        #[cfg(feature = "sysroot-abi")]
        {
            if info.version_string == CRABLANGC_VERSION_STRING {
            ...
            // want that to panic - this could mean that the format of `crablangc
            // --version` no longer matches the format of the version string
            // stored in the `.crablangc` section, and we want to catch that in-tree
            // with `x.py test`
            #[cfg(test)]
            {
                let allow_mismatch = std::env::var("PROC_MACRO_SRV_ALLOW_SYSROOT_MISMATCH");
                if let Ok("1") = allow_mismatch.as_deref() {
                    // only used by crablang-analyzer developers, when working on the
                    // sysroot ABI from the crablang-analyzer repository - which should
                    // only happen pre-subtree. this can be removed later.
                } else {
                    panic!(
                        "sysroot ABI mismatch: dylib crablangc version (read from .crablangc section): {:?} != proc-macro-srv version (read from 'crablangc --version'): {:?}",
                        info.version_string, CRABLANGC_VERSION_STRING
                    );
                }
            }
        }
        ...
        // https://github.com/crablang/crablang/issues/37854
