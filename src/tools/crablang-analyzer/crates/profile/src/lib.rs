//! A collection of tools for profiling crablang-analyzer.
...
/// 6. Use something like `pprof -svg target/release/crablang-analyzer ./out.profile` to see the results.
///
...
/// $ nix-shell --run 'cargo test --release --package crablang-analyzer --lib -- benchmarks::benchmark_integrated_highlighting --exact --nocapture'
/// $ pprof -svg target/release/deps/crablang_analyzer-8739592dc93d63cb crates/crablang-analyzer/out.profile > profile.svg
/// ```
///
/// See this diff for how to profile completions:
///
/// <https://github.com/crablang/crablang-analyzer/pull/5306>
#[derive(Debug)]
pub struct CpuSpan {
    _private: (),
}
