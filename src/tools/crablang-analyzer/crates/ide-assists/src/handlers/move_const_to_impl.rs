...
pub(crate) fn move_const_to_impl(acc: &mut Assists, ctx: &AssistContext<'_>) -> Option<()> {
    ...
    if ty
        .iterate_assoc_items(db, ty.krate(db), |assoc| {
            ...
            // of brevity (inherent associated types exist in nightly CrabLang, but it's *very*
    ...
    acc.add(
        ...
        |builder| {
            ...
            //
            // We're assuming the code is formatted according to CrabLang's standard style guidelines
