...
//  nesting is only supported in CrabLang versions later than 1.24.
// - `module`: Merge imports from the same module into a single use statement.
// - `item`: Don't merge imports at all, creating one import per item.
// - `preserve`: Do not change the granularity of any imports. For auto-import this has the same
//  effect as `item`.
//
// In `VS Code` the configuration for this is `crablang-analyzer.imports.granularity.group`.
//
// .Import Prefix
//
...
//
// In `VS Code` the configuration for this is `crablang-analyzer.imports.prefix`.
//
