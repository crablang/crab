...
fn load_crablang_project(file: &str) -> CrateGraph {
    let data = get_test_json_file(file);
    let project = rooted_project_json(data);
    let sysroot = Ok(get_fake_sysroot());
    let project_workspace = ProjectWorkspace::Json { project, sysroot, crablangc_cfg: Vec::new() };
    to_crate_graph(project_workspace)
}
            ...
            CrateGraph {
                arena: {
                    ...
                    CrateId(
                        ...
                        origin: CratesIo {
                            repo: Some(
                                "https://github.com/crablang/libc",
                            ),
                            name: Some(
                                "libc",
                            ),
...
#[test]
fn crablang_project_hello_world_project_model() {
    let crate_graph = load_crablang_project("hello-world-project.json");
    check_crate_graph(
        crate_graph,
        expect![[r#"
            CrateGraph {
                arena: {
                    CrateId(
                        ...
                        target_layout: Err(
                            "crablang-project.json projects have no target layout set",
                        ),
                        env: Env {
                            entries: {},
                        },
...
#[test]
fn crablang_project_is_proc_macro_has_proc_macro_dep() {
    let crate_graph = load_crablang_project("is-proc-macro-project.json");
