//! In crablang-analyzer, we maintain a strict separation between pure abstract
...
pub use crate::{
    ...
    cargo_workspace::{
        ...
        CrabLangLibSource, Target, TargetData, TargetKind, UnsetTestCrates,
    },
...
impl ProjectManifest {
    pub fn from_manifest_file(path: AbsPathBuf) -> Result<ProjectManifest> {
        ...
        if path.file_name().unwrap_or_default() == "crablang-project.json" {
            return Ok(ProjectManifest::ProjectJson(path));
        }
        if path.file_name().unwrap_or_default() == "Cargo.toml" {
            return Ok(ProjectManifest::CargoToml(path));
        }
        bail!("project root must point to Cargo.toml or crablang-project.json: {}", path.display());
    }
    ...
    pub fn discover(path: &AbsPath) -> io::Result<Vec<ProjectManifest>> {
        if let Some(project_json) = find_in_parent_dirs(path, "crablang-project.json") {
