...
impl WorkspaceBuildScripts {
    fn build_command(config: &CargoConfig) -> io::Result<Command> {
        ...
        if config.wrap_crablangc_in_build_scripts {
            // Setup CRABLANGC_WRAPPER to point to `crablang-analyzer` binary itself. We use
            // that to compile only proc macros and build scripts during the initial
            // `cargo check`.
            let myself = std::env::current_exe()?;
            cmd.env("CRABLANGC_WRAPPER", myself);
            cmd.env("RA_CRABLANGC_WRAPPER", "1");
        }
    ...
    pub(crate) fn run_for_workspace(
        ...
        const CRABLANG_1_62: Version = Version::new(1, 62, 0);
        ...
        match Self::run_per_ws(Self::build_command(config)?, workspace, current_dir, progress) {
            Ok(WorkspaceBuildScripts { error: Some(error), .. })
                if toolchain.as_ref().map_or(false, |it| *it >= CRABLANG_1_62) =>
            {
                // building build scripts failed, attempt to build with --keep-going so
                // that we potentially get more build data
                let mut cmd = Self::build_command(config)?;
                cmd.args(["-Z", "unstable-options", "--keep-going"]).env("CRABLANGC_BOOTSTRAP", "1");
                    ...
                    Message::BuildScriptExecuted(mut message) => {
                        with_output_for(&message.package_id.repr, &mut |name, data| {
                            ...
                            if !out_dir.is_empty() {
                                ...
                                // NOTE: cargo and crablangc seem to hide non-UTF-8 strings from env! and option_env!()
                                if let Some(out_dir) =
    ...
    pub(crate) fn crablangc_crates(
        crablangc: &CargoWorkspace,
        current_dir: &AbsPath,
        extra_env: &FxHashMap<String, String>,
    ) -> Self {
        let mut bs = WorkspaceBuildScripts::default();
        for p in crablangc.packages() {
        ...
        let res = (|| {
            let target_libdir = (|| {
                ...
                cargo_config
                    ...
                    .args(["crablangc", "-Z", "unstable-options", "--print", "target-libdir"])
            ...
            tracing::info!("Loading crablangc proc-macro paths from {}", target_libdir.display());
            ...
            let proc_macro_dylibs: Vec<(String, AbsPathBuf)> = std::fs::read_dir(target_libdir)?
                ...
                .collect();
            for p in crablangc.packages() {
                let package = &crablangc[p];
                if package.targets.iter().any(|&it| crablangc[it].is_proc_macro) {
            ...
            if tracing::enabled!(tracing::Level::INFO) {
                for package in crablangc.packages() {
                    let package_build_data = &bs.outputs[package];
                    if !package_build_data.is_unchanged() {
                        tracing::info!(
                            "{}: {:?}",
                            crablangc[package].manifest.parent().display(),
                            package_build_data,
                        );
                    }
                }
