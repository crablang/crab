//! `crablang-project.json` file format.
//!
...
//! system to generate `crablang-project.json` which can be ingested by
//! crablang-analyzer.
//!
//! This short file is a somewhat big conceptual piece of the architecture of
//! crablang-analyzer, so it's worth elaborating on the underlying ideas and
//! motivation.
//!
//! For crablang-analyzer to function, it needs some information about the project.
...
//! have a monorepo with an infinite amount of CrabLang code which is built with bazel or
//! some such.
//!
...
//! right? :)
//!
//! `crablang-project.json` also isn't necessary a file. Architecturally, we support
...
//!   of crablang-project.json to crablang-analyzer, no need to write anything to disk)
//!
...
//! project.
//!
//! In general, it is assumed that if you are going to use `crablang-project.json`,
...
//! crablang-project.json over time via configuration request!)
...
/// Roots and crates that compose this CrabLang project.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ProjectJson {
    /// e.g. `path/to/sysroot`
    pub(crate) sysroot: Option<AbsPathBuf>,
    /// e.g. `path/to/sysroot/lib/crablanglib/src/crablang`
...
impl ProjectJson {
    ...
    /// # Arguments
    ///
    /// * `base` - The path to the workspace root (i.e. the folder containing `crablang-project.json`)
    /// * `data` - The parsed contents of `crablang-project.json`, or project json that's passed via
    ///            configuration.
