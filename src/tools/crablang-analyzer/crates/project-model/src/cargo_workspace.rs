...
///
/// Note that internally, crablang-analyzer uses a different structure:
...
/// Describes how to set the crablangc source directory.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum CrabLangLibSource {
    /// Explicit path for the crablangc source directory.
    Path(AbsPathBuf),
    /// Try to automatically detect where the crablangc source directory is.
    Discover,
}
...
pub struct CargoConfig {
    ...
    pub sysroot: Option<CrabLangLibSource>,
    pub sysroot_src: Option<AbsPathBuf>,
    /// crablangc private crate source
    pub crablangc_source: Option<CrabLangLibSource>,
    /// crates to disable `#[cfg(test)]` on
    pub unset_test_crates: UnsetTestCrates,
    /// Invoke `cargo check` through the CRABLANGC_WRAPPER.
    pub wrap_crablangc_in_build_scripts: bool,
...
pub struct PackageData {
    ...
    /// CrabLang edition for this package
    pub edition: Edition,
    ...
    pub id: String,
    /// The contents of [package.metadata.crablang-analyzer]
    pub metadata: CrabLangAnalyzerPackageMetaData,
}

#[derive(Deserialize, Default, Debug, Clone, Eq, PartialEq)]
pub struct CrabLangAnalyzerPackageMetaData {
    pub crablangc_private: bool,
}
...
struct PackageMetadata {
    #[serde(rename = "crablang-analyzer")]
    crablang_analyzer: Option<CrabLangAnalyzerPackageMetaData>,
}
    ...
    pub fn new(mut meta: cargo_metadata::Metadata) -> CargoWorkspace {
        ...
        for meta_pkg in meta.packages {
            ...
            let pkg = packages.alloc(PackageData {
                ...
                metadata: meta.crablang_analyzer.unwrap_or_default(),
            });
...
fn find_list_of_build_targets(config: &CargoConfig, cargo_toml: &ManifestPath) -> Vec<String> {
    ...
    crablangc_discover_host_triple(cargo_toml, &config.extra_env).into_iter().collect()
}

fn crablangc_discover_host_triple(
    cargo_toml: &ManifestPath,
    extra_env: &FxHashMap<String, String>,
) -> Option<String> {
    let mut crablangc = Command::new(toolchain::crablangc());
    crablangc.envs(extra_env);
    crablangc.current_dir(cargo_toml.parent()).arg("-vV");
    tracing::debug!("Discovering host platform by {:?}", crablangc);
    match utf8_stdout(crablangc) {
        Ok(stdout) => {
            let field = "host: ";
            ...
            if let Some(target) = target {
                ...
                tracing::info!("crablangc -vV did not report host platform, got:\n{}", stdout);
                None
            }
        }
        Err(e) => {
