...
//! crablang-analyzer should be capable to process `#[path = r"C:\bar.rs"]` on Unix.
//!
//! The second try is to use a `String`. This also fails, however. Consider a
//! hypothetical scenario, where crablang-analyzer operates in a
//! networked/distributed mode. There's one global instance of crablang-analyzer,
