...
fn parser_fuzz_tests() {
    for (_, text) in collect_crablang_files(&test_data_dir(), &["parser/fuzz-failures"]) {
...
fn reparse_fuzz_tests() {
    for (_, text) in collect_crablang_files(&test_data_dir(), &["reparse/fuzz-failures"]) {
        let check = fuzz::CheckReparse::from_data(text.as_bytes()).unwrap();
        check.run();
    }
}

/// Test that CrabLang-analyzer can parse and validate the crablang-analyzer
#[test]
fn self_hosting_parsing() {
    let crates_dir = project_root().join("crates");

    let mut files = ::sourcegen::list_crablang_files(&crates_dir);
    files.retain(|path| {
...
{
    for (path, input_code) in collect_crablang_files(test_data_dir, paths) {
...
fn collect_crablang_files(root_dir: &Path, paths: &[&str]) -> Vec<(PathBuf, String)> {
    paths
        .iter()
        .flat_map(|path| {
            let path = root_dir.to_owned().join(path);
            crablang_files_in_dir(&path).into_iter()
        })
        .map(|path| {
...
fn crablang_files_in_dir(dir: &Path) -> Vec<PathBuf> {
    let mut acc = Vec::new();
...
///
/// `crablangc` seems to always normalize `\r\n` newlines to `\n`:
///
/// ```
/// let s = "
/// ";
