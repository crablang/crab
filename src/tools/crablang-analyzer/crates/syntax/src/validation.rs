...
use rowan::Direction;
use crablangc_lexer::unescape::{self, unescape_byte, unescape_char, unescape_literal, Mode};
...
fn crablangc_unescape_error_to_string(err: unescape::EscapeError) -> &'static str {
...
fn validate_literal(literal: ast::Literal, acc: &mut Vec<SyntaxError>) {
    // FIXME: move this function to outer scope (https://github.com/crablang/crablang-analyzer/pull/2834#discussion_r366196658)
    ...
    // FIXME: lift this lambda refactor to `fn` (https://github.com/crablang/crablang-analyzer/pull/2834#discussion_r366199205)
    let mut push_err = |prefix_len, (off, err): (usize, unescape::EscapeError)| {
        let off = token.text_range().start() + TextSize::try_from(off + prefix_len).unwrap();
        acc.push(SyntaxError::new_at_offset(crablangc_unescape_error_to_string(err), off));
    };
