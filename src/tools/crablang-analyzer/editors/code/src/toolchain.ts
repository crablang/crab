...
export class Cargo {
    ...
    static artifactSpec(args: readonly string[]): ArtifactSpec {
        ...
        // see crates\crablang-analyzer\src\main_loop\handlers.rs, handle_code_lens
        switch (cargoArgs[0]) {
            case "run":
                cargoArgs[0] = "build";
                break;
        ...
        if (cargoArgs[0] === "test" || cargoArgs[0] === "bench") {
            // for instance, `crates\crablang-analyzer\tests\heavy_tests\main.rs` tests
...
export async function getSysroot(dir: string): Promise<string> {
    const crablangcPath = await getPathForExecutable("crablangc");

    // do not memoize the result because the toolchain may change between runs
    return await execute(`${crablangcPath} --print sysroot`, { cwd: dir });
}

export async function getCrabLangcId(dir: string): Promise<string> {
    const crablangcPath = await getPathForExecutable("crablangc");

    // do not memoize the result because the toolchain may change between runs
    const data = await execute(`${crablangcPath} -V -v`, { cwd: dir });
...
export const getPathForExecutable = memoizeAsync(
    ...
    async (executableName: "cargo" | "crablangc" | "crablangup"): Promise<string> => {
        {
