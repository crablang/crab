...
const CRABLANG_PROJECT_CONTEXT_NAME = "inCrabLangProject";

export interface CrabLangAnalyzerExtensionApi {
    readonly client?: lc.LanguageClient;
}

export async function deactivate() {
    await setContextValue(CRABLANG_PROJECT_CONTEXT_NAME, undefined);
}

export async function activate(
    context: vscode.ExtensionContext
): Promise<CrabLangAnalyzerExtensionApi> {
    if (vscode.extensions.getExtension("crablang.crablang")) {
        vscode.window
            .showWarningMessage(
                `You have both the crablang-analyzer (crablang.crablang-analyzer) and CrabLang (crablang.crablang) ` +
    ...
    const api = await activateServer(ctx).catch((err) => {
        void vscode.window.showErrorMessage(
            `Cannot activate crablang-analyzer extension: ${err.message}`
        );
        throw err;
    });
    await setContextValue(CRABLANG_PROJECT_CONTEXT_NAME, true);
    return api;
}

async function activateServer(ctx: Ctx): Promise<CrabLangAnalyzerExtensionApi> {
...
function createCommands(): Record<string, CommandFactory> {
    return {
        ...
        reload: {
            enabled: (ctx) => async () => {
                void vscode.window.showInformationMessage("Reloading crablang-analyzer...");
                await ctx.restart();
            },
            disabled: (ctx) => async () => {
                void vscode.window.showInformationMessage("Reloading crablang-analyzer...");
