...
export class Config {
    readonly extensionId = "crablang.crablang-analyzer";
    configureLang: vscode.Disposable | undefined;

    readonly rootSection = "crablang-analyzer";
    ...
    private async onDidChangeConfiguration(event: vscode.ConfigurationChangeEvent) {
        ...
        if (this.restartServerOnConfigChange) {
            await vscode.commands.executeCommand("crablang-analyzer.reload");
            return;
        }
        ...
        if (userResponse) {
            const command = "crablang-analyzer.reload";
    ...
    private configureLanguage() {
        ...
        this.configureLang = vscode.languages.setLanguageConfiguration("crablang", {
            onEnterRules,
        });
    }
     ...
     *  .workspace
     *  .getConfiguration
     *  .getConfiguration("crablang-analyzer")
    ...
    get debug() {
        ...
        if (sourceFileMap !== "auto") {
            // "/crablangc/<id>" used by suggestions only.
            const { ["/crablangc/<id>"]: _, ...trimmed } =
    ...
    get previewCrabLangcOutput() {
        return this.get<boolean>("diagnostics.previewCrabLangcOutput");
    }

    get useCrabLangcErrorCode() {
        return this.get<boolean>("diagnostics.useCrabLangcErrorCode");
    }
}

// the optional `cb?` parameter is meant to be used to add additional
// key/value pairs to the VS Code configuration. This needed for, e.g.,
// including a `crablang-project.json` into the `linkedProjects` key as part
...
// undesirable behavior because crablang-project.json files can be tens of
