...
export async function selectRunnable(
    ...
    if (items.length === 0) {
        ...
        // see crates\crablang-analyzer\src\main_loop\handlers.rs, handle_runnables
...
export function prepareEnv(
    ...
    const env: Record<string, string> = { CRABLANG_BACKTRACE: "short" };
...
export async function createTask(runnable: ra.Runnable, config: Config): Promise<vscode.Task> {
    if (runnable.kind !== "cargo") {
        // crablang-analyzer supports only one kind, "cargo"
