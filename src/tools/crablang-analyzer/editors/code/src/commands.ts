...
import { isCrabLangDocument, isCargoTomlDocument, sleep, isCrabLangEditor } from "./util";
...
export function analyzerStatus(ctx: CtxInit): Cmd {
    const tdcp = new (class implements vscode.TextDocumentContentProvider {
        readonly uri = vscode.Uri.parse("crablang-analyzer-status://status");
        ...
        async provideTextDocumentContent(_uri: vscode.Uri): Promise<string> {
            ...
            const doc = ctx.activeCrabLangEditor?.document;
            if (doc != null) {
    ...
    ctx.pushExtCleanup(
        vscode.workspace.registerTextDocumentContentProvider("crablang-analyzer-status", tdcp)
    );
...
export function memoryUsage(ctx: CtxInit): Cmd {
    const tdcp = new (class implements vscode.TextDocumentContentProvider {
        readonly uri = vscode.Uri.parse("crablang-analyzer-memory://memory");
        readonly eventEmitter = new vscode.EventEmitter<vscode.Uri>();
    ...
    ctx.pushExtCleanup(
        vscode.workspace.registerTextDocumentContentProvider("crablang-analyzer-memory", tdcp)
    );
...
export function matchingBrace(ctx: CtxInit): Cmd {
    return async () => {
        const editor = ctx.activeCrabLangEditor;
        if (!editor) return;
...
export function parentModule(ctx: CtxInit): Cmd {
    return async () => {
        ...
        if (!editor) return;
        if (!(isCrabLangDocument(editor.document) || isCargoTomlDocument(editor.document))) return;
...
export function serverVersion(ctx: CtxInit): Cmd {
    return async () => {
        if (!ctx.serverPath) {
            void vscode.window.showWarningMessage(`crablang-analyzer server is not running`);
            return;
        }
        const { stdout } = spawnSync(ctx.serverPath, ["--version"], { encoding: "utf8" });
        const versionString = stdout.slice(`crablang-analyzer `.length).trim();

        void vscode.window.showInformationMessage(`crablang-analyzer version: ${versionString}`);
    };
}
...
export function syntaxTree(ctx: CtxInit): Cmd {
    const tdcp = new (class implements vscode.TextDocumentContentProvider {
        readonly uri = vscode.Uri.parse("crablang-analyzer-syntax-tree://syntaxtree/tree.rast");
        readonly eventEmitter = new vscode.EventEmitter<vscode.Uri>();
        constructor() {
        ...
        private onDidChangeTextDocument(event: vscode.TextDocumentChangeEvent) {
            if (isCrabLangDocument(event.document)) {
        ...
        private onDidChangeActiveTextEditor(editor: vscode.TextEditor | undefined) {
            if (editor && isCrabLangEditor(editor)) {
        ...
        async provideTextDocumentContent(
            ...
            const crablangEditor = ctx.activeCrabLangEditor;
            if (!crablangEditor) return "";
            const client = ctx.client;

            // When the range based query is enabled we take the range of the selection
            const range =
                uri.query === "range=true" && !crablangEditor.selection.isEmpty
                    ? client.code2ProtocolConverter.asRange(crablangEditor.selection)
                    : null;

            const params = { textDocument: { uri: crablangEditor.document.uri.toString() }, range };
    ...
    ctx.pushExtCleanup(
        vscode.workspace.registerTextDocumentContentProvider("crablang-analyzer-syntax-tree", tdcp)
    );
    ctx.pushExtCleanup(
...
function viewHirOrMir(ctx: CtxInit, xir: "hir" | "mir"): Cmd {
    ...
    const tdcp = new (class implements vscode.TextDocumentContentProvider {
        readonly uri = vscode.Uri.parse(`crablang-analyzer-${xir}://${viewXir}/${xir}.rs`);
        readonly eventEmitter = new vscode.EventEmitter<vscode.Uri>();
        constructor() {
            vscode.workspace.onDidChangeTextDocument(
                this.onDidChangeTextDocument,
        ...
        async provideTextDocumentContent(
            ...
            const params = {
                textDocument: client.code2ProtocolConverter.asTextDocumentIdentifier(
                    crablangEditor.document
                ),
                position: client.code2ProtocolConverter.asPosition(crablangEditor.selection.active),
            };
    ...
    ctx.pushExtCleanup(
        vscode.workspace.registerTextDocumentContentProvider(`crablang-analyzer-${xir}`, tdcp)
    );
...
export function viewFileText(ctx: CtxInit): Cmd {
    const tdcp = new (class implements vscode.TextDocumentContentProvider {
        readonly uri = vscode.Uri.parse("crablang-analyzer-file-text://viewFileText/file.rs");
        readonly eventEmitter = new vscode.EventEmitter<vscode.Uri>();
        constructor() {
            vscode.workspace.onDidChangeTextDocument(
                this.onDidChangeTextDocument,
    ...
    ctx.pushExtCleanup(
        vscode.workspace.registerTextDocumentContentProvider("crablang-analyzer-file-text", tdcp)
    );
...
export function viewItemTree(ctx: CtxInit): Cmd {
    const tdcp = new (class implements vscode.TextDocumentContentProvider {
        readonly uri = vscode.Uri.parse("crablang-analyzer-item-tree://viewItemTree/itemtree.rs");
        readonly eventEmitter = new vscode.EventEmitter<vscode.Uri>();
        constructor() {
            vscode.workspace.onDidChangeTextDocument(
                this.onDidChangeTextDocument,
    ...
    ctx.pushExtCleanup(
        vscode.workspace.registerTextDocumentContentProvider("crablang-analyzer-item-tree", tdcp)
    );
...
export function expandMacro(ctx: CtxInit): Cmd {
    ...
    const tdcp = new (class implements vscode.TextDocumentContentProvider {
        uri = vscode.Uri.parse("crablang-analyzer-expand-macro://expandMacro/[EXPANSION].rs");
    ...
    ctx.pushExtCleanup(
        vscode.workspace.registerTextDocumentContentProvider("crablang-analyzer-expand-macro", tdcp)
    );
...
export function addProject(ctx: CtxInit): Cmd {
    return async () => {
        ...
        // this is a workaround to avoid needing writing the `crablang-project.json` into
        // a workspace-level VS Code-specific settings folder. We'd like to keep the
        // `crablang-project.json` entirely in-memory.
...
export function applyActionGroup(_ctx: CtxInit): Cmd {
    return async (actions: { label: string; arguments: lc.CodeAction }[]) => {
        ...
        await vscode.commands.executeCommand(
            "crablang-analyzer.resolveCodeAction",
