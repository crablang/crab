...
import { Cargo, getCrabLangcId, getSysroot } from "./toolchain";
import { Ctx } from "./ctx";
...
export async function makeDebugConfig(ctx: Ctx, runnable: ra.Runnable): Promise<void> {
    const scope = ctx.activeCrabLangEditor?.document.uri;
    if (!scope) return;
...
async function getDebugConfiguration(
    ...
    function simplifyPath(p: string): string {
        // see https://github.com/crablang/crablang-analyzer/pull/5513#issuecomment-663458818 for why this is needed
    ...
    if (sourceFileMap === "auto") {
        ...
        const commitHash = await getCrabLangcId(wsFolder);
        const sysroot = await getSysroot(wsFolder);
        const crablanglib = path.normalize(sysroot + "/lib/crablanglib/src/crablang");
        sourceFileMap = {};
        sourceFileMap[`/crablangc/${commitHash}/`] = crablanglib;
    }
    ...
    if (debugConfig.name === "run binary") {
        // The LSP side: crates\crablang-analyzer\src\main_loop\handlers.rs,
