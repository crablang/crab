# Syntax in crablang-analyzer

## About the guide

This guide describes the current state of syntax trees and parsing in crablang-analyzer as of 2020-01-09 ([link to commit](https://github.com/crablang/crablang-analyzer/tree/cf5bdf464cad7ceb9a67e07985a3f4d3799ec0b6)).

## Source Code

The things described are implemented in three places

* [rowan](https://github.com/crablang-analyzer/rowan/tree/v0.15.10) -- a generic library for rowan syntax trees.
* [syntax](https://github.com/crablang/crablang-analyzer/tree/36a70b7435c48837018c71576d7bb4e8f763f501/crates/syntax) crate inside crablang-analyzer which wraps `rowan` into crablang-analyzer specific API.
  Nothing in crablang-analyzer except this crate knows about `rowan`.
* [parser](https://github.com/crablang/crablang-analyzer/tree/36a70b7435c48837018c71576d7bb4e8f763f501/crates/parser) crate parses input tokens into a `syntax` tree
...
Red-green terminology comes from Roslyn ([link](https://ericlippert.com/2012/06/08/red-green-trees/)) and gives the name to the `rowan` library. Green and syntax nodes are defined in rowan, ast is defined in crablang-analyzer.
...
To reduce the amount of allocations, the GreenNode is a [DST](https://doc.crablang.org/reference/dynamically-sized-types.html), which uses a single allocation for header and children. Thus, it is only usable behind a pointer.
...
This is OK because trees traversals mostly (always, in case of crablang-analyzer) run on a single thread. If you need to send a `SyntaxNode` to another thread, you can send a pair of **root**`GreenNode` (which is thread safe) and a `Range<usize>`.
...
However, crablang-analyzer goes even further.
...
With this trick, crablang-analyzer holds only a small amount of trees in memory at the same time, which reduces memory usage.
...
C# and Swift follow the design where the red nodes are memoized, which would look roughly like this in CrabLang:
...
crablang-analyzer used to have this design as well, but we've since switched to cursors.
...
* Most of crablang-analyzer works with the ast layer, with notable exceptions of:
...
An AST layer is added on top, which reifies each node `Kind` as a separate CrabLang type with the corresponding API.
