...
need to adjust this doc as well and ping this issue:
  ...
  https://github.com/crablang/crablang-analyzer/issues/4604

--->

# LSP Extensions

This document describes LSP extensions used by crablang-analyzer.
It's a best effort document, when in doubt, consult the source (and send a PR with clarification ;-) ).
We aim to upstream all non CrabLang-specific extensions to the protocol, but this is not a top priority.
...
Requests, which are likely to always remain specific to `crablang-analyzer` are under `crablang-analyzer/` namespace.

If you want to be notified about the changes to this document, subscribe to [#4604](https://github.com/crablang/crablang-analyzer/issues/4604).

## Configuration in `initializationOptions`

**Upstream Issue:** https://github.com/microsoft/language-server-protocol/issues/567

The `initializationOptions` field of the `InitializeParams` of the initialization request should contain the `"crablang-analyzer"` section of the configuration.

`crablang-analyzer` normally sends a `"workspace/configuration"` request with `{ "items": ["crablang-analyzer"] }` payload.
...
If a language client does not know about `crablang-analyzer`'s configuration options it can get sensible defaults by doing any of the following:
...
At the moment, crablang-analyzer guarantees that only a single edit will have `InsertTextFormat.Snippet`.
...
* An alternative would be to use a more general "gotoSuper" request, which would work for super methods, super classes and super modules.
  This is the approach IntelliJ CrabLang is taking.
...
crablang-analyzer supports only one `kind`, `"cargo"`. The `args` for `"cargo"` look like this:
...
**Method:** `crablang-analyzer/analyzerStatus`
...
**Method:** `crablang-analyzer/reloadWorkspace`
...
**Method:** `crablang-analyzer/runFlycheck`
...
**Method:** `crablang-analyzer/clearFlycheck`
...
**Method:** `crablang-analyzer/cancelFlycheck`
...
**Method:** `crablang-analyzer/syntaxTree`
...
Primarily for debugging, but very useful for all people working on crablang-analyzer itself.

## View Hir

**Method:** `crablang-analyzer/viewHir`
...
For debugging or when working on crablang-analyzer itself.

## View Mir

**Method:** `crablang-analyzer/viewMir`
...
**Method:** `crablang-analyzer/viewFileText`
...
**Method:** `crablang-analyzer/viewItemTree`
...
**Method:** `crablang-analyzer/viewCrateGraph`
...
**Method:** `crablang-analyzer/expandMacro`
...
**Upstream Issue:** https://github.com/crablang/crablang-analyzer/issues/6462
...
**Method:** `crablang-analyzer/relatedTests`
...
**Upstream Issue:** https://github.com/crablang/crablang-analyzer/issues/6823
