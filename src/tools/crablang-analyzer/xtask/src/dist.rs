...
fn dist_server(sh: &Shell, release: &str, target: &Target) -> anyhow::Result<()> {
    ...
    cmd!(sh, "cargo build --manifest-path ./crates/crablang-analyzer/Cargo.toml --bin crablang-analyzer --target {target_name} --release").run()?;
...
impl Target {
    fn get(project_root: &Path) -> Self {
        ...
        let (exe_suffix, symbols_path) = if name.contains("-windows-") {
            (".exe".into(), Some(out_path.join("crablang_analyzer.pdb")))
        } else {
            (String::new(), None)
        };
        let server_path = out_path.join(format!("crablang-analyzer{exe_suffix}"));
        let artifact_name = format!("crablang-analyzer-{name}{exe_suffix}");
