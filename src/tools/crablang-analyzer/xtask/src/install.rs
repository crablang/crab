//! Installs crablang-analyzer language server and/or editor plugin.
...
fn install_client(sh: &Shell, client_opt: ClientOpt) -> Result<()> {
    ...
    let installed_extensions = if cfg!(unix) {
        cmd!(sh, "{code} --install-extension crablang-analyzer.vsix --force").run()?;
        cmd!(sh, "{code} --list-extensions").read()?
    } else {
        cmd!(sh, "cmd.exe /c {code}.cmd --install-extension crablang-analyzer.vsix --force").run()?;
        cmd!(sh, "cmd.exe /c {code}.cmd --list-extensions").read()?
    };

    if !installed_extensions.contains("crablang-analyzer") {
        bail!(
...
fn install_server(sh: &Shell, opts: ServerOpt) -> Result<()> {
    ...
    let cmd = cmd!(sh, "cargo install --path crates/crablang-analyzer --locked --force --features force-always-assert {features...}");
    cmd.run()?;
    Ok(())
}
