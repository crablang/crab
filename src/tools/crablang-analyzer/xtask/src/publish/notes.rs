...
impl Macro {
    ...
    fn process(&self) -> anyhow::Result<String> {
        ...
        let text = match name.as_str() {
            ...
            "pr" => {
                let pr = &self.target;
                let url = format!("https://github.com/crablang-analyzer/crablang-analyzer/pull/{pr}");
                format!("[`#{pr}`]({url})")
            }
            "commit" => {
                let hash = &self.target;
                let short = &hash[0..7];
                let url = format!("https://github.com/crablang-analyzer/crablang-analyzer/commit/{hash}");
                format!("[`{short}`]({url})")
            }
            "release" => {
                let date = &self.target;
                let url = format!("https://github.com/crablang-analyzer/crablang-analyzer/releases/{date}");
                format!("[`{date}`]({url})")
            }
