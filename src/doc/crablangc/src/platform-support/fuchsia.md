...
`@crablangbot ping fuchsia` to contact the team on GitHub.

## Table of contents

1. [Requirements](#requirements)
1. [Walkthrough structure](#walkthrough-structure)
1. [Compiling a CrabLang binary targeting Fuchsia](#compiling-a-crablang-binary-targeting-fuchsia)
    1. [Targeting Fuchsia with crablangup and cargo](#targeting-fuchsia-with-crablangup-and-cargo)
...
1. Compiling a CrabLang binary targeting Fuchsia.
...
## Compiling a CrabLang binary targeting Fuchsia

Today, there are two main ways to build a CrabLang binary targeting Fuchsia
using the Fuchsia SDK:
1. Allow [crablangup] to handle the installation of Fuchsia targets for you.
1. Build a toolchain locally that can target Fuchsia.

### Targeting Fuchsia with crablangup and cargo

The easiest way to build a CrabLang binary targeting Fuchsia is by allowing [crablangup]
to handle the installation of Fuchsia targets for you. This can be done by issuing
the following commands:

```sh
crablangup target add x86_64-unknown-fuchsia
crablangup target add aarch64-unknown-fuchsia
```

After installing our Fuchsia targets, we can now compile a CrabLang binary that targets
Fuchsia.

To create our CrabLang project, we can use [`cargo`][cargo] as follows:
...
Finally, we can build our crablang binary as:

```sh
cargo build --target x86_64-unknown-fuchsia
```

Now we have a CrabLang binary at `target/x86_64-unknown-fuchsia/debug/hello_fuchsia`,
...
`crablangc` built from source.

Before building CrabLang for Fuchsia, you'll need a clang toolchain that supports
Fuchsia as well. A recent version (14+) of clang should be sufficient to compile
CrabLang for Fuchsia.
...
By default, the CrabLang compiler installs itself to `/usr/local` on most UNIX
...
prefix = "<CRABLANG_SRC_PATH>/install"
```
...
export CARGO_TARGET_AARCH64_UNKNOWN_FUCHSIA_CRABLANGFLAGS="-C link-arg=--sysroot=${SDK_PATH}/arch/arm64/sysroot -Lnative=${SDK_PATH}/arch/arm64/sysroot/lib -Lnative=${SDK_PATH}/arch/arm64/lib"
...
export CARGO_TARGET_X86_64_UNKNOWN_FUCHSIA_CRABLANGFLAGS="-C link-arg=--sysroot=${SDK_PATH}/arch/x64/sysroot -Lnative=${SDK_PATH}/arch/x64/sysroot/lib -Lnative=${SDK_PATH}/arch/x64/lib"
```

Finally, the CrabLang compiler can be built and installed:

```sh
(source config-env.sh && ./x.py install)
```

Once `crablangc` is installed, we can create a new working directory to work from,
...
Using your freshly installed `crablangc`, you can compile a binary for Fuchsia using
...
With our CrabLang binary built, we can move to creating a Fuchsia package.
...
**`pkg/hello_fuchsia.manifest` if using crablangc**
```txt
...
`bin/hello_fuchsia=` entry will be different depending on how your CrabLang binary
...
* If using the compiled `crablangc`, you can simply pass `--test`
to the `crablangc` invocation and then repackage and rerun the Fuchsia package.
...
local CrabLang source checkout:

```sh
cd ${CRABLANG_SRC_PATH}
```

To run the CrabLang test suite on an emulated Fuchsia device, you must install the
CrabLang compiler locally. See "[Targeting Fuchsia with a compiler built from source](#targeting-fuchsia-with-a-compiler-built-from-source)"
...
src/ci/docker/scripts/fuchsia-test-runner.py start
    --crablang ${CRABLANG_SRC_PATH}/install
    --sdk ${SDK_PATH}
    --target {x86_64-unknown-fuchsia|aarch64-unknown-fuchsia}
```

Where `${CRABLANG_SRC_PATH}/install` is the `prefix` set in `config.toml` and
...
( \
    ...
    --target x86_64-unknown-fuchsia                                           \
    --run=always                                                              \
    --test-args --target-crablangcflags                                           \
...
CrabLang toolchain with `-Cpanic=abort`, you need to tell `x.py` to compile test
...
section describes how you can display CrabLang and/or Fuchsia source code in your
debugging session.
...
`--build-dir`. For example, to display the CrabLang and Fuchsia
source code:

```sh
${SDK_PATH}/tools/${ARCH}/ffx debug connect -- \
    --symbol-path target/x86_64-unknown-fuchsia/debug \
    --build-dir ${CRABLANG_SRC_PATH}/crablang \
...
src/ci/docker/scripts/fuchsia-test-runner.py debug                            \
    --crablang-src ${CRABLANG_SRC_PATH}                                               \
    --fuchsia-src ${FUCHSIA_SRC_PATH}                                         \
    --test ${TEST}
```

where `${TEST}` is relative to CrabLang's `tests` directory (e.g. `ui/abi/...`).
...
test suite binaries. We can do this by passing some new arguments to `crablangc`
...
*If you built your CrabLang toolchain with `panic=abort`, make sure to include the
...
[Fuchsia team]: https://team-api.infra.crablang.org/v1/teams/fuchsia.json
