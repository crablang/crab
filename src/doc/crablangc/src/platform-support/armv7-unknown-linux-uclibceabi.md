...
[crablang-bootstrap-armv7-unknown-linux-uclibceabi](https://github.com/lancethepants/crablang-bootstrap-armv7-unknown-linux-uclibceabi).
...
* `apt update && apt install crablang`

If you bootstrap crablang on your own using the project above, it will create a .deb file that you then can install with
```text
dpkg -i crablang_1.xx.x-x_arm.deb
```

After completing these steps you can use crablang normally in a native environment.

### Cross Compilation

To cross compile, you'll need to:

* Build the crablang cross toochain using  [crablang-bootstrap-armv7-unknown-linux-uclibceabi](https://github.com/lancethepants/crablang-bootstrap-armv7-unknown-linux-uclibceabi) or your own built toolchain.
* Link your built toolchain with

    ```text
    crablangup toolchain link stage2 \
    ${HOME}/crablang-bootstrap-armv7-unknown-linux-uclibceabi/src/crablang/crablang/build/x86_64-unknown-linux-gnu/stage2
    ```
* Build with:
    ```text
    ...
    CARGO_TARGET_ARMV7_UNKNOWN_LINUX_UCLIBCEABI_CRABLANGFLAGS='-Clink-arg=-s -Clink-arg=-Wl,--dynamic-linker=/mmc/lib/ld-uClibc.so.1 -Clink-arg=-Wl,-rpath,/mmc/lib' \
    cargo +stage2 \
    build \
    --target armv7-unknown-linux-uclibceabi \
    --release
...
We specify `CC`, `CXX`, `AR`, `CFLAGS`, and `CXXFLAGS` environment variables because sometimes a project or a subproject requires the use of your `'C'` cross toolchain. Since Tomatoware has a modified sysroot we also pass via CRABLANGFLAGS the location of the dynamic-linker and rpath.
...
* Extract `tar zxvf arm-soft-mmc.tgz -C /mmc`
* Add `/mmc/bin:/mmc:sbin/` to your PATH, or `source /mmc/etc/profile`
* `sudo /mmc/bin/apt update && sudo /mmc/bin/apt install crablang`

After completing these steps you can use crablang normally in a chroot environment.
