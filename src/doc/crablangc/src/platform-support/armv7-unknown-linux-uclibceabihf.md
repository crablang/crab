...
This tier supports the ARMv7 processor running a Linux kernel and uClibc-ng standard library.  It provides full support for crablang and the crablang standard library.
...
Compiling crablang for this target has been tested on `x86_64` linux hosts.  Other host types have not been tested, but may work, if you can find a suitable cross compilation toolchain for them.
...
The target can be built by enabling it for a `crablangc` build, by placing the following in `config.toml`:
...
* Link your built toolchain via:
  * `crablangup toolchain link stage2 ${CRABLANG}/build/x86_64-unknown-linux-gnu/stage2`
