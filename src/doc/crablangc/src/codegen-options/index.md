...
All of these options are passed to `crablangc` via the `-C` flag, short for "codegen." You can see
a version of this list for your exact compiler by running `crablangc -C help`.
...
Supported values can also be discovered by running `crablangc --print code-models`.
...
LLVM bitcode is required when crablangc is performing link-time optimization (LTO).
It is also required on some targets like iOS ones where vendors look for LLVM
bitcode. Embedded bitcode will appear in crablangc-generated object files inside of
...
are building directly with `crablangc` we recommend using `-C embed-bitcode=no`
whenever you are not using LTO.

If combined with `-C lto`, `-C embed-bitcode=no` will cause `crablangc` to abort
at start-up, because the combination is invalid.

> **Note**: if you're building CrabLang code with LTO then you probably don't even
...
> with and without LTO. For example CrabLang's standard library ships with embedded
...
> reason for that is that it's how it was for crablangc 1.44 and prior. In 1.45 this
...
This flag allows you to enable incremental compilation, which allows `crablangc`
...
linker will use libraries and objects shipped with CrabLang instead or those in the system.
It takes one of the following values:

* no value: crablangc will use heuristic to disable self-contained mode if system has necessary tools.
* `y`, `yes`, `on`, `true`: use only libraries/objects shipped with CrabLang.
* `n`, `no`, `off` or `false`: rely on the user or the linker to provide non-CrabLang libraries/objects.

This allows overriding cases when detection fails or user wants to use shipped libraries.

## linker

This flag controls which linker `crablangc` invokes to link your code. It takes a
...
This flag controls the linker flavor used by `crablangc`. If a linker is given with
...
determine the linker to use. Every `crablangc` target defaults to some linker
...
* `ptx-linker`: use
  [`crablang-ptx-linker`](https://github.com/denzp/crablang-ptx-linker) for Nvidia
  NVPTX GPGPU support.
* `bpf-linker`: use
...
Note that crablangc can also read its own object files produced with
...
By default, `crablangc` prefers to statically link dependencies. This option will
...
Supported values can also be discovered by running `crablangc --print relocation-models`.
...
This option controls whether `crablangc` generates code that emulates floating
...
that `crablangc` generates. The default behavior of this option is
...
This option controls the [name mangling] format for encoding CrabLang item names
...
This instructs `crablangc` to generate code specifically for a particular processor.

You can run `crablangc --print target-cpus` to see the valid options to pass
...
To see the valid options and an example of use, run `crablangc --print
target-features`.
...
This instructs `crablangc` to schedule code specifically for a particular
