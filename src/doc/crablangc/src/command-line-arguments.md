...
Here's a list of command-line arguments to `crablangc` and what they do.

<a id="option-help"></a>
## `-h`/`--help`: get help

This flag will print out help information for `crablangc`.
...
`+verbatim` means that crablangc itself won't add any target-specified library prefixes or suffixes
(like `lib` or `.a`) to the library name, and will try its best to ask for the same thing from the
linker.

For `ld`-like linkers supporting GNU extensions crablangc will use the `-l:filename` syntax (note the
...
`-verbatim` means that crablangc will either add a target-specific prefix and suffix to the library
...
`-Clink-arg=/my/absolute/path` can be used for doing this from stable `crablangc`.

<a id="option-crate-type"></a>
## `--crate-type`: a list of types of crates for the compiler to emit

This instructs `crablangc` on which crate type to build. This flag accepts a
...
- `lib` — Generates a library kind preferred by the compiler, currently
  defaults to `rlib`.
- `rlib` — A CrabLang static library.
- `staticlib` — A native static library.
- `dylib` — A CrabLang dynamic library.
...
This informs `crablangc` of the name of your crate.
...
- `mir` — Generates a file containing crablangc's mid-level intermediate
...
- `link-args` — This flag does not disable the `--emit` step. When linking,
  this flag causes `crablangc` to print the full linker invocation in a
...
Each error of `crablangc`'s comes with an error code; this will print
out a longer explanation of a given error.

<a id="option-test"></a>
## `--test`: build a test harness

When compiling this crate, `crablangc` will ignore your `main` function
...
This flag will allow you to set unstable options of crablangc. In order to set multiple options,
the -Z flag can be used multiple times. For example: `crablangc -Z verbose -Z time-passes`.
Specifying options with -Z is only available on nightly. To view all available options
run: `crablangc -Z help`, or see [The Unstable Book](../unstable-book/index.html).
...
This flag will print out `crablangc`'s version.
...
The "sysroot" is where `crablangc` looks for the crates that come with the CrabLang
...
crablangc then all of the compiler's diagnostic output will be emitted in the form
...
- `diagnostic-rendered-ansi` - by default JSON blobs in their `rendered` field
  ...
  to be used to colorize the message in the manner crablangc typically already does
...
- `artifacts` - this instructs crablangc to emit a JSON blob for each artifact that
