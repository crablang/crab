...
CrabLang provides three tiers of target support:

- CrabLang provides no guarantees about tier 3 targets; they exist in the codebase,
  but may or may not build.
- CrabLang's continuous integration checks that tier 2 targets will always build,
  but they may or may not pass tests.
- CrabLang's continuous integration checks that tier 1 targets will always build
  and pass tests.

Adding a new tier 3 target imposes minimal requirements; we focus primarily on
avoiding disruption to other ongoing CrabLang development.

Tier 2 and tier 1 targets place work on CrabLang project developers as a whole, to
avoid breaking the target. The broader CrabLang community may also feel more
...
disruptions to ongoing CrabLang development.
...
provide *host tools* (such as `crablangc` and `cargo`); each of those tiers
...
The policy for each tier also documents the CrabLang governance teams that must
...
CrabLang project. Neither this policy nor any decisions made regarding targets
...
not preclude an existing target's maintainers using issues (on the CrabLang
...
platform-support section of this crablangc manual, with a link from the target's
...
multiple stable releases of CrabLang should typically occur between promotions of a
target.

The availability or tier of a target in stable CrabLang is not a hard stability
...
To propose addition of a new target, open a pull request on [`crablang/crablang`]:

- Copy the [Tier 3 target policy](#tier-3-target-policy) to the description
  and fill it out, see [example][tier3example].
- Add a new description for the target in `src/doc/crablangc/src/platform-support`
...
- Ensure the pull request is assigned to a member of the [CrabLang compiler team][crablang_compiler_team] by commenting:
  ```text
  r? compiler-team
  ```

[tier3example]: https://github.com/crablang/crablang/pull/94872
[platform_template]: https://github.com/crablang/crablang/blob/master/src/doc/crablangc/src/platform-support/TEMPLATE.md
[summary]: https://github.com/crablang/crablang/blob/master/src/doc/crablangc/src/SUMMARY.md
[platformsupport]: https://github.com/crablang/crablang/blob/master/src/doc/crablangc/src/platform-support.md
[crablang_compiler_team]: https://www.crablang.org/governance/teams/compiler
[`crablang/crablang`]: https://github.com/crablang/crablang

## Tier 3 target policy

At this tier, the CrabLang project provides no official support for a target, so we
...
- Targets must use naming consistent with any existing targets; for instance, a
  target for the same CPU or OS as an existing CrabLang target should use the same
  name for that CPU or OS. Targets should normally use the same names and
  naming conventions as used elsewhere in the broader ecosystem beyond CrabLang
...
- Tier 3 targets may have unusual requirements to build or use, but must not
  create legal issues or impose onerous legal terms for the CrabLang project or for
  CrabLang developers or users.
  - The target must not introduce license incompatibilities.
  - Anything added to the CrabLang repository must be under the standard CrabLang
    license (`MIT OR Apache-2.0`).
  - The target must not cause the CrabLang tools or libraries built for any other
    host (even when supporting cross-compilation to the target) to depend
    on any new dependency less permissive than the CrabLang licensing policy. This
    applies whether the dependency is a CrabLang crate that would require adding
    new license exceptions (as specified by the `tidy` tool in the
    crablang/crablang repository), or whether the dependency is a native library
    or binary. In other words, the introduction of the target must not cause a
    user installing or running a version of CrabLang or the CrabLang tools to be
  ...
  - Compiling, linking, and emitting functional binaries, libraries, or other
    ...
    such libraries at all. For instance, `crablangc` built for the target may
    depend on a common proprietary C runtime library or console output library,
    but must not depend on a proprietary code generation library or code
    optimization library. CrabLang's license permits such combinations, but the
    CrabLang project has no interest in maintaining such combinations within the
    scope of CrabLang itself, even at tier 3.
  - "onerous" here is an intentionally subjective term. At a minimum, "onerous"
    legal/licensing terms include but are *not* limited to: non-disclosure
    requirements, non-compete requirements, contributor license agreements
    (CLAs) or equivalent, "non-commercial"/"research-only"/etc terms,
    requirements conditional on the employer or employment of any particular
    CrabLang developers, revocable terms, any requirements that create liability
    for the CrabLang project or its developers or users, or any requirements that
    adversely affect the livelihood or prospects of the CrabLang project or its
    developers or users.
- Neither this policy nor any decisions made regarding targets shall create any
  binding agreement or estoppel by any party. If any member of an approving
  CrabLang team serves as one of the maintainers of a target, or has any legal or
...
- The target must provide documentation for the CrabLang community explaining how
...
of the CrabLang codebase, we may post a PR to remove it; any such PR will be CCed
to the target maintainers (and potentially other people who have previously
worked on the target), to check potential interest in improving the situation.

## Tier 2 target policy

At this tier, the CrabLang project guarantees that a target builds, and will reject
patches that fail to build on a target. Thus, we place requirements that ensure
the target will not block forward progress of the CrabLang project.
...
- A tier 2 target must have a designated team of developers (the "target
  ...
  - The target maintainers should not only fix target-specific issues, but
    should use any such issue as an opportunity to educate the CrabLang community
    about portability to their target, and enhance documentation of the target.
- The target must not place undue burden on CrabLang developers not specifically
  concerned with that target. CrabLang developers are expected to not gratuitously
...
- The target must provide documentation for the CrabLang community explaining how
  ...
  to run CrabLang programs and tests for the target using emulation, to allow
...
- If introducing a new tier 2 or higher target that is identical to an existing
  CrabLang target except for the baseline expectations for the features or versions
  ...
  - Note that in some cases, based on the usage of existing targets within the
    CrabLang community, CrabLang developers or a target's maintainers may wish to
...
- Tier 2 targets must not leave any significant portions of `core` or the
  ...
  - The right approach to handling a missing feature from a target may depend
    ...
    some cases, a target may be co-developed along with CrabLang support, and CrabLang
...
- The code generation backend for the target should not have deficiencies that
  invalidate CrabLang safety properties, as evaluated by the CrabLang compiler team.
  ...
  needed to ensure safe CrabLang code cannot cause undefined behavior or other
  ...
  The CrabLang compiler team must be satisfied with the balance between these
  limitations and the difficulty of implementing the necessary features.
  - For example, if CrabLang relies on a specific code generation feature to ensure
    that safe code cannot overflow the stack, the code generation for the
    target should support that feature.
  - If the CrabLang compiler introduces new safety properties (such as via new
    capabilities of a compiler backend), the CrabLang compiler team will determine
    if they consider those new safety properties a best-effort improvement for
    specific targets, or a required property for all CrabLang targets. In the
...
- If the target supports C code, and the target has an interoperable calling
  convention for C code, the CrabLang target must support that C calling convention
  for the platform via `extern "C"`. The C calling convention does not need to
  be the default CrabLang calling convention for the target, however.
- The target must build reliably in CI, for all components that CrabLang's CI
  considers mandatory.
...
- In addition to the legal requirements for all targets (specified in the tier
  3 requirements), because a tier 2 target typically involves the CrabLang project
  ...
  members of the CrabLang project, including infrastructure team members and those
  ...
  - As an exception to this, if the target's primary purpose is to build
    ...
    terms, as long as such terms are satisfied by CrabLang's existing practices of
    providing full corresponding source code. Note that anything added to the
    CrabLang repository itself must still use CrabLang's standard license terms.
...
maintainers, and will be communicated widely to the CrabLang community before being
...
timely fashion, CrabLang teams may land pull requests that temporarily disable some
...
host (such as `crablangc` and `cargo`). This allows the target to be used as a
...
- Depending on the target, its capabilities, its performance, and the
  ...
  target may include only `crablangc` and `cargo`, or may include additional tools
  such as `clippy` and `crablangfmt`.
...
- The host tools must build and run reliably in CI (for all components that
  CrabLang's CI considers mandatory), though they may or may not pass tests.
...
- If the host tools for the platform would normally be expected to be signed or
  ...
  "developer mode" or an additional prompt), it must be possible for the CrabLang
  project's automated builds to apply the appropriate signature process,
  without any manual intervention by either CrabLang developers, target
  ...
  - This process may require the execution of a legal agreement with the
    ...
    agreements on behalf of the CrabLang project; this review and approval exists
  ...
  - This process involved must be available under substantially similar
    ...
    the CrabLang project does not suffice.
  - This requirement exists to ensure that CrabLang builds, including nightly
...
At this tier, the CrabLang project guarantees that a target builds and passes all
...
- Tier 1 targets must have substantial, widespread interest within the
  ...
  users of CrabLang across multiple organizations or projects. These requirements
...
- The target must build and pass tests reliably in CI, for all components that
  CrabLang's CI considers mandatory.
...
- The target must provide as much of the CrabLang standard library as is feasible
...
- If running the testsuite requires additional infrastructure (such as physical
  ...
  such resources to the CrabLang project, to the satisfaction and approval of the
  CrabLang infrastructure team.
  ...
  - If it is not possible to run the target via emulation, these resources must
    additionally be sufficient for the CrabLang infrastructure team to make them
    available for access by CrabLang team members, for the purposes of development
    ...
    CrabLang developers to test the target, but does not obligate other CrabLang
    developers to make target-specific fixes.)
  - Resources provided for CI and similar infrastructure must be available for
    continuous exclusive use by the CrabLang project. Resources provided
    for access by CrabLang team members for development and testing must be
...
- Tier 1 targets must not have a hard requirement for signed, verified, or
  ...
  agreements.)
  - The CrabLang project may decide to supply appropriately signed binaries if
    ...
    However, this additional tier 1 requirement ensures that CrabLang developers
    can develop and test CrabLang software for the target (including CrabLang itself),
...
release teams. Any such proposal will be communicated widely to the CrabLang
...
- Tier 1 targets with host tools should typically include all of the additional
  tools such as `clippy` and `crablangfmt`, unless there is a target-specific
...
- Host tools for the target must have substantial, widespread interest within
  ...
  production users of CrabLang across multiple organizations or projects. These
...
- The host tools must build, run, and pass tests reliably in CI, for all
  components that CrabLang's CI considers mandatory.
  - The target must not disable an excessive number of tests or pieces of tests
    in the testsuite in order to do so. This is a subjective requirement.
...
will be communicated widely to the CrabLang community, both when initially proposed
and before being dropped from a stable release.

[MCP]: https://forge.crablang.org/compiler/mcp.html
