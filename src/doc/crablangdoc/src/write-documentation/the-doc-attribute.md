...
The `#[doc]` attribute lets you control various aspects of how `crablangdoc` does
its job.
...
and the CrabLang edition respectively.
...
given. `crablangdoc` uses this number, plus the base URL given here, to link to
the tracking issue.
...
generating links to external crates. When crablangdoc needs to generate a link to
...
By default, `crablangdoc` will include the source code of your program, with links
...
By default, `crablangdoc` will automatically add a line with `extern crate my_crate;` into each doctest.
...
up. For example, consider this CrabLang code:
...
to link to. `crablangdoc` will inline these definitions, and so we end up in the same case
...
One special case: In CrabLang 2018 and later, if you `pub use` one of your dependencies, `crablangdoc` will
...
It takes a pointer to an `Obj` type and returns an integer. In CrabLang, it might
...
However, if you want to look for the CrabLang equivalent of `lib_name_do_something`,
