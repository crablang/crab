//! This module implements [RFC 1946]: Intra-crablangdoc-links
//!
//! [RFC 1946]: https://github.com/crablang/rfcs/blob/master/text/1946-intra-crablangdoc-links.md

use pulldown_cmark::LinkType;
use crablangc_ast::util::comments::may_have_doc_links;
use crablangc_data_structures::{
    fx::{FxHashMap, FxHashSet},
    intern::Interned,
};
use crablangc_errors::{Applicability, Diagnostic};
use crablangc_hir::def::Namespace::*;
use crablangc_hir::def::{DefKind, Namespace, PerNS};
use crablangc_hir::def_id::{DefId, CRATE_DEF_ID};
use crablangc_hir::Mutability;
use crablangc_middle::ty::{fast_reject::TreatProjections, Ty, TyCtxt};
use crablangc_middle::{bug, ty};
use crablangc_resolve::crablangdoc::{has_primitive_or_keyword_docs, prepare_to_doc_link_resolution};
use crablangc_resolve::crablangdoc::{strip_generics_from_path, MalformedGenerics};
use crablangc_session::lint::Lint;
...
type ResolveRes = crablangc_hir::def::Res<crablangc_ast::NodeId>;
...
pub(crate) enum UrlFragment {
    Item(DefId),
    /// A part of a page that isn't a crablang item.
    ///
...
impl<'a, 'tcx> LinkCollector<'a, 'tcx> {
    ...
    ///
    /// [enum struct variant]: crablangc_hir::VariantData::Struct
    ...
    fn resolve<'path>(
        ...
        let path_root = split
            ...
            // So we can be sure that `crablangc_resolve` was accurate when it said it wasn't resolved.
            .ok_or_else(|| {
        ...
        // links to primitives when `#[crablangc_doc_primitive]` is present. It should give an ambiguity
        // error instead and special case *only* modules with `#[crablangc_doc_primitive]`, not all
        // primitives.
    ...
    fn primitive_type_to_ty(&mut self, prim: PrimitiveType) -> Option<Ty<'tcx>> {
        ...
        // See https://github.com/crablang/crablang/issues/90703#issuecomment-1004263455
        Some(match prim {
            Bool => tcx.types.bool,
            Str => tcx.types.str_,
            Char => tcx.types.char,
    ...
    fn resolve_associated_item(
        ...
        match root_res {
            ...
            Res::Def(
                ...
                // because they are real CrabLang syntax (unlike the intra-doc links
...
impl LinkCollector<'_, '_> {
    fn resolve_links(&mut self, item: &Item) {
        ...
        for (item_id, doc) in prepare_to_doc_link_resolution(&item.attrs.doc_strings) {
            ...
            // This is a degenerate case and it's not supported by crablangdoc.
        ...
        match res {
            Res::Primitive(_) => {
                if let Some(UrlFragment::Item(id)) = fragment {
                    ...
                    // thinks `register_res` is only needed for cross-crate re-exports, but CrabLang
                    // doesn't allow statements like `use str::trim;`, making this a (hopefully)
                    // valid omission. See https://github.com/crablang/crablang/pull/80660#discussion_r551585677
            ...
            Res::Def(kind, id) => {
                ...
                let page_id = clean::register_res(self.cx, crablangc_hir::def::Res::Def(kind, id));
                Some(ItemLink {
                    link: Box::<str>::from(&*ori_link.link),
                    link_text: link_text.clone(),
                    page_id,
    ...
    fn verify_disambiguator(
        ...
        // item can be non-local e.g. when using `#[crablangc_doc_primitive = "pointer"]`
    ...
    fn report_disambiguator_mismatch(
        ...
        let callback = |diag: &mut Diagnostic, sp: Option<crablangc_span::Span>| {
            let note = format!(
    ...
    fn report_rawptr_assoc_feature_gate(&self, dox: &str, ori_link: &Range<usize>, item: &Item) {
        ...
        crablangc_session::parse::feature_err(
            &self.cx.tcx.sess.parse_sess,
            sym::intra_doc_pointers,
            span,
            "linking to associated items of raw pointers is experimental",
        )
        .note("crablangdoc does not allow disambiguating between `*const` and `*mut`, and pointers are unstable until it does")
        .emit();
    }
            ...
            Some(expected_ns) => {
                match self.resolve(path_str, expected_ns, item_id, module_id) {
                    ...
                    Err(err) => {
                        ...
                        // See https://github.com/crablang/crablang/pull/76955#discussion_r493953382 for a good approach.
...
enum Disambiguator {
    /// `prim@`
    ///
    /// This is buggy, see <https://github.com/crablang/crablang/pull/77875#discussion_r503583103>
    Primitive,
    /// `struct@` or `f()`
    Kind(DefKind),
    /// `type@`
...
impl Suggestion {
    ...
    ) -> Vec<(crablangc_span::Span, String)> {
...
fn report_diagnostic(
    ...
    decorate: impl FnOnce(&mut Diagnostic, Option<crablangc_span::Span>),
) {
...
fn disambiguator_error(
    ...
    report_diagnostic(cx.tcx, BROKEN_INTRA_DOC_LINKS, msg, &diag_info, |diag, _sp| {
        let msg = format!(
            "see {}/crablangdoc/write-documentation/linking-to-items-by-name.html#namespaces-and-disambiguators for more info about disambiguators",
        ...
        |diag, sp| {
            let note = match err {
                ...
                MalformedGenerics::HasFullyQualifiedSyntax => {
                    diag.note(
                        "see https://github.com/crablang/crablang/issues/74563 for more information",
                    );
