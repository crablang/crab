...
use crablangc_middle::lint::LintLevelSource;
...
impl crate::doctest::Tester for Tests {
    fn add_test(&mut self, _: String, config: LangString, _: usize) {
        if config.crablang && config.ignore == Ignore::None {
            self.found_tests += 1;
        }
    }
}
...
pub(crate) fn look_for_tests<'tcx>(cx: &DocContext<'tcx>, dox: &str, item: &Item) {
    ...
    if tests.found_tests == 0 && cx.tcx.features().crablangdoc_missing_doc_code_examples {
