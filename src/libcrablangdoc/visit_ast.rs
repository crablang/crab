//! The CrabLang AST Visitor. Extracts useful information and massages it into a form
//! usable for `clean`.

use crablangc_data_structures::fx::{FxHashSet, FxIndexMap};
use crablangc_hir as hir;
use crablangc_hir::def::{DefKind, Res};
use crablangc_hir::def_id::{DefId, DefIdMap, LocalDefId, LocalDefIdSet};
use crablangc_hir::intravisit::{walk_item, Visitor};
use crablangc_hir::{Node, CRATE_HIR_ID};
use crablangc_middle::hir::nested_filter;
use crablangc_middle::ty::TyCtxt;
use crablangc_span::def_id::{CRATE_DEF_ID, LOCAL_CRATE};
...
/// This module is used to store stuff from CrabLang's AST in a more convenient
...
pub(crate) struct CrabLangdocVisitor<'a, 'tcx> {
...
impl<'a, 'tcx> CrabLangdocVisitor<'a, 'tcx> {
    pub(crate) fn new(cx: &'a mut core::DocContext<'tcx>) -> CrabLangdocVisitor<'a, 'tcx> {
    ...
    fn visit_mod_contents(&mut self, def_id: LocalDefId, m: &'tcx hir::Mod<'tcx>) {
        ...
        for &i in m.item_ids {
            ...
            // Later passes in crablangdoc will de-duplicate by name and kind, so if glob-
...
impl<'a, 'tcx> Visitor<'tcx> for CrabLangdocVisitor<'a, 'tcx> {
    ...
    fn visit_label(&mut self, _: &crablangc_ast::Label) {
        // Unneeded.
    }
