...
use crablangc_errors::{ColorConfig, ErrorGuaranteed, FatalError, TerminalUrl};
use crablangc_hir::def_id::{LocalDefId, CRATE_DEF_ID, LOCAL_CRATE};
use crablangc_hir::{self as hir, intravisit, CRATE_HIR_ID};
...
use crablangc_parse::parser::attr::InnerAttrPolicy;
use crablangc_session::config::{self, CrateType, ErrorOutputType};
...
use crablangc_span::{BytePos, FileName, Pos, Span, DUMMY_SP};
...
use crate::config::Options as CrabLangdocOptions;
use crate::html::markdown::{self, ErrorCodes, Ignore, LangString};
use crate::lint::init_lints;
use crate::passes::span_of_attrs;

/// Options that apply to all doctests in a crate or Markdown file (for `crablangdoc foo.md`).
...
pub(crate) fn run(options: CrabLangdocOptions) -> Result<(), ErrorGuaranteed> {
    ...
    let config = interface::Config {
        ...
        output_dir: None,
        file_loader: None,
        locale_resources: crablangc_driver::DEFAULT_LOCALE_RESOURCES,
        lint_caps,
...
pub(crate) fn run_tests(
    ...
    test_args.insert(0, "crablangdoctest".to_string());
    if nocapture {
...
fn scrape_test_config(attrs: &[ast::Attribute]) -> GlobalTestOptions {
    ...
    for attr in attrs {
        ...
        {
            for item in l {
                opts.attrs.push(ppcrablang::meta_list_item_to_string(item));
            }
        }
    }
...
// NOTE: Keep this in sync with the equivalent structs in crablangc
// and cargo.
// We could unify this struct the one in crablangc but they have different
...
fn run_test(
    ...
    crate_name: &str,
    line: usize,
    crablangdoc_options: CrabLangdocOptions,
    ...
    let crablang_out = add_exe_suffix("crablang_out".to_owned(), &target);
    let output_file = outdir.path().join(crablang_out);

    let crablangc_binary = crablangdoc_options
        .test_builder
        .as_deref()
        .unwrap_or_else(|| crablangc_interface::util::crablangc_path().expect("found crablangc"));
    let mut compiler = Command::new(&crablangc_binary);
    compiler.arg("--crate-type").arg("bin");
    for cfg in &crablangdoc_options.cfgs {
        compiler.arg("--cfg").arg(&cfg);
    }
    if !crablangdoc_options.check_cfgs.is_empty() {
        compiler.arg("-Z").arg("unstable-options");
        for check_cfg in &crablangdoc_options.check_cfgs {
            compiler.arg("--check-cfg").arg(&check_cfg);
        }
    }
    if let Some(sysroot) = crablangdoc_options.maybe_sysroot {
        compiler.arg("--sysroot").arg(sysroot);
    }
    compiler.arg("--edition").arg(&edition.to_string());
    compiler.env("UNSTABLE_CRABLANGDOC_TEST_PATH", path);
    compiler.env("UNSTABLE_CRABLANGDOC_TEST_LINE", format!("{}", line as isize - line_offset as isize));
    compiler.arg("-o").arg(&output_file);
    if lang_string.test_harness {
        compiler.arg("--test");
    }
    if crablangdoc_options.json_unused_externs.is_enabled() && !lang_string.compile_fail {
        compiler.arg("--error-format=json");
        compiler.arg("--json").arg("unused-externs");
        compiler.arg("-Z").arg("unstable-options");
        compiler.arg("-W").arg("unused_crate_dependencies");
    }
    for lib_str in &crablangdoc_options.lib_strs {
        compiler.arg("-L").arg(&lib_str);
    }
    for extern_str in &crablangdoc_options.extern_strs {
        compiler.arg("--extern").arg(&extern_str);
    }
    compiler.arg("-Ccodegen-units=1");
    for codegen_options_str in &crablangdoc_options.codegen_options_strs {
        compiler.arg("-C").arg(&codegen_options_str);
    }
    for unstable_option_str in &crablangdoc_options.unstable_opts_strs {
        compiler.arg("-Z").arg(&unstable_option_str);
    }
    if no_run && !lang_string.compile_fail && crablangdoc_options.persist_doctests.is_none() {
        compiler.arg("--emit=metadata");
    }
    compiler.arg("--target").arg(match target {
        TargetTriple::TargetTriple(s) => s,
        TargetTriple::TargetJson { path_for_crablangdoc, .. } => {
            path_for_crablangdoc.to_str().expect("target path must be valid unicode").to_string()
        }
    });
    if let ErrorOutputType::HumanReadable(kind) = crablangdoc_options.error_format {
    ...
    let mut child = compiler.spawn().expect("Failed to spawn crablangc process");
    {
    ...
    if let Some(run_directory) = crablangdoc_options.test_run_directory {
        cmd.current_dir(run_directory);
    }

    let result = if crablangdoc_options.nocapture {
...
/// Transforms a test into code that can be compiled into a CrabLang binary, and returns the number of
...
pub(crate) fn make_test(
    ...
    // Uses libcrablangc_ast to parse the doctest and find if there's a main fn and the extern
    // crate already is included.
    let result = crablangc_driver::catch_fatal_errors(|| {
        crablangc_span::create_session_if_not_set_then(edition, |_| {
            use crablangc_errors::emitter::{Emitter, EmitterWriter};
            use crablangc_errors::Handler;
            ...
            // send all the errors that libcrablangc_ast emits directly into a `Sink` instead of stderr.
            let sm = Lrc::new(SourceMap::new(FilePathMapping::empty()));
            let fallback_bundle = crablangc_errors::fallback_fluent_bundle(
                crablangc_driver::DEFAULT_LOCALE_RESOURCES.to_vec(),
                false,
            );
    ...
    // https://github.com/crablang/crablang/issues/56898
            ...
            PartitionState::Attrs => {
                    ...
                    || trimline.starts_with("#[macro_use] extern crate")
                {
                    ...
                    if !mod_attr_pending.is_empty() {
                        ...
                        // prepended before crablangdoc's inserts.
                        PartitionState::Attrs
                    } else {
...
pub(crate) struct Collector {
    ...
    //
    // In tests from CrabLang source, this is the path to the item
...
impl Tester for Collector {
    fn add_test(&mut self, test: String, config: LangString, line: usize) {
        ...
        let edition = config.edition.unwrap_or(self.crablangdoc_options.edition);
        let crablangdoc_options = self.crablangdoc_options.clone();
        let runtool = self.crablangdoc_options.runtool.clone();
        let runtool_args = self.crablangdoc_options.runtool_args.clone();
        let target = self.crablangdoc_options.target.clone();
        let target_str = target.to_string();
        let unused_externs = self.unused_extern_reports.clone();
        let no_run = config.no_run || crablangdoc_options.no_run;
        ...
        let outdir = if let Some(mut path) = crablangdoc_options.persist_doctests.clone() {
            path.push(&test_id);
            ...
            DirState::Temp(
                TempFileBuilder::new()
                    .prefix("crablangdoctest")
                    .tempdir()
                    .expect("crablangdoc needs a tempdir"),
            )
        };
    ...
    fn register_header(&mut self, name: &str, level: u32) {
        if self.use_headers {
            ...
            let name = name
                ...
                .enumerate()
                .map(|(i, c)| {
                    if (i == 0 && crablangc_lexer::is_id_start(c))
                        || (i != 0 && crablangc_lexer::is_id_continue(c))
                    {
                        c
                    } else {
                        '_'
    ...
    fn visit_item(&mut self, item: &'hir hir::Item<'_>) {
        let name = match &item.kind {
            hir::ItemKind::Impl(impl_) => {
                crablangc_hir_pretty::id_to_string(&self.map, impl_.self_ty.hir_id)
            }
