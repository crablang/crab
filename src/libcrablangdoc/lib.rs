#![doc(
    html_root_url = "https://doc.crablang.org/nightly/",
...
// the crablangdoc artifacts aren't stored in crablangc's cargo target directory.
// So if `crablangc` was specified in Cargo.toml, this would spuriously rebuild crates.
//
...
extern crate crablangc_expand;
extern crate crablangc_feature;
...
// See docs in https://github.com/crablang/crablang/blob/master/compiler/crablangc/src/main.rs
// about jemalloc.
...
use crablangc_driver::abort_on_err;
use crablangc_errors::ErrorGuaranteed;
use crablangc_interface::interface;
use crablangc_middle::ty::TyCtxt;
use crablangc_session::config::{make_crate_type_option, ErrorOutputType, CrabLangcOptGroup};
use crablangc_session::getopts;
use crablangc_session::{early_error, early_warn};

use crate::clean::utils::DOC_CRABLANG_LANG_ORG_CHANNEL;
...
macro_rules! map {
    ($( $key: expr => $val: expr ),* $(,)*) => {{
        let mut map = ::crablangc_data_structures::fx::FxHashMap::default();
        $( map.insert($key, $val); )*
        map
    }}
}
...
pub fn main() {
    ...
    // with `--features=static_max_level_info`, which disables almost all crablangdoc logging. To avoid
    // this, compile our own version of `tracing` that logs all levels.
    // NOTE: this compiles both versions of tracing unconditionally, because
    // - The compile time hit is not that bad, especially compared to crablangdoc's incremental times, and
    ...
    // `debug_logging = true` is because all crablangc logging goes to its version of tracing (the one
    // in the sysroot), and all of crablangdoc's logging goes to its version (the one in Cargo.toml).
    init_logging();
    crablangc_driver::init_env_logger("CRABLANGDOC_LOG");

    let exit_code = crablangc_driver::catch_with_exit_code(|| match get_args() {
...
fn init_logging() {
    let color_logs = match std::env::var("CRABLANGDOC_LOG_COLOR").as_deref() {
        Ok("always") => true,
        Ok("never") => false,
    ...
    let filter = tracing_subscriber::EnvFilter::from_env("CRABLANGDOC_LOG");
...
fn opts() -> Vec<CrabLangcOptGroup> {
    let stable: fn(_, fn(&mut getopts::Options) -> &mut _) -> _ = CrabLangcOptGroup::stable;
    let unstable: fn(_, fn(&mut getopts::Options) -> &mut _) -> _ = CrabLangcOptGroup::unstable;
    vec![
        stable("h", |o| o.optflagmulti("h", "help", "show this help message")),
        stable("V", |o| o.optflagmulti("V", "version", "print crablangdoc's version")),
        ...
        stable("cfg", |o| o.optmulti("", "cfg", "pass a --cfg to crablangc", "")),
        unstable("check-cfg", |o| o.optmulti("", "check-cfg", "pass a --check-cfg to crablangc", "")),
        stable("extern", |o| o.optmulti("", "extern", "pass an --extern to crablangc", "NAME[=PATH]")),
        ...
        stable("C", |o| {
            o.optmulti("C", "codegen", "pass a codegen option to crablangc", "OPT[=VALUE]")
        }),
        ...
        stable("e", |o| {
            o.optopt(
                ...
                "To add some CSS rules with a given file to generate doc with your \
                 own theme. However, your theme might break if the crablangdoc's generated HTML \
        ...
        stable("default-theme", |o| {
            o.optopt(
                ...
                "Set the default theme. THEME should be the theme name, generally lowercase. \
                 ...
                 The set of themes, and the crablangdoc built-in default, are not stable.",
                "THEME",
            )
        }),
        unstable("default-setting", |o| {
            o.optmulti(
                "",
                "default-setting",
                "Default value for a crablangdoc setting (used when \"crablangdoc-SETTING\" is absent \
        ...
        stable("edition", |o| {
            o.optopt(
                "",
                "edition",
                "edition to use when compiling crablang code (default: 2015)",
                "EDITION",
            )
        }),
        stable("color", |o| {
        ...
        unstable("test-builder", |o| {
            o.optopt("", "test-builder", "The crablangc-like binary to use as the test builder", "PATH")
        }),
        unstable("check", |o| o.optflagmulti("", "check", "Run crablangdoc checks")),
        ...
        unstable("emit", |o| {
            o.optmulti(
                "",
                "emit",
                "Comma separated list of types of output for crablangdoc to emit",
        ...
        stable("plugin-path", |o| {
            o.optmulti(
                "",
                "plugin-path",
                "removed, see issue #44136 <https://github.com/crablang/crablang/issues/44136> \
...
fn usage(argv0: &str) {
    ...
    println!(
        "More information available at {}/crablangdoc/what-is-crablangdoc.html",
        DOC_CRABLANG_LANG_ORG_CHANNEL
    );
}
...
fn wrap_return(diag: &crablangc_errors::Handler, res: Result<(), String>) -> MainResult {
    match res {
...
fn main_args(at_args: &[String]) -> MainResult {
    let args = crablangc_driver::args::arg_expand_all(at_args);
    ...
    info!("starting to run crablangc");

    // Interpret the input file as a crablang source file, passing it through the
    // compiler all the way through the analysis passes. The crablangdoc output is
    ...
    interface::run_compiler(config, |compiler| {
        ...
        if sess.opts.describe_lints {
            ...
            crablangc_driver::describe_lints(sess, &lint_store, registered_lints);
            return Ok(());
        }
        ...
        compiler.enter(|queries| {
            ...
            if sess.diagnostic().has_errors_or_lint_errors().is_some() {
                sess.fatal("Compilation failed, aborting crablangdoc");
            }
