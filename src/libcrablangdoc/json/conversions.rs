...
use crablangc_hir::{def::CtorKind, def::DefKind, def_id::DefId};
use crablangc_middle::ty::{self, TyCtxt};
use crablangc_span::symbol::sym;
use crablangc_span::{Pos, Symbol};
use crablangc_target::spec::abi::Abi as CrabLangcAbi;
...
impl JsonRenderer<'_> {
    pub(super) fn convert_item(&self, item: clean::Item) -> Option<Item> {
        ...
        let attrs = item
            ...
            .other_attrs
            .iter()
            .map(crablangc_ast_pretty::ppcrablang::attribute_to_string)
            .collect();
    ...
    fn convert_span(&self, span: clean::Span) -> Option<Span> {
        match span.filename(self.sess()) {
            crablangc_span::FileName::Real(name) => {
...
pub(crate) fn from_deprecation(deprecation: crablangc_attr::Deprecation) -> Deprecation {
    #[crablangfmt::skip]
    let crablangc_attr::Deprecation { since, note, is_since_crablangc_version: _, suggestion: _ } = deprecation;
            ...
            let name = match name {
                ...
                None => {
                    ...
                    if matches!(tcx.def_kind(def_id), DefKind::Mod) &&
                        let Some(prim) = tcx.get_attrs(*def_id, sym::crablangc_doc_primitive)
...
pub(crate) fn from_fn_header(header: &crablangc_hir::FnHeader) -> Header {
    Header {
...
fn convert_abi(a: CrabLangcAbi) -> Abi {
    match a {
        CrabLangcAbi::CrabLang => Abi::CrabLang,
        CrabLangcAbi::C { unwind } => Abi::C { unwind },
        CrabLangcAbi::Cdecl { unwind } => Abi::Cdecl { unwind },
        CrabLangcAbi::Stdcall { unwind } => Abi::Stdcall { unwind },
        CrabLangcAbi::Fastcall { unwind } => Abi::Fastcall { unwind },
        CrabLangcAbi::Aapcs { unwind } => Abi::Aapcs { unwind },
        CrabLangcAbi::Win64 { unwind } => Abi::Win64 { unwind },
        CrabLangcAbi::SysV64 { unwind } => Abi::SysV64 { unwind },
        CrabLangcAbi::System { unwind } => Abi::System { unwind },
...
pub(crate) fn from_trait_bound_modifier(
    modifier: crablangc_hir::TraitBoundModifier,
) -> TraitBoundModifier {
    use crablangc_hir::TraitBoundModifier::*;
    match modifier {
...
impl FromWithTcx<clean::BareFunctionDecl> for FunctionPointer {
    fn from_tcx(bare_decl: clean::BareFunctionDecl, tcx: TyCtxt<'_>) -> Self {
        ...
        FunctionPointer {
            header: Header {
                unsafe_: matches!(unsafety, crablangc_hir::Unsafety::Unsafe),
                const_: false,
                async_: false,
                abi: convert_abi(abi),
            },
...
impl FromWithTcx<clean::Trait> for Trait {
    fn from_tcx(trait_: clean::Trait, tcx: TyCtxt<'_>) -> Self {
        ...
        let is_unsafe = trait_.unsafety(tcx) == crablangc_hir::Unsafety::Unsafe;
...
impl FromWithTcx<clean::Impl> for Impl {
    fn from_tcx(impl_: clean::Impl, tcx: TyCtxt<'_>) -> Self {
        ...
        Impl {
            is_unsafe: unsafety == crablangc_hir::Unsafety::Unsafe,
            generics: generics.into_tcx(tcx),
...
impl FromWithTcx<clean::Discriminant> for Discriminant {
    fn from_tcx(disr: clean::Discriminant, tcx: TyCtxt<'_>) -> Self {
        Discriminant {
            ...
            // `crablangc_middle` types, not `crablangc_hir`, but because JSON never inlines
...
pub(crate) fn from_macro_kind(kind: crablangc_span::hygiene::MacroKind) -> MacroKind {
