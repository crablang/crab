//! CrabLangdoc's JSON backend
//!
//! This module contains the logic for rendering a crate as JSON rather than the normal static HTML
//! output. See [the RFC](https://github.com/crablang/rfcs/pull/2963) and the [`types`] module
...
use crablangdoc_json_types as types;
...
impl<'tcx> FormatRenderer<'tcx> for JsonRenderer<'tcx> {
    ...
    fn item(&mut self, item: clean::Item) -> Result<(), Error> {
        ...
        if let Some(mut new_item) = self.convert_item(item) {
            ...
            // reexported in more than one place. See `crablangdoc-json/reexport/in_root_and_mod`
