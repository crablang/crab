...
use crablangc_data_structures::sync::{self, Lrc};
use crablangc_data_structures::unord::UnordSet;
use crablangc_errors::emitter::{Emitter, EmitterWriter};
use crablangc_errors::json::JsonEmitter;
use crablangc_errors::TerminalUrl;
use crablangc_feature::UnstableFeatures;
use crablangc_hir::def::Res;
use crablangc_hir::def_id::{DefId, DefIdMap, DefIdSet, LocalDefId};
use crablangc_hir::intravisit::{self, Visitor};
use crablangc_hir::{HirId, Path};
use crablangc_interface::interface;
use crablangc_middle::hir::nested_filter;
use crablangc_middle::ty::{ParamEnv, Ty, TyCtxt};
use crablangc_session::config::{self, CrateType, ErrorOutputType, ResolveDocLinks};
...
use crate::config::{Options as CrabLangdocOptions, OutputFormat, RenderOptions};
use crate::formats::cache::Cache;
use crate::passes::{self, Condition::*};

pub(crate) use crablangc_session::config::{Input, Options, UnstableOptions};

pub(crate) struct DocContext<'tcx> {
    pub(crate) tcx: TyCtxt<'tcx>,
    /// Used for normalization.
    ///
    /// Most of this logic is copied from crablangc_lint::late.
    ...
    /// The options given to crablangdoc that could be relevant to a pass.
    pub(crate) render_options: RenderOptions,
    /// This same cache is used throughout crablangdoc, including in [`crate::html::render`].
...
pub(crate) fn new_handler(
    ...
    crablangc_errors::Handler::with_emitter_and_flags(
        emitter,
...
pub(crate) fn create_config(
...
) -> crablangc_interface::Config {
    // Add the doc cfg into the doc build.
    cfgs.push("doc".to_string());

    let input = Input::File(input);

    // By default, crablangdoc ignores all lints.
    // Specifically unblock lints relevant to documentation or the lint machinery itself.
    let mut lints_to_show = vec![
        // it's unclear whether these should be part of crablangdoc directly (#77364)
        crablangc_lint::builtin::MISSING_DOCS.name.to_string(),
        crablangc_lint::builtin::INVALID_DOC_ATTRIBUTES.name.to_string(),
        // these are definitely not part of crablangdoc, but we want to warn on them anyway.
        crablangc_lint::builtin::RENAMED_AND_REMOVED_LINTS.name.to_string(),
        crablangc_lint::builtin::UNKNOWN_LINTS.name.to_string(),
        crablangc_lint::builtin::UNEXPECTED_CFGS.name.to_string(),
        // this lint is needed to support `#[expect]` attributes
        crablangc_lint::builtin::UNFULFILLED_LINT_EXPECTATIONS.name.to_string(),
    ];
    lints_to_show.extend(crate::lint::CRABLANGDOC_LINTS.iter().map(|lint| lint.name.to_string()));
    ...
    interface::Config {
        ...
        override_queries: Some(|_sess, providers, _external_providers| {
            ...
            // Prevent `crablangc_hir_analysis::check_crate` from calling `typeck` on all bodies.
            ...
            providers.typeck = move |tcx, def_id| {
                ...
                (crablangc_interface::DEFAULT_QUERY_PROVIDERS.typeck)(tcx, def_id)
            };
        }),
...
pub(crate) fn run_global_ctxt(
    ...
    // (see https://github.com/crablang/crablang/pull/73566#issuecomment-656954425),
    // so type-check everything other than function bodies in this crate before running lints.

    // NOTE: this does not call `tcx.analysis()` so that we won't
    // typeck function bodies or run the default crablangc lints.
    ...
    tcx.sess.time("missing_docs", || {
        crablangc_lint::check_crate(tcx, crablangc_lint::builtin::MissingDoc::new);
    });
    ...
    if krate.module.doc_value().map(|d| d.is_empty()).unwrap_or(true) {
        let help = format!(
            ...
            {}/crablangdoc/how-to-write-documentation.html",
            crate::DOC_CRABLANG_LANG_ORG_CHANNEL
        );
    ...
    fn report_deprecated_attr(name: &str, diag: &crablangc_errors::Handler, sp: Span) {
        let mut msg =
            diag.struct_span_warn(sp, &format!("the `#![doc({})]` attribute is deprecated", name));
        msg.note(
            "see issue #44136 <https://github.com/crablang/crablang/issues/44136> \
    ...
    tcx.sess.time("check_lint_expectations", || tcx.check_expectations(Some(sym::crablangdoc)));
...
/// Due to <https://github.com/crablang/crablang/pull/73566>,
...
impl<'tcx> Visitor<'tcx> for EmitIgnoredResolutionErrors<'tcx> {
    ...
    fn visit_path(&mut self, path: &Path<'tcx>, _id: HirId) {
        ...
        if path.res == Res::Err {
            // We have less context here than in crablangc_resolve,
            // so we can only emit the name and span.
            // However we can give a hint that crablangc_resolve will have more info.
            let label = format!(
            ...
            let mut err = crablangc_errors::struct_span_err!(
                self.tcx.sess,
                path.span,
                E0433,
                "failed to resolve: {}",
            ...
            err.note("this error was originally ignored because you are running `crablangdoc`");
            err.note("try running again with `crablangc` or `cargo check` and you may get a more detailed error");
            err.emit();
        }
