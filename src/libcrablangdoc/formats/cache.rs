...
use crablangc_hir::def_id::{CrateNum, DefId, DefIdMap, DefIdSet};
...
use crate::visit_lib::CrabLangdocEffectiveVisibilities;
...
pub(crate) struct Cache {
    ...
    pub(crate) effective_visibilities: CrabLangdocEffectiveVisibilities,

    /// The version of the crate being documented, if given from the `--crate-version` flag.
    pub(crate) crate_version: Option<String>,

    /// Whether to document private items.
    /// This is stored in `Cache` so it doesn't need to be passed through all crablangdoc functions.
    pub(crate) document_private: bool,

    /// Crates marked with [`#[doc(masked)]`][doc_masked].
    ///
    /// [doc_masked]: https://doc.crablang.org/nightly/unstable-book/language-features/doc-masked.html
                ...
                (parent, Some(path)) if is_inherent_impl_item || !self.cache.stripped_mod => {
                    ...
                    if item.item_id.as_def_id().map_or(false, |idx| !idx.is_crate_root()) {
                            ...
                            || u16::from_str_radix(s.as_str(), 10).is_err()
                        {
                            ...
                            // not useful for crablangdoc search.
        ...
        match *item.kind {
            ...
            | clean::VariantItem(..) => {
                if !self.cache.stripped_mod {
                    ...
                    // in the crablangdoc ast that we're looking at. We know,
