...
//!
//! This module uses libcrablangc_ast's lexer to provide token-based highlighting for
//! the HTML documentation generated by crablangdoc.
//!
//! Use the `render_with_highlighting` to highlight some crablang code.
...
use crablangc_lexer::{Cursor, LiteralKind, TokenKind};
...
pub(crate) fn render_example_with_highlighting(
    ...
    write_header(out, "crablang-example-rendered", None, tooltip);
...
pub(crate) fn render_item_decl_with_highlighting(src: &str, out: &mut Buffer) {
    write!(out, "<pre class=\"crablang item-decl\">");
    write_code(out, src, None, None);
    write!(out, "</pre>");
}
...
fn write_header(out: &mut Buffer, class: &str, extra_content: Option<Buffer>, tooltip: Tooltip) {
    ...
    if class.is_empty() {
        write!(out, "<pre class=\"crablang\">");
    } else {
        write!(out, "<pre class=\"crablang {class}\">");
    }
    write!(out, "<code>");
}
...
impl Class {
    ...
    /// Returns the css class expected by crablangdoc for each `Class`.
...
impl<'src> Classifier<'src> {
    /// Takes as argument the source code to HTML-ify, the crablang edition to use and the source code
...
/// * If `klass` is `Some` and has a [`crablangc_span::Span`], it then tries to generate a link (`<a>`
///   element) by retrieving the link information from the `span_correspondance_map` that was filled
///   in `span_map.rs::collect_spans_and_sources`. If it cannot retrieve the information, then it's
///   the same as the second point (`klass` is `Some` but doesn't have a [`crablangc_span::Span`]).
    ...
    if let Some(href_context) = href_context {
        if let Some(href) =
            href_context.context.shared.span_correspondance_map.get(&def_span).and_then(|href| {
                ...
                // https://github.com/crablang/crablang/blob/60f1a2fc4b535ead9c85ce085fdce49b1b097531/src/libcrablangdoc/html/render/context.rs#L315-L338
                match href {
