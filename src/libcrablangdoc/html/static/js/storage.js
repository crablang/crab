// storage.js is loaded in the `<head>` of all crablangdoc pages and doesn't
...
function getSettingValue(settingName) {
    ...
    if (current === null && settingsDataset !== null) {
        ...
        // `Options::from_matches` in `libcrablangdoc/config.rs`.
...
function updateLocalStorage(name, value) {
    try {
        window.localStorage.setItem("crablangdoc-" + name, value);
    } catch (e) {
...
function getCurrentValue(name) {
    try {
        return window.localStorage.getItem("crablangdoc-" + name);
    } catch (e) {
        return null;
    }
}

// Get a value from the crablangdoc-vars div, which is used to convey data from
// CrabLang to the JS. If there is no such element, return null.
const getVar = (function getVar(name) {
    const el = document.getElementById("crablangdoc-vars");
