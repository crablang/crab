//! CrabLangdoc's HTML rendering module.
//!
//! This modules contains the bulk of the logic necessary for rendering a
//! crablangdoc `clean::Crate` instance to a set of static HTML pages. This
...
use crablangc_attr::{ConstStability, Deprecation, StabilityLevel};
use crablangc_data_structures::captures::Captures;
use crablangc_data_structures::fx::{FxHashMap, FxHashSet};
use crablangc_hir::def_id::{DefId, DefIdSet};
...
use crate::try_none;
use crate::DOC_CRABLANG_LANG_ORG_CHANNEL;
...
fn scrape_examples_help(shared: &SharedContext<'_>) -> String {
    ...
    content.push_str(&format!(
      "## More information\n\n\
      If you want more information about this feature, please read the [corresponding chapter in the CrabLangdoc book]({}/crablangdoc/scraped-examples.html).",
...
fn short_item_info(
    ...
    if let Some(depr @ Deprecation { note, since, is_since_crablangc_version: _, suggestion: _ }) =
        item.deprecation(cx.tcx())
    {
        // We display deprecation messages for #[deprecated], but only display
        // the future-deprecation messages for crablangc versions.
        let mut message = if let Some(since) = since {
            let since = since.as_str();
            if !stability::deprecation_in_effect(&depr) {
                if since == "TBD" {
                    String::from("Deprecating in a future CrabLang version")
                } else {
    ...
    // Render unstable items. But don't render "crablangc_private" crates (internal compiler crates).
    // Those crates are permanently unstable so it makes no sense to render "unstable" everywhere.
    if let Some((StabilityLevel::Unstable { reason: _, issue, .. }, feature)) = item
        .stability(cx.tcx())
        .as_ref()
        .filter(|stab| stab.feature != sym::crablangc_private)
...
fn assoc_method(
    ...
    // FIXME: Once https://github.com/crablang/crablang/issues/67792 is implemented, we can remove
    // this condition.
...
fn render_stability_since_raw_with_extra(
    ...
    if let Some(ver) = stable_version {
        ...
        title.push_str(&format!("Stable since CrabLang version {}", ver));
    }
    ...
    let const_title_and_stability = match const_stability {
        ...
        Some(ConstStability { level: StabilityLevel::Unstable { issue, .. }, feature, .. }) => {
            let unstable = if let Some(n) = issue {
                format!(
                    r#"<a href="https://github.com/crablang/crablang/issues/{}" title="Tracking issue for {}">unstable</a>"#,
                    n, feature
                )
            } else {
    ...
    it.attrs
        ...
        .filter_map(|attr| {
            if ALLOWED_ATTRIBUTES.contains(&attr.name_or_empty()) {
                Some(
                    ppcrablang::attribute_to_string(attr)
                        .replace("\\\n", "")
                        .replace('\n', "")
                        .replace("  ", " "),
                )
...
pub(crate) fn small_url_encode(s: String) -> String {
    ...
    for (idx, b) in s.bytes().enumerate() {
        ...
        if b == b' ' {
            ...
            // While the same is not true for hashes, crablangdoc only needs to be
...
fn render_call_locations<W: fmt::Write>(mut w: W, cx: &mut Context<'_>, item: &clean::Item) {
    ...
    let write_example = |mut w: &mut W, (path, call_data): (&PathBuf, &CallData)| -> bool {
        let contents = match fs::read_to_string(&path) {
            ...
            Err(err) => {
                let span = item.span(tcx).map_or(crablangc_span::DUMMY_SP, |span| span.inner());
                tcx.sess
        ...
        let file_span = (|| {
            ...
            Some(crablangc_span::Span::with_root_ctxt(
    ...
    //      directory structure in CrabLangdoc, so we proxy this by prioritizing code that comes from
    //      a --crate-type bin.
    ...
    //      ordering of examples from randomly changing between CrabLangdoc invocations.
