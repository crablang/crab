...
use crablangc_middle::ty::layout::LayoutError;
use crablangc_middle::ty::{self, Adt, TyCtxt};
use crablangc_span::hygiene::MacroKind;
use crablangc_span::symbol::{kw, sym, Symbol};
use crablangc_target::abi::{LayoutS, Primitive, TagEncoding, Variants};
...
fn extra_info_tags<'a, 'tcx: 'a>(
    ...
    display_fn(move |f| {
        ...
        // The "crablangc_private" crates are permanently unstable so it makes no sense
        // to render "unstable" everywhere.
        if item.stability(tcx).as_ref().map(|s| s.is_unstable() && s.feature != sym::crablangc_private)
            == Some(true)
        {
...
fn item_trait(w: &mut Buffer, cx: &mut Context<'_>, it: &clean::Item, t: &clean::Trait) {
    ...
    //
    // This is complicated by the way crablangdoc is invoked, which is basically
    // the same way crablangc is invoked: it gets called, one at a time, for each
    // crate. When building the crablangdocs for the current crate, crablangdoc can
    ...
    // surrounded by a tiny wrapper that the CrabLang side ignores, but allows the
    ...
    // data if something changes. That is, `crablangdoc` is idempotent. The other
...
where
    F: FnOnce(&mut Buffer),
{
    w.write_str(r#"<pre class="crablang item-decl"><code>"#);
    f(w);
    ...
    fn write_size_of_layout(mut w: impl fmt::Write, layout: &LayoutS, tag_size: u64) {
        if layout.abi.is_unsized() {
            ...
            if layout.abi.is_uninhabited() {
                write!(
                    w,
                    " (<a href=\"https://doc.crablang.org/stable/reference/glossary.html#uninhabited\">uninhabited</a>)"
                ).unwrap();
            }
        }
    }
        ...
        match tcx.layout_of(param_env.and(ty)) {
            Ok(ty_layout) => {
                writeln!(
                    ...
                    Please see the CrabLang Reference’s \
                    <a href=\"https://doc.crablang.org/reference/type-layout.html\">“Type Layout”</a> \
