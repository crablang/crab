...
impl<'tcx> SpanMapVisitor<'tcx> {
    ...
    fn handle_path(&mut self, path: &crablangc_hir::Path<'_>) {
...
impl<'tcx> Visitor<'tcx> for SpanMapVisitor<'tcx> {
    ...
    fn visit_path(&mut self, path: &crablangc_hir::Path<'tcx>, _id: HirId) {
    ...
    fn visit_expr(&mut self, expr: &'tcx crablangc_hir::Expr<'tcx>) {
        if let ExprKind::MethodCall(segment, ..) = expr.kind {
            ...
            //
            // See discussion in https://github.com/crablang/crablang/issues/69426#issuecomment-1019412352
