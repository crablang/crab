...
/// CrabLangdoc writes out two kinds of shared files:
///  - Static files, which are embedded in the crablangdoc binary and are written with a
...
pub(super) fn write_shared(
    ...
    // Write out the shared files. Note that these are shared among all crablangdoc
    // docs placed in the output directory, so this needs to be a synchronized
    // operation with respect to all other crablangdocs running around.
    ...
    // with crablangdoc running in parallel.
    all_indexes.sort();
...
"#,
    ...
    for (&did, imps) in &cache.implementors {
        // Private modules can leak through to this phase of crablangdoc, which
        ...
        // identically even with crablangdoc running in parallel.
