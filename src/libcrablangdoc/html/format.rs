...
//! various types in `crablangdoc::clean`.
//!
...
use crablangc_attr::{ConstStability, StabilityLevel};
...
use crablangc_metadata::creader::{CStore, LoadedMacro};
        ...
        display_fn(move |f| match self {
            ...
            clean::GenericBound::TraitBound(ty, modifier) => {
                let modifier_str = match modifier {
                    ...
                    // ~const is experimental; do not display those bounds in crablangdoc
...
pub(crate) enum HrefError {
    /// This item is known to crablangdoc, but from a crate that does not have documentation generated.
    ///
    ...
    /// We document `b` but **not** `a` (we only _build_ the latter – with `crablangc`):
    ///
    /// ```sh
    /// crablangc a.rs --crate-type=lib
    /// crablangdoc b.rs --crate-type=lib --extern=a=liba.rlib
    /// ```
    ///
...
fn generate_macro_def_id_path(
    ...
    // More information in <https://crablang.github.io/rfcs/1584-macros.html>.
...
pub(crate) fn visibility_print_with_space<'a, 'tcx: 'a>(
    ...
    let to_print: Cow<'static, str> = match visibility {
        ...
        Some(ty::Visibility::Restricted(vis_did)) => {
            ...
            //                 However, crablangdoc currently never displays a module's
