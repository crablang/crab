use crablangc_ast::token::{self, BinOpToken, Delimiter};
use crablangc_ast::tokenstream::{TokenStream, TokenTree};
use crablangc_ast_pretty::ppcrablang::state::State as Printer;
use crablangc_ast_pretty::ppcrablang::PrintState;
...
fn snippet_equal_to_token(tcx: TyCtxt<'_>, matcher: &TokenTree) -> Option<String> {
    // Find what crablangc thinks is the source snippet.
    ...
    // Create a Parser.
    let sess =
        ParseSess::new(crablangc_driver::DEFAULT_LOCALE_RESOURCES.to_vec(), FilePathMapping::empty());
    let file_name = source_map.span_to_filename(span);
    let mut parser =
        match crablangc_parse::maybe_new_parser_from_source_str(&sess, file_name, snippet.clone()) {
            Ok(parser) => parser,
            Err(diagnostics) => {
                drop(diagnostics);
                return None;
