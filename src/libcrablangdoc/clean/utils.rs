...
use crablangc_ast::tokenstream::TokenTree;
...
pub(crate) fn krate(cx: &mut DocContext<'_>) -> Crate {
    let module = crate::visit_ast::CrabLangdocVisitor::new(cx).visit();

    // Clean the crate, translating the entire libcrablangc_ast AST to one that is
    // understood by crablangdoc.
...
/// This means that the output is not necessarily valid CrabLang code.
///
/// Currently, only
///
...
pub(crate) fn print_const_expr(tcx: TyCtxt<'_>, body: hir::BodyId) -> String {
    ...
    && let Ok(snippet) = tcx.sess.source_map().span_to_snippet(value.span) {
        ...
        crablangc_hir_pretty::id_to_string(&hir, body.hir_id)
...
pub(crate) fn get_auto_trait_and_blanket_impls(
    ...
    // More information in <https://github.com/crablang/crablang/pull/106930>.
...
pub(crate) fn has_doc_flag(tcx: TyCtxt<'_>, did: DefId, flag: Symbol) -> bool {
    tcx.get_attrs(did, sym::doc).any(|attr| {
        attr.meta_item_list().map_or(false, |l| crablangc_attr::list_contains_name(&l, flag))
    })
}

/// A link to `doc.crablang.org` that includes the channel name. Use this instead of manual links
/// so that the channel is consistent.
///
/// Set by `bootstrap::Builder::doc_crablang_lang_org_channel` in order to keep tests passing on beta/stable.
pub(crate) const DOC_CRABLANG_LANG_ORG_CHANNEL: &str = env!("DOC_CRABLANG_LANG_ORG_CHANNEL");
