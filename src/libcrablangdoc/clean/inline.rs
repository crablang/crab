...
use crablangc_hir::def_id::{DefId, DefIdSet, LocalDefId};
...
pub(crate) fn build_impls(
    ...
    // See also:
    //
    // * https://github.com/crablang/crablang/issues/103170 — where it didn't used to get documented
    // * https://github.com/crablang/crablang/pull/99917 — where the feature got used
    // * https://github.com/crablang/crablang/issues/53487 — overall tracking issue for Error
    if tcx.has_attr(did, sym::crablangc_has_incoherent_inherent_impls) {
...
pub(crate) fn build_impl(
    ...
    // reachable in crablangdoc generated documentation
    if !did.is_local() && let Some(traitref) = associated_trait {
        ...
        if let Some(stab) = tcx.lookup_stability(did) &&
            stab.is_unstable() &&
            stab.feature == sym::crablangc_private
        {
            return;
        }
    }
    ...
    if !did.is_local() {
        if let Some(did) = for_.def_id(&cx.cache) {
            ...
            if let Some(stab) = tcx.lookup_stability(did) {
                if stab.is_unstable() && stab.feature == sym::crablangc_private {
                    return;
                }
            }
        }
...
pub(crate) fn print_inlined_const(tcx: TyCtxt<'_>, did: DefId) -> String {
    if let Some(did) = did.as_local() {
        ...
        crablangc_hir_pretty::id_to_string(&tcx.hir(), hir_id)
    } else {
