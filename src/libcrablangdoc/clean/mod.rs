...
use crablangc_data_structures::fx::{FxHashMap, FxHashSet, FxIndexMap, FxIndexSet, IndexEntry};
use crablangc_hir as hir;
use crablangc_hir::def::{CtorKind, DefKind, Res};
use crablangc_hir::def_id::{DefId, DefIdMap, DefIdSet, LocalDefId, LOCAL_CRATE};
use crablangc_hir::PredicateOrigin;
use crablangc_hir_analysis::hir_ty_to_ty;
use crablangc_infer::infer::region_constraints::{Constraint, RegionConstraintData};
use crablangc_middle::metadata::Reexport;
use crablangc_middle::middle::resolve_bound_vars as rbv;
use crablangc_middle::ty::fold::TypeFolder;
use crablangc_middle::ty::InternalSubsts;
use crablangc_middle::ty::TypeVisitableExt;
use crablangc_middle::ty::{self, AdtKind, EarlyBinder, Ty, TyCtxt};
use crablangc_middle::{bug, span_bug};
use crablangc_span::hygiene::{AstPass, MacroKind};
use crablangc_span::symbol::{kw, sym, Ident, Symbol};
use crablangc_span::{self, ExpnKind};
...
pub(crate) fn clean_doc_module<'tcx>(doc: &DocModule<'tcx>, cx: &mut DocContext<'tcx>) -> Item {
    ...
    // but there's already an item with the same namespace and same name. CrabLang gives
...
///
/// See `lifetime_to_generic_param` in `crablangc_ast_lowering` for more information.
...
fn clean_ty_generics<'tcx>(
    ...
    // predicates field (see crablangc_hir_analysis::collect::ty_generics), so remove
    // them.
...
/// `crablangc_legacy_const_generics`. More information in
/// <https://github.com/crablang/crablang/issues/83167>.
fn clean_fn_decl_legacy_const_generics(func: &mut Function, attrs: &[ast::Attribute]) {
    for meta_item_list in attrs
        .iter()
        .filter(|a| a.has_name(sym::crablangc_legacy_const_generics))
                    ...
                    WherePredicate::BoundPredicate {
                        ...
                        match &assoc.args {
                            ...
                            GenericArgs::Parenthesized { .. } => {
                                // The only time this happens is if we're inside the crablangdoc for Fn(),
    ...
    match qpath {
        ...
        hir::QPath::TypeRelative(qself, segment) => {
            ...
            let res = match ty.kind() {
                ...
                // CrabLangdoc handles `ty::Error`s by turning them into `Type::Infer`s.
...
pub(crate) fn clean_ty<'tcx>(ty: &hir::Ty<'tcx>, cx: &mut DocContext<'tcx>) -> Type {
    ...
    match ty.kind {
        ...
        // CrabLangdoc handles `TyKind::Err`s by turning them into `Type::Infer`s.
...
fn normalize<'tcx>(
    ...
    use crate::crablangc_trait_selection::infer::TyCtxtInferExt;
    use crate::crablangc_trait_selection::traits::query::normalize::QueryNormalizeExt;
...
pub(crate) fn clean_middle_ty<'tcx>(
    ...
    match *bound_ty.skip_binder().kind() {
        ...
        ty::Dynamic(obj, ref reg, _) => {
            ...
            // https://doc.crablang.org/reference/lifetime-elision.html#default-trait-object-lifetimes
        ...
        ty::Error(_) => crablangc_errors::FatalError.raise(),
    }
}
...
fn clean_extern_crate<'tcx>(
    ...
    // FIXME: using `from_def_id_and_kind` breaks `crablangdoc/masked` for some reason
    vec![Item {
        name: Some(name),
