...
use crablangc_attr::{ConstStability, Deprecation, Stability, StabilityLevel};
use crablangc_const_eval::const_eval::is_unstable_const_fn;
use crablangc_data_structures::fx::{FxHashMap, FxHashSet};
use crablangc_hir as hir;
use crablangc_hir::def::{CtorKind, DefKind, Res};
use crablangc_hir::def_id::{CrateNum, DefId, LOCAL_CRATE};
use crablangc_hir::lang_items::LangItem;
use crablangc_hir::{BodyId, Mutability};
use crablangc_hir_analysis::check::intrinsic::intrinsic_operation_unsafety;
use crablangc_index::vec::IndexVec;
use crablangc_middle::ty::fast_reject::SimplifiedType;
use crablangc_middle::ty::{self, TyCtxt, Visibility};
use crablangc_resolve::crablangdoc::{add_doc_fragment, attrs_to_doc_fragments, inner_docs, DocFragment};
use crablangc_session::Session;
use crablangc_span::hygiene::MacroKind;
use crablangc_span::symbol::{kw, sym, Ident, Symbol};
use crablangc_span::{self, FileName, Loc};
use crablangc_target::abi::VariantIdx;
...
pub(crate) struct Crate {
    ...
    /// Only here so that they can be filtered through the crablangdoc passes.
...
impl ExternalCrate {
    ...
    pub(crate) fn location(
        ...
        // WARNING: since crablangdoc creates these directories as it generates documentation, this check is only accurate before rendering starts.
    ...
    pub(crate) fn primitives(&self, tcx: TyCtxt<'_>) -> ThinVec<(DefId, PrimitiveType)> {
        ...
        // item tagged with `#[crablangc_doc_primitive]` then we would also have to
        // search the entirety of external modules for items tagged
        // `#[crablangc_doc_primitive]`, which is a pretty inefficient process (decoding
        // all that metadata unconditionally).
        //
        // In order to keep the metadata load under control, the
        // `#[crablangc_doc_primitive]` feature is explicitly designed to only allow the
        ...
        let as_primitive = |res: Res<!>| {
            ...
            tcx.get_attrs(def_id, sym::crablangc_doc_primitive).find_map(|attr| {
...
pub(crate) fn crablangc_span(def_id: DefId, tcx: TyCtxt<'_>) -> Span {
        ...
        match kind {
            ...
            ItemKind::ImplItem(box Impl { kind: ImplKind::Blanket(_), .. }) => {
                if let ItemId::Blanket { impl_id, .. } = self.item_id {
                    Some(crablangc_span(impl_id, tcx))
                } else {
                    panic!("blanket impl item has non-blanket ID")
                }
            }
            _ => self.item_id.as_def_id().map(|did| crablangc_span(did, tcx)),
        }
    }

    pub(crate) fn attr_span(&self, tcx: TyCtxt<'_>) -> crablangc_span::Span {
        crate::passes::span_of_attrs(&self.attrs)
            .unwrap_or_else(|| self.span(tcx).map_or(crablangc_span::DUMMY_SP, |span| span.inner()))
    }
    ...
    pub(crate) fn fn_header(&self, tcx: TyCtxt<'_>) -> Option<hir::FnHeader> {
        ...
        let header = match *self.kind {
            ItemKind::ForeignFunctionItem(_) => {
                ...
                hir::FnHeader {
                    unsafety: if abi == Abi::CrabLangIntrinsic {
                        intrinsic_operation_unsafety(tcx, self.item_id.as_def_id().unwrap())
                    } else {
                        hir::Unsafety::Unsafe
                    },
                    abi,
                    constness: if abi == Abi::CrabLangIntrinsic
...
pub(crate) enum ItemKind {
    ...
    /// An item that has been stripped by a crablangdoc pass
                ...
                if let Some(list) = attr.meta().as_ref().and_then(|mi| mi.meta_item_list()) {
                    for item in list {
                        ...
                        if let Some(cfg_mi) = item
                            .meta_item()
                            .and_then(|item| crablangc_expand::config::parse_cfg(item, sess))
                        {
...
impl GenericBound {
    ...
    pub(crate) fn is_sized_bound(&self, cx: &DocContext<'_>) -> bool {
        use crablangc_hir::TraitBoundModifier as TBM;
...
pub(crate) struct Argument {
    ...
    pub(crate) name: Symbol,
    /// This field is used to represent "const" arguments from the `crablangc_legacy_const_generics`
    /// feature. More information in <https://github.com/crablang/crablang/issues/83167>.
...
/// CrabLangdoc's representation of types, mostly based on the [`hir::Ty`].
#[derive(Clone, PartialEq, Eq, Debug, Hash)]
pub(crate) enum Type {
    /// A named type, which could be a trait.
    ///
    /// This is mostly CrabLangdoc's version of [`hir::Path`].
    /// It has to be different because CrabLangdoc's [`PathSegment`] can contain cleaned generics.
    Path { path: Path },
...
impl Type {
    ...
    /// use crablangdoc::format::cache::Cache;
    /// use crablangdoc::clean::types::{Type, PrimitiveType};
...
impl PrimitiveType {
    ...
    pub(crate) fn simplified_types() -> &'static SimplifiedTypes {
        ...
        CELL.get_or_init(move || {
            map! {
                ...
                // with different lengths, they won't show in crablangdoc.
                //
    ...
    /// Returns the DefId of the module with `crablangc_doc_primitive` for this primitive type.
    ...
    /// `crablangc_doc_primitive`, then it's entirely random whether `std` or the other crate is picked.
...
/// Small wrapper around [`crablangc_span::Span`] that adds helper methods
/// and enforces calling [`crablangc_span::Span::source_callsite()`].
#[derive(Copy, Clone, Debug)]
pub(crate) struct Span(crablangc_span::Span);

impl Span {
    /// Wraps a [`crablangc_span::Span`]. In case this span is the result of a macro expansion, the
    /// span will be updated to point to the macro invocation instead of the macro definition.
    ///
    /// (See crablang/crablang#39726)
    pub(crate) fn new(sp: crablangc_span::Span) -> Self {
        Self(sp.source_callsite())
    }

    pub(crate) fn inner(&self) -> crablangc_span::Span {
        self.0
    }
