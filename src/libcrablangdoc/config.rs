...
use crablangc_driver::print_flag_list;
use crablangc_session::config::{
    self, parse_crate_types_from_list, parse_externs, parse_target_triple, CrateType,
};
use crablangc_session::config::{get_cmd_lint_options, nightly_options};
use crablangc_session::config::{
...
use crablangc_target::spec::TargetTriple;
...
/// Configuration options for crablangdoc.
#[derive(Clone)]
pub(crate) struct Options {
    // Basic options / Options passed directly to crablangc
    ...
    /// List of `cfg` flags to hand to the compiler. Always includes `crablangdoc`.
    ...
    /// Whether to ask crablangc to describe the lints it knows.
    ...
    /// The path to a crablangc-like binary to build tests with. If not set, we
    /// default to loading from `$sysroot/bin/crablangc`.
    ...
    /// If this option is set to `true`, crablangdoc will only run checks and not generate
    /// documentation.
    ...
    pub(crate) unstable_features: crablangc_feature::UnstableFeatures,
}
...
impl Options {
    ...
    pub(crate) fn from_matches(
        ...
        if args.is_empty() || matches.opt_present("h") || matches.opt_present("help") {
            crate::usage("crablangdoc");
            return Err(0);
        } else if matches.opt_present("version") {
            crablangc_driver::version!("crablangdoc", matches);
            return Err(0);
        }
        ...
        if matches.opt_strs("passes") == ["list"] {
            println!("Available passes for running crablangdoc:");
            for pass in passes::PASSES {
                println!("{:>20} - {}", pass.name, pass.description);
            }
            println!("\nDefault passes for crablangdoc:");
        ...
        {
            crablangc_session::early_error(
                error_format,
                "the -Z unstable-options flag must be passed to enable --output-format for documentation generation (see https://github.com/crablang/crablang/issues/76578)",
            );
        }
        ...
        if !to_check.is_empty() {
            ...
            println!("crablangdoc: [check-theme] Starting tests! (Ignoring all other arguments)");
        ...
        if matches.opt_present("theme") {
                ...
                matches.opt_strs("theme").iter().map(|s| (PathBuf::from(&s), s.to_owned()))
            {
                ...
                if !success {
                    ...
                    .help(&format!(
                        "to see what rules are missing, call `crablangdoc --check-theme \"{}\"`",
                        theme_s
                    ))
                    .emit();
                }
        ...
        let unstable_features =
            crablangc_feature::UnstableFeatures::from_environment(crate_name.as_deref());
        let options = Options {
            input,
            bin_crate,
            proc_macro_crate,
...
fn check_deprecated_options(matches: &getopts::Matches, diag: &crablangc_errors::Handler) {
