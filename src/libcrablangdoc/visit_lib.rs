...
// FIXME: this may not be exhaustive, but is sufficient for crablangdocs current uses

#[derive(Default)]
pub(crate) struct CrabLangdocEffectiveVisibilities {
...
impl CrabLangdocEffectiveVisibilities {
...
/// Similar to `libcrablangc_privacy::EmbargoVisitor`, but also takes
/// specific crablangdoc annotations into account (i.e., `doc(hidden)`)
