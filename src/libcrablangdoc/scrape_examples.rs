...
impl SyntaxRange {
    fn new(span: crablangc_span::Span, file: &SourceFile) -> Option<Self> {
...
impl CallLocation {
    fn new(
        expr_span: crablangc_span::Span,
        ident_span: crablangc_span::Span,
        enclosing_item_span: crablangc_span::Span,
    ...
    'tcx: 'a,
{
    ...
    fn visit_expr(&mut self, ex: &'tcx hir::Expr<'tcx>) {
        ...
        // test/run-make/crablangdoc-scrape-examples-invalid-expr for an example.
        let hir = tcx.hir();
...
pub(crate) fn run(
    ...
    let inner = move || -> Result<(), String> {
        ...
        if tcx.sess.diagnostic().has_errors_or_lint_errors().is_some() {
            return Err(String::from("Compilation failed, aborting crablangdoc"));
        }
