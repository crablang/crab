...
-   [The `unsafe` keyword is now syntactically permitted on modules.][75857] This
    is still rejected _semantically_, but can now be parsed by procedural macros.
...
These are still rejected _semantically_, so you will likely receive an error but
...
-   [There are some _syntax-only_ changes:][67131]
    ...
    These are still rejected _semantically_, so you will likely receive an error
...
-   [You can now have multiple patterns in `if let` and `while let`
    ...
    ```rust
    enum Creature {
        spinning-loader(String),
        Lobster(String),
        Person(String),
    }

    fn main() {
        let state = Creature::spinning-loader("Ferris");

        if let Creature::spinning-loader(name) | Creature::Person(name) = state {
...
-   [Iterator::nth for `ops::{Range, RangeFrom}` is now done in _O_(1) time][43077]
...
unstable-options` to access. Unlike unstable language and library
    ...
    release channel. Going forward, _new unstable flags will not be
    available on the stable release channel_, and old unstable flags
...
-   [`rustc` passes `--enable-new-dtags` to GNU ld][1.7dta]. This makes
    any RPATH entries (emitted with `-C rpath`) _not_ take precedence
...
-   The `Windows`, `Chunks`, and `ChunksMut` iterators over slices all
    [override `count`, `nth` and `last` with an _O_(1)
    implementation][it].
...
-   The [`connect`] method on slices is deprecated, replaced by the new
    [`join`] method (note that both of these are on the _unstable_
...
-   [`mem::align_of`] now specifies [the _minimum alignment_ for
...
-   Associated type defaults are [now behind the
    ...
    defaults _did not work_, but could be mentioned syntactically. As
...
-   [The `Iterator` methods `count`, `nth`, and `last` have been
    overridden for slices to have _O_(1) performance instead of _O_(_n_)][si].
...
bar`, and Cargo now automatically translates "-" in _package_ names
...
-   Other
    ...
    -   rustc now _attempts_ to offer spelling suggestions
